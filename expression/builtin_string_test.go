...
func (s *testEvaluatorSuite) TestLengthAndOctetLength(c *C) {
	cases := []struct {
		...
		{types.NewTime(types.FromGoTime(time.Now()), allegrosql.TypeDatetime, 6), 26, false, false},
	...
	for _, lengthMethod := range lengthMethods {
		for _, t := range cases {
			f, err := newFunctionForTest(s.ctx, lengthMethod, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.args})...)
			c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestASCII(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.ASCII, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.args})...)
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestConcat(c *C) {
	cases := []struct {
	...
	}{
		{
			[]interface{}{nil},
			true, false, "",
			&types.FieldType{Tp: allegrosql.TypeVarString, Flen: 0, Decimal: types.UnspecifiedLength, Charset: charset.CharsetBin, DefCauslate: charset.DefCauslationBin, Flag: allegrosql.BinaryFlag},
		},
		{
			[]interface{}{"a", "b",
				1, 2,
				...
				types.NewTime(types.FromDate(2000, 1, 1, 12, 01, 01, 0), allegrosql.TypeDatetime, types.DefaultFsp),
				types.Duration{
					Duration: 12*time.Hour + 1*time.Minute + 1*time.Second,
					Fsp:      types.DefaultFsp},
			},
			false, false, "ab121.11.21.12000-01-01 12:01:0112:01:01",
			&types.FieldType{Tp: allegrosql.TypeVarString, Flen: 40, Decimal: types.UnspecifiedLength, Charset: charset.CharsetBin, DefCauslate: charset.DefCauslationBin, Flag: allegrosql.BinaryFlag},
		},
		{
			[]interface{}{"a", "b", nil, "c"},
			true, false, "",
			&types.FieldType{Tp: allegrosql.TypeVarString, Flen: 3, Decimal: types.UnspecifiedLength, Charset: charset.CharsetBin, DefCauslate: charset.DefCauslationBin, Flag: allegrosql.BinaryFlag},
		},
		{
			[]interface{}{errors.New("must error")},
			false, true, "",
			&types.FieldType{Tp: allegrosql.TypeVarString, Flen: types.UnspecifiedLength, Decimal: types.UnspecifiedLength, Charset: charset.CharsetBin, DefCauslate: charset.DefCauslationBin, Flag: allegrosql.BinaryFlag},
		},
	}
	fcName := ast.Concat
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, fcName, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestConcatSig(c *C) {
	...
	resultType := &types.FieldType{Tp: allegrosql.TypeVarchar, Flen: 1000}
	args := []Expression{
		&DeferredCauset{Index: 0, RetType: defCausTypes[0]},
		&DeferredCauset{Index: 1, RetType: defCausTypes[1]},
	}
	...
	for _, t := range cases {
		input := chunk.NewChunkWithCapacity(defCausTypes, 10)
		input.AppendString(0, t.args[0].(string))
		input.AppendString(1, t.args[1].(string))

		res, isNull, err := concat.evalString(input.GetEvent(0))
...
func (s *testEvaluatorSuite) TestConcatWS(c *C) {
	...
	_, err := newFunctionForTest(s.ctx, fcName, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{nil})...)
	c.Assert(err, NotNil)

	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, fcName, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		val, err1 := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err1, NotNil)
		} else {
			c.Assert(err1, IsNil)
			if t.isNil {
				c.Assert(val.HoTT(), Equals, types.HoTTNull)
			} else {
	...
	_, err = funcs[ast.ConcatWS].getFunction(s.ctx, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{nil, nil}))
	c.Assert(err, IsNil)
}
...
func (s *testEvaluatorSuite) TestConcatWSSig(c *C) {
	...
	args := []Expression{
		&DeferredCauset{Index: 0, RetType: defCausTypes[0]},
		&DeferredCauset{Index: 1, RetType: defCausTypes[1]},
		&DeferredCauset{Index: 2, RetType: defCausTypes[2]},
	}
	base := baseBuiltinFunc{args: args, ctx: s.ctx, tp: resultType}
...
func (s *testEvaluatorSuite) TestLeft(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Left, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		v, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestRight(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Right, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		v, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestRepeat(c *C) {
	...
	fc := funcs[ast.Repeat]
	f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(args...)))
	c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestRepeatSig(c *C) {
	...
	for _, t := range cases {
		...
		res, isNull, err := repeat.evalString(input.GetEvent(0))
		c.Assert(res, Equals, t.res)
		c.Assert(err, IsNil)
		if t.warning == 0 {
			c.Assert(isNull, IsFalse)
...
func (s *testEvaluatorSuite) TestLower(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Lower, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		v, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestUpper(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Upper, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		v, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestReverse(c *C) {
	...
	for _, t := range dtbl {
		...
		d, err = evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(d, solitonutil.CausetEquals, t["Expect"][0])
	}
}
...
func (s *testEvaluatorSuite) TestStrcmp(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Strcmp, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestReplace(c *C) {
	cases := []struct {
		...
		{[]interface{}{"www.allegrosql.com", "allegrosql", "whtcorpsinc"}, false, false, "www.whtcorpsinc.com", 17},
		{[]interface{}{"www.allegrosql.com", "w", 1}, false, false, "111.allegrosql.com", 260},
	...
	for i, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Replace, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		...
		if t.getErr {
			...
			c.Assert(err, IsNil, Commentf("test %v", i))
			if t.isNil {
				c.Assert(d.HoTT(), Equals, types.HoTTNull, Commentf("test %v", i))
			} else {
...
func (s *testEvaluatorSuite) TestSubstring(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Substring, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestConvert(c *C) {
	...
	for _, v := range tbl {
		...
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(v.str, v.cs)))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		retType := f.getRetTp()
		c.Assert(retType.Charset, Equals, strings.ToLower(v.cs))
		defCauslate, err := charset.GetDefaultDefCauslation(strings.ToLower(v.cs))
		c.Assert(err, IsNil)
		c.Assert(retType.DefCauslate, Equals, defCauslate)
		c.Assert(allegrosql.HasBinaryFlag(retType.Flag), Equals, v.hasBinaryFlag)

		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(r.HoTT(), Equals, types.HoTTString)
	...
	fc := funcs[ast.Convert]
	f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets("haha", "utf8")))
	c.Assert(err, IsNil)
	c.Assert(f, NotNil)
	wrongFunction := f.(*builtinConvertSig)
	wrongFunction.tp.Charset = "wrongcharset"
	_, err = evalBuiltinFunc(wrongFunction, chunk.Event{})
...
func (s *testEvaluatorSuite) TestSubstringIndex(c *C) {
	cases := []struct {
		...
		{[]interface{}{"www.whtcorpsinc.com", ".", 2}, false, false, "www.whtcorpsinc"},
		{[]interface{}{"www.whtcorpsinc.com", ".", -2}, false, false, "whtcorpsinc.com"},
		{[]interface{}{"www.whtcorpsinc.com", ".", 0}, false, false, ""},
		{[]interface{}{"www.whtcorpsinc.com", ".", 100}, false, false, "www.whtcorpsinc.com"},
		{[]interface{}{"www.whtcorpsinc.com", ".", -100}, false, false, "www.whtcorpsinc.com"},
		{[]interface{}{"www.whtcorpsinc.com", "d", 0}, false, false, ""},
		{[]interface{}{"www.whtcorpsinc.com", "d", 1}, false, false, "www.whtcorpsinc.com"},
		{[]interface{}{"www.whtcorpsinc.com", "d", -1}, false, false, "www.whtcorpsinc.com"},
		{[]interface{}{"www.whtcorpsinc.com", "", 0}, false, false, ""},
		{[]interface{}{"www.whtcorpsinc.com", "", 1}, false, false, ""},
		{[]interface{}{"www.whtcorpsinc.com", "", -1}, false, false, ""},
		{[]interface{}{"", ".", 0}, false, false, ""},
		{[]interface{}{"", ".", 1}, false, false, ""},
		{[]interface{}{"", ".", -1}, false, false, ""},
		{[]interface{}{nil, ".", 1}, true, false, ""},
		{[]interface{}{"www.whtcorpsinc.com", nil, 1}, true, false, ""},
		{[]interface{}{"www.whtcorpsinc.com", ".", nil}, true, false, ""},
		{[]interface{}{errors.New("must error"), ".", 1}, false, true, ""},
	}
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.SubstringIndex, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestSpace(c *C) {
	...
	cases := []struct {
		...
		{0, false, false, ""},
		{3, false, false, "   "},
		{allegrosql.MaxBlobWidth + 1, true, false, ""},
		{-1, false, false, ""},
		{"abc", false, false, ""},
		{"3", false, false, "   "},
		{1.2, false, false, " "},
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Space, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.arg})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestSpaceSig(c *C) {
	...
	res, isNull, err := space.evalString(input.GetEvent(0))
	c.Assert(res, Equals, "      ")
	c.Assert(isNull, IsFalse)
	c.Assert(err, IsNil)
	res, isNull, err = space.evalString(input.GetEvent(1))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)
	warnings := s.ctx.GetStochaseinstein_dbars().StmtCtx.GetWarnings()
...
func (s *testEvaluatorSuite) TestLocate(c *C) {
	...
	for i, t := range Dtbl {
		f, err := instr.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(t["Args"]))
		c.Assert(err, IsNil)
	...
	for i, t := range Dtbl2 {
		exprs := s.datumsToCouplingConstantWithRadixs(t["Args"])
...
func (s *testEvaluatorSuite) TestTrim(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Trim, s.primitiveValsToCouplingConstantWithRadixs(t.args)...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestLTrim(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.LTrim, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.arg})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestRTrim(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.RTrim, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.arg})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestHexFunc(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Hex, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.arg})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestUnhexFunc(c *C) {
	cases := []struct {
		...
		{"4D7953514C", false, false, "MyALLEGROSQL"},
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Unhex, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.arg})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestBitLength(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.BitLength, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.args})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestChar(c *C) {
	...
	for _, v := range tbl {
		for _, char := range []interface{}{"utf8", nil} {
			...
			f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(v.str, v.iNum, v.fNum, char)))
			c.Assert(err, IsNil)
			c.Assert(f, NotNil)
			r, err := evalBuiltinFunc(f, chunk.Event{})
			c.Assert(err, IsNil, Commentf("err: %v", err))
			c.Assert(r, solitonutil.CausetEquals, types.NewCauset(v.result))
		}
	}

	fc := funcs[ast.CharFunc]
	f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets("65", 66, nil)))
	c.Assert(err, IsNil)
	r, err := evalBuiltinFunc(f, chunk.Event{})
	c.Assert(err, IsNil)
	c.Assert(r, solitonutil.CausetEquals, types.NewCauset("AB"))
}
...
func (s *testEvaluatorSuite) TestCharLength(c *C) {
	...
	for _, v := range tbl {
		...
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(v.input)))
		c.Assert(err, IsNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(r, solitonutil.CausetEquals, types.NewCauset(v.result))
	...
	for _, v := range tbl {
		fc := funcs[ast.CharLength]
		arg := s.datumsToCouplingConstantWithRadixs(types.MakeCausets(v.input))
		tp := arg[0].GetType()
...
func (s *testEvaluatorSuite) TestFindInSet(c *C) {
	for _, t := range []struct {
		...
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(t.str, t.strlst)))
		c.Assert(err, IsNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(r, solitonutil.CausetEquals, types.NewCauset(t.ret), Commentf("FindInSet(%s, %s)", t.str, t.strlst))
	}
}
...
func (s *testEvaluatorSuite) TestField(c *C) {
	...
	for _, t := range tbl {
		fc := funcs[ast.Field]
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(t.argLst...)))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestLpad(c *C) {
	...
	for _, test := range tests {
		str := types.NewStringCauset(test.str)
		length := types.NewIntCauset(test.len)
		padStr := types.NewStringCauset(test.padStr)
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs([]types.Causet{str, length, padStr}))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
...
func (s *testEvaluatorSuite) TestRpadSig(c *C) {
	...
	res, isNull, err := rpad.evalString(input.GetEvent(0))
	c.Assert(res, Equals, "abc123")
	c.Assert(isNull, IsFalse)
	c.Assert(err, IsNil)

	res, isNull, err = rpad.evalString(input.GetEvent(1))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestInsertBinarySig(c *C) {
	...
	resultType := &types.FieldType{Tp: allegrosql.TypeVarchar, Flen: 3}

	args := []Expression{
		&DeferredCauset{Index: 0, RetType: defCausTypes[0]},
		&DeferredCauset{Index: 1, RetType: defCausTypes[1]},
		&DeferredCauset{Index: 2, RetType: defCausTypes[2]},
		&DeferredCauset{Index: 3, RetType: defCausTypes[3]},
	}

	base := baseBuiltinFunc{args: args, ctx: s.ctx, tp: resultType}
	insert := &builtinInsertSig{base, 3}

	input := chunk.NewChunkWithCapacity(defCausTypes, 2)
	input.AppendString(0, "abc")
	input.AppendString(0, "abc")
	input.AppendString(0, "abc")
	input.AppendNull(0)
	...
	res, isNull, err := insert.evalString(input.GetEvent(0))
	c.Assert(res, Equals, "abd")
	c.Assert(isNull, IsFalse)
	c.Assert(err, IsNil)

	res, isNull, err = insert.evalString(input.GetEvent(1))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)

	res, isNull, err = insert.evalString(input.GetEvent(2))
	c.Assert(res, Equals, "abc")
	c.Assert(isNull, IsFalse)
	c.Assert(err, IsNil)

	res, isNull, err = insert.evalString(input.GetEvent(3))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)

	res, isNull, err = insert.evalString(input.GetEvent(4))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)

	res, isNull, err = insert.evalString(input.GetEvent(5))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)

	res, isNull, err = insert.evalString(input.GetEvent(6))
	c.Assert(res, Equals, "")
	c.Assert(isNull, IsTrue)
	c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestMakeSet(c *C) {
	...
	for _, t := range tbl {
		...
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(t.argList...)))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestFormat(c *C) {
	...
	for _, tt := range formatTests {
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(tt.number, tt.precision, tt.locale)))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(r, solitonutil.CausetEquals, types.NewCauset(tt.ret))
	}

	origConfig := s.ctx.GetStochaseinstein_dbars().StmtCtx.TruncateAsWarning
	s.ctx.GetStochaseinstein_dbars().StmtCtx.TruncateAsWarning = true
	for _, tt := range formatTests1 {
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(tt.number, tt.precision)))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(r, solitonutil.CausetEquals, types.NewCauset(tt.ret), Commentf("test %v", tt))
		if tt.warnings > 0 {
			warnings := s.ctx.GetStochaseinstein_dbars().StmtCtx.GetWarnings()
			...
			s.ctx.GetStochaseinstein_dbars().StmtCtx.SetWarnings([]stmtctx.ALLEGROSQLWarn{})
		}
	}
	s.ctx.GetStochaseinstein_dbars().StmtCtx.TruncateAsWarning = origConfig

	f2, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(formatTests2.number, formatTests2.precision, formatTests2.locale)))
	c.Assert(err, IsNil)
	c.Assert(f2, NotNil)
	r2, err := evalBuiltinFunc(f2, chunk.Event{})
	c.Assert(types.NewCauset(err), solitonutil.CausetEquals, types.NewCauset(errors.New("not implemented")))
	c.Assert(r2, solitonutil.CausetEquals, types.NewCauset(formatTests2.ret))

	f3, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(formatTests3.number, formatTests3.precision, formatTests3.locale)))
	c.Assert(err, IsNil)
	c.Assert(f3, NotNil)
	r3, err := evalBuiltinFunc(f3, chunk.Event{})
	c.Assert(types.NewCauset(err), solitonutil.CausetEquals, types.NewCauset(errors.New("not support for the specific locale")))
	c.Assert(r3, solitonutil.CausetEquals, types.NewCauset(formatTests3.ret))

	f4, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(formatTests4.number, formatTests4.precision, formatTests4.locale)))
	c.Assert(err, IsNil)
	c.Assert(f4, NotNil)
	r4, err := evalBuiltinFunc(f4, chunk.Event{})
	c.Assert(err, IsNil)
	c.Assert(r4, solitonutil.CausetEquals, types.NewCauset(formatTests4.ret))
	warnings := s.ctx.GetStochaseinstein_dbars().StmtCtx.GetWarnings()
	c.Assert(len(warnings), Equals, 1)
...
func (s *testEvaluatorSuite) TestFromBase64(c *C) {
	...
	for _, test := range tests {
		f, err := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs(types.MakeCausets(test.args)))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		result, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestFromBase64Sig(c *C) {
	...
	for _, test := range tests {
		resultType := &types.FieldType{Tp: allegrosql.TypeVarchar, Flen: allegrosql.MaxBlobWidth}
		base := baseBuiltinFunc{args: args, ctx: s.ctx, tp: resultType}
		fromBase64 := &builtinFromBase64Sig{base, test.maxAllowPacket}

		input := chunk.NewChunkWithCapacity(defCausTypes, 1)
		input.AppendString(0, test.args)
		res, isNull, err := fromBase64.evalString(input.GetEvent(0))
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestOrd(c *C) {
	...
	for _, t := range cases {
		f, err := newFunctionForTest(s.ctx, ast.Ord, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{t.args})...)
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestExportSet(c *C) {
	...
	for _, t := range estd {
		...
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		exportSetRes, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestBin(c *C) {
	...
	for _, t := range dtbl {
		f, err := fc.getFunction(ctx, s.datumsToCouplingConstantWithRadixs(t["Input"]))
		c.Assert(err, IsNil)
		c.Assert(f, NotNil)
		r, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(r, solitonutil.CausetEquals, types.NewCauset(t["Expected"][0]))
	}
}
...
func (s *testEvaluatorSuite) TestToBase64(c *C) {
	...
	for _, test := range tests {
		f, err := newFunctionForTest(s.ctx, ast.ToBase64, s.primitiveValsToCouplingConstantWithRadixs([]interface{}{test.args})...)
		c.Assert(err, IsNil)
		d, err := f.Eval(chunk.Event{})
		if test.getErr {
			c.Assert(err, NotNil)
...
func (s *testEvaluatorSuite) TestToBase64Sig(c *C) {
	...
	for _, test := range tests {
		resultType := &types.FieldType{Tp: allegrosql.TypeVarchar, Flen: base64NeededEncodedLength(len(test.args))}
		base := baseBuiltinFunc{args: args, ctx: s.ctx, tp: resultType}
		toBase64 := &builtinToBase64Sig{base, test.maxAllowPacket}

		input := chunk.NewChunkWithCapacity(defCausTypes, 1)
		input.AppendString(0, test.args)
		res, isNull, err := toBase64.evalString(input.GetEvent(0))
		c.Assert(err, IsNil)
		if test.isNil {
			c.Assert(isNull, IsTrue)
...
func (s *testEvaluatorSuite) TestStringRight(c *C) {
	...
	for _, test := range tests {
		str := types.NewCauset(test.str)
		length := types.NewCauset(test.length)
		f, _ := fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs([]types.Causet{str, length}))
		result, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		if result.IsNull() {
...
func (s *testEvaluatorSuite) TestWeightString(c *C) {
	...
	for _, test := range tests {
		str := types.NewCauset(test.expr)
		var f builtinFunc
		var err error
		if test.padding == "NONE" {
			f, err = fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs([]types.Causet{str}))
		} else {
			padding := types.NewCauset(test.padding)
			length := types.NewCauset(test.length)
			f, err = fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs([]types.Causet{str, padding, length}))
		}
		c.Assert(err, IsNil)
		// Reset warnings.
		s.ctx.GetStochaseinstein_dbars().StmtCtx.ResetForRetry()
		result, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		if result.IsNull() {
			c.Assert(test.expect, IsNil)
		...
		if test.padding == "BINARY" && test.length < len(strExpr) {
			...
			obtainedWarns := s.ctx.GetStochaseinstein_dbars().StmtCtx.GetWarnings()
...
func (s *testEvaluatorSerialSuites) TestCIWeightString(c *C) {
	...
	checkResult := func(defCauslation string, tests []weightStringTest) {
		fc := funcs[ast.WeightString]
		for _, test := range tests {
			str := types.NewDefCauslationStringCauset(test.str, defCauslation, utf8.RuneCountInString(test.str))
			var f builtinFunc
			var err error
			if test.padding == "NONE" {
				f, err = fc.getFunction(s.ctx, s.datumsToCouplingConstantWithRadixs([]types.Causet{str}))
