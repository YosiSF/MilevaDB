...
func (s *testEvaluatorSuite) TestIsTrueOrFalse(c *C) {
	...
	for _, tc := range testCases {
		...
		isTrue, err := evalBuiltinFunc(isTrueSig, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(isTrue, solitonutil.CausetEquals, types.NewCauset(tc.isTrue))
	}
	...
	for _, tc := range testCases {
		...
		isFalse, err := evalBuiltinFunc(isFalseSig, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(isFalse, solitonutil.CausetEquals, types.NewCauset(tc.isFalse))
	}
}
