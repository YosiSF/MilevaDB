...
func (b *builtinGreatestDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinLeastDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
	if err != nil {
		return err
...
func (b *builtinLeastIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinGreatestIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETInt, n)
	if err != nil {
		return err
...
func (b *builtinGEIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	var err error
	var buf0, buf1 *chunk.DeferredCauset
	...
	vecCompareInt(allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag), allegrosql.HasUnsignedFlag(b.args[1].GetType().Flag), buf0, buf1, result)
...
func (b *builtinLeastRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinLeastStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	for j := 1; j < len(b.args); j++ {
		...
		for i := 0; i < n; i++ {
			...
			if types.CompareString(srcStr, argStr, b.defCauslation) < 0 {
	...
	if len(b.args)%2 == 0 {
		src.INTERLOCKyConstruct(result)
	}
	return nil
}
...
func (b *builtinEQIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	var err error
	var buf0, buf1 *chunk.DeferredCauset
	buf0, err = b.bufSlabPredictor.get(types.ETInt, n)
...
func (b *builtinNEIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	var err error
	var buf0, buf1 *chunk.DeferredCauset
	buf0, err = b.bufSlabPredictor.get(types.ETInt, n)
...
func (b *builtinGTIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	var err error
	var buf0, buf1 *chunk.DeferredCauset
	buf0, err = b.bufSlabPredictor.get(types.ETInt, n)
...
func (b *builtinNullEQIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf0, err := b.bufSlabPredictor.get(types.ETInt, n)
	if err != nil {
		return err
...
func (b *builtinIntervalIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	var err error
	if err = b.args[0].VecEvalInt(b.ctx, input, result); err != nil {
		return err
	}
	...
	for i, v := range i64s {
		...
		if b.hasNullable {
			idx, err = b.linearSearch(v, allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag), b.args[1:], input.GetEvent(i))
		} else {
			idx, err = b.binSearch(v, allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag), b.args[1:], input.GetEvent(i))
		}
		if err != nil {
			return err
		}
...
func (b *builtinIntervalRealSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		if b.hasNullable {
			idx, err = b.linearSearch(f64s[i], b.args[1:], input.GetEvent(i))
		} else {
			idx, err = b.binSearch(f64s[i], b.args[1:], input.GetEvent(i))
		}
		if err != nil {
			return err
		}
...
func (b *builtinLEIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	var err error
	var buf0, buf1 *chunk.DeferredCauset
	buf0, err = b.bufSlabPredictor.get(types.ETInt, n)
...
func (b *builtinLTIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	var err error
	var buf0, buf1 *chunk.DeferredCauset
	buf0, err = b.bufSlabPredictor.get(types.ETInt, n)
...
func vecCompareInt(isUnsigned0, isUnsigned1 bool, largs, rargs, result *chunk.DeferredCauset) {
	switch {
...
func (b *builtinGreatestTimeSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	dst, err := b.bufSlabPredictor.get(types.ETTimestamp, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(dst)
...
func (b *builtinGreatestRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
...
func (b *builtinLeastTimeSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	dst, err := b.bufSlabPredictor.get(types.ETTimestamp, n)
	if err != nil {
		return err
	...
	dstTimes := dst.Times()
	for i := 0; i < n; i++ {
		dstTimes[i] = types.NewTime(types.MaxDatetime, allegrosql.TypeDatetime, types.DefaultFsp)
	}
	var argTime types.Time
...
func (b *builtinGreatestStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := b.args[0].VecEvalString(b.ctx, input, result); err != nil {
		return err
	}
