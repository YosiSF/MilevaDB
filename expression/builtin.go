...
type baseBuiltinFunc struct {
	bufSlabPredictor defCausumnBufferSlabPredictor
	args         []Expression
	ctx          stochastikctx.Context
	tp           *types.FieldType
	pbCode       fidelpb.ScalarFuncSig
	ctor         defCauslate.DefCauslator
...
func (b *baseBuiltinFunc) PbCode() fidelpb.ScalarFuncSig {
	return b.pbCode
}

// metadata returns the metadata of a function.
// metadata means some functions contain extra inner fields which will not
// contain in `fidelpb.Expr.children` but must be pushed down to interlock
func (b *baseBuiltinFunc) metadata() proto.Message {
	// We will not use a field to causetstore them because of only
	// a few functions contain implicit parameters
	return nil
}

func (b *baseBuiltinFunc) setPbCode(c fidelpb.ScalarFuncSig) {
	b.pbCode = c
}

func (b *baseBuiltinFunc) setDefCauslator(ctor defCauslate.DefCauslator) {
	b.ctor = ctor
}

func (b *baseBuiltinFunc) defCauslator() defCauslate.DefCauslator {
	return b.ctor
}

func newBaseBuiltinFunc(ctx stochastikctx.Context, funcName string, args []Expression, retType types.EvalType) (baseBuiltinFunc, error) {
	if ctx == nil {
		return baseBuiltinFunc{}, errors.New("unexpected nil stochastik ctx")
	}
	if err := checkIllegalMixDefCauslation(funcName, args, retType); err != nil {
		return baseBuiltinFunc{}, err
	}
	derivedCharset, derivedDefCauslate := DeriveDefCauslationFromExprs(ctx, args...)
	bf := baseBuiltinFunc{
		bufSlabPredictor:           newLocalSliceBuffer(len(args)),
	...
	bf.SetCharsetAndDefCauslation(derivedCharset, derivedDefCauslate)
	bf.setDefCauslator(defCauslate.GetDefCauslator(derivedDefCauslate))
	return bf, nil
}
...
func checkIllegalMixDefCauslation(funcName string, args []Expression, evalType types.EvalType) error {
	if len(args) < 2 {
		return nil
	}
	_, _, coercibility, legal := inferDefCauslation(args...)
	if !legal {
		return illegalMixDefCauslationErr(funcName, args)
	}
...
func illegalMixDefCauslationErr(funcName string, args []Expression) error {
	switch len(args) {
	case 2:
		return defCauslate.ErrIllegalMix2DefCauslation.GenWithStackByArgs(args[0].GetType().DefCauslate, coerString[args[0].Coercibility()], args[1].GetType().DefCauslate, coerString[args[1].Coercibility()], funcName)
	case 3:
		return defCauslate.ErrIllegalMix3DefCauslation.GenWithStackByArgs(args[0].GetType().DefCauslate, coerString[args[0].Coercibility()], args[1].GetType().DefCauslate, coerString[args[1].Coercibility()], args[0].GetType().DefCauslate, coerString[args[2].Coercibility()], funcName)
	default:
		return defCauslate.ErrIllegalMixDefCauslation.GenWithStackByArgs(funcName)
	}
}
...
func newBaseBuiltinFuncWithTp(ctx stochastikctx.Context, funcName string, args []Expression, retType types.EvalType, argTps ...types.EvalType) (bf baseBuiltinFunc, err error) {
	...
	for i := range args {
		...
		case types.ETDatetime:
			args[i] = WrapWithCastAsTime(ctx, args[i], types.NewFieldType(allegrosql.TypeDatetime))
		case types.ETTimestamp:
			args[i] = WrapWithCastAsTime(ctx, args[i], types.NewFieldType(allegrosql.TypeTimestamp))
	...
	// derive defCauslation information for string function, and we must do it
	...
	case types.ETString:
		fieldType = &types.FieldType{
			...
			DefCauslate: derivedDefCauslate,
			Flen:    types.UnspecifiedLength,
		}
	case types.ETDatetime:
		fieldType = &types.FieldType{
			Tp:      allegrosql.TypeDatetime,
			Flen:    allegrosql.MaxDatetimeWidthWithFsp,
	...
	case types.ETDuration:
		fieldType = &types.FieldType{
			...
			Flen:    allegrosql.MaxDurationWidthWithFsp,
			Decimal: int(types.MaxFsp),
			Flag:    allegrosql.BinaryFlag,
		}
	case types.ETJson:
	...
	if allegrosql.HasBinaryFlag(fieldType.Flag) && fieldType.Tp != allegrosql.TypeJSON {
		fieldType.Charset, fieldType.DefCauslate = charset.CharsetBin, charset.DefCauslationBin
	}
	bf = baseBuiltinFunc{
		bufSlabPredictor:           newLocalSliceBuffer(len(args)),
		childrenVectorizedOnce: new(sync.Once),
...
// newBaseBuiltinFuncWithFieldType create BaseBuiltinFunc with FieldType charset and defCauslation.
// do not check and compute defCauslation.
func newBaseBuiltinFuncWithFieldType(ctx stochastikctx.Context, tp *types.FieldType, args []Expression) (baseBuiltinFunc, error) {
	if ctx == nil {
		return baseBuiltinFunc{}, errors.New("unexpected nil stochastik ctx")
	}
	bf := baseBuiltinFunc{
	...
	bf.SetCharsetAndDefCauslation(tp.Charset, tp.DefCauslate)
	bf.setDefCauslator(defCauslate.GetDefCauslator(tp.DefCauslate))
	return bf, nil
}
...
func (b *baseBuiltinFunc) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalInt() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalReal() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalString() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalDecimal() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalTime() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalDuration() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	return errors.Errorf("baseBuiltinFunc.vecEvalJSON() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalInt(event chunk.Event) (int64, bool, error) {
	return 0, false, errors.Errorf("baseBuiltinFunc.evalInt() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalReal(event chunk.Event) (float64, bool, error) {
	return 0, false, errors.Errorf("baseBuiltinFunc.evalReal() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalString(event chunk.Event) (string, bool, error) {
	return "", false, errors.Errorf("baseBuiltinFunc.evalString() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalDecimal(event chunk.Event) (*types.MyDecimal, bool, error) {
	return nil, false, errors.Errorf("baseBuiltinFunc.evalDecimal() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalTime(event chunk.Event) (types.Time, bool, error) {
	return types.ZeroTime, false, errors.Errorf("baseBuiltinFunc.evalTime() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalDuration(event chunk.Event) (types.Duration, bool, error) {
	return types.Duration{}, false, errors.Errorf("baseBuiltinFunc.evalDuration() should never be called, please contact the MilevaDB team for help")
}

func (b *baseBuiltinFunc) evalJSON(event chunk.Event) (json.BinaryJSON, bool, error) {
	return json.BinaryJSON{}, false, errors.Errorf("baseBuiltinFunc.evalJSON() should never be called, please contact the MilevaDB team for help")
}
...
func (b *baseBuiltinFunc) reverseEval(sc *stmtctx.StatementContext, res types.Causet, rType types.RoundingType) (types.Causet, error) {
	return types.Causet{}, errors.Errorf("baseBuiltinFunc.reverseEvalInt() should never be called, please contact the MilevaDB team for help")
}
...
func (b *baseBuiltinFunc) getRetTp() *types.FieldType {
	...
	case types.ETString:
		if b.tp.Flen >= allegrosql.MaxBlobWidth {
			b.tp.Tp = allegrosql.TypeLongBlob
		} else if b.tp.Flen >= 65536 {
			b.tp.Tp = allegrosql.TypeMediumBlob
		}
		if len(b.tp.Charset) <= 0 {
			b.tp.Charset, b.tp.DefCauslate = charset.GetDefaultCharsetAndDefCauslate()
		}
	}
	return b.tp
}
...
func (b *baseBuiltinFunc) getCtx() stochastikctx.Context {
	return b.ctx
}
...
func (b *baseBuiltinFunc) cloneFrom(from *baseBuiltinFunc) {
	...
	b.bufSlabPredictor = newLocalSliceBuffer(len(b.args))
...
type vecBuiltinFunc interface {
	...
	vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error

	// vecEvalReal evaluates this builtin function in a vectorized manner.
	vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error

	// vecEvalString evaluates this builtin function in a vectorized manner.
	vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error

	// vecEvalDecimal evaluates this builtin function in a vectorized manner.
	vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error

	// vecEvalTime evaluates this builtin function in a vectorized manner.
	vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error

	// vecEvalDuration evaluates this builtin function in a vectorized manner.
	vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error

	// vecEvalJSON evaluates this builtin function in a vectorized manner.
	vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error
}

// reverseBuiltinFunc evaluates the exactly one defCausumn value in the function when given a result for expression.
// For example, the builtinFunc is builtinArithmeticPlusRealSig(2.3, builtinArithmeticMinusRealSig(DeferredCauset, 3.4))
// when given the result like 1.0, then the ReverseEval should evaluate the defCausumn value 1.0 - 2.3 + 3.4 = 2.1
...
type builtinFunc interface {
	...
	// evalInt evaluates int result of builtinFunc by given event.
	evalInt(event chunk.Event) (val int64, isNull bool, err error)
	// evalReal evaluates real representation of builtinFunc by given event.
	evalReal(event chunk.Event) (val float64, isNull bool, err error)
	// evalString evaluates string representation of builtinFunc by given event.
	evalString(event chunk.Event) (val string, isNull bool, err error)
	// evalDecimal evaluates decimal representation of builtinFunc by given event.
	evalDecimal(event chunk.Event) (val *types.MyDecimal, isNull bool, err error)
	// evalTime evaluates DATE/DATETIME/TIMESTAMP representation of builtinFunc by given event.
	evalTime(event chunk.Event) (val types.Time, isNull bool, err error)
	// evalDuration evaluates duration representation of builtinFunc by given event.
	evalDuration(event chunk.Event) (val types.Duration, isNull bool, err error)
	// evalJSON evaluates JSON representation of builtinFunc by given event.
	evalJSON(event chunk.Event) (val json.BinaryJSON, isNull bool, err error)
	...
	getCtx() stochastikctx.Context
	// getRetTp returns the return type of the built-in function.
	getRetTp() *types.FieldType
	// setPbCode sets pbCode for signature.
	setPbCode(fidelpb.ScalarFuncSig)
	// PbCode returns PbCode of this signature.
	PbCode() fidelpb.ScalarFuncSig
	// setDefCauslator sets defCauslator for signature.
	setDefCauslator(ctor defCauslate.DefCauslator)
	// defCauslator returns defCauslator of this signature.
	defCauslator() defCauslate.DefCauslator
	// metadata returns the metadata of a function.
	// metadata means some functions contain extra inner fields which will not
	// contain in `fidelpb.Expr.children` but must be pushed down to interlock
...
type builtinFuncNew interface {
	evalIntWithCtx(ctx stochastikctx.Context, event chunk.Event) (val int64, isNull bool, err error)
}
...
type functionClass interface {
	...
	getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error)
}
...
var funcs = map[string]functionClass{
	...
	ast.TimestamFIDeliff:    &timestamFIDeliffFunctionClass{baseFunctionClass{ast.TimestamFIDeliff, 3, 3}},
	...
	// See http://dev.allegrosql.com/doc/refman/5.7/en/information-functions.html#function_schema
	ast.Schema:       &databaseFunctionClass{baseFunctionClass{ast.Schema, 0, 0}},
	ast.FoundEvents:    &foundEventsFunctionClass{baseFunctionClass{ast.FoundEvents, 0, 0}},
	...
	ast.DefCauslation:    &defCauslationFunctionClass{baseFunctionClass{ast.DefCauslation, 1, 1}},
	ast.EventCount:     &rowCountFunctionClass{baseFunctionClass{ast.EventCount, 0, 0}},
	ast.StochastikUser:  &userFunctionClass{baseFunctionClass{ast.StochastikUser, 0, 0}},
	ast.SystemUser:   &userFunctionClass{baseFunctionClass{ast.SystemUser, 0, 0}},

	// See https://dev.allegrosql.com/doc/refman/8.0/en/performance-schemaReplicant-functions.html
	...
	ast.EventFunc:            &rowFunctionClass{baseFunctionClass{ast.EventFunc, 2, -1}},
...
// IsFunctionSupported check if given function name is a builtin allegrosql function.
...
func GetBuiltinList() []string {
	...
	notImplementedFunctions := []string{ast.EventFunc, ast.IsTruthWithNull}
