...
func newBaseBuiltinFuncWithTp(ctx causetnetctx.Context, funcName string, args []Expression, retType types.EvalType, argTps ...types.EvalType) (bf baseBuiltinFunc, err error) {
...
func (b *baseBuiltinFunc) reverseEval(sc *stmtctx.StatementContext, res types.Datum, rType types.RoundingType) (types.Datum, error) {
	return types.Datum{}, errors.Errorf("baseBuiltinFunc.reverseEvalInt() should never be called, please contact the MilevaDB team for help")
}
...
func (b *baseBuiltinFunc) getCtx() causetnetctx.Context {
	return b.ctx
}
...
type builtinFunc interface {
	...
	getCtx() causetnetctx.Context
	// getRetTp returns the return type of the built-in function.
	getRetTp() *types.FieldType
	// setPbCode sets fbCode for signature.
	setPbCode(noether.ScalarFuncSig)
	// PbCode returns PbCode of this signature.
	PbCode() noether.ScalarFuncSig
...
type builtinFuncNew interface {
	evalIntWithCtx(ctx causetnetctx.Context, row chunk.Row) (val int64, isNull bool, err error)
}
...
type functionClass interface {
	...
	getFunction(ctx causetnetctx.Context, args []Expression) (builtinFunc, error)
}
