...
func (b *builtinLowerSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinRepeatSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinStringIsNullSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinUpperUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := b.args[0].VecEvalString(b.ctx, input, result); err != nil {
		return err
	}
...
func (b *builtinUpperSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinLeftUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinRightUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinSpaceSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	for i := 0; i < n; i++ {
		...
		if num > allegrosql.MaxBlobWidth {
			result.AppendNull()
			continue
		}
...
func (b *builtinReverseUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := b.args[0].VecEvalString(b.ctx, input, result); err != nil {
		return err
	}
	for i := 0; i < input.NumEvents(); i++ {
...
func (b *builtinConcatSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinLocate3ArgsUTF8Sig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	// causetstore positions in result
	...
	i64s := result.Int64s()
	ci := defCauslate.IsCIDefCauslation(b.defCauslation)
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
			continue
		}
...
func (b *builtinHexStrArgSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinLTrimSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinQuoteSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinInsertSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	str, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(str)
	...
	defer b.bufSlabPredictor.put(pos)
	...
	defer b.bufSlabPredictor.put(length)
	if err := b.args[2].VecEvalInt(b.ctx, input, length); err != nil {
		return err
	}
	newstr, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(newstr)
...
func (b *builtinConcatWSSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	argsLen := len(b.args)

	bufs := make([]*chunk.DeferredCauset, argsLen)
	var err error
	for i := 0; i < argsLen; i++ {
		bufs[i], err = b.bufSlabPredictor.get(types.ETString, n)
		if err != nil {
			return err
		}
		defer b.bufSlabPredictor.put(bufs[i])
...
func (b *builtinConvertSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	expr, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(expr)
...
func (b *builtinSubstringIndexSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinUnHexSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinExportSet3ArgSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	defer b.bufSlabPredictor.put(bits)
	if err := b.args[0].VecEvalInt(b.ctx, input, bits); err != nil {
		return err
	}
	on, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(on)
	if err := b.args[1].VecEvalString(b.ctx, input, on); err != nil {
		return err
	}
	off, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(off)
...
func (b *builtinASCIISig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinLpadSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	strBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(strBuf)
	...
	defer b.bufSlabPredictor.put(lenBuf)
	if err := b.args[1].VecEvalInt(b.ctx, input, lenBuf); err != nil {
		return err
	}
	padBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(padBuf)
...
// vecEvalString evals LPAD(str,len,padstr).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_lpad
func (b *builtinLpadUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	buf2, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(buf2)
...
func (b *builtinFindInSetSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	str, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	strlist, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(strlist)
...
func (b *builtinLeftSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinReverseSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := b.args[0].VecEvalString(b.ctx, input, result); err != nil {
		return err
	}
	for i := 0; i < input.NumEvents(); i++ {
...
func (b *builtinRTrimSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinStrcmpSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	leftBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(leftBuf)
	if err := b.args[0].VecEvalString(b.ctx, input, leftBuf); err != nil {
		return err
	}
	rightBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(rightBuf)
	...
	for i := 0; i < n; i++ {
		...
		i64s[i] = int64(types.CompareString(leftBuf.GetString(i), rightBuf.GetString(i), b.defCauslation))
	}
	return nil
}
...
func (b *builtinLocate2ArgsSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf0, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate3ArgsSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf0, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinExportSet4ArgSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	separator, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(separator)
...
func (b *builtinRpadSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	strBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinFormatWithLocaleSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	defer b.bufSlabPredictor.put(dBuf)
	if err := b.args[1].VecEvalInt(b.ctx, input, dBuf); err != nil {
		return err
	}
	dInt64s := dBuf.Int64s()

	localeBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(localeBuf)
	...
	if b.args[0].GetType().EvalType() == types.ETDecimal {
		xBuf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
		if err != nil {
			return err
		}
		defer b.bufSlabPredictor.put(xBuf)
	...
	// real x
	xBuf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(xBuf)
...
func (b *builtinSubstring2ArgsSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinSubstring2ArgsUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinTrim2ArgsSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinInstrUTF8Sig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	str, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	substr, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(substr)
...
func (b *builtinOctStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinEltSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	for i := 0; i < n; i++ {
		...
		if bufs[j] == nil {
			bufs[j], err = b.bufSlabPredictor.get(types.ETString, n)
			if err != nil {
				return err
			}
			defer b.bufSlabPredictor.put(bufs[j])
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_insert
func (b *builtinInsertUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	buf3, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(buf3)
...
func (b *builtinExportSet5ArgSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	defer b.bufSlabPredictor.put(numberOfBits)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinTrim3ArgsSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf0, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinOrdSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinInstrSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	str, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinLengthSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate2ArgsUTF8Sig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinBitLengthSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCharSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	l := len(b.args)
	...
	for i := 0; i < len(b.args)-1; i++ {
		...
		buf[i] = te
		defer b.bufSlabPredictor.put(buf[i])
		if err := b.args[i].VecEvalInt(b.ctx, input, buf[i]); err != nil {
			return err
		}
	}
	bufstr, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(bufstr)
...
func (b *builtinReplaceSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinMakeSetSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	defer b.bufSlabPredictor.put(bitsBuf)
	if err := b.args[0].VecEvalInt(b.ctx, input, bitsBuf); err != nil {
		return err
	}

	strBuf := make([]*chunk.DeferredCauset, len(b.args)-1)
	for i := 1; i < len(b.args); i++ {
		strBuf[i-1], err = b.bufSlabPredictor.get(types.ETString, nr)
		if err != nil {
			return err
		}
		defer b.bufSlabPredictor.put(strBuf[i-1])
...
func (b *builtinToBase64Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinTrim1ArgSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
// vecEvalString evals RPAD(str,len,padstr).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_rpad
func (b *builtinRpadUTF8Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCharLengthBinarySig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinRightSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
// vecEvalString evals SUBSTR(str,pos,len), SUBSTR(str FROM pos FOR len), SUBSTR() is a synonym for SUBSTRING().
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinFromBase64Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCharLengthUTF8Sig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func formatDecimal(sctx stochastikctx.Context, xBuf *chunk.DeferredCauset, dInt64s []int64, result *chunk.DeferredCauset, localeBuf *chunk.DeferredCauset) error {
	...
	for i := range xDecimals {
		...
		localeFormatFunction := allegrosql.GetLocaleFormatFunction(locale)
...
func formatReal(sctx stochastikctx.Context, xBuf *chunk.DeferredCauset, dInt64s []int64, result *chunk.DeferredCauset, localeBuf *chunk.DeferredCauset) error {
