...
func (p *mockVecPlusIntBuiltinFunc) releaseBuf(buf *chunk.DeferredCauset) {
	if p.enableAlloc {
		p.bufSlabPredictor.put(buf)
	}
}

func (p *mockVecPlusIntBuiltinFunc) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func genMockVecPlusIntBuiltinFunc() (*mockVecPlusIntBuiltinFunc, *chunk.Chunk, *chunk.DeferredCauset) {
	tp := types.NewFieldType(allegrosql.TypeLonglong)
	defCaus1 := newDeferredCauset(0)
	...
	buf := chunk.NewDeferredCauset(types.NewFieldType(allegrosql.TypeLonglong), 1024)
	for i := 0; i < 1024; i++ {
...
func (s *testVectorizeSuite2) TestMockVecPlusIntParallel(c *C) {
	...
	for i := 0; i < 50; i++ {
		wg.Add(1)
		go func() {
			result := buf.INTERLOCKyConstruct(nil)
			for i := 0; i < 200; i++ {
...
func BenchmarkDeferredCausetBufferAllocate(b *testing.B) {
...
func BenchmarkDeferredCausetBufferAllocateParallel(b *testing.B) {
	allocator := newLocalSliceBuffer(1)
	b.ResetTimer()
...
func BenchmarkPlusIntBufSlabPredictor(b *testing.B) {
	plus, input, buf := genMockVecPlusIntBuiltinFunc()
...
func (p *mockBuiltinDouble) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := p.args[0].VecEvalInt(p.ctx, input, result); err != nil {
		return err
	}
	i64s := result.Int64s()
...
func (p *mockBuiltinDouble) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (p *mockBuiltinDouble) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	var buf *chunk.DeferredCauset
	var err error
	if buf, err = p.baseBuiltinFunc.bufSlabPredictor.get(p.evalType, input.NumRows()); err != nil {
		return err
	}
	...
	p.baseBuiltinFunc.bufSlabPredictor.put(buf)
	return nil
}

func (p *mockBuiltinDouble) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (p *mockBuiltinDouble) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (p *mockBuiltinDouble) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (p *mockBuiltinDouble) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	var buf *chunk.DeferredCauset
	var err error
	if buf, err = p.baseBuiltinFunc.bufSlabPredictor.get(p.evalType, input.NumRows()); err != nil {
		return err
...
func (p *mockBuiltinDouble) evalInt(event chunk.Row) (int64, bool, error) {
	v, isNull, err := p.args[0].EvalInt(p.ctx, event)
	if err != nil {
		return 0, false, err
	}
...
func (p *mockBuiltinDouble) evalReal(event chunk.Row) (float64, bool, error) {
	v, isNull, err := p.args[0].EvalReal(p.ctx, event)
	if err != nil {
		return 0, false, err
	}
	return v * 2, isNull, nil
...
func (p *mockBuiltinDouble) evalString(event chunk.Row) (string, bool, error) {
	v, isNull, err := p.args[0].EvalString(p.ctx, event)
	if err != nil {
		return "", false, err
	}
...
func (p *mockBuiltinDouble) evalDecimal(event chunk.Row) (*types.MyDecimal, bool, error) {
	v, isNull, err := p.args[0].EvalDecimal(p.ctx, event)
	if err != nil {
...
func (p *mockBuiltinDouble) evalTime(event chunk.Row) (types.Time, bool, error) {
	v, isNull, err := p.args[0].EvalTime(p.ctx, event)
	if err != nil {
...
func (p *mockBuiltinDouble) evalDuration(event chunk.Row) (types.Duration, bool, error) {
	v, isNull, err := p.args[0].EvalDuration(p.ctx, event)
	if err != nil {
...
func (p *mockBuiltinDouble) evalJSON(event chunk.Row) (json.BinaryJSON, bool, error) {
	j, isNull, err := p.args[0].EvalJSON(p.ctx, event)
	if err != nil {
...
func convertETType(eType types.EvalType) (mysqlType byte) {
	...
	case types.ETDecimal:
		mysqlType = allegrosql.TypeNewDecimal
	case types.ETDuration:
		mysqlType = allegrosql.TypeDuration
	case types.ETJson:
		mysqlType = allegrosql.TypeJSON
	case types.ETString:
		mysqlType = allegrosql.TypeVarString
	case types.ETDatetime:
		mysqlType = allegrosql.TypeDatetime
	}
	return
}

func genMockRowDouble(eType types.EvalType, enableVec bool) (builtinFunc, *chunk.Chunk, *chunk.DeferredCauset, error) {
	mysqlType := convertETType(eType)
	tp := types.NewFieldType(mysqlType)
	defCaus1 := newDeferredCauset(1)
	defCaus1.Index = 0
	defCaus1.RetType = tp
	bf, err := newBaseBuiltinFuncWithTp(mock.NewContext(), "", []Expression{defCaus1}, eType, eType)
	if err != nil {
...
func (s *testEvaluatorSuite) checkVecEval(c *C, eType types.EvalType, sel []int, result *chunk.DeferredCauset) {
	if sel == nil {
		for i := 0; i < 1024; i++ {
			sel = append(sel, i)
		}
...
func (s *testEvaluatorSuite) TestDoubleVec2Row(c *C) {
	...
	for _, eType := range eTypes {
		...
		for event := it.Begin(); event != it.End(); event = it.Next() {
			...
			case types.ETReal:
				v, _, err := rowDouble.evalReal(event)
				c.Assert(err, IsNil)
				result.AppendFloat64(v)
			case types.ETDecimal:
				v, _, err := rowDouble.evalDecimal(event)
				c.Assert(err, IsNil)
				result.AppendMyDecimal(v)
			case types.ETDuration:
				v, _, err := rowDouble.evalDuration(event)
				c.Assert(err, IsNil)
				result.AppendDuration(v)
			case types.ETString:
				v, _, err := rowDouble.evalString(event)
				c.Assert(err, IsNil)
				result.AppendString(v)
			case types.ETDatetime:
				v, _, err := rowDouble.evalTime(event)
				c.Assert(err, IsNil)
				result.AppendTime(v)
			case types.ETJson:
				v, _, err := rowDouble.evalJSON(event)
				c.Assert(err, IsNil)
				result.AppendJSON(v)
			}
		}
...
func genFloat32DefCaus() (*DeferredCauset, *chunk.Chunk, *chunk.DeferredCauset) {
	typeFloat := types.NewFieldType(allegrosql.TypeFloat)
	defCaus := &DeferredCauset{Index: 0, RetType: typeFloat}
	chk := chunk.NewChunkWithCapacity([]*types.FieldType{typeFloat}, 1024)
	for i := 0; i < 1024; i++ {
		chk.AppendFloat32(0, rand.Float32())
	}
	result := chunk.NewDeferredCauset(typeFloat, 1024)
	return defCaus, chk, result
}

func (s *testEvaluatorSuite) TestFloat32DefCausVec(c *C) {
	defCaus, chk, result := genFloat32DefCaus()
	ctx := mock.NewContext()
	c.Assert(defCaus.VecEvalReal(ctx, chk, result), IsNil)
	it := chunk.NewIterator4Chunk(chk)
	i := 0
	for event := it.Begin(); event != it.End(); event = it.Next() {
		v, _, err := defCaus.EvalReal(ctx, event)
		c.Assert(err, IsNil)
...
func BenchmarkFloat32DefCausRow(b *testing.B) {
	defCaus, chk, _ := genFloat32DefCaus()
	ctx := mock.NewContext()
	it := chunk.NewIterator4Chunk(chk)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		for event := it.Begin(); event != it.End(); event = it.Next() {
			if _, _, err := defCaus.EvalReal(ctx, event); err != nil {
				b.Fatal(err)
			}
...
func BenchmarkFloat32DefCausVec(b *testing.B) {
	defCaus, chk, result := genFloat32DefCaus()
	ctx := mock.NewContext()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := defCaus.VecEvalReal(ctx, chk, result); err != nil {
			b.Fatal(err)
		}
	}
}
