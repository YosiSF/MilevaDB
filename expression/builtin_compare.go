...
func (c *coalesceFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (sig builtinFunc, err error) {
	...
	if resultFieldType.Tp == allegrosql.TypeNull {
		...
		for _, argTp := range fieldTps {
			...
			if !allegrosql.HasUnsignedFlag(argTp.Flag) {
				argIntLen--
			}
		...
		if resultEvalType == types.CausetEDN || resultEvalType == types.ETDecimal {
			...
			if !allegrosql.HasUnsignedFlag(resultFieldType.Flag) {
				resultFieldType.Flen++
			}
		...
		if bf.tp.Flen > allegrosql.MaxDecimalWidth {
			bf.tp.Flen = allegrosql.MaxDecimalWidth
		}
	}
	...
	case types.ETReal:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_CoalesceReal)
	case types.ETDecimal:
		sig = &builtinCoalesceDecimalSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_CoalesceDecimal)
	case types.ETString:
		sig = &builtinCoalesceStringSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_CoalesceString)
	case types.ETDatetime, types.ETTimestamp:
		sig = &builtinCoalesceTimeSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_CoalesceTime)
	case types.ETDuration:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_CoalesceDuration)
	case types.ETJson:
		sig = &builtinCoalesceJSONSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_CoalesceJson)
	}
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_coalesce
...
func (b *builtinCoalesceIntSig) evalInt(event chunk.Event) (res int64, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalInt(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func (b *builtinCoalesceRealSig) evalReal(event chunk.Event) (res float64, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalReal(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func (b *builtinCoalesceDecimalSig) evalDecimal(event chunk.Event) (res *types.MyDecimal, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalDecimal(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func (b *builtinCoalesceStringSig) evalString(event chunk.Event) (res string, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalString(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func (b *builtinCoalesceTimeSig) evalTime(event chunk.Event) (res types.Time, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalTime(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func (b *builtinCoalesceDurationSig) evalDuration(event chunk.Event) (res types.Duration, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalDuration(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func (b *builtinCoalesceJSONSig) evalJSON(event chunk.Event) (res json.BinaryJSON, isNull bool, err error) {
	for _, a := range b.getArgs() {
		res, isNull, err = a.EvalJSON(b.ctx, event)
		if err != nil || !isNull {
			break
		}
	}
...
func temporalWithDateAsNumEvalType(argTp *types.FieldType) (argEvalType types.EvalType, isStr bool, isTemporalWithDate bool) {
	...
	isStr, isTemporalWithDate = argEvalType.IsStringHoTT(), types.IsTemporalWithDate(argTp.Tp)
...
func (c *greatestFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (sig builtinFunc, err error) {
	if err = c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	case types.ETReal:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_GreatestReal)
	case types.ETDecimal:
		sig = &builtinGreatestDecimalSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_GreatestDecimal)
	case types.ETString:
		sig = &builtinGreatestStringSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_GreatestString)
	case types.ETDatetime:
		sig = &builtinGreatestTimeSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_GreatestTime)
	}
	return sig, nil
}
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_greatest
func (b *builtinGreatestIntSig) evalInt(event chunk.Event) (max int64, isNull bool, err error) {
	max, isNull, err = b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return max, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
		var v int64
		v, isNull, err = b.args[i].EvalInt(b.ctx, event)
		if isNull || err != nil {
			return max, isNull, err
		}
		if v > max {
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_greatest
func (b *builtinGreatestRealSig) evalReal(event chunk.Event) (max float64, isNull bool, err error) {
	max, isNull, err = b.args[0].EvalReal(b.ctx, event)
	if isNull || err != nil {
		return max, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
		var v float64
		v, isNull, err = b.args[i].EvalReal(b.ctx, event)
		if isNull || err != nil {
			return max, isNull, err
		}
		if v > max {
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_greatest
func (b *builtinGreatestDecimalSig) evalDecimal(event chunk.Event) (max *types.MyDecimal, isNull bool, err error) {
	max, isNull, err = b.args[0].EvalDecimal(b.ctx, event)
	if isNull || err != nil {
		return max, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
		var v *types.MyDecimal
		v, isNull, err = b.args[i].EvalDecimal(b.ctx, event)
		if isNull || err != nil {
			return max, isNull, err
		}
		if v.Compare(max) > 0 {
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_greatest
func (b *builtinGreatestStringSig) evalString(event chunk.Event) (max string, isNull bool, err error) {
	max, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return max, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
		...
		if types.CompareString(v, max, b.defCauslation) > 0 {
			max = v
		}
	}
	return
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_greatest
func (b *builtinGreatestTimeSig) evalString(event chunk.Event) (_ string, isNull bool, err error) {
	var (
		v string
		t types.Time
	)
...
func (c *leastFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (sig builtinFunc, err error) {
	if err = c.verifyArgs(args); err != nil {
		return nil, err
	}
	tp, cmpAsDatetime := GetCmpTp4MinMax(args), false
	...
	case types.ETReal:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_LeastReal)
	case types.ETDecimal:
		sig = &builtinLeastDecimalSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_LeastDecimal)
	case types.ETString:
		sig = &builtinLeastStringSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_LeastString)
	case types.ETDatetime:
		sig = &builtinLeastTimeSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_LeastTime)
	}
	return sig, nil
}
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_least
func (b *builtinLeastIntSig) evalInt(event chunk.Event) (min int64, isNull bool, err error) {
	min, isNull, err = b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#functionleast
func (b *builtinLeastRealSig) evalReal(event chunk.Event) (min float64, isNull bool, err error) {
	min, isNull, err = b.args[0].EvalReal(b.ctx, event)
	if isNull || err != nil {
		return min, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#functionleast
func (b *builtinLeastDecimalSig) evalDecimal(event chunk.Event) (min *types.MyDecimal, isNull bool, err error) {
	min, isNull, err = b.args[0].EvalDecimal(b.ctx, event)
	if isNull || err != nil {
		return min, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#functionleast
func (b *builtinLeastStringSig) evalString(event chunk.Event) (min string, isNull bool, err error) {
	min, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return min, isNull, err
	}
	for i := 1; i < len(b.args); i++ {
		...
		if types.CompareString(v, min, b.defCauslation) < 0 {
			min = v
		}
	}
	return
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#functionleast
func (b *builtinLeastTimeSig) evalString(event chunk.Event) (res string, isNull bool, err error) {
	var (
		v string
		t types.Time
	)
...
func (c *intervalFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	allInt := true
	hasNullable := false
	// if we have nullable defCausumns in the argument list, we won't do a binary search, instead we will linearly scan the arguments.
	// this behavior is in line with MyALLEGROSQL's, see MyALLEGROSQL's source code here:
	// https://github.com/allegrosql/allegrosql-server/blob/f8cdce86448a211511e8a039c62580ae16cb96f5/allegrosql/item_cmpfunc.cc#L2713-L2788
	...
	if allInt {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_IntervalInt)
	} else {
		sig = &builtinIntervalRealSig{bf, hasNullable}
		sig.setPbCode(fidelpb.ScalarFuncSig_IntervalReal)
	}
	return sig, nil
}
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_interval
func (b *builtinIntervalIntSig) evalInt(event chunk.Event) (int64, bool, error) {
	...
	isUint1 := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag)
	var idx int
	if b.hasNullable {
		idx, err = b.linearSearch(arg0, isUint1, b.args[1:], event)
	} else {
		idx, err = b.binSearch(arg0, isUint1, b.args[1:], event)
	}
...
func (b *builtinIntervalIntSig) linearSearch(target int64, isUint1 bool, args []Expression, event chunk.Event) (i int, err error) {
	i = 0
	for ; i < len(args); i++ {
		isUint2 := allegrosql.HasUnsignedFlag(args[i].GetType().Flag)
		arg, isNull, err := args[i].EvalInt(b.ctx, event)
		if err != nil {
			return 0, err
		}
		var less bool
...
func (b *builtinIntervalIntSig) binSearch(target int64, isUint1 bool, args []Expression, event chunk.Event) (_ int, err error) {
	i, j, cmp := 0, len(args), false
	for i < j {
		mid := i + (j-i)/2
		v, isNull, err1 := args[mid].EvalInt(b.ctx, event)
		if err1 != nil {
			err = err1
			break
		}
		...
		isUint2 := allegrosql.HasUnsignedFlag(args[mid].GetType().Flag)
		switch {
		case !isUint1 && !isUint2:
			cmp = target < v
		case isUint1 && isUint2:
...
// See http://dev.allegrosql.com/doc/refman/5.7/en/comparison-operators.html#function_interval
func (b *builtinIntervalRealSig) evalInt(event chunk.Event) (int64, bool, error) {
	...
	var idx int
	if b.hasNullable {
		idx, err = b.linearSearch(arg0, b.args[1:], event)
	} else {
		idx, err = b.binSearch(arg0, b.args[1:], event)
	}
	return int64(idx), err != nil, err
}

func (b *builtinIntervalRealSig) linearSearch(target float64, args []Expression, event chunk.Event) (i int, err error) {
	i = 0
	for ; i < len(args); i++ {
		arg, isNull, err := args[i].EvalReal(b.ctx, event)
		if err != nil {
			return 0, err
		}
...
func (b *builtinIntervalRealSig) binSearch(target float64, args []Expression, event chunk.Event) (_ int, err error) {
	i, j := 0, len(args)
	for i < j {
		mid := i + (j-i)/2
		v, isNull, err1 := args[mid].EvalReal(b.ctx, event)
		if err1 != nil {
			err = err1
			break
		}
...
func GetAccurateCmpType(lhs, rhs Expression) types.EvalType {
	...
	if (lhsEvalType.IsStringHoTT() && rhsFieldType.Tp == allegrosql.TypeJSON) ||
		(lhsFieldType.Tp == allegrosql.TypeJSON && rhsEvalType.IsStringHoTT()) {
	...
	} else if lhsFieldType.Tp == allegrosql.TypeDuration && rhsFieldType.Tp == allegrosql.TypeDuration {
		...
		} else if isTemporalDeferredCauset(lhs) && isRHSConst ||
			isTemporalDeferredCauset(rhs) && isLHSConst {
			/*
				<temporal defCausumn> <cmp> <non-temporal constant>
				or
				<non-temporal constant> <cmp> <temporal defCausumn>

				Convert the constant to temporal type.
			*/
			defCaus, isLHSDeferredCauset := lhs.(*DeferredCauset)
			if !isLHSDeferredCauset {
				defCaus = rhs.(*DeferredCauset)
			}
			if defCaus.GetType().Tp == allegrosql.TypeDuration {
				cmpType = types.ETDuration
			} else {
				cmpType = types.ETDatetime
			}
...
func GetCmpFunction(ctx stochastikctx.Context, lhs, rhs Expression) CompareFunc {
	...
	case types.ETString:
		_, dstDefCauslation := DeriveDefCauslationFromExprs(ctx, lhs, rhs)
		return genCompareString(dstDefCauslation)
	case types.ETDuration:
...
// isTemporalDeferredCauset checks if a expression is a temporal defCausumn,
// temporal defCausumn indicates time defCausumn or duration defCausumn.
func isTemporalDeferredCauset(expr Expression) bool {
	ft := expr.GetType()
	if _, isDefCaus := expr.(*DeferredCauset); !isDefCaus {
		return false
	}
	if !types.IsTypeTime(ft.Tp) && ft.Tp != allegrosql.TypeDuration {
		return false
	}
	return true
}
...
func RefineComparedCouplingConstantWithRadix(ctx stochastikctx.Context, targetFieldType types.FieldType, con *CouplingConstantWithRadix, op opcode.Op) (_ *CouplingConstantWithRadix, isExceptional bool) {
	...
	c, err := intCauset.CompareCauset(sc, &con.Value)
	if err != nil {
		return con, false
	}
	if c == 0 {
	...
	case opcode.NullEQ, opcode.EQ:
		...
		case types.ETString:
			...
			var doubleCauset types.Causet
			doubleCauset, err = dt.ConvertTo(sc, types.NewFieldType(allegrosql.TypeDouble))
			if err != nil {
				return con, false
			}
			if c, err = doubleCauset.CompareCauset(sc, &intCauset); err != nil {
				return con, false
			}
			if c != 0 {
				return con, true
...
// refineArgs will rewrite the arguments if the compare expression is `int defCausumn <cmp> non-int constant` or
// `non-int constant <cmp> int defCausumn`. E.g., `a < 1.1` will be rewritten to `a < 2`.
func (c *compareFunctionClass) refineArgs(ctx stochastikctx.Context, args []Expression) []Expression {
	if ContainMublockConst(ctx, args) {
		return args
	}
...
func (c *compareFunctionClass) getFunction(ctx stochastikctx.Context, rawArgs []Expression) (sig builtinFunc, err error) {
...
func (c *compareFunctionClass) generateCmpSigs(ctx stochastikctx.Context, args []Expression, tp types.EvalType) (sig builtinFunc, err error) {
	...
	case types.CausetEDN:
		...
		case opcode.LT:
			...
			sig.setPbCode(fidelpb.ScalarFuncSig_LTInt)
		case opcode.LE:
			sig = &builtinLEIntSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LEInt)
		case opcode.GT:
			sig = &builtinGTIntSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTInt)
		case opcode.EQ:
			sig = &builtinEQIntSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQInt)
		case opcode.GE:
			sig = &builtinGEIntSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GEInt)
		case opcode.NE:
			sig = &builtinNEIntSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NEInt)
		case opcode.NullEQ:
			sig = &builtinNullEQIntSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQInt)
		}
	case types.ETReal:
		switch c.op {
		case opcode.LT:
			sig = &builtinLTRealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LTReal)
		case opcode.LE:
			sig = &builtinLERealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LEReal)
		case opcode.GT:
			sig = &builtinGTRealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTReal)
		case opcode.GE:
			sig = &builtinGERealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GEReal)
		case opcode.EQ:
			sig = &builtinEQRealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQReal)
		case opcode.NE:
			sig = &builtinNERealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NEReal)
		case opcode.NullEQ:
			sig = &builtinNullEQRealSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQReal)
		}
	case types.ETDecimal:
		switch c.op {
		case opcode.LT:
			sig = &builtinLTDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LTDecimal)
		case opcode.LE:
			sig = &builtinLEDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LEDecimal)
		case opcode.GT:
			sig = &builtinGTDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTDecimal)
		case opcode.GE:
			sig = &builtinGEDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GEDecimal)
		case opcode.EQ:
			sig = &builtinEQDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQDecimal)
		case opcode.NE:
			sig = &builtinNEDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NEDecimal)
		case opcode.NullEQ:
			sig = &builtinNullEQDecimalSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQDecimal)
		}
	case types.ETString:
		switch c.op {
		case opcode.LT:
			sig = &builtinLTStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LTString)
		case opcode.LE:
			sig = &builtinLEStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LEString)
		case opcode.GT:
			sig = &builtinGTStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTString)
		case opcode.GE:
			sig = &builtinGEStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GEString)
		case opcode.EQ:
			sig = &builtinEQStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQString)
		case opcode.NE:
			sig = &builtinNEStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NEString)
		case opcode.NullEQ:
			sig = &builtinNullEQStringSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQString)
		}
	case types.ETDuration:
		switch c.op {
		case opcode.LT:
			sig = &builtinLTDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LTDuration)
		case opcode.LE:
			sig = &builtinLEDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LEDuration)
		case opcode.GT:
			sig = &builtinGTDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTDuration)
		case opcode.GE:
			sig = &builtinGEDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GEDuration)
		case opcode.EQ:
			sig = &builtinEQDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQDuration)
		case opcode.NE:
			sig = &builtinNEDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NEDuration)
		case opcode.NullEQ:
			sig = &builtinNullEQDurationSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQDuration)
		}
	case types.ETDatetime, types.ETTimestamp:
		switch c.op {
		case opcode.LT:
			sig = &builtinLTTimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LTTime)
		case opcode.LE:
			sig = &builtinLETimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LETime)
		case opcode.GT:
			sig = &builtinGTTimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTTime)
		case opcode.GE:
			sig = &builtinGETimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GETime)
		case opcode.EQ:
			sig = &builtinEQTimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQTime)
		case opcode.NE:
			sig = &builtinNETimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NETime)
		case opcode.NullEQ:
			sig = &builtinNullEQTimeSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQTime)
		}
	case types.ETJson:
		switch c.op {
		case opcode.LT:
			sig = &builtinLTJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LTJson)
		case opcode.LE:
			sig = &builtinLEJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_LEJson)
		case opcode.GT:
			sig = &builtinGTJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GTJson)
		case opcode.GE:
			sig = &builtinGEJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_GEJson)
		case opcode.EQ:
			sig = &builtinEQJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_EQJson)
		case opcode.NE:
			sig = &builtinNEJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NEJson)
		case opcode.NullEQ:
			sig = &builtinNullEQJSONSig{bf}
			sig.setPbCode(fidelpb.ScalarFuncSig_NullEQJson)
		}
	}
	return
}
...
func (b *builtinLTIntSig) evalIntWithCtx(ctx stochastikctx.Context, event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareInt(ctx, b.args[0], b.args[1], event, event))
}

func (b *builtinLTIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareInt(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLTRealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareReal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLTDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareDecimal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLTStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareStringWithDefCauslationInfo(b.ctx, b.args[0], b.args[1], event, event, b.defCauslation))
}
...
func (b *builtinLTDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareDuration(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLTTimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareTime(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLTJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLT(CompareJSON(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLEIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareInt(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLERealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareReal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLEDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareDecimal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLEStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareStringWithDefCauslationInfo(b.ctx, b.args[0], b.args[1], event, event, b.defCauslation))
}
...
func (b *builtinLEDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareDuration(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLETimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareTime(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinLEJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfLE(CompareJSON(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGTIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareInt(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGTRealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareReal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGTDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareDecimal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGTStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareStringWithDefCauslationInfo(b.ctx, b.args[0], b.args[1], event, event, b.defCauslation))
}
...
func (b *builtinGTDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareDuration(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGTTimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareTime(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGTJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGT(CompareJSON(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGEIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareInt(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGERealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareReal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGEDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareDecimal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGEStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareStringWithDefCauslationInfo(b.ctx, b.args[0], b.args[1], event, event, b.defCauslation))
}
...
func (b *builtinGEDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareDuration(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGETimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareTime(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinGEJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfGE(CompareJSON(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinEQIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareInt(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinEQRealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareReal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinEQDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareDecimal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinEQStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareStringWithDefCauslationInfo(b.ctx, b.args[0], b.args[1], event, event, b.defCauslation))
}
...
func (b *builtinEQDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareDuration(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinEQTimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareTime(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinEQJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfEQ(CompareJSON(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNEIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareInt(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNERealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareReal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNEDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareDecimal(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNEStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareStringWithDefCauslationInfo(b.ctx, b.args[0], b.args[1], event, event, b.defCauslation))
}
...
func (b *builtinNEDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareDuration(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNETimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareTime(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNEJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	return resOfNE(CompareJSON(b.ctx, b.args[0], b.args[1], event, event))
}
...
func (b *builtinNullEQIntSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	...
	isUnsigned0, isUnsigned1 := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag), allegrosql.HasUnsignedFlag(b.args[1].GetType().Flag)
	var res int64
	switch {
...
func (b *builtinNullEQRealSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalReal(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	arg1, isNull1, err := b.args[1].EvalReal(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	var res int64
...
func (b *builtinNullEQDecimalSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalDecimal(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	arg1, isNull1, err := b.args[1].EvalDecimal(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	var res int64
...
func (b *builtinNullEQStringSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalString(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	arg1, isNull1, err := b.args[1].EvalString(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	var res int64
	...
	case isNull0 != isNull1:
		break
	case types.CompareString(arg0, arg1, b.defCauslation) == 0:
		res = 1
	}
	return res, false, nil
}
...
func (b *builtinNullEQDurationSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalDuration(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	arg1, isNull1, err := b.args[1].EvalDuration(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	var res int64
...
func (b *builtinNullEQTimeSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalTime(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	arg1, isNull1, err := b.args[1].EvalTime(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	var res int64
...
func (b *builtinNullEQJSONSig) evalInt(event chunk.Event) (val int64, isNull bool, err error) {
	arg0, isNull0, err := b.args[0].EvalJSON(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	arg1, isNull1, err := b.args[1].EvalJSON(b.ctx, event)
	if err != nil {
		return 0, true, err
	}
	var res int64
...
type CompareFunc = func(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error)

// CompareInt compares two integers.
func CompareInt(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalInt(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalInt(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
	...
	isUnsigned0, isUnsigned1 := allegrosql.HasUnsignedFlag(lhsArg.GetType().Flag), allegrosql.HasUnsignedFlag(rhsArg.GetType().Flag)
	var res int
	switch {
	case isUnsigned0 && isUnsigned1:
...
func genCompareString(defCauslation string) func(sctx stochastikctx.Context, lhsArg Expression, rhsArg Expression, lhsEvent chunk.Event, rhsEvent chunk.Event) (int64, bool, error) {
	return func(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
		return CompareStringWithDefCauslationInfo(sctx, lhsArg, rhsArg, lhsEvent, rhsEvent, defCauslation)
	}
}

// CompareStringWithDefCauslationInfo compares two strings with the specified defCauslation information.
func CompareStringWithDefCauslationInfo(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event, defCauslation string) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalString(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalString(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
	...
	return int64(types.CompareString(arg0, arg1, defCauslation)), false, nil
}

// CompareReal compares two float-point values.
func CompareReal(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalReal(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalReal(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
...
func CompareDecimal(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalDecimal(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalDecimal(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
...
func CompareTime(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalTime(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalTime(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
...
func CompareDuration(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalDuration(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalDuration(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
...
func CompareJSON(sctx stochastikctx.Context, lhsArg, rhsArg Expression, lhsEvent, rhsEvent chunk.Event) (int64, bool, error) {
	arg0, isNull0, err := lhsArg.EvalJSON(sctx, lhsEvent)
	if err != nil {
		return 0, true, err
	}

	arg1, isNull1, err := rhsArg.EvalJSON(sctx, rhsEvent)
	if err != nil {
		return 0, true, err
	}
