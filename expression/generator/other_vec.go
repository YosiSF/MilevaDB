...
{{ define "BufSlabPredictor" }}
	buf0, err := b.bufSlabPredictor.get(types.ET{{ .Input.ETName }}, n)
	if err != nil {
		return err
	}
	...
	buf1, err := b.bufSlabPredictor.get(types.ET{{ .Input.ETName }}, n)
	if err != nil {
		return err
	}
...
{{ define "Compare" }}
	...
	{{- else -}}
		compareResult = types.Compare{{ .Input.TypeNameInDeferredCauset }}(arg0, arg1)
	{{- end -}}
{{ end }}
...
func (b *{{.SigName}}) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	{{- template "BufSlabPredictor" . }}
	{{- if $InputFixed }}
		args0 := buf0.{{.Input.TypeNameInDeferredCauset}}s()
	{{- end }}
	...
	for j := 1; j < len(args); j++ {
		...
		{{- end }}
		{{- if $InputFixed }}
			args1 := buf1.{{.Input.TypeNameInDeferredCauset}}s()
