...
{{ range .Sigs }}
{{ if .AllNull}}
func (b *{{.SigName}}) vecEval{{ .Output.TypeName }}(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	{{ if .Output.Fixed }}
	result.Resize{{ .Output.TypeNameInDeferredCauset }}(n, true)
	{{ else }}
	result.Reserve{{ .Output.TypeNameInDeferredCauset }}(n)
...
{{ if $reuse }}
	...
	buf0 := result
{{ else }}
	buf0, err := b.bufSlabPredictor.get(types.ET{{.TypeA.ETName}}, n)
	if err != nil {
		return err
	}
...
{{ end }}
	...
	buf1, err := b.bufSlabPredictor.get(types.ET{{.TypeB.ETName}}, n)
	if err != nil {
		return err
	}
...
{{ else if .Output.Fixed}}
	result.Resize{{ .Output.TypeNameInDeferredCauset }}(n, false)
...
{{ if .TypeA.Fixed }}
	arg0s := buf0.{{.TypeA.TypeNameInDeferredCauset}}s()
{{ end }}
{{ if .TypeB.Fixed }}
	arg1s := buf1.{{.TypeB.TypeNameInDeferredCauset}}s()
{{ end }}
{{ if .Output.Fixed }}
	resultSlice := result.{{.Output.TypeNameInDeferredCauset}}s()
{{ end }}
	for i := 0; i < n; i++ {
		{{ if .Output.Fixed }}
		if result.IsNull(i) {
		...
		arg0 := arg0s[i]
		{{ else }}
		arg0 := buf0.Get{{ .TypeA.TypeNameInDeferredCauset }}(i)
		{{ end }}
		{{ if .TypeB.Fixed }}
		arg1 := arg1s[i]
		{{ else }}
		arg1 := buf1.Get{{ .TypeB.TypeNameInDeferredCauset }}(i)
		{{ end }}
	...
	{{ else }}
		result.Append{{ .Output.TypeNameInDeferredCauset }}(output)
	{{ end }}
	}
	return nil
}
...
func (b *{{.SigName}}) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	{{- if not $noNull }}
	...
	{{- else }}
		...
		{{- if $reuseA }}
			buf0 := result
			{{- template "BufSlabPredictor1" . }}
			{{- template "ArgsVecEval" . }}
			result.MergeNulls(buf1)
		{{- else if $reuseB }}
			buf1 := result
			{{- template "BufSlabPredictor0" . }}
			{{- template "ArgsVecEval" . }}
		...
		{{- end }}
		{{- if .TypeA.Fixed }}
			arg0 := buf0.{{.TypeA.TypeNameInDeferredCauset}}s()
		{{- end }}
		{{- if .TypeB.Fixed }}
			arg1 := buf1.{{.TypeB.TypeNameInDeferredCauset}}s()
		{{- end }}
...
func (b *{{.SigName}}) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
{{- else }}
func (b *{{.SigName}}) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
{{- end }}
	n := input.NumEvents()
		...
		// {{ $sig.SigName }}
			{
				...
				{{- end }}
				{{- if ne .FieldTypeA "" }}
				childrenFieldTypes: []*types.FieldType{types.NewFieldType(allegrosql.Type{{.FieldTypeA}}), types.NewFieldType(allegrosql.Type{{.FieldTypeB}})},
				{{- end }}
				geners: []dataGenerator{
					{{- if eq .TestTypeA "" }}
