...
func init() {
	...
	// If we don't SetSystemTZ() here, the value would change unpredicblock.
...
func (s *testEvaluatorSuite) TestDate(c *C) {
	...
	for _, t := range dtblDate {
		...
		c.Assert(err, IsNil)
		c.Assert(v, solitonutil.CausetEquals, t["Expect"][0])
	}
	...
	for ith, t := range dtbl {
		...
		v, err = evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(v, solitonutil.CausetEquals, t["YearWeek"][0], Commentf("no.%d", ith))
	}
	...
	for _, t := range dtblNil {
		...
		v, err = evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(v, solitonutil.CausetEquals, t["YearWeek"][0])
	}
...
func (s *testEvaluatorSuite) TestMonthName(c *C) {
	...
	cases := []struct {
		...
		{"2020-12-01", "December", false, false},
		{"2020-00-01", "", true, false},
...
func (s *testEvaluatorSuite) TestDayName(c *C) {
	...
	cases := []struct {
		...
		{"2020-12-01", "Friday", false, false},
		{"0000-12-01", "Friday", false, false},
		{"2020-00-01", "", true, false},
		{"2020-01-00", "", true, false},
		{"0000-00-00", "", true, false},
		{"0000-00-00 00:00:00.000000", "", true, false},
		{"0000-00-00 00:00:11.000000", "", true, false},
	}
...
func (s *testEvaluatorSuite) TestDayOfWeek(c *C) {
	...
	cases := []struct {
		...
		{"2020-12-01", 6, false, false},
		{"0000-00-00", 1, true, false},
		{"2020-00-00", 1, true, false},
		{"2020-00-00 12:12:12", 1, true, false},
...
func (s *testEvaluatorSuite) TestDayOfMonth(c *C) {
	...
	cases := []struct {
		...
		{"2020-12-01", 1, false, false},
		{"0000-00-00", 0, false, false},
		{"2020-00-00", 0, false, false},
		{"2020-00-00 12:12:12", 0, false, false},
...
func (s *testEvaluatorSuite) TestDayOfYear(c *C) {
	...
	cases := []struct {
		...
		{"2020-12-01", 335, false, false},
		{"0000-00-00", 1, true, false},
		{"2020-00-00", 0, true, false},
		{"2020-00-00 12:12:12", 0, true, false},
...
func (s *testEvaluatorSuite) TestDateFormat(c *C) {
	...
	tblDate := []struct {
		...
		{[]string{"0000-01-01 00:00:00.123456",
			...
			// so the result of "%U %u %V %Y" is different from MyALLEGROSQL.
		...
		{[]string{"2020-09-3 00:59:59.123456",
			`abc%b %M %m %c %D %d %e %j %k %h %i %p %r %T %s %f %U %u %V %v %a %W %w %X %x %Y %y!123 %%xyz %z`},
			`abcSep September 09 9 3rd 03 3 247 0 12 59 AM 12:59:59 AM 00:59:59 59 123456 35 35 35 35 Sat Saturday 6 2020 2020 2020 16!123 %xyz z`},
	...
	for i, t := range dtblDate {
		...
		v, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(v, solitonutil.CausetEquals, t["Expect"][0], Commentf(`no.%d \nobtain:%v \nexpect:%v\n`, i,
...
func (s *testEvaluatorSuite) TestClock(c *C) {
	...
	for _, t := range dtbl {
		...
		v, err = evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(v, solitonutil.CausetEquals, t["Time"][0])
	}
...
func (s *testEvaluatorSuite) TestTime(c *C) {
	...
	for _, t := range cases {
		...
		c.Assert(tp.Flen, Equals, allegrosql.MaxDurationWidthWithFsp)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
		} else {
...
func (s *testEvaluatorSuite) TestAddTimeSig(c *C) {
	tbl := []struct {
		...
		{"2020-01-01 01:01:01.11", "01:01:01.11111", "2020-01-01 02:02:02.221110"},
		{"2007-12-31 23:59:59.999999", "1 1:1:1.000002", "2008-01-02 01:01:01.000001"},
		{"2020-12-01 01:01:01.000001", "1 1:1:1.000002", "2020-12-02 02:02:02.000003"},
		{"2020-12-31 23:59:59", "00:00:01", "2020-01-01 00:00:00"},
		{"2020-12-31 23:59:59", "1", "2020-01-01 00:00:00"},
		{"2007-12-31 23:59:59.999999", "2 1:1:1.000002", "2008-01-03 01:01:01.000001"},
		{"2020-08-16 20:21:01", "00:00:00.000001", "2020-08-16 20:21:01.000001"},
	}
	...
	tbll := []struct {
		...
		{20171010123456, 1, "2020-10-10 12:34:57"},
		{123456, 1, "12:34:57"},
	}
	for _, t := range tbll {
	...
	tblWarning := []struct {
		...
		{types.CurrentTime(allegrosql.TypeTimestamp), "-32073", types.ErrTruncatedWrongVal},
		{types.CurrentTime(allegrosql.TypeDate), "-32073", types.ErrTruncatedWrongVal},
		{types.CurrentTime(allegrosql.TypeDatetime), "-32073", types.ErrTruncatedWrongVal},
	}
...
func (s *testEvaluatorSuite) TestSubTimeSig(c *C) {
	tbl := []struct {
		...
		{"110:00:00", "1 02:00:00", "84:00:00"},
		{"2020-01-01 01:01:01.11", "01:01:01.11111", "2020-12-31 23:59:59.998890"},
	...
	tbll := []struct {
		...
		{20171010123456, 1, "2020-10-10 12:34:55"},
		{123456, 1, "12:34:55"},
	}
	for _, t := range tbll {
		tmpInput := types.NewIntCauset(t.Input)
...
func convertToTimeWithFsp(sc *stmtctx.StatementContext, arg types.Causet, tp byte, fsp int8) (d types.Causet, err error) {
	if fsp > types.MaxFsp {
		fsp = types.MaxFsp
	}
...
func convertToTime(sc *stmtctx.StatementContext, arg types.Causet, tp byte) (d types.Causet, err error) {
...
func builtinDateFormat(ctx stochastikctx.Context, args []types.Causet) (d types.Causet, err error) {
	...
	d.SetString(str, allegrosql.DefaultDefCauslationName)
	return
}
...
func (s *testEvaluatorSuite) TestFromUnixTime(c *C) {
	tbl := []struct {
		...
		{false, 1451606400, 0, 0, "", "2020-01-01 00:00:00"},
		{true, 1451606400, 123456000, 1451606400.123456, "", "2020-01-01 00:00:00.123456"},
		{true, 1451606400, 999999000, 1451606400.999999, "", "2020-01-01 00:00:00.999999"},
		{true, 1451606400, 999999900, 1451606400.9999999, "", "2020-01-01 00:00:01.000000"},
		{false, 1451606400, 0, 0, `%Y %D %M %h:%i:%s %x`, "2020-01-01 00:00:00"},
		{true, 1451606400, 123456000, 1451606400.123456, `%Y %D %M %h:%i:%s %x`, "2020-01-01 00:00:00.123456"},
	...
	for _, t := range tbl {
		...
		if len(t.format) == 0 {
			...
			v, err := evalBuiltinFunc(f, chunk.Event{})
			c.Assert(err, IsNil)
			result, err := builtinDateFormat(s.ctx, []types.Causet{types.NewStringCauset(t.expect), format})
			c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestStrToDate(c *C) {
	tests := []struct {
		...
		{"20161122165022", `%Y%m%d%H%i%s`, true, time.Date(2020, 11, 22, 16, 50, 22, 0, time.Local)},
		{"2020 11 22 16 50 22", `%Y%m%d%H%i%s`, true, time.Date(2020, 11, 22, 16, 50, 22, 0, time.Local)},
		{"16-50-22 2020 11 22", `%H-%i-%s%Y%m%d`, true, time.Date(2020, 11, 22, 16, 50, 22, 0, time.Local)},
		{"16-50 2020 11 22", `%H-%i-%s%Y%m%d`, false, time.Time{}},
	...
	for _, test := range tests {
		...
		c.Assert(result.HoTT(), Equals, types.HoTTMysqlTime)
...
func (s *testEvaluatorSuite) TestFromDays(c *C) {
	...
	tests := []struct {
		...
		{-140, "0000-00-00"},   // allegrosql FROM_DAYS returns 0000-00-00 for any day <= 365.
		{140, "0000-00-00"},    // allegrosql FROM_DAYS returns 0000-00-00 for any day <= 365.
...
func (s *testEvaluatorSuite) TestDateDiff(c *C) {
	// Test cases from https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_datediff
	tests := []struct {
		t1     string
		t2     string
		expect int64
	...
	tests2 := []struct {
		...
		{"2007-00-31 23:59:59", "2020-01-13"},
		{"2007-10-31 23:59:59", "2020-01-00"},
...
func (s *testEvaluatorSuite) TestTimeDiff(c *C) {
	...
	tests := []struct {
		...
		{[]interface{}{"2020-12-00 12:00:00", "2020-12-01 12:00:00"}, "-24:00:00", false, 0, false},
		{[]interface{}{"10:10:10", "10:9:0"}, "00:01:10", false, 0, false},
		{[]interface{}{"2020-12-00 12:00:00", "10:9:0"}, "", true, 0, false},
		{[]interface{}{"2020-12-00 12:00:00", ""}, "", true, 0, true},
	}
	...
	for _, t := range tests {
		...
		c.Assert(tp.Charset, Equals, charset.CharsetBin)
		c.Assert(tp.DefCauslate, Equals, charset.DefCauslationBin)
		c.Assert(tp.Flag, Equals, allegrosql.BinaryFlag)
		c.Assert(tp.Flen, Equals, allegrosql.MaxDurationWidthWithFsp)
		d, err := f.Eval(chunk.Event{})
		if t.getWarning {
			c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestWeek(c *C) {
	// Test cases from https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_week
	tests := []struct {
		t      string
		mode   int64
		expect int64
...
func (s *testEvaluatorSuite) TestTimestamFIDeliff(c *C) {
	tests := []struct {
		unit   string
		t1     string
		t2     string
	...
	fc := funcs[ast.TimestamFIDeliff]
	for _, test := range tests {
		args := []types.Causet{
			types.NewStringCauset(test.unit),
			types.NewStringCauset(test.t1),
...
func (s *testEvaluatorSuite) TestUnixTimestamp(c *C) {
	...
	tests := []struct {
		...
		{0, types.NewIntCauset(151113), types.HoTTInt64, "1447372800"}, // YYMMDD
		// TODO: Uncomment the line below after fixing #4232
		// {5, types.NewFloat64Causet(151113.12345), types.HoTTMysqlDecimal, "1447372800.00000"},                                           // YYMMDD
		{0, types.NewIntCauset(20151113), types.HoTTInt64, "1447372800"}, // YYYYMMDD
		// TODO: Uncomment the line below after fixing #4232
		// {5, types.NewFloat64Causet(20151113.12345), types.HoTTMysqlDecimal, "1447372800.00000"},                                         // YYYYMMDD
		{0, types.NewIntCauset(151113102019), types.HoTTInt64, "1447410019"},                                                            // YYMMDDHHMMSS
		{0, types.NewFloat64Causet(151113102019), types.HoTTInt64, "1447410019"},                                                        // YYMMDDHHMMSS
		{2, types.NewFloat64Causet(151113102019.12), types.HoTTMysqlDecimal, "1447410019.12"},                                           // YYMMDDHHMMSS
		{0, types.NewDecimalCauset(types.NewDecFromStringForTest("151113102019")), types.HoTTInt64, "1447410019"},                       // YYMMDDHHMMSS
		{2, types.NewDecimalCauset(types.NewDecFromStringForTest("151113102019.12")), types.HoTTMysqlDecimal, "1447410019.12"},          // YYMMDDHHMMSS
		{7, types.NewDecimalCauset(types.NewDecFromStringForTest("151113102019.1234567")), types.HoTTMysqlDecimal, "1447410019.123457"}, // YYMMDDHHMMSS
		{0, types.NewIntCauset(20151113102019), types.HoTTInt64, "1447410019"},                                                          // YYYYMMDDHHMMSS
		{0, types.NewStringCauset("2020-11-13 10:20:19"), types.HoTTInt64, "1447410019"},
		{0, types.NewStringCauset("2020-11-13 10:20:19.012"), types.HoTTMysqlDecimal, "1447410019.012"},
		{0, types.NewStringCauset("1970-01-01 00:00:00"), types.HoTTInt64, "0"},                               // Min timestamp
		{0, types.NewStringCauset("2038-01-19 03:14:07.999999"), types.HoTTMysqlDecimal, "2147483647.999999"}, // Max timestamp
		{0, types.NewStringCauset("2020-00-02"), types.HoTTInt64, "0"},                                        // Invalid date
		{0, types.NewStringCauset("1969-12-31 23:59:59.999999"), types.HoTTMysqlDecimal, "0"},                 // Invalid timestamp
		{0, types.NewStringCauset("2038-01-19 03:14:08"), types.HoTTInt64, "0"},                               // Invalid timestamp
		// Below tests irregular inputs.
		//{0, types.NewIntCauset(0), types.HoTTInt64, "0"},
		//{0, types.NewIntCauset(-1), types.HoTTInt64, "0"},
		//{0, types.NewIntCauset(12345), types.HoTTInt64, "0"},
	}
	...
	for _, test := range tests {
		...
		d, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil, Commentf("%+v", test))
		c.Assert(d.HoTT(), Equals, test.expectHoTT, Commentf("%+v", test))
		str, err := d.ToString()
		c.Assert(err, IsNil, Commentf("%+v", test))
...
func (s *testEvaluatorSuite) TestDateArithFuncs(c *C) {
	date := []string{"2020-12-31", "2020-01-01"}
	...
	testMonths := []struct {
		...
		{"2020-07-31", 1, "2020-08-31"},
		{"2020-08-31", 1, "2020-09-30"},
		{"2020-07-31", 2, "2020-09-30"},
		{"2020-01-31", 27, "2020-04-30"},
...
func (s *testEvaluatorSuite) TestTimestamp(c *C) {
	tests := []struct {
		...
		// one argument
		{[]types.Causet{types.NewStringCauset("2020-01-18")}, "2020-01-18 00:00:00"},
		{[]types.Causet{types.NewStringCauset("20170118")}, "2020-01-18 00:00:00"},
		{[]types.Causet{types.NewStringCauset("170118")}, "2020-01-18 00:00:00"},
		{[]types.Causet{types.NewStringCauset("20170118123056")}, "2020-01-18 12:30:56"},
		{[]types.Causet{types.NewStringCauset("2020-01-18 12:30:56")}, "2020-01-18 12:30:56"},
		{[]types.Causet{types.NewIntCauset(170118)}, "2020-01-18 00:00:00"},
		{[]types.Causet{types.NewFloat64Causet(20170118)}, "2020-01-18 00:00:00"},
		{[]types.Causet{types.NewStringCauset("20170118123050.999")}, "2020-01-18 12:30:50.999"},
		{[]types.Causet{types.NewStringCauset("20170118123050.1234567")}, "2020-01-18 12:30:50.123457"},
		// TODO: Parse int should use ParseTimeFromNum, rather than convert int to string for parsing.
		// {[]types.Causet{types.NewIntCauset(11111111111)}, "2001-11-11 11:11:11"},
		{[]types.Causet{types.NewStringCauset("11111111111")}, "2011-11-11 11:11:01"},
		{[]types.Causet{types.NewFloat64Causet(20170118.999)}, "2020-01-18 00:00:00.000"},

		// two arguments
		{[]types.Causet{types.NewStringCauset("2020-01-18"), types.NewStringCauset("12:30:59")}, "2020-01-18 12:30:59"},
		{[]types.Causet{types.NewStringCauset("2020-01-18"), types.NewStringCauset("12:30:59")}, "2020-01-18 12:30:59"},
		{[]types.Causet{types.NewStringCauset("2020-01-18 01:01:01"), types.NewStringCauset("12:30:50")}, "2020-01-18 13:31:51"},
		{[]types.Causet{types.NewStringCauset("2020-01-18 01:01:01"), types.NewStringCauset("838:59:59")}, "2020-02-22 00:01:00"},

		{[]types.Causet{types.NewDecimalCauset(types.NewDecFromStringForTest("20170118123950.123"))}, "2020-01-18 12:39:50.123"},
		{[]types.Causet{types.NewDecimalCauset(types.NewDecFromStringForTest("20170118123950.999"))}, "2020-01-18 12:39:50.999"},
		{[]types.Causet{types.NewDecimalCauset(types.NewDecFromStringForTest("20170118123950.999"))}, "2020-01-18 12:39:50.999"},
	}
...
func (s *testEvaluatorSuite) TestMakeDate(c *C) {
	...
	for _, t := range cases {
		...
		c.Assert(tp.DefCauslate, Equals, charset.DefCauslationBin)
		c.Assert(tp.Flag, Equals, allegrosql.BinaryFlag)
		c.Assert(tp.Flen, Equals, allegrosql.MaxDateWidth)
		d, err := f.Eval(chunk.Event{})
		if t.getErr {
			c.Assert(err, NotNil)
		} else {
...
func (s *testEvaluatorSuite) TestMakeTime(c *C) {
	...
	for idx, t := range Dtbl {
		...
		c.Assert(err, IsNil)
		if t["Want"][0].HoTT() == types.HoTTNull {
			c.Assert(got.HoTT(), Equals, types.HoTTNull, Commentf("[%v] - args:%v", idx, t["Args"]))
		} else {
...
func (s *testEvaluatorSuite) TestQuarter(c *C) {
	...
	tests := []struct {
		...
		// Test case from https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_quarter
		{"2008-04-01", 2},
...
func (s *testEvaluatorSuite) TestPeriodAdd(c *C) {
	...
	for _, test := range tests {
		...
		c.Assert(err, IsNil)
		c.Assert(result.HoTT(), Equals, types.HoTTInt64)
...
func (s *testEvaluatorSuite) TestTimeToSec(c *C) {
	...
	tests := []struct {
		...
		{types.NewStringCauset("22:23:00"), 80580},
		{types.NewStringCauset("00:39:38"), 2378},
		{types.NewStringCauset("23:00"), 82800},
		{types.NewStringCauset("00:00"), 0},
		{types.NewStringCauset("00:00:00"), 0},
		{types.NewStringCauset("23:59:59"), 86399},
		{types.NewStringCauset("1:0"), 3600},
		{types.NewStringCauset("1:00"), 3600},
		{types.NewStringCauset("1:0:0"), 3600},
		{types.NewStringCauset("-02:00"), -7200},
		{types.NewStringCauset("-02:00:05"), -7205},
		{types.NewStringCauset("020005"), 7205},
		// {types.NewStringCauset("20171222020005"), 7205},
		// {types.NewIntCauset(020005), 7205},
		// {types.NewIntCauset(20171222020005), 7205},
...
func (s *testEvaluatorSuite) TestSecToTime(c *C) {
	...
	tests := []struct {
		...
		{0, types.NewIntCauset(2378), "00:39:38"},
		{0, types.NewIntCauset(3864000), "838:59:59"},
		{0, types.NewIntCauset(-3864000), "-838:59:59"},
		{1, types.NewFloat64Causet(86401.4), "24:00:01.4"},
		{1, types.NewFloat64Causet(-86401.4), "-24:00:01.4"},
		{5, types.NewFloat64Causet(86401.54321), "24:00:01.54321"},
		{-1, types.NewFloat64Causet(86401.54321), "24:00:01.543210"},
		{0, types.NewStringCauset("123.4"), "00:02:03.400000"},
		{0, types.NewStringCauset("123.4567891"), "00:02:03.456789"},
		{0, types.NewStringCauset("123"), "00:02:03.000000"},
...
func (s *testEvaluatorSuite) TestWithTimeZone(c *C) {
	...
	timeToGoTime := func(d types.Causet, loc *time.Location) time.Time {
		result, _ := d.GetMysqlTime().GoTime(loc)
		return result
	}
	durationToGoTime := func(d types.Causet, loc *time.Location) time.Time {
		t, _ := d.GetMysqlDuration().ConvertToTime(sv.StmtCtx, allegrosql.TypeDatetime)
	...
	tests := []struct {
		method        string
		Input         []types.Causet
		convertToTime func(types.Causet, *time.Location) time.Time
	}{
		{ast.Sysdate, makeCausets(2), timeToGoTime},
		{ast.Sysdate, nil, timeToGoTime},
		{ast.Curdate, nil, timeToGoTime},
		{ast.CurrentTime, makeCausets(2), durationToGoTime},
...
func (s *testEvaluatorSuite) TestMilevaDBParseTso(c *C) {
	...
	tests := []struct {
		...
		{404411537129996288, "2020-11-20 09:53:04.877000"},
		{"404411537129996288", "2020-11-20 09:53:04.877000"},
