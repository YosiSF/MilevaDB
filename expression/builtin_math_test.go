...
func (s *testEvaluatorSuite) TestCeil(c *C) {
	...
	cases := []testCase{
		...
		{"milevadb", float64(0), false, false},
...
func (s *testEvaluatorSuite) TestFloor(c *C) {
	...
	genTime := func(y, m, d int) types.Time {
		return types.NewTime(types.FromDate(y, m, d, 0, 0, 0, 0), allegrosql.TypeDatetime, types.DefaultFsp)
	}
...
func (s *testEvaluatorSuite) TestRand(c *C) {
	...
	for i := 0; i < 3; i++ {
		v, err = evalBuiltinFunc(f2, chunk.Event{})
		c.Assert(err, IsNil)
...
func (s *testEvaluatorSuite) TestRound(c *C) {
	...
	for _, t := range Dtbl {
		...
		case *builtinRoundWithFracIntSig:
			c.Assert(f.PbCode(), Equals, fidelpb.ScalarFuncSig_RoundWithFracInt)
		case *builtinRoundWithFracDecSig:
			c.Assert(f.PbCode(), Equals, fidelpb.ScalarFuncSig_RoundWithFracDec)
		case *builtinRoundWithFracRealSig:
			c.Assert(f.PbCode(), Equals, fidelpb.ScalarFuncSig_RoundWithFracReal)
		case *builtinRoundIntSig:
			c.Assert(f.PbCode(), Equals, fidelpb.ScalarFuncSig_RoundInt)
		case *builtinRoundDecSig:
			c.Assert(f.PbCode(), Equals, fidelpb.ScalarFuncSig_RoundDec)
		case *builtinRoundRealSig:
			c.Assert(f.PbCode(), Equals, fidelpb.ScalarFuncSig_RoundReal)
		}
		v, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil)
		c.Assert(v, solitonutil.CausetEquals, t["Ret"][0])
...
func (s *testEvaluatorSuite) TestCRC32(c *C) {
	tbl := []struct {
		...
		{[]interface{}{"-1"}, 808273962},
		{[]interface{}{"allegrosql"}, 2501908538},
		{[]interface{}{"MyALLEGROSQL"}, 3259397556},
...
func (s *testEvaluatorSuite) TestConv(c *C) {
	cases := []struct {
		...
		{[]interface{}{"MilevaDB", 10, 8}, "0", false, false},
	...
	for _, t := range cases {
		...
		c.Assert(tp.DefCauslate, Equals, charset.DefCauslationUTF8MB4)
...
func (s *testEvaluatorSuite) TestSign(c *C) {
	...
	for _, t := range []struct {
		...
		v, err := evalBuiltinFunc(f, chunk.Event{})
		c.Assert(err, IsNil, Commentf("%v", t))
		c.Assert(v, solitonutil.CausetEquals, types.NewCauset(t.ret), Commentf("%v", t))
	}
}
...
func (s *testEvaluatorSuite) TestPi(c *C) {
	...
	pi, err := evalBuiltinFunc(f, chunk.Event{})
	c.Assert(err, IsNil)
	c.Assert(pi, solitonutil.CausetEquals, types.NewCauset(math.Pi))
}
...
func (s *testEvaluatorSuite) TestAtan(c *C) {
	tests := []struct {
		...
		{[]interface{}{"milevadb"}, 0, false, true},
	}
...
func (s *testEvaluatorSuite) TestTan(c *C) {
	cases := []struct {
		...
		{-math.Pi / 4, float64(-1), false, false},
		{math.Pi * 3 / 4, math.Tan(math.Pi * 3 / 4), false, false}, //in allegrosql and golang, it equals -1.0000000000000002, not -1
