...
func (b *builtinMonthSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	}
...
func (b *builtinYearSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	for i := 0; i < len(times); i++ {
		...
		if times[i].IsZero() {
			...
			times[i].SetType(allegrosql.TypeDate)
		}
	}
	return nil
}
...
func (b *builtinFromUnixTime2ArgSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinExtractDatetimeSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinDayNameSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	result.ReserveString(n)
...
func (b *builtinDayNameSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinDayNameSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	result.ResizeInt64(n, false)
	i64s := result.Int64s()
...
func (b *builtinWeekDaySig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinTimeFormatSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(buf)
	if err := b.args[0].VecEvalDuration(b.ctx, input, buf); err != nil {
		...
		// in this case which follows the behavior of allegrosql.
		// Use the non-vectorized method to handle this HoTT of error.
		return vecEvalStringByEvents(b, input, result)
	}

	buf1, err1 := b.bufSlabPredictor.get(types.ETString, n)
	if err1 != nil {
		return err1
	}
	defer b.bufSlabPredictor.put(buf1)
...
func (b *builtinUnixTimestampCurrentSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	nowTs, err := getStmtTimestamp(b.ctx)
	if err != nil {
		return err
	}
...
func (b *builtinYearWeekWithoutModeSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinPeriodDiffSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinNowWithArgSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	defer b.bufSlabPredictor.put(bufFsp)
...
func (b *builtinGetFormatSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinLastDaySig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	if err := b.args[0].VecEvalTime(b.ctx, input, result); err != nil {
		return err
	}
	...
	for i := 0; i < n; i++ {
		...
		times[i] = types.NewTime(types.FromDate(year, month, lastDay, 0, 0, 0, 0), allegrosql.TypeDate, types.DefaultFsp)
	}
	return nil
}
...
func (b *builtinStrToDateDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	bufStrings, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(bufStrings)
	if err := b.args[0].VecEvalString(b.ctx, input, bufStrings); err != nil {
		return err
	}

	bufFormats, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(bufFormats)
...
func (b *builtinMilevaDBParseTsoSig) vectorized() bool {
	return true
}
...
func (b *builtinMicroSecondSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
	...
	defer b.bufSlabPredictor.put(buf)
	if err = b.args[0].VecEvalDuration(b.ctx, input, buf); err != nil {
		return vecEvalIntByEvents(b, input, result)
	}
...
func (b *builtinQuarterSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinWeekWithModeSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf1, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinExtractDurationSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	unit, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(unit)
	if err := b.args[0].VecEvalString(b.ctx, input, unit); err != nil {
		return err
	}
	dur, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(dur)
...
func (b *builtinStrToDateDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	bufStrings, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinToSecondsSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinMinuteSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinSecondSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinNowWithoutArgSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinTimestampLiteralSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	result.ResizeTime(n, false)
	times := result.Times()
	for i := range times {
...
func (b *builtinMakeDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	for i := 0; i < n; i++ {
		...
		startTime := types.NewTime(types.FromDate(int(years[i]), 1, 1, 0, 0, 0, 0), allegrosql.TypeDate, 0)
...
func (b *builtinWeekOfYearSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinTimeToSecSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinStrToDateDatetimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	dateBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(dateBuf)
	if err = b.args[0].VecEvalString(b.ctx, input, dateBuf); err != nil {
		return err
	}

	formatBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(formatBuf)
...
func (b *builtinUTCDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	nowTs, err := getStmtTimestamp(b.ctx)
	if err != nil {
		return err
	}
	year, month, day := nowTs.UTC().Date()
	utcDate := types.NewTime(types.FromGoTime(time.Date(year, month, day, 0, 0, 0, 0, time.UTC)), allegrosql.TypeDate, types.UnspecifiedFsp)
...
func (b *builtinWeekWithoutModeSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinUnixTimestamFIDelecSig) vectorized() bool {
	return true
}

func (b *builtinUnixTimestamFIDelecSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	result.ResizeDecimal(n, false)
	ts := result.Decimals()
	timeBuf, err := b.bufSlabPredictor.get(types.ETTimestamp, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(timeBuf)
	if err := b.args[0].VecEvalTime(b.ctx, input, timeBuf); err != nil {
		for i := 0; i < n; i++ {
			temp, isNull, err := b.evalDecimal(input.GetEvent(i))
			if err != nil {
				return err
			}
			ts[i] = *temp
...
func (b *builtinPeriodAddSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := b.args[0].VecEvalInt(b.ctx, input, result); err != nil {
		return err
	}
...
func (b *builtinTimestampAddSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	buf2, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(buf2)
...
func (b *builtinToDaysSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinDateFormatSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()

	dateBuf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(dateBuf)
...
func (b *builtinHourSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinSecToTimeSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_utc-time
func (b *builtinUTCTimeWithoutArgSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	nowTs, err := getStmtTimestamp(b.ctx)
	if err != nil {
		return err
...
func (b *builtinDateDiffSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinCurrentDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	nowTs, err := getStmtTimestamp(b.ctx)
	if err != nil {
		return err
	}
...
func (b *builtinMakeTimeSig) getVecIntParam(arg Expression, input *chunk.Chunk, defCaus *chunk.DeferredCauset) (err error) {
	if arg.GetType().EvalType() == types.ETReal {
		err = arg.VecEvalReal(b.ctx, input, defCaus)
		if err != nil {
			return err
		}
	...
	err = arg.VecEvalInt(b.ctx, input, defCaus)
	return err
}

func (b *builtinMakeTimeSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	result.ResizeInt64(n, false)
	hoursBuf := result
	var err error
	...
	defer b.bufSlabPredictor.put(minutesBuf)
	if err = b.getVecIntParam(b.args[1], input, minutesBuf); err != nil {
		return err
	}
	secondsBuf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(secondsBuf)
	...
	hourUnsignedFlag := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag)
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
			continue
		}
...
func (b *builtinDayOfYearSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinFromUnixTime1ArgSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_yearweek
func (b *builtinYearWeekWithModeSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf1, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinTimestamFIDeliffSig) vectorized() bool {
	return true
}

// vecEvalInt evals a builtinTimestamFIDeliffSig.
// See https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_timestamFIDeliff
func (b *builtinTimestamFIDeliffSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	unitBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(unitBuf)
	if err := b.args[0].VecEvalString(b.ctx, input, unitBuf); err != nil {
		return err
	}
	lhsBuf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(lhsBuf)
	if err := b.args[1].VecEvalTime(b.ctx, input, lhsBuf); err != nil {
		return err
	}
	rhsBuf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(rhsBuf)
	...
	for i := 0; i < n; i++ {
		...
		i64s[i] = types.TimestamFIDeliff(unitBuf.GetString(i), lhs[i], rhs[i])
	}
	return nil
}
...
func (b *builtinUnixTimestampIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	...
	if err := b.args[0].VecEvalTime(b.ctx, input, buf); err != nil {
		var isNull bool
		for i := 0; i < n; i++ {
			i64s[i], isNull, err = b.evalInt(input.GetEvent(i))
			if err != nil {
				return err
			}
			if isNull {
...
func (b *builtinCurrentTime0ArgSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	nowTs, err := getStmtTimestamp(b.ctx)
	if err != nil {
		return err
...
func (b *builtinTimeSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinTimeLiteralSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	result.ResizeGoDuration(n, false)
	d64s := result.GoDurations()
	for i := 0; i < n; i++ {
...
func (b *builtinMonthNameSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinDayOfWeekSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/date-and-time-functions.html#function_utc-timestamp
func (b *builtinUTCTimestampWithoutArgSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinConvertTzSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	if err := b.args[0].VecEvalTime(b.ctx, input, result); err != nil {
		return err
	}

	fromTzBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(fromTzBuf)
	if err := b.args[1].VecEvalString(b.ctx, input, fromTzBuf); err != nil {
		return err
	}

	toTzBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(toTzBuf)
...
func (b *builtinTimestamp1ArgSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinTimestamp2ArgsSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf0, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinDayOfMonthSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
