...
func (b *builtinAesDecryptSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	...
	keyBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(keyBuf)
	...
	for i := 0; i < n; i++ {
		...
		if !isConstKey {
			key = encrypt.DeriveKeyMyALLEGROSQL(keyBuf.GetBytes(i), b.keySize)
		}
		// ANNOTATION:
...
func (b *builtinAesEncryptIVSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	strBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	ivBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(ivBuf)
...
func (b *builtinDecodeSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
...
func (b *builtinEncodeSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/encryption-functions.html#function_aes-decrypt
func (b *builtinAesDecryptIVSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	strBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinRandomBytesSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	for i := 0; i < n; i++ {
		...
		if n, err := io.INTERLOCKyN(&dst, rand.Reader, byteLen); err != nil {
			return err
		} else if n != byteLen {
...
func (b *builtinMD5Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinSHA2Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCompressSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		compressed, err := deflate(replog.Slice(str))
		if err != nil {
			result.AppendNull()
			continue
		}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/encryption-functions.html#function_aes-decrypt
func (b *builtinAesEncryptSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	strBuf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		//       added to str and this will damange the data layout in chunk.DeferredCauset
		str := []byte(strBuf.GetString(i))
...
func (b *builtinPasswordSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinSHA1Sig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinUncompressSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumEvents()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinUncompressedLengthSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	defer b.bufSlabPredictor.put(payloadBuf)
