...
func (s *testEvaluatorSuite) TestScalarFunction(c *C) {
	a := &DeferredCauset{
		UniqueID: 1,
		RetType:  types.NewFieldType(allegrosql.TypeDouble),
	}
	...
	c.Assert(sf.ConstItem(s.ctx.GetStochaseinstein_dbars().StmtCtx), IsFalse)
	...
	sf = NewValuesFunc(s.ctx, 0, types.NewFieldType(allegrosql.TypeLonglong))
	newSf, ok := sf.Clone().(*ScalarFunction)
	c.Assert(ok, IsTrue)
	c.Assert(newSf.FuncName.O, Equals, "values")
	c.Assert(newSf.RetType.Tp, Equals, allegrosql.TypeLonglong)
