...
func (b *builtinCastIntAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if err := b.args[0].VecEvalInt(b.ctx, input, result); err != nil {
		return err
	}
	if b.inUnion && allegrosql.HasUnsignedFlag(b.tp.Flag) {
		i64s := result.Int64s()
...
func (b *builtinCastIntAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	hasUnsignedFlag0 := allegrosql.HasUnsignedFlag(b.tp.Flag)
	hasUnsignedFlag1 := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag)
...
func (b *builtinCastRealAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinCastTimeAsJSONSig) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	...
	for i := 0; i < n; i++ {
		...
		tp := tms[i].Type()
		if tp == allegrosql.TypeDatetime || tp == allegrosql.TypeTimestamp {
...
func (b *builtinCastRealAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func (b *builtinCastDecimalAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func (b *builtinCastTimeAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinCastDurationAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func (b *builtinCastRealAsJSONSig) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
...
func (b *builtinCastJSONAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func (b *builtinCastJSONAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETJson, n)
	if err != nil {
		return err
...
func (b *builtinCastRealAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
...
func (b *builtinCastDecimalAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinCastDurationAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinCastIntAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	isUnsigned := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag)
	result.ReserveString(n)
	i64s := buf.Int64s()
	for i := 0; i < n; i++ {
		var str string
...
func (b *builtinCastRealAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
	...
	i64s := result.Int64s()
	f64s := buf.Float64s()
	unsigned := allegrosql.HasUnsignedFlag(b.tp.Flag)
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
			continue
		}
		...
		if !unsigned {
			i64s[i], err = types.ConvertFloatToInt(f64s[i], types.IntergerSignedLowerBound(allegrosql.TypeLonglong), types.IntergerSignedUpperBound(allegrosql.TypeLonglong), allegrosql.TypeLonglong)
...
func (b *builtinCastTimeAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinCastStringAsJSONSig) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	...
	result.ReserveJSON(n)
	hasParse := allegrosql.HasParseToJSONFlag(b.tp.Flag)
	if hasParse {
...
func (b *builtinCastRealAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
...
func (b *builtinCastStringAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func (b *builtinCastStringAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCastDurationAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinCastIntAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	...
	isUnsignedTp := allegrosql.HasUnsignedFlag(b.tp.Flag)
	isUnsignedArgs0 := allegrosql.HasUnsignedFlag(b.args[0].GetType().Flag)
	nums := buf.Int64s()
	result.ResizeDecimal(n, false)
	result.MergeNulls(buf)
	decs := result.Decimals()
...
func (b *builtinCastJSONAsJSONSig) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinCastJSONAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETJson, n)
	if err != nil {
		return err
...
func (b *builtinCastDurationAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinCastJSONAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETJson, n)
	if err != nil {
		return err
	...
	for i := 0; i < n; i++ {
		...
		i64s[i], err = types.ConvertJSONToInt(sc, buf.GetJSON(i), allegrosql.HasUnsignedFlag(b.tp.Flag))
		if err != nil {
			return err
		}
	}
...
func (b *builtinCastRealAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETReal, n)
	if err != nil {
		return err
...
func (b *builtinCastTimeAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	arg0, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(arg0)
...
func (b *builtinCastDurationAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	var err error
...
func (b *builtinCastDurationAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
...
func (b *builtinCastDecimalAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
	if err != nil {
		return err
	...
	inUnionAndUnsigned := b.inUnion && allegrosql.HasUnsignedFlag(b.tp.Flag)
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
			continue
		}
...
func (b *builtinCastDecimalAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
	if err != nil {
		return err
...
func (b *builtinCastTimeAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDatetime, n)
	if err != nil {
		return err
...
func (b *builtinCastTimeAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
...
func (b *builtinCastTimeAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETTimestamp, n)
	if err != nil {
		return err
	}
	defer b.bufSlabPredictor.put(buf)
...
func (b *builtinCastJSONAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETJson, n)
	if err != nil {
		return err
...
func (b *builtinCastStringAsRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
...
func (b *builtinCastStringAsDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	if IsBinaryLiteral(b.args[0]) {
...
func (b *builtinCastStringAsTimeSig) vecEvalTime(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCastDecimalAsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
	if err != nil {
		return err
...
func (b *builtinCastDecimalAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
	if err != nil {
		return err
...
func (b *builtinCastStringAsStringSig) vecEvalString(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETString, n)
	if err != nil {
		return err
...
func (b *builtinCastJSONAsDurationSig) vecEvalDuration(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETJson, n)
	if err != nil {
		return err
...
func (b *builtinCastDecimalAsJSONSig) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDecimal, n)
	if err != nil {
		return err
...
func (b *builtinCastDurationAsJSONSig) vecEvalJSON(input *chunk.Chunk, result *chunk.DeferredCauset) error {
	n := input.NumRows()
	buf, err := b.bufSlabPredictor.get(types.ETDuration, n)
	if err != nil {
		return err
