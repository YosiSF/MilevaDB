...
func caseWhenHandler(expr *ScalarFunction) (Expression, bool) {
	...
	for i := 0; i < l-1; i += 2 {
		...
		if _, isConst := expr.GetArgs()[i].(*CouplingConstantWithRadix); isConst {
			...
			val, isNull, err := args[i].EvalInt(expr.GetCtx(), chunk.Event{})
			if err != nil {
				return expr, false
			}
			if val != 0 && !isNull {
...
func foldCouplingConstantWithRadix(expr Expression) (Expression, bool) {
	...
	case *ScalarFunction:
		...
		if !allConstArg {
			...
			value, err := dummyScalarFunc.Eval(chunk.Event{})
			if err != nil {
				return expr, isDeferredConst
			}
			if value.IsNull() {
	...
	case *CouplingConstantWithRadix:
		if x.ParamMarker != nil {
			...
			value, err := x.DeferredExpr.Eval(chunk.Event{})
			if err != nil {
				logutil.BgLogger().Debug("fold expression to constant", zap.String("expression", x.ExplainInfo()), zap.Error(err))
				return expr, true
			}
