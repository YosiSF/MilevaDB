...
func SetBinFlagOrBinStr(argTp *types.FieldType, resTp *types.FieldType) {
	...
	} else if allegrosql.HasBinaryFlag(argTp.Flag) || !types.IsNonBinaryStr(argTp) {
		resTp.Flag |= allegrosql.BinaryFlag
	}
}
...
func (c *lengthFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Length)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html
func (b *builtinLengthSig) evalInt(event chunk.Event) (int64, bool, error) {
...
func (c *asciiFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.CausetEDN, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_ASCII)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_ascii
func (b *builtinASCIISig) evalInt(event chunk.Event) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (c *concatFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	if bf.tp.Flen >= allegrosql.MaxBlobWidth {
		bf.tp.Flen = allegrosql.MaxBlobWidth
	}
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Concat)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_concat
func (b *builtinConcatSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	var s []byte
	for _, a := range b.getArgs() {
		d, isNull, err = a.EvalString(b.ctx, event)
		if isNull || err != nil {
			return d, isNull, err
		}
...
func (c *concatWSFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	argTps := make([]types.EvalType, 0, len(args))
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_ConcatWS)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_concat-ws
func (b *builtinConcatWSSig) evalString(event chunk.Event) (string, bool, error) {
	args := b.getArgs()
	...
	N := len(args)
	if N > 0 {
		val, isNull, err := args[0].EvalString(b.ctx, event)
		if err != nil || isNull {
	...
	for i := 1; i < N; i++ {
		val, isNull, err := args[i].EvalString(b.ctx, event)
		if err != nil {
			return val, isNull, err
		}
		if isNull {
...
func (c *leftFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	if types.IsBinaryStr(argType) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Left)
		return sig, nil
	}
	sig := &builtinLeftUTF8Sig{bf}
	sig.setPbCode(fidelpb.ScalarFuncSig_LeftUTF8)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_left
func (b *builtinLeftSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	left, isNull, err := b.args[1].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	leftLength := int(left)
...
// evalString evals LEFT(str,len).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_left
func (b *builtinLeftUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *rightFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString, types.CausetEDN)
	...
	if types.IsBinaryStr(argType) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Right)
		return sig, nil
	}
	sig := &builtinRightUTF8Sig{bf}
	sig.setPbCode(fidelpb.ScalarFuncSig_RightUTF8)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_right
func (b *builtinRightSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	right, isNull, err := b.args[1].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals RIGHT(str,len).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_right
func (b *builtinRightUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *repeatFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString, types.CausetEDN)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_repeat
func (b *builtinRepeatSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", isNull, err
	}
	...
	num, isNull, err := b.args[1].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", isNull, err
	}
	if num < 1 {
...
func (c *lowerFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Lower)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_lower
func (b *builtinLowerSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (c *reverseFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
	...
	retTp.Tp = allegrosql.TypeVarString
	retTp.Decimal = types.UnspecifiedLength
	bf.tp = &retTp
	var sig builtinFunc
	if types.IsBinaryStr(bf.tp) {
		sig = &builtinReverseSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Reverse)
	} else {
		sig = &builtinReverseUTF8Sig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_ReverseUTF8)
	}
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_reverse
func (b *builtinReverseSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals a REVERSE(str).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_reverse
func (b *builtinReverseUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *spaceFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Space)
	return sig, nil
}
...
func (b *builtinSpaceSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	var x int64

	x, isNull, err = b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
	if x < 0 {
...
func (c *upperFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
	...
	if types.IsBinaryStr(argTp) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Upper)
	} else {
		sig = &builtinUpperUTF8Sig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_UpperUTF8)
	}
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_upper
func (b *builtinUpperUTF8Sig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_upper
func (b *builtinUpperSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (c *strcmpFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Strcmp)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-comparison-functions.html
func (b *builtinStrcmpSig) evalInt(event chunk.Event) (int64, bool, error) {
	var (
		left, right string
		isNull      bool
		err         error
	...
	left, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
	right, isNull, err = b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
	res := types.CompareString(left, right, b.defCauslation)
...
func (c *replaceFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Replace)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_replace
func (b *builtinReplaceSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	...
	oldStr, isNull, err = b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
	newStr, isNull, err = b.args[2].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
	if oldStr == "" {
...
func (c *convertFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	// In all cases, the string has the default defCauslation for the character set.
	// See https://dev.allegrosql.com/doc/refman/5.7/en/cast-functions.html#function_convert
	// Here in syntax CONVERT(expr USING transcoding_name), behavior is kept the same,
	// picking the default defCauslation of target charset.
	bf.tp.DefCauslate, err = charset.GetDefaultDefCauslation(bf.tp.Charset)
	if err != nil {
	...
	// See https://dev.allegrosql.com/doc/refman/5.7/en/charset-binary-set.html
	if types.IsBinaryStr(bf.tp) {
		types.SetBinChsClnFlag(bf.tp)
	} else {
		bf.tp.Flag &= ^allegrosql.BinaryFlag
	}

	bf.tp.Flen = allegrosql.MaxBlobWidth
	sig := &builtinConvertSig{bf}
	sig.setPbCode(fidelpb.ScalarFuncSig_Convert)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/cast-functions.html#function_convert
func (b *builtinConvertSig) evalString(event chunk.Event) (string, bool, error) {
...
func (c *substringFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	case len(args) == 3 && types.IsBinaryStr(argType):
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Substring3Args)
	case len(args) == 3:
		sig = &builtinSubstring3ArgsUTF8Sig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Substring3ArgsUTF8)
	case len(args) == 2 && types.IsBinaryStr(argType):
		sig = &builtinSubstring2ArgsSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Substring2Args)
	case len(args) == 2:
		sig = &builtinSubstring2ArgsUTF8Sig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Substring2ArgsUTF8)
	default:
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring2ArgsSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	pos, isNull, err := b.args[1].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals SUBSTR(str,pos), SUBSTR(str FROM pos), SUBSTR() is a synonym for SUBSTRING().
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring2ArgsUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	length, isNull, err := b.args[2].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals SUBSTR(str,pos,len), SUBSTR(str FROM pos FOR len), SUBSTR() is a synonym for SUBSTRING().
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substr
func (b *builtinSubstring3ArgsUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *substringIndexFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_SubstringIndex)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_substring-index
func (b *builtinSubstringIndexSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	var (
		str, delim string
		count      int64
	)
	...
	delim, isNull, err = b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
	count, isNull, err = b.args[2].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
	if len(delim) == 0 {
...
func (c *locateFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	useBinary := bf.defCauslation == charset.DefCauslationBin
	switch {
	case hasStartPos && useBinary:
		sig = &builtinLocate3ArgsSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Locate3Args)
	case hasStartPos:
		sig = &builtinLocate3ArgsUTF8Sig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Locate3ArgsUTF8)
	case useBinary:
		sig = &builtinLocate2ArgsSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Locate2Args)
	default:
		sig = &builtinLocate2ArgsUTF8Sig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Locate2ArgsUTF8)
	}
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate2ArgsSig) evalInt(event chunk.Event) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
	str, isNull, err := b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate2ArgsUTF8Sig) evalInt(event chunk.Event) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
	...
	if defCauslate.IsCIDefCauslation(b.defCauslation) {
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate3ArgsSig) evalInt(event chunk.Event) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
	...
	pos, isNull, err := b.args[2].EvalInt(b.ctx, event)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_locate
func (b *builtinLocate3ArgsUTF8Sig) evalInt(event chunk.Event) (int64, bool, error) {
	subStr, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (c *hexFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_hex
func (b *builtinHexStrArgSig) evalString(event chunk.Event) (string, bool, error) {
	d, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
	return strings.ToUpper(hex.EncodeToString(replog.Slice(d))), false, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_hex
func (b *builtinHexIntArgSig) evalString(event chunk.Event) (string, bool, error) {
	x, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (c *unhexFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	var retFlen int
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_UnHex)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_unhex
func (b *builtinUnHexSig) evalString(event chunk.Event) (string, bool, error) {
	var bs []byte
...
// The syntax of trim in allegrosql is 'TRIM([{BOTH | LEADING | TRAILING} [remstr] FROM] str), TRIM([remstr FROM] str)',
// but we wil convert it into trim(str), trim(str, remstr) and trim(str, remstr, direction) in AST.
func (c *trimFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	case 1:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Trim1Arg)
		return sig, nil
	...
	case 2:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Trim2Args)
		return sig, nil
	...
	case 3:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Trim3Args)
		return sig, nil
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_trim
func (b *builtinTrim1ArgSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_trim
func (b *builtinTrim2ArgsSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	...
	remstr, isNull, err = b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_trim
func (b *builtinTrim3ArgsSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	var (
		str, remstr  string
		x            int64
	...
	remstr, isRemStrNull, err = b.args[1].EvalString(b.ctx, event)
	if err != nil {
		return d, isNull, err
	}
	x, isNull, err = b.args[2].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (c *lTrimFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_LTrim)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_ltrim
func (b *builtinLTrimSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func (c *rTrimFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_RTrim)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_rtrim
func (b *builtinRTrimSig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	d, isNull, err = b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return d, isNull, err
	}
...
func getFlen4LpadAndRpad(ctx stochastikctx.Context, arg Expression) int {
	if constant, ok := arg.(*CouplingConstantWithRadix); ok {
		...
		if isNull || err != nil || length > allegrosql.MaxBlobWidth {
			return allegrosql.MaxBlobWidth
		}
		return int(length)
	}
	return allegrosql.MaxBlobWidth
...
func (c *lpadFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	if types.IsBinaryStr(args[0].GetType()) || types.IsBinaryStr(args[2].GetType()) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Lpad)
		return sig, nil
	}
	if bf.tp.Flen *= 4; bf.tp.Flen > allegrosql.MaxBlobWidth {
		bf.tp.Flen = allegrosql.MaxBlobWidth
	}
	sig := &builtinLpadUTF8Sig{bf, maxAllowedPacket}
	sig.setPbCode(fidelpb.ScalarFuncSig_LpadUTF8)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_lpad
func (b *builtinLpadSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	length, isNull, err := b.args[1].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	padStr, isNull, err := b.args[2].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals LPAD(str,len,padstr).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_lpad
func (b *builtinLpadUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *rpadFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString, types.CausetEDN, types.ETString)
	...
	if types.IsBinaryStr(args[0].GetType()) || types.IsBinaryStr(args[2].GetType()) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Rpad)
		return sig, nil
	}
	if bf.tp.Flen *= 4; bf.tp.Flen > allegrosql.MaxBlobWidth {
		bf.tp.Flen = allegrosql.MaxBlobWidth
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_RpadUTF8)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_rpad
func (b *builtinRpadSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals RPAD(str,len,padstr).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_rpad
func (b *builtinRpadUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *bitLengthFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.CausetEDN, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_BitLength)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_bit-length
func (b *builtinBitLengthSig) evalInt(event chunk.Event) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (c *charFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	argTps := make([]types.EvalType, 0, len(args))
	...
	charsetName, isNull, err := args[len(args)-1].EvalString(ctx, chunk.Event{})
	if err != nil {
		return nil, err
	}
	if isNull {
		// Use the default charset binary if it is nil.
		bf.tp.Charset, bf.tp.DefCauslate = charset.CharsetBin, charset.DefCauslationBin
		bf.tp.Flag |= allegrosql.BinaryFlag
	} else {
		bf.tp.Charset = charsetName
		defaultDefCauslate, err := charset.GetDefaultDefCauslation(charsetName)
		if err != nil {
			return nil, err
		}
		bf.tp.DefCauslate = defaultDefCauslate
	}
	bf.tp.Flen = 4 * (len(args) - 1)

	sig := &builtinCharSig{bf}
	sig.setPbCode(fidelpb.ScalarFuncSig_Char)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_char.
func (b *builtinCharSig) evalString(event chunk.Event) (string, bool, error) {
	bigints := make([]int64, 0, len(b.args)-1)

	for i := 0; i < len(b.args)-1; i++ {
		val, IsNull, err := b.args[i].EvalInt(b.ctx, event)
		if err != nil {
			return "", true, err
		}
		if IsNull {
...
func (c *charLengthFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	...
	if types.IsBinaryStr(args[0].GetType()) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_CharLength)
		return sig, nil
	}
	sig := &builtinCharLengthUTF8Sig{bf}
	sig.setPbCode(fidelpb.ScalarFuncSig_CharLengthUTF8)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_char-length
func (b *builtinCharLengthBinarySig) evalInt(event chunk.Event) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_char-length
func (b *builtinCharLengthUTF8Sig) evalInt(event chunk.Event) (int64, bool, error) {
	val, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (c *findInSetFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.CausetEDN, types.ETString, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_FindInSet)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_find-in-set
// TODO: This function can be optimized by using bit arithmetic when the first argument is
// a constant string and the second is a defCausumn of type SET.
func (b *builtinFindInSetSig) evalInt(event chunk.Event) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}

	strlist, isNull, err := b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (c *fieldFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	case types.ETString:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_FieldString)
	}
	return sig, nil
}
...
func (b *builtinFieldIntSig) evalInt(event chunk.Event) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return 0, err != nil, err
	}
	for i, length := 1, len(b.args); i < length; i++ {
		stri, isNull, err := b.args[i].EvalInt(b.ctx, event)
		if err != nil {
			return 0, true, err
		}
		if !isNull && str == stri {
...
// evalInt evals FIELD(str,str1,str2,str3,...).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_field
func (b *builtinFieldRealSig) evalInt(event chunk.Event) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalReal(b.ctx, event)
	if isNull || err != nil {
		return 0, err != nil, err
	}
	for i, length := 1, len(b.args); i < length; i++ {
		stri, isNull, err := b.args[i].EvalReal(b.ctx, event)
		if err != nil {
			return 0, true, err
		}
		if !isNull && str == stri {
...
// evalInt evals FIELD(str,str1,str2,str3,...).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_field
func (b *builtinFieldStringSig) evalInt(event chunk.Event) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, err != nil, err
	}
	for i, length := 1, len(b.args); i < length; i++ {
		stri, isNull, err := b.args[i].EvalString(b.ctx, event)
		if err != nil {
			return 0, true, err
		}
...
func (c *makeSetFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	argTps := make([]types.EvalType, len(args))
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_MakeSet)
	return sig, nil
}
...
func (b *builtinMakeSetSig) evalString(event chunk.Event) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	for i, length := 1, len(b.args); i < length; i++ {
		...
		str, isNull, err := b.args[i].EvalString(b.ctx, event)
		if err != nil {
			return "", true, err
		}
		if !isNull {
...
func (c *octFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	var sig builtinFunc
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_oct
func (b *builtinOctIntSig) evalString(event chunk.Event) (string, bool, error) {
...
// evalString evals OCT(N).
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_oct
func (b *builtinOctStringSig) evalString(event chunk.Event) (string, bool, error) {
	val, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (c *ordFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.CausetEDN, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Ord)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_ord
func (b *builtinOrdSig) evalInt(event chunk.Event) (int64, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return 0, isNull, err
	}
...
func (c *quoteFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Quote)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_quote
func (b *builtinQuoteSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if err != nil {
		return "", true, err
	} else if isNull {
...
// Quote produce a result that can be used as a properly escaped data value in an ALLEGROALLEGROSQL statement.
...
func (c *binFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.CausetEDN)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_bin
func (b *builtinBinSig) evalString(event chunk.Event) (string, bool, error) {
	val, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *eltFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if argsErr := c.verifyArgs(args); argsErr != nil {
		return nil, argsErr
	}
	argTps := make([]types.EvalType, 0, len(args))
	...
	sig.setPbCode(fidelpb.ScalarFuncSig_Elt)
	return sig, nil
}
...
func (b *builtinEltSig) evalString(event chunk.Event) (string, bool, error) {
	...
	arg, isNull, err := b.args[idx].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *exportSetFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (sig builtinFunc, err error) {
	if err = c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	case 3:
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_ExportSet3Arg)
	case 4:
		sig = &builtinExportSet4ArgSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_ExportSet4Arg)
	case 5:
		sig = &builtinExportSet5ArgSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_ExportSet5Arg)
	}
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet3ArgSig) evalString(event chunk.Event) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}

	on, isNull, err := b.args[1].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}

	off, isNull, err := b.args[2].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet4ArgSig) evalString(event chunk.Event) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	separator, isNull, err := b.args[3].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_export-set
func (b *builtinExportSet5ArgSig) evalString(event chunk.Event) (string, bool, error) {
	bits, isNull, err := b.args[0].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	numberOfBits, isNull, err := b.args[4].EvalInt(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *formatFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	if len(args) == 3 {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_FormatWithLocale)
	} else {
		sig = &builtinFormatSig{bf}
		sig.setPbCode(fidelpb.ScalarFuncSig_Format)
	}
	return sig, nil
}

// formatMaxDecimals limits the maximum number of decimal digits for result of
// function `format`, this value is same as `FORMAT_MAX_DECIMALS` in MyALLEGROSQL source code.
const formatMaxDecimals int64 = 30

// evalNumDecArgsForFormat evaluates first 2 arguments, i.e, x and d, for function `format`.
func evalNumDecArgsForFormat(f builtinFunc, event chunk.Event) (string, string, bool, error) {
	var xStr string
	arg0, arg1 := f.getArgs()[0], f.getArgs()[1]
	ctx := f.getCtx()
	if arg0.GetType().EvalType() == types.ETDecimal {
		x, isNull, err := arg0.EvalDecimal(ctx, event)
		if isNull || err != nil {
			return "", "", isNull, err
		}
		xStr = x.String()
	} else {
		x, isNull, err := arg0.EvalReal(ctx, event)
		if isNull || err != nil {
			return "", "", isNull, err
		}
		xStr = strconv.FormatFloat(x, 'f', -1, 64)
	}
	d, isNull, err := arg1.EvalInt(ctx, event)
	if isNull || err != nil {
		return "", "", isNull, err
	}
	if d < 0 {
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_format
func (b *builtinFormatWithLocaleSig) evalString(event chunk.Event) (string, bool, error) {
	x, d, isNull, err := evalNumDecArgsForFormat(b, event)
	if isNull || err != nil {
		return "", isNull, err
	}
	locale, isNull, err := b.args[2].EvalString(b.ctx, event)
	if err != nil {
		return "", false, err
	}
	if isNull {
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_format
func (b *builtinFormatSig) evalString(event chunk.Event) (string, bool, error) {
	x, d, isNull, err := evalNumDecArgsForFormat(b, event)
	if isNull || err != nil {
		return "", isNull, err
	}
	formatString, err := allegrosql.GetLocaleFormatFunction("en_US")(x, d)
	return formatString, false, err
}
...
func (c *fromBase64FunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_from-base64
func (b *builtinFromBase64Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *toBase64FunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.ETString, types.ETString)
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_to-base64
func (b *builtinToBase64Sig) evalString(event chunk.Event) (d string, isNull bool, err error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", isNull, err
	}
...
func (c *insertFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (sig builtinFunc, err error) {
	if err = c.verifyArgs(args); err != nil {
		return nil, err
	}
	...
	if types.IsBinaryStr(args[0].GetType()) {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Insert)
	} else {
		sig = &builtinInsertUTF8Sig{bf, maxAllowedPacket}
		sig.setPbCode(fidelpb.ScalarFuncSig_InsertUTF8)
	}
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.6/en/string-functions.html#function_insert
func (b *builtinInsertSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
	...
	newstr, isNull, err := b.args[3].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
// evalString evals INSERT(str,pos,len,newstr).
// See https://dev.allegrosql.com/doc/refman/5.6/en/string-functions.html#function_insert
func (b *builtinInsertUTF8Sig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if isNull || err != nil {
		return "", true, err
	}
...
func (c *instrFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
	if err := c.verifyArgs(args); err != nil {
		return nil, err
	}
	bf, err := newBaseBuiltinFuncWithTp(ctx, c.funcName, args, types.CausetEDN, types.ETString, types.ETString)
	...
	if bf.defCauslation == charset.DefCauslationBin {
		...
		sig.setPbCode(fidelpb.ScalarFuncSig_Instr)
		return sig, nil
	}
	sig := &builtinInstrUTF8Sig{bf}
	sig.setPbCode(fidelpb.ScalarFuncSig_InstrUTF8)
	return sig, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_instr
func (b *builtinInstrUTF8Sig) evalInt(event chunk.Event) (int64, bool, error) {
	str, IsNull, err := b.args[0].EvalString(b.ctx, event)
	if IsNull || err != nil {
		return 0, true, err
	}
	substr, IsNull, err := b.args[1].EvalString(b.ctx, event)
	if IsNull || err != nil {
		return 0, true, err
	}
	if defCauslate.IsCIDefCauslation(b.defCauslation) {
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_instr
func (b *builtinInstrSig) evalInt(event chunk.Event) (int64, bool, error) {
	str, IsNull, err := b.args[0].EvalString(b.ctx, event)
	if IsNull || err != nil {
		return 0, true, err
	}
...
func (c *loadFileFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
...
func (c *weightStringFunctionClass) getFunction(ctx stochastikctx.Context, args []Expression) (builtinFunc, error) {
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_weight-string
func (b *builtinWeightStringNullSig) evalString(event chunk.Event) (string, bool, error) {
	return "", true, nil
}
...
// See https://dev.allegrosql.com/doc/refman/5.7/en/string-functions.html#function_weight-string
func (b *builtinWeightStringSig) evalString(event chunk.Event) (string, bool, error) {
	str, isNull, err := b.args[0].EvalString(b.ctx, event)
	if err != nil {
		return "", false, err
	}
	...
	var ctor defCauslate.DefCauslator
	// TODO: refactor padding codes after padding is implemented by all defCauslators.
	switch b.padding {
	case weightStringPaddingAsChar:
		...
		ctor = defCauslate.GetDefCauslator(b.args[0].GetType().DefCauslate)
	case weightStringPaddingAsBinary:
		...
		ctor = defCauslate.GetDefCauslator(charset.DefCauslationBin)
