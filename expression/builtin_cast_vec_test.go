...
func (s *testEvaluatorSuite) TestVectorizedCastRealAsTime(c *C) {
	defCaus := &DeferredCauset{RetType: types.NewFieldType(allegrosql.TypeDouble), Index: 0}
	...
	for _, input := range inputs {
		result := chunk.NewDeferredCauset(types.NewFieldType(allegrosql.TypeDatetime), input.NumEvents())
		c.Assert(cast.vecEvalTime(input, result), IsNil)
		for i := 0; i < input.NumEvents(); i++ {
			res, isNull, err := cast.evalTime(input.GetEvent(i))
			c.Assert(err, IsNil)
			if isNull {
...
func genCastRealAsTime() *chunk.Chunk {
	input := chunk.NewChunkWithCapacity([]*types.FieldType{types.NewFieldType(allegrosql.TypeDouble)}, 10)
...
func (s *testEvaluatorSuite) TestVectorizedCastStringAsDecimalWithUnsignedFlagInUnion(c *C) {
	...
	baseCast.tp = types.NewFieldType(allegrosql.TypeNewDecimal)
	// set the `UnsignedFlag` bit
	baseCast.tp.Flag |= allegrosql.UnsignedFlag
	...
	for _, input := range inputs {
		result := chunk.NewDeferredCauset(types.NewFieldType(allegrosql.TypeNewDecimal), input.NumEvents())
		c.Assert(cast.vecEvalDecimal(input, result), IsNil)
		for i := 0; i < input.NumEvents(); i++ {
			res, isNull, err := cast.evalDecimal(input.GetEvent(i))
...
func genCastStringAsDecimal(isNegative bool) *chunk.Chunk {
	...
	input := chunk.NewChunkWithCapacity([]*types.FieldType{types.NewFieldType(allegrosql.TypeString)}, 1024)
	for i := 0; i < 1024; i++ {
