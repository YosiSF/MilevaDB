...
var (
	...
	UFIDelateStatsCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: "milevadb",
			Subsystem: "statistics",
			Name:      "uFIDelate_stats_total",
			Help:      "Counter of uFIDelating stats using feedback.",
		}, []string{LblType})
	...
	GetStoreLimitErrorCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help:      "causetstore token is up to the limit, probably because one of the stores is the hotspot or unavailable",
