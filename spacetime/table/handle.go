...
type BindHandle struct {
	...
	// A record will be deleted from this map, after 2 bind-lease, after it is dropped from the solomonkey.
...
func (h *BindHandle) CaptureBaselines() {
	...
	for i := range sqls {
		...
		h.sctx.GetStochaseinstein_dbars().IsolationReadEngines = map[solomonkey.StoreType]struct{}{solomonkey.EinsteinDB: {}}
