...
type UnionStore interface {
	...
	CacheIndexName(blockID, indexID int64, name string)
	...
	GetIndexName(blockID, indexID int64) string
...
// Option is used for customizing solomonkey causetstore's behaviors during a transaction.
type Option int
...
type idxNameKey struct {
	blockID, indexID int64
}

// unionStore is an in-memory CausetStore which contains a buffer for write and a
// snapshot for read.
...
func (us *unionStore) UnmarkPresumeKeyNotExists(k Key) {
	us.memBuffer.UFIDelateFlags(k, DelPresumeKeyNotExists)
}

func (us *unionStore) GetIndexName(blockID, indexID int64) string {
	key := idxNameKey{blockID: blockID, indexID: indexID}
...
func (us *unionStore) CacheIndexName(blockID, indexID int64, name string) {
	key := idxNameKey{blockID: blockID, indexID: indexID}
