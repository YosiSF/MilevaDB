...
type Getter interface {
	// Get gets the value for key k from solomonkey causetstore.
	// If corresponding solomonkey pair does not exist, it returns nil and ErrNotExist.
...
type Mutator interface {
	// Set sets the value for key k as v into solomonkey causetstore.
	// v must NOT be nil or empty, otherwise it returns ErrCannotSetNilValue.
	Set(k Key, v []byte) error
	// Delete removes the entry for key k from solomonkey causetstore.
	Delete(k Key) error
}
...
// MemBuffer is an in-memory solomonkey collection, can be used to buffer write operations.
...
type Transaction interface {
	...
	GetVars() *Variables
	// BatchGet gets solomonkey from the memory buffer of statement and transaction, and the solomonkey storage.
...
// Request represents a solomonkey request.
type Request struct {
...
// SplitblockStore is the solomonkey causetstore which supports split regions.
