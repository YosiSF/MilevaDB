...
func (s *mockStorage) Describe() string {
	return "KVMockStorage is a mock CausetStore implementation, only for unittests in KV package"
}
...
func newMockStorage() CausetStorage {
	return &mockStorage{}
}

type mockSnapshot struct {
	causetstore MemBuffer
}

func (s *mockSnapshot) Get(ctx context.Context, k Key) ([]byte, error) {
	return s.causetstore.Get(ctx, k)
}
