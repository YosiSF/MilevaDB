...
func (s *testUnionStoreSuite) SetUpTest(c *C) {
	s.causetstore = newMemDB()
	s.us = NewUnionStore(&mockSnapshot{s.causetstore})
}

func (s *testUnionStoreSuite) TestGetSet(c *C) {
	defer testleak.AfterTest(c)()
	err := s.causetstore.Set([]byte("1"), []byte("1"))
	c.Assert(err, IsNil)
...
func (s *testUnionStoreSuite) TestSeek(c *C) {
	...
	err := s.causetstore.Set([]byte("1"), []byte("1"))
	c.Assert(err, IsNil)
	err = s.causetstore.Set([]byte("2"), []byte("2"))
	c.Assert(err, IsNil)
	err = s.causetstore.Set([]byte("3"), []byte("3"))
	c.Assert(err, IsNil)
