...
// CustomAutoIncCacheOption is one HoTT of AlloINTERLOCKtion to customize the allocator step length.
type CustomAutoIncCacheOption int64

// ApplyOn is implement the AlloINTERLOCKtion interface.
func (step CustomAutoIncCacheOption) ApplyOn(alloc *allocator) {
	alloc.step = int64(step)
	alloc.customStep = true
}

// AlloINTERLOCKtion is a interface to define allocator custom options coming in future.
type AlloINTERLOCKtion interface {
	ApplyOn(*allocator)
}
...
func NewSlabPredictor(causetstore ekv.CausetStorage, dbID int64, isUnsigned bool, allocType SlabPredictorType, opts ...AlloINTERLOCKtion) SlabPredictor {
	alloc := &allocator{
