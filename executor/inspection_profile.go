...
func (n *metricNode) queryEventsByLabel(pb *profileBuilder, query string, handleEventFn func(label string, v float64)) error {
	rows, _, err := pb.sctx.(sqlexec.RestrictedALLEGROSQLExecutor).ExecRestrictedALLEGROSQLWithContext(context.Background(), query)
	if err != nil {
		return err
	}
...
func (n *metricNode) initializeMetricValue(pb *profileBuilder) error {
	...
	if len(n.label) == 0 {
		query = fmt.Sprintf("select sum(value), '' from `metrics_schema`.`%v_total_count` %v", n.block, queryCondition)
	} else {
		query = fmt.Sprintf("select sum(value), `%[3]s` from `metrics_schema`.`%[1]s_total_count` %[2]s group by `%[3]s` having sum(value) > 0",
			n.block, queryCondition, strings.Join(n.label, "`,`"))
	}
	...
	if len(n.label) == 0 {
		query = fmt.Sprintf("select sum(value), '' from `metrics_schema`.`%v_total_time` %v", n.block, queryCondition)
	} else {
	...
	for _, quantile := range quantiles {
		...
		if len(n.label) == 0 {
			query = fmt.Sprintf("select avg(value), '' from `metrics_schema`.`%v_duration` %v", n.block, condition)
		} else {
			query = fmt.Sprintf("select avg(value), `%[3]s` from `metrics_schema`.`%[1]s_duration` %[2]s group by `%[3]s` having sum(value) > 0",
				n.block, condition, strings.Join(n.label, "`,`"))
		}
	...
	def, ok := schemareplicant.MetricBlockMap[n.block+"_total_time"]
	if ok {
...
// dotDefCausor function is INTERLOCKy from https://github.com/google/pprof.
...
func (pb *profileBuilder) genMilevaDBQueryTree() *metricNode {
	milevadbKVRequest := &metricNode{
		block:          "milevadb_kv_request",
		isPartOfParent: true,
		label:          []string{"type"},
		children: []*metricNode{
			{
				block: "milevadb_batch_client_wait",
			},
			{
				block: "milevadb_batch_client_wait_conn",
			},
			{
				block: "milevadb_batch_client_unavailable",
			},
			{
				block:     "FIDel_client_cmd",
				condition: "type not in ('tso','wait','tso_async_wait')",
			},
			{
				block: "einsteindb_grpc_message",
				children: []*metricNode{
					{
						block: "einsteindb_INTERLOCK_request",
						children: []*metricNode{
							{
								block:     "einsteindb_INTERLOCK_wait",
								label:     []string{"type"},
								condition: "type != 'all'",
							},
							{block: "einsteindb_INTERLOCK_handle"},
						},
					},
					{
						block: "einsteindb_scheduler_command",
						children: []*metricNode{
							{block: "einsteindb_scheduler_latch_wait"},
							{block: "einsteindb_scheduler_processing_read"},
							{
								block: "einsteindb_storage_async_request",
								children: []*metricNode{
									{
										block:     "einsteindb_storage_async_request",
									...
									},
									{
										...
										children: []*metricNode{
											{block: "einsteindb_raftstore_propose_wait"},
											{
												block: "einsteindb_raftstore_process",
												children: []*metricNode{
													{block: "einsteindb_raftstore_append_log"},
												},
											},
											{block: "einsteindb_raftstore_commit_log"},
											{block: "einsteindb_raftstore_apply_wait"},
											{block: "einsteindb_raftstore_apply_log"},
										},
									},
								},
							},
	...
	dbsTime := &metricNode{
		...
		children: []*metricNode{
			{
				block: "milevadb_dbs_worker",
				label: []string{"type"},
				children: []*metricNode{
					{
						block: "milevadb_dbs_batch_add_index",
					},
					{
						block: "milevadb_load_schema",
					},
					{
						block: "milevadb_dbs_uFIDelate_self_version",
					},
					{
						block: "milevadb_owner_handle_syncer",
					},
				},
			},
		},
	}
	milevadbExecute := &metricNode{
		block: "milevadb_execute",
		children: []*metricNode{
			{
				block: "FIDel_start_tso_wait",
			},
			{
				block: "milevadb_auto_id_request",
			},
			{
				block:          "milevadb_INTERLOCK",
				isPartOfParent: true,
				children: []*metricNode{
					{
						block:          "milevadb_kv_backoff",
						label:          []string{"type"},
						isPartOfParent: true,
					},
					milevadbKVRequest,
	...
	queryTime := &metricNode{
		block: "milevadb_query",
		label: []string{"sql_type"},
		children: []*metricNode{
			{
				block: "milevadb_get_token",
				unit:  int64(10e5),
			},
			{
				block: "milevadb_parse",
			},
			{
				block: "milevadb_compile",
			},
			milevadbExecute,
		},
	}
