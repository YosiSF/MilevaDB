...
func (s *testSuiteAgg) SetUpSuite(c *C) {
	...
	var err error
	s.testData, err = solitonutil.LoadTestSuiteData("testdata", "agg_suite")
	c.Assert(err, IsNil)
}
...
func (s *testSuiteAgg) TestAggregation(c *C) {
	...
	tk.MustQuery("select bit_and(c) from t where NULL").Check(testkit.Events("18446744073709551615"))
	tk.MustQuery("select bit_or(c) from t where NULL").Check(testkit.Events("0"))
	tk.MustQuery("select bit_xor(c) from t where NULL").Check(testkit.Events("0"))
	tk.MustQuery("select approx_count_distinct(c) from t where NULL").Check(testkit.Events("0"))
	...
	result.Check(testkit.Events("<nil> 1", "1 2", "1 3"))
	...
	result.Check(testkit.Events("1", "2", "2"))
	...
	result.Check(testkit.Events("4", "3"))
	...
	result.Check(testkit.Events("36 1", "4 3"))
	...
	result.Check(testkit.Events("12"))
	...
	result.Check(testkit.Events("<nil> 0 <nil>", "8 12 1", "5 2 3"))
	...
	result.Check(testkit.Events("-2", "-2"))
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (keywords varchar(20), type int)")
	...
	result.Check(testkit.Events("测试", "test"))
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (c int, d int)")
	tk.MustExec("insert t values (1, -1)")
	...
	result.Check(testkit.Events("1 0 1", "0 1 1", "-1 2 1"))
	result = tk.MustQuery("select d, 1-d as d, c as d from t order by d+1")
	result.Check(testkit.Events("-1 2 1", "0 1 1", "1 0 1"))
	...
	result.Check(testkit.Events("-1 -1 2 1", "0 0 1 1", "1 1 0 1"))
	result = tk.MustQuery("select d*d as d1, c as d1 from t group by d1 order by d1")
	result.Check(testkit.Events("0 1", "1 1"))
	...
	result.Check(testkit.Events("1 -1", "1 0", "1 1"))
	...
	result.Check(testkit.Events("1 -1 2"))
	tk.MustExec("drop block if exists t1")
	tk.MustExec("create block t1(a float, b int default 3)")
	...
	result.Check(testkit.Events("2 2 2"))
	...
	tk.MustExec("drop block if exists t1")
	tk.MustExec("create block t1(a int, b int, index(a))")
	...
	result.Check(testkit.Events("1", "2", "3"))
	tk.MustExec("drop block if exists t1")
	tk.MustExec("create block t1(a int, b int, index(b, a))")
	tk.MustExec("insert into t1 (a, b) values (1, 1),(2, 2),(3, 3),(1, 4), (1,1),(3, 5), (2,2), (3,5), (3,3)")
	result = tk.MustQuery("select avg(distinct b) from (select * from t1) t group by a order by a")
	result.Check(testkit.Events("2.5000", "2.0000", "4.0000"))
	result = tk.MustQuery("select sum(distinct b) from (select * from t1) t group by a order by a")
	...
	result.Check(testkit.Events("1", "2", "3"))
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (id int primary key, ds date)")
	...
	result.Check(testkit.Events("3 1991-09-06", "3 1991-09-05"))
	tk.MustExec("drop block if exists t1")
	tk.MustExec("drop block if exists t2")
	tk.MustExec("create block t1 (defCaus0 int, defCaus1 int)")
	tk.MustExec("create block t2 (defCaus0 int, defCaus1 int)")
	tk.MustExec("insert into t1 values(83, 0), (26, 0), (43, 81)")
	tk.MustExec("insert into t2 values(22, 2), (3, 12), (38, 98)")
	result = tk.MustQuery("SELECT COALESCE ( + 1, cor0.defCaus0 ) + - CAST( NULL AS DECIMAL ) FROM t2, t1 AS cor0, t2 AS cor1 GROUP BY cor0.defCaus1")
	result.Check(testkit.Events("<nil>", "<nil>"))

	tk.MustExec("drop block if exists t1")
	tk.MustExec("drop block if exists t2")
	tk.MustExec("create block t1 (c1 int)")
	tk.MustExec("create block t2 (c1 int)")
	...
	result.Check(testkit.Events("2 1"))
	...
	tk.MustExec("create block idx_agg (a int, b int, index (b))")
	tk.MustExec("insert idx_agg values (1, 1), (1, 2), (2, 2)")
	tk.MustQuery("select sum(a), sum(b) from idx_agg where b > 0 and b < 10").Check(testkit.Events("4 5"))

	// test without any aggregate function
	tk.MustQuery("select 10 from idx_agg group by b").Check(testkit.Events("10", "10"))
	tk.MustQuery("select 11 from idx_agg group by a").Check(testkit.Events("11", "11"))

	tk.MustExec("set @@milevadb_init_chunk_size=1;")
	tk.MustQuery("select group_concat(b) from idx_agg group by b;").Sort().Check(testkit.Events("1", "2,2"))
	tk.MustExec("set @@milevadb_init_chunk_size=2;")

	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int(11), b decimal(15,2))")
	tk.MustExec("insert into t values(1,771.64),(2,378.49),(3,920.92),(4,113.97)")
	tk.MustQuery("select a, max(b) from t group by a order by a limit 2").Check(testkit.Events("1 771.64", "2 378.49"))

	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int(11), b char(15))")
	tk.MustExec("insert into t values(1,771.64),(2,378.49),(3,920.92),(4,113.97)")
	tk.MustQuery("select a, max(b) from t group by a order by a limit 2").Check(testkit.Events("1 771.64", "2 378.49"))
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (id int(11) NOT NULL, tags json DEFAULT NULL)")
	...
	tk.MustQuery("select sum(tags->'$.i') from t").Check(testkit.Events("14"))
	...
	result.Check(testkit.Events("<nil> 0 <nil> <nil> 0 18446744073709551615 0 <nil> <nil> <nil>"))
	...
	result.Check(testkit.Events("<nil> 0 <nil> <nil> 0 18446744073709551615 0 <nil> <nil> <nil> 0"))
	tk.MustExec(`insert into t values (1, '{"i": 1, "n": "n1"}')`)
	result = tk.MustQuery("select t.id, count(95), sum(95), avg(95), bit_or(95), bit_and(95), bit_or(95), max(95), min(95), group_concat(95), approx_count_distinct(95) from t left join s on t.id = s.id")
	result.Check(testkit.Events("1 1 95 95.0000 95 95 95 95 95 95 1"))
	tk.MustExec("set @@milevadb_hash_join_concurrency=5")

	// test agg bit defCaus
	...
	// This result is compatible with MyALLEGROSQL, the readable result is shown in the next case.
	result = tk.MustQuery("select max(a) from t group by a order by a")
	result.Check(testkit.Events(string([]byte{0x0}), string([]byte{0x1})))
	...
	result.Check(testkit.Events("0 0 0", "1 1 1"))

	tk.MustExec("drop block t")
	tk.MustExec("create block t(a int, b int)")
	tk.MustExec("insert into t value(null, null)")
	tk.MustQuery("select group_concat(a), group_concat(distinct a) from t").Check(testkit.Events("<nil> <nil>"))
	tk.MustExec("insert into t value(1, null), (null, 1), (1, 2), (3, 4)")
	tk.MustQuery("select group_concat(a, b), group_concat(distinct a,b) from t").Check(testkit.Events("12,34 12,34"))
	tk.MustExec("set @@stochastik.milevadb_opt_distinct_agg_push_down = 0")
	tk.MustQuery("select count(distinct a) from t;").Check(testkit.Events("2"))
	tk.MustExec("set @@stochastik.milevadb_opt_distinct_agg_push_down = 1")
	tk.MustQuery("select count(distinct a) from t;").Check(testkit.Events("2"))
	tk.MustExec("set @@stochastik.milevadb_opt_distinct_agg_push_down = 0")
	tk.MustQuery("select approx_count_distinct( a) from t;").Check(testkit.Events("2"))

	tk.MustExec("drop block t")
	tk.MustExec("create block t(a decimal(10, 4))")
	tk.MustQuery("select 10 from t group by a").Check(testkit.Events())
	tk.MustExec("insert into t value(0), (-0.9871), (-0.9871)")
	tk.MustQuery("select 10 from t group by a").Check(testkit.Events("10", "10"))
	tk.MustQuery("select sum(a) from (select a from t union all select a from t) tmp").Check(testkit.Events("-3.9484"))

	tk.MustExec("drop block t")
	tk.MustExec("create block t(a tinyint, b smallint, c mediumint, d int, e bigint, f float, g double, h decimal)")
	...
	result.Check(testkit.Events("0.25 0.25 0.25 0.25 0.25 0.25 0.25"))
	...
	result.Check(testkit.Events("1 0.25 0.25", "1 0.25 0.25", "2 0 0"))
	...
	result.Check(testkit.Events("1 0.25 0.25 0.25 0.25 0.25 0.25 0.25"))

	tk.MustExec("drop block t")
	tk.MustExec("create block t(a int, b bigint, c float, d double, e decimal)")
	...
	result.Check(testkit.Events("2584338.6666666665 29.840000178019228 1.1808222222222229 12.666666666666666"))
	...
	result.Check(testkit.Events("2364075.6875 29.840000178019228 1.1808222222222229 12.666666666666666"))
	...
	result.Check(testkit.Events("1 2364075.6875", "1 2364075.6875", "1 2364075.6875", "1 2364075.6875", "2 0"))
	...
	// For issue #14072: wrong result when using generated defCausumn with aggregate statement
	tk.MustExec("drop block if exists t1;")
	tk.MustExec("create block t1 (a int, b int generated always as (-a) virtual, c int generated always as (-a) stored);")
	tk.MustExec("insert into t1 (a) values (2), (1), (1), (3), (NULL);")
	tk.MustQuery("select sum(a) from t1 group by b order by b;").Check(testkit.Events("<nil>", "3", "2", "2"))
	tk.MustQuery("select sum(a) from t1 group by c order by c;").Check(testkit.Events("<nil>", "3", "2", "2"))
	tk.MustQuery("select sum(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "-2", "-2", "-3"))
	tk.MustQuery("select sum(b) from t1 group by c order by c;").Check(testkit.Events("<nil>", "-3", "-2", "-2"))
	tk.MustQuery("select sum(c) from t1 group by a order by a;").Check(testkit.Events("<nil>", "-2", "-2", "-3"))
	tk.MustQuery("select sum(c) from t1 group by b order by b;").Check(testkit.Events("<nil>", "-3", "-2", "-2"))

	// For stddev_pop()/std()/stddev() function
	tk.MustExec("drop block if exists t1;")
	tk.MustExec(`create block t1 (grp int, a bigint unsigned, c char(10) not null);`)
	...
	tk.MustQuery(`select stddev_pop(all a) from t1;`).Check(testkit.Events("1.707825127659933"))
	tk.MustQuery(`select stddev_pop(a) from t1 group by grp order by grp;`).Check(testkit.Events("0", "0.5", "0.816496580927726"))
	tk.MustQuery(`select sum(a)+count(a)+avg(a)+stddev_pop(a) as sum from t1 group by grp order by grp;`).Check(testkit.Events("3", "10", "23.816496580927726"))
	tk.MustQuery(`select std(all a) from t1;`).Check(testkit.Events("1.707825127659933"))
	tk.MustQuery(`select std(a) from t1 group by grp order by grp;`).Check(testkit.Events("0", "0.5", "0.816496580927726"))
	tk.MustQuery(`select sum(a)+count(a)+avg(a)+std(a) as sum from t1 group by grp order by grp;`).Check(testkit.Events("3", "10", "23.816496580927726"))
	tk.MustQuery(`select stddev(all a) from t1;`).Check(testkit.Events("1.707825127659933"))
	tk.MustQuery(`select stddev(a) from t1 group by grp order by grp;`).Check(testkit.Events("0", "0.5", "0.816496580927726"))
	tk.MustQuery(`select sum(a)+count(a)+avg(a)+stddev(a) as sum from t1 group by grp order by grp;`).Check(testkit.Events("3", "10", "23.816496580927726"))
	// test null
	tk.MustExec("drop block if exists t1;")
	tk.MustExec("CREATE TABLE t1 (a int, b int);")
	tk.MustQuery("select  stddev_pop(b) from t1;").Check(testkit.Events("<nil>"))
	tk.MustQuery("select  std(b) from t1;").Check(testkit.Events("<nil>"))
	tk.MustQuery("select  stddev(b) from t1;").Check(testkit.Events("<nil>"))
	tk.MustExec("insert into t1 values (1,null);")
	tk.MustQuery("select stddev_pop(b) from t1 group by a order by a;").Check(testkit.Events("<nil>"))
	tk.MustQuery("select std(b) from t1 group by a order by a;").Check(testkit.Events("<nil>"))
	tk.MustQuery("select stddev(b) from t1 group by a order by a;").Check(testkit.Events("<nil>"))
	tk.MustExec("insert into t1 values (1,null);")
	tk.MustExec("insert into t1 values (2,null);")
	tk.MustQuery("select  stddev_pop(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "<nil>"))
	tk.MustQuery("select  std(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "<nil>"))
	tk.MustQuery("select  stddev(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "<nil>"))
	tk.MustExec("insert into t1 values (2,1);")
	tk.MustQuery("select  stddev_pop(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "0"))
	tk.MustQuery("select  std(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "0"))
	tk.MustQuery("select  stddev(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "0"))
	tk.MustExec("insert into t1 values (3,1);")
	tk.MustQuery("select  stddev_pop(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "0", "0"))
	tk.MustQuery("select  std(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "0", "0"))
	tk.MustQuery("select  stddev(b) from t1 group by a order by a;").Check(testkit.Events("<nil>", "0", "0"))
	...
	result.Check(testkit.Events("1 0.816496580927726 0.6666666666666666 1 1", "2 1.118033988749895 1.25 1.2909944487358056 1.6666666666666667"))

	// For issue #19676 The result of stddev_pop(distinct xxx) is wrong
	tk.MustExec("drop block if exists t1;")
	tk.MustExec("CREATE TABLE t1 (id int);")
	tk.MustExec("insert into t1 values (1),(2);")
	tk.MustQuery("select  stddev_pop(id) from t1;").Check(testkit.Events("0.5"))
	tk.MustExec("insert into t1 values (1);")
	tk.MustQuery("select  stddev_pop(distinct id) from t1;").Check(testkit.Events("0.5"))
}
...
func (s *testSuiteAgg) TestAggPrune(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(id int primary key, b varchar(50), c int)")
	tk.MustExec("insert into t values(1, '1ff', NULL), (2, '234.02', 1)")
	tk.MustQuery("select id, sum(b) from t group by id").Check(testkit.Events("1 1", "2 234.02"))
	tk.MustQuery("select sum(b) from t").Check(testkit.Events("235.02"))
	tk.MustQuery("select id, count(c) from t group by id").Check(testkit.Events("1 0", "2 1"))
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(id int primary key, b float, c float)")
	tk.MustExec("insert into t values(1, 1, 3), (2, 1, 6)")
	tk.MustQuery("select sum(b/c) from t group by id").Check(testkit.Events("0.3333333333333333", "0.16666666666666666"))
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(id int primary key, b float, c float, d float)")
	...
	tk.MustQuery("select count(distinct b, c, d) from t group by id").Check(testkit.Events("0", "0", "0", "0", "0", "0", "0", "1"))
	tk.MustQuery("select approx_count_distinct( b, c, d) from t group by id order by id").Check(testkit.Events("0", "0", "0", "0", "0", "0", "0", "1"))

	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int primary key, b varchar(10))")
	tk.MustExec("insert into t value(1, 11),(3, NULL)")
	tk.MustQuery("SELECT a, MIN(b), MAX(b) FROM t GROUP BY a").Check(testkit.Events("1 11 11", "3 <nil> <nil>"))
}
...
func (s *testSuiteAgg) TestGroupConcatAggr(c *C) {
	...
	result.Check(testkit.Events("1 10,20,30", "2 20", "3 200,500"))
	...
	result = tk.MustQuery("select id, group_concat(name SEPARATOR '') from test group by id order by id")
	result.Check(testkit.Events("1 102030", "2 20", "3 200500"))

	result = tk.MustQuery("select id, group_concat(name SEPARATOR '123') from test group by id order by id")
	result.Check(testkit.Events("1 101232012330", "2 20", "3 200123500"))

	tk.MustQuery("select group_concat(id ORDER BY name) from (select * from test order by id, name limit 2,2) t").Check(testkit.Events("2,1"))
	tk.MustQuery("select group_concat(id ORDER BY name desc) from (select * from test order by id, name limit 2,2) t").Check(testkit.Events("1,2"))
	tk.MustQuery("select group_concat(name ORDER BY id) from (select * from test order by id, name limit 2,2) t").Check(testkit.Events("30,20"))
	tk.MustQuery("select group_concat(name ORDER BY id desc) from (select * from test order by id, name limit 2,2) t").Check(testkit.Events("20,30"))

	result = tk.MustQuery("select group_concat(name ORDER BY name desc SEPARATOR '++') from test;")
	result.Check(testkit.Events("500++200++30++20++20++10"))

	result = tk.MustQuery("select group_concat(id ORDER BY name desc, id asc SEPARATOR '--') from test;")
	result.Check(testkit.Events("3--3--1--1--2--1"))
	...
	result.Check(testkit.Events("500++200++30++20++20++10 3--3--1--1--2--1"))

	result = tk.MustQuery("select group_concat(distinct name order by name desc) from test;")
	result.Check(testkit.Events("500,200,30,20,10"))
	...
	tk.MustExec("create block test2(id varchar(20), name varchar(20));")
	tk.MustExec("insert into test2 select * from test;")

	tk.MustQuery("select group_concat(id ORDER BY name) from (select * from test2 order by id, name limit 2,2) t").Check(testkit.Events("2,1"))
	tk.MustQuery("select group_concat(id ORDER BY name desc) from (select * from test2 order by id, name limit 2,2) t").Check(testkit.Events("1,2"))
	tk.MustQuery("select group_concat(name ORDER BY id) from (select * from test2 order by id, name limit 2,2) t").Check(testkit.Events("30,20"))
	tk.MustQuery("select group_concat(name ORDER BY id desc) from (select * from test2 order by id, name limit 2,2) t").Check(testkit.Events("20,30"))
	...
	result.Check(testkit.Events("500++30++200++20++20++10 3--1--3--1--2--1"))
	...
	tk.MustQuery("select 1, 2, 3, 4, 5 , group_concat(name, id ORDER BY 2 desc, name SEPARATOR '++') from test;").Check(testkit.Events("1 2 3 4 5 2003++5003++202++101++201++301"))
	err = tk.ExecToErr("select 1, 2, 3, 4, 5 , group_concat(name, id ORDER BY 3 desc, name SEPARATOR '++') from test;")
	c.Assert(err.Error(), Equals, "[planner:1054]Unknown defCausumn '3' in 'order clause'")

	// test Param Marker
	tk.MustExec(`prepare s1 from "select 1, 2, 3, 4, 5 , group_concat(name, id ORDER BY floor(id/?) desc, name SEPARATOR '++') from test";`)
	tk.MustExec("set @a=2;")
	tk.MustQuery("execute s1 using @a;").Check(testkit.Events("1 2 3 4 5 202++2003++5003++101++201++301"))

	tk.MustExec(`prepare s1 from "select 1, 2, 3, 4, 5 , group_concat(name, id ORDER BY ? desc, name SEPARATOR '++') from test";`)
	tk.MustExec("set @a=2;")
	tk.MustQuery("execute s1 using @a;").Check(testkit.Events("1 2 3 4 5 2003++5003++202++101++201++301"))
	tk.MustExec("set @a=3;")
	...
	tk.MustQuery("execute s1 using @a;").Check(testkit.Events("1 2 3 4 5 101++202++201++301++2003++5003"))
	...
	for i := 0; i <= 1; i++ {
		for j := 0; j <= 1; j++ {
			tk.MustExec(fmt.Sprintf("set stochastik milevadb_opt_distinct_agg_push_down = %v", i))
			tk.MustExec(fmt.Sprintf("set stochastik milevadb_opt_agg_push_down = %v", j))

			result = tk.MustQuery("select /*+ agg_to_INTERLOCK */ group_concat(name ORDER BY name desc SEPARATOR '++'), group_concat(id ORDER BY name desc, id asc SEPARATOR '--') from ptest;")
			result.Check(testkit.Events("500++200++30++20++20++10 3--3--1--1--2--1"))

			result = tk.MustQuery("select /*+ agg_to_INTERLOCK */ group_concat(distinct name order by name desc) from ptest;")
			result.Check(testkit.Events("500,200,30,20,10"))
		}
	}

	// issue #9920
	tk.MustQuery("select group_concat(123, null)").Check(testkit.Events("<nil>"))
}
...
func (s *testSuiteAgg) TestAggPushDown(c *C) {
	...
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (a int, b int, c int)")
	tk.MustExec("alter block t add index idx(a, b, c)")
	// test for empty block
	tk.MustQuery("select count(a) from t group by a;").Check(testkit.Events())
	tk.MustQuery("select count(a) from t;").Check(testkit.Events("0"))
	// test for one event
	tk.MustExec("insert t values(0,0,0)")
	tk.MustQuery("select distinct b from t").Check(testkit.Events("0"))
	tk.MustQuery("select count(b) from t group by a;").Check(testkit.Events("1"))
	// test for rows
	tk.MustExec("insert t values(1,1,1),(3,3,6),(3,2,5),(2,1,4),(1,1,3),(1,1,2);")
	tk.MustQuery("select count(a) from t where b>0 group by a, b;").Sort().Check(testkit.Events("1", "1", "1", "3"))
	tk.MustQuery("select count(a) from t where b>0 group by a, b order by a;").Check(testkit.Events("3", "1", "1", "1"))
	tk.MustQuery("select count(a) from t where b>0 group by a, b order by a limit 1;").Check(testkit.Events("3"))

	tk.MustExec("drop block if exists t, tt")
	tk.MustExec("create block t(a int primary key, b int, c int)")
	tk.MustExec("create block tt(a int primary key, b int, c int)")
	tk.MustExec("insert into t values(1, 1, 1), (2, 1, 1)")
	tk.MustExec("insert into tt values(1, 2, 1)")
	tk.MustQuery("select max(a.b), max(b.b) from t a join tt b on a.a = b.a group by a.c").Check(testkit.Events("1 2"))
	tk.MustQuery("select a, count(b) from (select * from t union all select * from tt) k group by a order by a").Check(testkit.Events("1 2", "2 1"))
}
...
func (s *testSuiteAgg) TestOnlyFullGroupBy(c *C) {
	...
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int not null primary key, b int not null, c int default null, d int not null, unique key I_b_c (b,c), unique key I_b_d (b,d))")
	tk.MustExec("create block x(a int not null primary key, b int not null, c int default null, d int not null, unique key I_b_c (b,c), unique key I_b_d (b,d))")
	...
	// test functional depend on unique not null defCausumns
	...
	// FixMe: test functional dependency of derived block
...
func (s *testSuiteAgg) TestIssue16279(c *C) {
	...
	tk.MustExec("create block s(a int);")
...
func (s *testSuiteAgg) TestAggPushDownPartitionBlock(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t1")
	...
	tk.MustExec("set @@milevadb_opt_agg_push_down = 1")
	tk.MustQuery("select /*+ AGG_TO_INTERLOCK() */ sum(a), sum(b) from t1 where a < 40 group by a").Sort().Check(testkit.Events(
		"0 0",
		"24 25",
		"28 29",
		"3 6",
...
func (s *testSuiteAgg) TestIssue13652(c *C) {
	...
	c.Assert(err.Error(), Equals, "[planner:1055]Expression #1 of SELECT list is not in GROUP BY clause and contains nonaggregated defCausumn 'test.t.a' which is not functionally dependent on defCausumns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by")
}
...
func (s *testSuiteAgg) TestHaving(c *C) {
	...
	tk.MustQuery("select c1 as c2, c3 from t having c2 = 2").Check(testkit.Events("2 1"))
	tk.MustQuery("select c1 as c2, c3 from t group by c2 having c2 = 2;").Check(testkit.Events("1 3"))
	tk.MustQuery("select c1 as c2, c3 from t group by c2 having sum(c2) = 2;").Check(testkit.Events("1 3"))
	tk.MustQuery("select c1 as c2, c3 from t group by c3 having sum(c2) = 2;").Check(testkit.Events("1 3"))
	tk.MustQuery("select c1 as c2, c3 from t group by c3 having sum(0) + c2 = 2;").Check(testkit.Events("2 1"))
	tk.MustQuery("select c1 as a from t having c1 = 1;").Check(testkit.Events("1"))
	tk.MustQuery("select t.c1 from t having c1 = 1;").Check(testkit.Events("1"))
	tk.MustQuery("select a.c1 from t as a having c1 = 1;").Check(testkit.Events("1"))
	tk.MustQuery("select c1 as a from t group by c3 having sum(a) = 1;").Check(testkit.Events("1"))
	tk.MustQuery("select c1 as a from t group by c3 having sum(a) + a = 2;").Check(testkit.Events("1"))
	tk.MustQuery("select a.c1 as c, a.c1 as d from t as a, t as b having c1 = 1 limit 1;").Check(testkit.Events("1 1"))

	tk.MustQuery("select sum(c1) as s from t group by c1 having sum(c1) order by s").Check(testkit.Events("1", "2", "3"))
	tk.MustQuery("select sum(c1) - 1 as s from t group by c1 having sum(c1) - 1 order by s").Check(testkit.Events("1", "2"))
	tk.MustQuery("select 1 from t group by c1 having sum(abs(c2 + c3)) = c1").Check(testkit.Events("1"))
}
...
func (s *testSuiteAgg) TestAggEliminator(c *C) {
	...
	tk.MustQuery("select min(a), min(a) from t").Check(testkit.Events("<nil> <nil>"))
	tk.MustExec("insert into t values(1, -1), (2, -2), (3, 1), (4, NULL)")
	tk.MustQuery("select max(a) from t").Check(testkit.Events("4"))
	tk.MustQuery("select min(b) from t").Check(testkit.Events("-2"))
	tk.MustQuery("select max(b*b) from t").Check(testkit.Events("4"))
	tk.MustQuery("select min(b*b) from t").Check(testkit.Events("1"))
	tk.MustQuery("select group_concat(b, b) from t group by a").Sort().Check(testkit.Events("-1-1", "-2-2", "11", "<nil>"))
}
...
func (s *testSuiteAgg) TestClusterIndexMaxMinEliminator(c *C) {
	...
	tk.MustQuery("select max(a), min(a+b) from t;").Check(testkit.Events("10 0"))
	tk.MustQuery("select max(a+b), min(a+b) from t;").Check(testkit.Events("20 0"))
	tk.MustQuery("select min(a), max(a), min(b), max(b) from t;").Check(testkit.Events("0 10 0 10"))
}
...
func (s *testSuiteAgg) TestMaxMinFloatScalaFunc(c *C) {
	...
	tk.MustQuery(`SELECT MAX(CASE B WHEN 'val_b'  THEN C ELSE 0 END) val_b FROM T WHERE cast(A as signed) = 0 GROUP BY a;`).Check(testkit.Events("12.190999984741211"))
	tk.MustQuery(`SELECT MIN(CASE B WHEN 'val_b'  THEN C ELSE 0 END) val_b FROM T WHERE cast(A as signed) = 0 GROUP BY a;`).Check(testkit.Events("12.190999984741211"))
}
...
func (s *testSuiteAgg) TestInjectProjBelowTopN(c *C) {
	...
	for i, tt := range input {
		s.testData.OnRecord(func() {
			output[i] = s.testData.ConvertEventsToStrings(tk.MustQuery(tt).Events())
		})
		tk.MustQuery(tt).Check(testkit.Events(output[i]...))
	}
}

func (s *testSuiteAgg) TestFirstEventEnum(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec(`use test;`)
	tk.MustExec(`drop block if exists t;`)
	tk.MustExec(`create block t(a enum('a', 'b'));`)
	tk.MustExec(`insert into t values('a');`)
	tk.MustQuery(`select a from t group by a;`).Check(testkit.Events(
		`a`,
	))
}
...
func (s *testSuiteAgg) TestAggJSON(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec(`drop block if exists t;`)
	tk.MustExec(`create block t(a datetime, b json, index idx(a));`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:00', '["a", "b", 1]');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:01', '["a", "b", 1]');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:02', '["a", "b", 1]');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:03', '{"k1": "value", "k2": [10, 20]}');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:04', '{"k1": "value", "k2": [10, 20]}');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:05', '{"k1": "value", "k2": [10, 20]}');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:06', '"hello"');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:07', '"hello"');`)
	tk.MustExec(`insert into t values('2020-03-20 21:50:08', '"hello"');`)
	tk.MustExec(`set @@sql_mode='';`)
	tk.MustQuery(`select b from t group by a order by a;`).Check(testkit.Events(
		`["a", "b", 1]`,
		`["a", "b", 1]`,
		`["a", "b", 1]`,
		`{"k1": "value", "k2": [10, 20]}`,
	...
	tk.MustQuery(`select min(b) from t group by a order by a;`).Check(testkit.Events(
		`["a", "b", 1]`,
		`["a", "b", 1]`,
		`["a", "b", 1]`,
		`{"k1": "value", "k2": [10, 20]}`,
	...
	tk.MustQuery(`select max(b) from t group by a order by a;`).Check(testkit.Events(
		`["a", "b", 1]`,
		`["a", "b", 1]`,
		`["a", "b", 1]`,
		`{"k1": "value", "k2": [10, 20]}`,
...
func (s *testSuiteAgg) TestIssue10099(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a char(10), b char(10))")
	tk.MustExec("insert into t values('1', '222'), ('12', '22')")
	tk.MustQuery("select count(distinct a, b) from t").Check(testkit.Events("2"))
	tk.MustQuery("select approx_count_distinct( a, b) from t").Check(testkit.Events("2"))
}
...
func (s *testSuiteAgg) TestIssue10098(c *C) {
	...
	tk.MustExec("create block t(a char(10), b char(10))")
	tk.MustExec("insert into t values('1', '222'), ('12', '22')")
	tk.MustQuery("select group_concat(distinct a, b) from t").Check(testkit.Events("1222,1222"))
}
...
func (s *testSuiteAgg) TestIssue10608(c *C) {
	...
	tk.MustQuery("select (select  /*+ stream_agg() */ group_concat(concat(123,'-')) from t where t.a = s.b group by t.a) as t from s;").Check(testkit.Events("123-", "123-"))
	tk.MustQuery("select (select  /*+ hash_agg() */ group_concat(concat(123,'-')) from t where t.a = s.b group by t.a) as t from s;").Check(testkit.Events("123-", "123-"))
...
func (s *testSuiteAgg) TestIssue12759HashAggCalledByApply(c *C) {
	...
	tk.MustExec(`drop block if exists test;`)
	tk.MustExec("create block test (a int);")
	tk.MustExec("insert into test value(1);")
	tk.MustQuery("select /*+ hash_agg() */ sum(a), (select NULL from test where tt.a = test.a limit 1),(select NULL from test where tt.a = test.a limit 1),(select NULL from test where tt.a = test.a limit 1) from test tt;").Check(testkit.Events("1 <nil> <nil> <nil>"))
...
func (s *testSuiteAgg) TestPR15242ShallowINTERLOCKy(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec(`drop block if exists t;`)
...
func (s *testSuiteAgg) TestIssue15690(c *C) {
	...
	tk.MustExec(`drop block if exists t;`)
	tk.MustExec(`create block t(a decimal(5,1));`)
...
func (s *testSuiteAgg) TestIssue15958(c *C) {
	...
	tk.MustQuery(`select sum(y) from t`).Check(testkit.Events("6070"))
	tk.MustQuery(`select avg(y) from t`).Check(testkit.Events("2023.3333"))
}
...
func (s *testSuiteAgg) TestIssue17216(c *C) {
	...
	tk.MustExec(`CREATE TABLE t1 (
	  pk int(11) NOT NULL,
	  defCaus1 decimal(40,20) DEFAULT NULL
	)`)
	...
	tk.MustQuery("SELECT count(distinct defCaus1) FROM t1").Check(testkit.Events("48"))
}
