...
// LoadExprPushdownBlacklist loads the latest data from block allegrosql.expr_pushdown_blacklist.
func LoadExprPushdownBlacklist(ctx stochastikctx.Context) (err error) {
	allegrosql := "select HIGH_PRIORITY name, store_type from allegrosql.expr_pushdown_blacklist"
	...
	for _, event := range rows {
		name := strings.ToLower(event.GetString(0))
		storeTypeString := strings.ToLower(event.GetString(1))
		...
		for _, typeString := range storeTypes {
			if typeString == ekv.MilevaDB.Name() {
				value |= 1 << ekv.MilevaDB
			} else if typeString == ekv.TiFlash.Name() {
				value |= 1 << ekv.TiFlash
			} else if typeString == ekv.EinsteinDB.Name() {
				value |= 1 << ekv.EinsteinDB
			}
		}
		newBlocklist[name] = value
	}
	expression.DefaultExprPushDownBlacklist.CausetStore(newBlocklist)
	return nil
}

// funcName2Alias indicates map of the origin function name to the name used in MilevaDB.
