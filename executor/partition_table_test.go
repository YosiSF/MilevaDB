...
func (s *partitionBlockSuite) TestPartitionIndexJoin(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec("drop block if exists p, t")
	tk.MustExec(`create block p (id int, c int, key i_id(id), key i_c(c)) partition by range (c) (
partition p0 values less than (4),
partition p1 values less than (7),
partition p2 values less than (10))`)
...
func (s *partitionBlockSuite) TestPartitionUnionScanIndexJoin(c *C) {
	...
	tk.MustExec("create block t2  (c_int int, c_str varchar(40), primary key (c_int, c_str)) partition by hash (c_int) partitions 4")
...
func (s *partitionBlockSuite) TestPosetDagBlockID(c *C) {
	// This test checks the block ID in the PosetDag is changed to partition ID in the nextPartition function.
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec("use test")
	tk.MustExec("create block employees (id int,store_id int not null)partition by hash(store_id) partitions 4;")
	...
	// Check block ID is changed to partition ID for each partition.
	c.Assert(m, HasLen, 4)
}
...
func (s *partitionBlockSuite) TestPartitionReaderUnderApply(c *C) {
	...
	// For issue 19450.
	tk.MustExec("drop block if exists t1, t2")
	tk.MustExec("create block t1  (c_int int, c_str varchar(40), c_decimal decimal(12, 6), primary key (c_int))")
	tk.MustExec("create block t2  (c_int int, c_str varchar(40), c_decimal decimal(12, 6), primary key (c_int)) partition by hash (c_int) partitions 4")
...
func (s *partitionBlockSuite) TestImproveCoverage(c *C) {
	tk := testkit.NewTestKitWithInit(c, s.causetstore)
	tk.MustExec("use test")
	tk.MustExec(`create block coverage_rr (
pk1 varchar(35) NOT NULL,
pk2 int NOT NULL,
c int,
PRIMARY KEY (pk1,pk2)) partition by hash(pk2) partitions 4;`)
	tk.MustExec("create block coverage_dt (pk1 varchar(35), pk2 int)")
	...
	tk.MustExec("set @@milevadb_partition_prune_mode = 'dynamic-only'")
