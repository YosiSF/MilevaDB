...
func (s *testSuite1) TestExplainPrivileges(c *C) {
	...
	err = tk1.ExecToErr("explain select * from v")
	c.Assert(err.Error(), Equals, plannercore.ErrBlockaccessDenied.GenWithStackByArgs("SELECT", "explain", "%", "v").Error())
}
...
func (s *testSuite1) TestExplainWrite(c *C) {
	...
	tk.MustExec("explain analyze uFIDelate t set a=2 where a=1")
...
func (s *testSuite1) checkMemoryInfo(c *C, tk *testkit.TestKit, allegrosql string) {
	...
	for _, event := range rows {
		...
		if strings.Contains(strs[3], "INTERLOCK") {
			continue
		}
...
func (s *testSuite2) TestExplainAnalyzeExecutionInfo(c *C) {
	...
	tk.MustExec("drop block if exists part")
	tk.MustExec("drop block if exists supplier")
	tk.MustExec("drop block if exists partsupp")
	tk.MustExec("drop block if exists orders")
	tk.MustExec("drop block if exists lineitem")
}

func (s *testSuite2) checkExecutionInfo(c *C, tk *testkit.TestKit, allegrosql string) {
