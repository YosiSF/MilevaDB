...
func (s *testSuite1) TestIndexAdvise(c *C) {
	...
	_, err := tk.Exec("index advise infile '/tmp/nonexistence.allegrosql'")
	...
	_, err = tk.Exec("index advise local infile '/tmp/nonexistence.allegrosql' lines terminated by ''")
	...
	tk.MustExec("index advise local infile '/tmp/index_advise.allegrosql' max_minutes 3 max_idxnum per_block 4 per_db 5")
