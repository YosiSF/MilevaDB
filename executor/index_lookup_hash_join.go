...
//    1. builds hash block from the outer rows
//    2. builds key ranges from outer rows and fetches inner rows
//    3. probes the hash block and sends the join result to the main thread channel.
...
type indexHashJoinInnerWorker struct {
	innerWorker
	matchedOuterPtrs  []chunk.EventPtr
	joiner            joiner
	joinChkResourceCh chan *chunk.Chunk
...
type indexHashJoinTask struct {
	*lookUpJoinTask
	...
	// len(matchedInnerEventPtrs) equals to
	// lookUpJoinTask.outerResult.NumChunks(), and the elements of every
	// matchedInnerEventPtrs[chkIdx][rowIdx] indicates the matched inner event ptrs
	// of the corresponding outer event.
...
func (e *IndexNestedLoopHashJoin) startWorkers(ctx context.Context) {
	...
	go soliton.WithRecovery(func() { ow.run(workerCtx) }, e.finishJoinWorkers)
	...
	for i := 0; i < concurrency; i++ {
		workerID := i
		go soliton.WithRecovery(func() { e.newInnerWorker(innerCh, workerID).run(workerCtx, cancelFunc) }, e.finishJoinWorkers)
	}
...
func (ow *indexHashJoinOuterWorker) run(ctx context.Context) {
	...
	for {
		...
		failpoint.Inject("testIndexHashJoinOuterWorkerErr", func() {
			err = errors.New("mocHoTTexHashJoinOuterWorkerErr")
		})
		if err != nil {
...
func (ow *indexHashJoinOuterWorker) buildTask(ctx context.Context) (*indexHashJoinTask, error) {
	...
	if ow.keepOuterOrder {
		...
		matchedInnerEventPtrs = make([][][]chunk.EventPtr, task.outerResult.NumChunks())
		for i := range matchedInnerEventPtrs {
			matchedInnerEventPtrs[i] = make([][]chunk.EventPtr, task.outerResult.GetChunk(i).NumEvents())
		}
	}
	numChks := task.outerResult.NumChunks()
	outerEventStatus := make([][]outerEventStatusFlag, numChks)
	for i := 0; i < numChks; i++ {
...
func (iw *indexHashJoinInnerWorker) run(ctx context.Context, cancelFunc context.CancelFunc) {
	...
	failpoint.Inject("testIndexHashJoinInnerWorkerErr", func() {
		joinResult.err = errors.New("mocHoTTexHashJoinInnerWorkerErr")
	})
	...
	if resultCh == iw.resultCh && joinResult.chk != nil && joinResult.chk.NumEvents() > 0 {
		select {
...
func (iw *indexHashJoinInnerWorker) buildHashBlockForOuterResult(ctx context.Context, task *indexHashJoinTask, h hash.Hash64) {
	if iw.stats != nil {
		start := time.Now()
		defer func() {
	...
	task.lookupMap = newUnsafeHashBlock(task.outerResult.Len())
	...
	OUTER:
		for rowIdx := 0; rowIdx < numEvents; rowIdx++ {
			...
			event := chk.GetEvent(rowIdx)
			keyDefCausIdx := iw.outerCtx.keyDefCauss
			for _, i := range keyDefCausIdx {
				if event.IsNull(i) {
					continue OUTER
				}
			}
			...
			rowPtr := chunk.EventPtr{ChkIdx: uint32(chkIdx), EventIdx: uint32(rowIdx)}
...
func (iw *indexHashJoinInnerWorker) handleTask(ctx context.Context, task *indexHashJoinTask, joinResult *indexHashJoinResult, h hash.Hash64, resultCh chan *indexHashJoinResult) error {
	...
	iw.wg.Add(1)
	// TODO(XuHuaiyu): we may always use the smaller side to build the hashblock.
	go soliton.WithRecovery(func() { iw.buildHashBlockForOuterResult(ctx, task, h) }, iw.handleHashJoinInnerWorkerPanic)
...
func (iw *indexHashJoinInnerWorker) doJoinUnordered(ctx context.Context, task *indexHashJoinTask, joinResult *indexHashJoinResult, h hash.Hash64, resultCh chan *indexHashJoinResult) error {
	...
	for event := iter.Begin(); event != iter.End(); event = iter.Next() {
		ok, joinResult = iw.joinMatchedInnerEvent2Chunk(ctx, event, task, joinResult, h, iw.joinKeyBuf)
		if !ok {
			return errors.New("indexHashJoinInnerWorker.doJoinUnordered failed")
		}
	}
	for chkIdx, outerEventStatus := range task.outerEventStatus {
		chk := task.outerResult.GetChunk(chkIdx)
		for rowIdx, val := range outerEventStatus {
			if val == outerEventMatched {
				continue
			}
			iw.joiner.onMissMatch(val == outerEventHasNull, chk.GetEvent(rowIdx), joinResult.chk)
...
func (iw *indexHashJoinInnerWorker) getMatchedOuterEvents(innerEvent chunk.Event, task *indexHashJoinTask, h hash.Hash64, buf []byte) (matchedEvents []chunk.Event, matchedEventPtr []chunk.EventPtr, err error) {
	...
	for _, ptr := range iw.matchedOuterPtrs {
		...
		if !ok || (task.outerEventStatus[ptr.ChkIdx][ptr.EventIdx] == outerEventMatched && isSemiJoin) {
			continue
		}
		matchedEvents = append(matchedEvents, outerEvent)
		matchedEventPtr = append(matchedEventPtr, chunk.EventPtr{ChkIdx: ptr.ChkIdx, EventIdx: ptr.EventIdx})
	}
	return matchedEvents, matchedEventPtr, nil
}

func (iw *indexHashJoinInnerWorker) joinMatchedInnerEvent2Chunk(ctx context.Context, innerEvent chunk.Event, task *indexHashJoinTask,
	joinResult *indexHashJoinResult, h hash.Hash64, buf []byte) (bool, *indexHashJoinResult) {
	matchedOuterEvents, matchedOuterEventPtr, err := iw.getMatchedOuterEvents(innerEvent, task, h, buf)
	if err != nil {
		joinResult.err = err
		return false, joinResult
	}
	if len(matchedOuterEvents) == 0 {
		return true, joinResult
	}
	var (
		ok     bool
		iter   = chunk.NewIterator4Slice(matchedOuterEvents)
		cursor = 0
	)
	for iter.Begin(); iter.Current() != iter.End(); {
		iw.outerEventStatus, err = iw.joiner.tryToMatchOuters(iter, innerEvent, joinResult.chk, iw.outerEventStatus)
		if err != nil {
			joinResult.err = err
			return false, joinResult
		}
		for _, status := range iw.outerEventStatus {
			chkIdx, rowIdx := matchedOuterEventPtr[cursor].ChkIdx, matchedOuterEventPtr[cursor].EventIdx
			if status == outerEventMatched || task.outerEventStatus[chkIdx][rowIdx] == outerEventUnmatched {
				task.outerEventStatus[chkIdx][rowIdx] = status
			}
			cursor++
		}
		if joinResult.chk.IsFull() {
...
func (iw *indexHashJoinInnerWorker) defCauslectMatchedInnerPtrs4OuterEvents(ctx context.Context, innerEvent chunk.Event, innerEventPtr chunk.EventPtr,
	task *indexHashJoinTask, h hash.Hash64, buf []byte) error {
	_, matchedOuterEventIdx, err := iw.getMatchedOuterEvents(innerEvent, task, h, buf)
	if err != nil {
		return err
	}
	for _, outerEventPtr := range matchedOuterEventIdx {
		chkIdx, rowIdx := outerEventPtr.ChkIdx, outerEventPtr.EventIdx
		task.matchedInnerEventPtrs[chkIdx][rowIdx] = append(task.matchedInnerEventPtrs[chkIdx][rowIdx], innerEventPtr)
	}
	return nil
}

// doJoinInOrder follows the following steps:
// 1. defCauslect all the matched inner event ptrs for every outer event
// 2. do the join work
//   2.1 defCauslect all the matched inner rows using the defCauslected ptrs for every outer event
//   2.2 call tryToMatchInners for every outer event
...
func (iw *indexHashJoinInnerWorker) doJoinInOrder(ctx context.Context, task *indexHashJoinTask, joinResult *indexHashJoinResult, h hash.Hash64, resultCh chan *indexHashJoinResult) (err error) {
	...
	for i, numChunks := 0, task.innerResult.NumChunks(); i < numChunks; i++ {
		for j, chk := 0, task.innerResult.GetChunk(i); j < chk.NumEvents(); j++ {
			event := chk.GetEvent(j)
			ptr := chunk.EventPtr{ChkIdx: uint32(i), EventIdx: uint32(j)}
			err = iw.defCauslectMatchedInnerPtrs4OuterEvents(ctx, event, ptr, task, h, iw.joinKeyBuf)
			if err != nil {
				return err
			}
		}
	}
	// TODO: matchedInnerEventPtrs and matchedInnerEvents can be moved to inner worker.
	matchedInnerEvents := make([]chunk.Event, len(task.matchedInnerEventPtrs))
	var hasMatched, hasNull, ok bool
	for chkIdx, innerEventPtrs4Chk := range task.matchedInnerEventPtrs {
		for outerEventIdx, innerEventPtrs := range innerEventPtrs4Chk {
			matchedInnerEvents, hasMatched, hasNull = matchedInnerEvents[:0], false, false
			outerEvent := task.outerResult.GetChunk(chkIdx).GetEvent(outerEventIdx)
			for _, ptr := range innerEventPtrs {
				matchedInnerEvents = append(matchedInnerEvents, task.innerResult.GetEvent(ptr))
			}
			iter := chunk.NewIterator4Slice(matchedInnerEvents)
			for iter.Begin(); iter.Current() != iter.End(); {
				matched, isNull, err := iw.joiner.tryToMatchInners(outerEvent, iter, joinResult.chk)
				if err != nil {
					return err
				}
			...
			if !hasMatched {
				iw.joiner.onMissMatch(hasNull, outerEvent, joinResult.chk)
			}
		}
	}
	return nil
