...
func NewLoadDataInfo(ctx stochastikctx.Context, event []types.Causet, tbl block.Block, defcaus []*block.DeferredCauset) *LoadDataInfo {
	insertVal := &InsertValues{baseExecutor: newBaseExecutor(ctx, nil, 0), Block: tbl}
	return &LoadDataInfo{
		event:        event,
...
// FieldMapping inticates the relationship between input field and block defCausumn or user variable
type FieldMapping struct {
	DeferredCauset *block.DeferredCauset
...
func (e *LoadDataInfo) initLoadDeferredCausets(defCausumnNames []string) error {
	...
	if len(defCausumnNames) != len(blockDefCauss) {
		...
		defcaus, missingDefCausName = block.FindDefCauss(blockDefCauss, defCausumnNames, e.Block.Meta().PKIsHandle)
	...
	for _, defCaus := range defcaus {
		...
		if defCaus.Name.L == perceptron.ExtraHandleName.L {
			if !e.ctx.GetStochaseinstein_dbars().AllowWriteEventID {
				return errors.Errorf("load data statement for _milevadb_rowid are not supported.")
			}
...
// initFieldMappings make a field mapping slice to implicitly map input field to block defCausumn or user defined variable
...
func (e *LoadDataInfo) initFieldMappings() []string {
	...
	var defCausumn *block.DeferredCauset

	for _, v := range e.DeferredCausetsAndUserVars {
		if v.DeferredCausetName != nil {
			defCausumn = block.FindDefCaus(blockDefCauss, v.DeferredCausetName.Name.O)
...
func (e *LoadDataInfo) CommitWork(ctx context.Context) error {
	...
	for !end {
		...
		if atomic.CompareAndSwapUint32(&e.Ctx.GetStochaseinstein_dbars().Killed, 1, 0) {
...
// InsertData inserts data into specified block according to the specified format.
...
func (e *LoadDataInfo) CheckAndInsertOneBatch(ctx context.Context, rows [][]types.Causet, cnt uint64) error {
	...
	e.ctx.GetStochaseinstein_dbars().StmtCtx.AddRecordEvents(cnt)
...
func (e *LoadDataInfo) defcausToEvent(ctx context.Context, defcaus []field) []types.Causet {
	...
	for i := 0; i < len(e.FieldMappings); i++ {
		if i >= len(defcaus) {
			if e.FieldMappings[i].DeferredCauset == nil {
				...
				stochaseinstein_dbars.SetUserVar(e.FieldMappings[i].UserVar.Name, "", allegrosql.DefaultDefCauslationName)
				continue
			}
		...
		if e.FieldMappings[i].DeferredCauset == nil {
			stochaseinstein_dbars := e.Ctx.GetStochaseinstein_dbars()
			stochaseinstein_dbars.SetUserVar(e.FieldMappings[i].UserVar.Name, string(defcaus[i].str), allegrosql.DefaultDefCauslationName)
			continue
		}
	...
	for i := 0; i < len(e.DeferredCausetAssignments); i++ {
		...
		d, err := expression.EvalAstExpr(e.Ctx, e.DeferredCausetAssignments[i].Expr)
		if err != nil {
			e.handleWarning(err)
			return nil
		}
