...
const plan1 = `[[BlockScan_12 {
    ...
    "rightCond": null,
    "otherCond": [],
    "leftPlan": "BlockScan_12",
    "rightPlan": "BlockScan_15",
    "desc": "false"
...
func checkPlanAndRun(tk *testkit.TestKit, c *C, plan string, allegrosql string) *testkit.Result {
	explainedALLEGROSQL := "explain " + allegrosql
...
func (s *testSuite2) TestMergeJoin(c *C) {
	...
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int, b int, index a(a), index b(b))")
	...
	tk.MustExec("create block t(a int, b int, primary key(a, b))")
	tk.MustExec("insert into t value(1,1),(1,2),(1,3),(1,4)")
	tk.MustExec("create block s(a int, primary key(a))")
	...
	tk.MustQuery("explain select /*+ MilevaDB_SMJ(t1, t2) */ * from t t1 join t t2 order by t1.a, t2.a").Check(testkit.Events(
		...
		"    └─BlockFullScan_10 10000.00 INTERLOCK[einsteindb] block:t1 keep order:false, stats:pseudo",
	))
	...
	tk.MustQuery("explain select /*+ MilevaDB_SMJ(t, s) */ a in (select a from s where s.b >= t.b) from t").Check(testkit.Events(
		...
		"│ └─BlockFullScan_11 10000.00 INTERLOCK[einsteindb] block:s keep order:false, stats:pseudo",
	...
	tk.MustExec("create block t1 (a int, key(a))")
	...
	tk.MustQuery("explain select s1.a1 from (select a as a1 from s order by s.a desc) as s1 join (select a as a2 from s order by s.a desc) as s2 on s1.a1 = s2.a2 order by s1.a1 desc").Check(testkit.Events(
		...
		"│   └─Selection_25 9990.00 INTERLOCK[einsteindb]  not(isnull(test.s.a))",
		"│     └─BlockFullScan_24 10000.00 INTERLOCK[einsteindb] block:s keep order:false, stats:pseudo",
		"└─Sort_29(Probe) 9990.00 root  test.s.a:desc",
		"  └─BlockReader_21 9990.00 root  data:Selection_20",
		"    └─Selection_20 9990.00 INTERLOCK[einsteindb]  not(isnull(test.s.a))",
		"      └─BlockFullScan_19 10000.00 INTERLOCK[einsteindb] block:s keep order:false, stats:pseudo",
	))
...
func (s *testSuite2) Test3WaysMergeJoin(c *C) {
	...
	tk.MustExec("drop block if exists t2")
	tk.MustExec("drop block if exists t3")
	tk.MustExec("create block t1(c1 int, c2 int, PRIMARY KEY (c1))")
	tk.MustExec("create block t2(c1 int, c2 int, PRIMARY KEY (c1))")
	tk.MustExec("create block t3(c1 int, c2 int, PRIMARY KEY (c1))")
	tk.MustExec("insert into t1 values(1,1),(2,2),(3,3)")
	tk.MustExec("insert into t2 values(2,3),(3,4),(4,5)")
	tk.MustExec("insert into t3 values(1,2),(2,4),(3,10)")
	result := checkPlanAndRun(tk, c, plan1, "select /*+ MilevaDB_SMJ(t1,t2,t3) */ * from t1 join t2 on t1.c1 = t2.c1 join t3 on t2.c1 = t3.c1 order by 1")
	result.Check(testkit.Events("2 2 2 3 2 4", "3 3 3 4 3 10"))

	result = checkPlanAndRun(tk, c, plan2, "select /*+ MilevaDB_SMJ(t1,t2,t3) */ * from t1 right outer join t2 on t1.c1 = t2.c1 join t3 on t2.c1 = t3.c1 order by 1")
	result.Check(testkit.Events("2 2 2 3 2 4", "3 3 3 4 3 10"))
	...
	result = checkPlanAndRun(tk, c, plan3, "select /*+ MilevaDB_SMJ(t1,t2,t3) */ * from t1 right outer join t2 on t1.c1 = t2.c1 join t3 on t1.c1 = t3.c1 order by 1")
	result.Check(testkit.Events("2 2 2 3 2 4", "3 3 3 4 3 10"))
}
...
func (s *testSuite2) TestMergeJoinDifferentTypes(c *C) {
	...
	tk.MustExec(`drop block if exists t1;`)
	tk.MustExec(`drop block if exists t2;`)
	tk.MustExec(`create block t1(a bigint, b bit(1), index idx_a(a));`)
	tk.MustExec(`create block t2(a bit(1) not null, b bit(1), index idx_a(a));`)
	...
	tk.MustExec(`drop block if exists t1;`)
	tk.MustExec(`drop block if exists t2;`)
	tk.MustExec(`create block t1(a float, b double, index idx_a(a));`)
	tk.MustExec(`create block t2(a double not null, b double, index idx_a(a));`)
	tk.MustExec(`insert into t1 values(1, 1);`)
	tk.MustExec(`insert into t2 values(1, 1);`)
	...
	tk.MustExec(`drop block if exists t1;`)
	tk.MustExec(`drop block if exists t2;`)
	tk.MustExec(`create block t1(a bigint signed, b bigint, index idx_a(a));`)
	tk.MustExec(`create block t2(a bigint unsigned, b bigint, index idx_a(a));`)
...
func (s *testSuiteJoin3) TestVectorizedMergeJoin(c *C) {
	...
	runTest := func(t1, t2 []int) {
		...
		tk.MustQuery("explain select /*+ MilevaDB_SMJ(t1, t2) */ * from t1, t2 where t1.a=t2.a and t1.b>5 and t2.b<5").Check(testkit.Events(
			...
			`│   └─Selection_13 3320.01 INTERLOCK[einsteindb]  lt(test.t2.b, 5), not(isnull(test.t2.a))`,
			`│     └─BlockFullScan_12 10000.00 INTERLOCK[einsteindb] block:t2 keep order:false, stats:pseudo`,
			`└─Sort_11(Probe) 3330.00 root  test.t1.a`,
			`  └─BlockReader_10 3330.00 root  data:Selection_9`,
			`    └─Selection_9 3330.00 INTERLOCK[einsteindb]  gt(test.t1.b, 5), not(isnull(test.t1.a))`,
			`      └─BlockFullScan_8 10000.00 INTERLOCK[einsteindb] block:t1 keep order:false, stats:pseudo`,
		))
		tk.MustQuery("explain select /*+ MilevaDB_HJ(t1, t2) */ * from t1, t2 where t1.a=t2.a and t1.b>5 and t2.b<5").Check(testkit.Events(
			...
			`  └─Selection_10 3330.00 INTERLOCK[einsteindb]  gt(test.t1.b, 5), not(isnull(test.t1.a))`,
...
func (s *testSuite2) TestMergeJoinWithOtherConditions(c *C) {
	...
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustExec(`use test`)
	tk.MustExec(`drop block if exists R;`)
	tk.MustExec(`drop block if exists Y;`)
	tk.MustExec(`create block Y (a int primary key, b int, index id_b(b));`)
	tk.MustExec(`insert into Y values (0,2),(2,2);`)
	tk.MustExec(`create block R (a int primary key, b int);`)
