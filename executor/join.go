...
type HashJoinExec struct {
	...
	probeKeys         []*expression.DeferredCauset
	buildKeys         []*expression.DeferredCauset
	isNullEQ          []bool
	...
	// closeCh add a dagger for closing executor.
	closeCh        chan struct{}
...
// fetchProbeSideChunks get chunks from fetches chunks from the big block in a background goroutine
...
// fetchBuildSideEvents fetches all rows from build side executor, and append them
// to e.buildSideResult.
func (e *HashJoinExec) fetchBuildSideEvents(ctx context.Context, chkCh chan<- *chunk.Chunk, doneCh <-chan struct{}) {
	defer close(chkCh)
	var err error
	for {
		if e.finished.Load().(bool) {
...
func (e *HashJoinExec) fetchAndProbeHashBlock(ctx context.Context) {
	...
	// Start e.concurrency join workers to probe hash block and join build side and
	// probe side rows.
	for i := uint(0); i < e.concurrency; i++ {
		e.joinWorkerWaitGroup.Add(1)
		workID := i
...
// Concurrently handling unmatched rows from the hash block
func (e *HashJoinExec) handleUnmatchedEventsFromHashBlock(workerID uint) {
...
func (e *HashJoinExec) waitJoinWorkersAndCloseResultChan() {
	...
	if e.useOuterToBuild {
		// Concurrently handling unmatched rows from the hash block at the tail
		for i := uint(0); i < e.concurrency; i++ {
			var workerID = i
			e.joinWorkerWaitGroup.Add(1)
...
func (e *HashJoinExec) runJoinWorker(workerID uint, probeKeyDefCausIdx []int) {
	probeTime := int64(0)
	if e.stats != nil {
		start := time.Now()
		defer func() {
...
func (e *HashJoinExec) joinMatchedProbeSideEvent2ChunkForOuterHashJoin(workerID uint, probeKey uint64, probeSideEvent chunk.Event, hCtx *hashContext,
...
func (e *HashJoinExec) joinMatchedProbeSideEvent2Chunk(workerID uint, probeKey uint64, probeSideEvent chunk.Event, hCtx *hashContext,
	joinResult *hashjoinWorkerResult) (bool, *hashjoinWorkerResult) {
...
// join2ChunkForOuterHashJoin joins chunks when using the outer to build a hash block (refer to outer hash join)
...
// step 1. fetch data from build side child and build a hash block;
// step 2. fetch data from probe child in a background goroutine and probe the hash block in multiple join workers.
...
func (e *HashJoinExec) handleFetchAndBuildHashBlockPanic(r interface{}) {
	if r != nil {
...
func (e *HashJoinExec) fetchAndBuildHashBlock(ctx context.Context) {
	if e.stats != nil {
		start := time.Now()
		defer func() {
	...
	// buildSideResultCh transfers build side chunk from build side fetch to build hash block.
	...
	// TODO: Parallel build hash block. Currently not support because `unsafeHashBlock` is not thread-safe.
...
// buildHashBlockForList builds hash block from `list`.
func (e *HashJoinExec) buildHashBlockForList(buildSideResultCh <-chan *chunk.Chunk) error {
...
func (e *NestedLoopApplyExec) fetchSelectedOuterEvent(ctx context.Context, chk *chunk.Chunk) (*chunk.Event, error) {
