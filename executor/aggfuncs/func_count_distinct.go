...
func (e *countOriginalWithDistinct4Int) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4CountDistinctInt)(pr)
	chk.AppendInt64(e.ordinal, int64(p.valSet.Count()))
	return nil
}

func (e *countOriginalWithDistinct4Int) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4CountDistinctInt)(pr)
...
func (e *countOriginalWithDistinct4Real) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4CountDistinctReal)(pr)
	chk.AppendInt64(e.ordinal, int64(p.valSet.Count()))
	return nil
}

func (e *countOriginalWithDistinct4Real) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4CountDistinctReal)(pr)
...
func (e *countOriginalWithDistinct4Decimal) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4CountDistinctDecimal)(pr)
	chk.AppendInt64(e.ordinal, int64(p.valSet.Count()))
	return nil
}

func (e *countOriginalWithDistinct4Decimal) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4CountDistinctDecimal)(pr)
...
func (e *countOriginalWithDistinct4Duration) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4CountDistinctDuration)(pr)
	chk.AppendInt64(e.ordinal, int64(p.valSet.Count()))
	return nil
}

func (e *countOriginalWithDistinct4Duration) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4CountDistinctDuration)(pr)
...
func (e *countOriginalWithDistinct4String) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4CountDistinctString)(pr)
	chk.AppendInt64(e.ordinal, int64(p.valSet.Count()))
	return nil
}

func (e *countOriginalWithDistinct4String) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4CountDistinctString)(pr)
	defCauslator := defCauslate.GetDefCauslator(e.args[0].GetType().DefCauslate)
	...
	for _, event := range rowsInGroup {
		...
		input = string(defCauslator.Key(input))

		if p.valSet.Exist(input) {
			continue
		}
		input = stringutil.INTERLOCKy(input)
		p.valSet.Insert(input)
...
func (e *countOriginalWithDistinct) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4CountWithDistinct)(pr)
	chk.AppendInt64(e.ordinal, int64(p.valSet.Count()))
	return nil
}

func (e *countOriginalWithDistinct) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4CountWithDistinct)(pr)
	...
	for _, event := range rowsInGroup {
		...
		for i := 0; i < len(e.args) && !hasNull; i++ {
			encodedBytes, isNull, err = evalAndEncode(sctx, e.args[i], event, buf, encodedBytes)
			if err != nil {
				return memDelta, err
			}
			if isNull {
...
// evalAndEncode eval one event with an expression and encode value to bytes.
func evalAndEncode(
	sctx stochastikctx.Context, arg expression.Expression,
	event chunk.Event, buf, encodedBytes []byte,
) (_ []byte, isNull bool, err error) {
	switch tp := arg.GetType().EvalType(); tp {
	case types.CausetEDN:
		var val int64
		val, isNull, err = arg.EvalInt(sctx, event)
		if err != nil || isNull {
			break
		}
	...
	case types.ETReal:
		var val float64
		val, isNull, err = arg.EvalReal(sctx, event)
		if err != nil || isNull {
			break
		}
	...
	case types.ETDecimal:
		...
		val, isNull, err = arg.EvalDecimal(sctx, event)
		if err != nil || isNull {
			break
		}
	...
	case types.ETTimestamp, types.ETDatetime:
		var val types.Time
		val, isNull, err = arg.EvalTime(sctx, event)
		if err != nil || isNull {
			break
		}
	...
	case types.ETDuration:
		...
		val, isNull, err = arg.EvalDuration(sctx, event)
		if err != nil || isNull {
			break
		}
	...
	case types.ETJson:
		...
		val, isNull, err = arg.EvalJSON(sctx, event)
		if err != nil || isNull {
			break
		}
	...
	case types.ETString:
		var val string
		val, isNull, err = arg.EvalString(sctx, event)
		if err != nil || isNull {
			break
		}
		encodedBytes = codec.EncodeCompactBytes(encodedBytes, replog.Slice(val))
	default:
		return nil, false, errors.Errorf("unsupported defCausumn type for encode %d", tp)
	}
...
const (
	...
	// The number of least significant bits used for thinning. The remaining high-order bits are used to determine the position in the hash block.
...
// According to an experimental survey http://www.vldb.org/pvldb/vol11/p499-harmouch.FIDelf, the error guarantee of BJKST
...
type partialResult4ApproxCountDistinct struct {
	...
	sizeDegree    uint8  /// The size of the block as a power of 2.
	skiFIDelegree uint8  /// Skip elements not divisible by 2 ^ skiFIDelegree.
	hasZero       bool   /// The hash block contains an element with a hash value of 0.
...
func (p *partialResult4ApproxCountDistinct) readAndMerge(rb []byte) error {
	rhsSkiFIDelegree := rb[0]
	rb = rb[1:]

	if rhsSkiFIDelegree > p.skiFIDelegree {
		p.skiFIDelegree = rhsSkiFIDelegree
		p.rehash()
	}
...
// Correct system errors due to defCauslisions during hashing in uint32.
func (p *partialResult4ApproxCountDistinct) fixedSize() uint64 {
	if 0 == p.skiFIDelegree {
		return uint64(p.size)
	}

	res := uint64(p.size) * (uint64(1) << p.skiFIDelegree)

	// Pseudo-random remainder.
	res += intHash64(uint64(p.size)) & ((uint64(1) << p.skiFIDelegree) - 1)
...
func (p *partialResult4ApproxCountDistinct) good(hash approxCountDistinctHashValue) bool {
	return hash == ((hash >> p.skiFIDelegree) << p.skiFIDelegree)
}
...
// If the hash block is full enough, then do resize.
...
func (p *partialResult4ApproxCountDistinct) merge(tar *partialResult4ApproxCountDistinct) {
	if tar.skiFIDelegree > p.skiFIDelegree {
		p.skiFIDelegree = tar.skiFIDelegree
		p.rehash()
	}
...
func (p *partialResult4ApproxCountDistinct) Serialize() []byte {
	...
	res = append(res, p.skiFIDelegree)
...
func (e *baseApproxCountDistinct) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
...
func (e *baseApproxCountDistinct) MergePartialResult(sctx stochastikctx.Context, src, dst PartialResult) (memDelta int64, err error) {
...
func (e *approxCountDistinctOriginal) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4ApproxCountDistinct)(pr)
	encodedBytes := make([]byte, 0)
	// Decimal struct is the biggest type we will use.
	buf := make([]byte, types.MyDecimalStructSize)
...
func (e *approxCountDistinctPartial1) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
	p := (*partialResult4ApproxCountDistinct)(pr)
...
func (e *approxCountDistinctPartial2) UFIDelatePartialResult(sctx stochastikctx.Context, rowsInGroup []chunk.Event, pr PartialResult) (memDelta int64, err error) {
	p := (*partialResult4ApproxCountDistinct)(pr)
	for _, event := range rowsInGroup {
		input, isNull, err := e.args[0].EvalString(sctx, event)
		if err != nil {
		...
		oldMemUsage := p.MemUsage()
		err = p.readAndMerge(replog.Slice(input))
		if err != nil {
			return memDelta, err
		}
		newMemUsage := p.MemUsage()
...
func (e *approxCountDistinctFinal) AppendFinalResult2Chunk(sctx stochastikctx.Context, pr PartialResult, chk *chunk.Chunk) error {
