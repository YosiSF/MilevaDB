...
func (s *testSuite) TestMergePartialResult4JsonObjectagg(c *C) {
	typeList := []byte{allegrosql.TypeLonglong, allegrosql.TypeDouble, allegrosql.TypeString, allegrosql.TypeJSON}
	...
	for k := 0; k < len(argCombines); k++ {
		...
		aggTest := buildMultiArgsAggTester(ast.AggFuncJsonObjectAgg, argTypes, allegrosql.TypeJSON, numEvents, json.CreateBinary(entries1), json.CreateBinary(entries2), json.CreateBinary(entries1))
...
func (s *testSuite) TestJsonObjectagg(c *C) {
	...
	for k := 0; k < len(argCombines); k++ {
		...
		aggTest := buildMultiArgsAggTester(ast.AggFuncJsonObjectAgg, argTypes, allegrosql.TypeJSON, numEvents, nil, json.CreateBinary(entries))
