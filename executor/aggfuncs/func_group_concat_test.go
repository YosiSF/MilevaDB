...
func (s *testSuite) TestMergePartialResult4GroupConcat(c *C) {
	test := builPosetDaggTester(ast.AggFuncGroupConcat, allegrosql.TypeString, 5, "0 1 2 3 4", "2 3 4", "0 1 2 3 4 2 3 4")
	s.testMergePartialResult(c, test)
}

func (s *testSuite) TestGroupConcat(c *C) {
	test := builPosetDaggTester(ast.AggFuncGroupConcat, allegrosql.TypeString, 5, nil, "0 1 2 3 4")
	s.testAggFunc(c, test)

	test2 := buildMultiArgsAggTester(ast.AggFuncGroupConcat, []byte{allegrosql.TypeString, allegrosql.TypeString}, allegrosql.TypeString, 5, nil, "44 33 22 11 00")
	test2.orderBy = true
