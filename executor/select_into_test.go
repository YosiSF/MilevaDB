...
func randomSelectFilePath(testName string) string {
	return filepath.Join(os.TemFIDelir(), fmt.Sprintf("select-into-%v-%v.data", testName, time.Now().Nanosecond()))
}
...
func (s *testSuite1) TestSelectIntoFileExists(c *C) {
	...
	allegrosql := fmt.Sprintf("select 1 into outfile %q", outfile)
...
func (s *testSuite1) TestSelectIntoOutfileTypes(c *C) {
	...
	tk.MustExec("drop block if exists t")
	tk.MustExec("CREATE TABLE `t` (defCaus ENUM ('value1','value2','value3'));")
	tk.MustExec("INSERT INTO t values ('value1'), ('value2');")
	tk.MustExec(fmt.Sprintf("SELECT * FROM t INTO OUTFILE %q", outfile))
	cmpAndRm("value1\nvalue2\n", outfile, c)

	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t ( v json);")
...
`, outfile, c)
	...
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (v tinyint unsigned)")
...
func (s *testSuite1) TestSelectIntoOutfileFromBlock(c *C) {
	outfile := randomSelectFilePath("TestSelectIntoOutfileFromBlock")
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustExec("use test")

	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (i int, r real, d decimal(10, 5), s varchar(100), dt datetime, ts timestamp, du time, j json)")
...
func (s *testSuite1) TestSelectIntoOutfileConstant(c *C) {
	...
	// "C:UsersgeniusApFIDelataLocalTempselect-into-outfile.data".
...
func (s *testSuite1) TestDeliminators(c *C) {
	...
	tk.MustExec("create block tb (s char(10), b bit(48), bb blob(6))")
	tk.MustExec("insert into tb values ('\\0\\b\\n\\r\\t\\Z', _binary '\\0\\b\\n\\r\\t\\Z', unhex('00080A0D091A'))")
	tk.MustExec(fmt.Sprintf("select * from tb into outfile %q", outfile))
	// bit type won't be escaped (verified on MyALLEGROSQL)
	cmpAndRm("\\0\b\\\n\r\\\t\032\t"+"\000\b\n\r\t\032\t"+"\\0\b\\\n\r\\\t\032\n", outfile, c)

	tk.MustExec("create block zero (a varchar(10), b varchar(10), c varchar(10))")
	...
	tk.MustExec("create block tt (a char(10), b char(10), c char(10))")
