...
func (s *testSequenceSuite) TestCreateSequence(c *C) {
	...
	// test unsupported block option in sequence.
	...
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[planner:1142]CREATE command denied to user 'myuser'@'localhost' for block 'my_seq'")
}
...
func (s *testSequenceSuite) TestDropSequence(c *C) {
	...
	tk.MustExec("create block seq3 (a int)")
	...
	// Test drop block when the object is a sequence.
	tk.MustExec("create sequence seq")
	_, err = tk.Exec("drop block seq")
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[schemaReplicant:1051]Unknown block 'test.seq'")
	...
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[planner:1142]DROP command denied to user 'myuser'@'localhost' for block 'my_seq'")
...
func (s *testSequenceSuite) TestShowCreateSequence(c *C) {
	...
	// Grant the myuser the access to block t in database test, but sequence seq.
	tk.MustExec("grant select on test.t to 'myuser'@'localhost'")

	tk1.MustExec("use test")
	tk1.MustExec("show create block t")
	_, err = tk1.Exec("show create sequence seq")
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[planner:1142]SHOW command denied to user 'myuser'@'localhost' for block 'seq'")
	...
	// Test show create sequence with a normal block.
	tk.MustExec("drop sequence if exists seq")
	tk.MustExec("create block seq (a int)")
	err = tk.QueryToErr("show create sequence seq")
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[executor:1347]'test.seq' is not SEQUENCE")
	tk.MustExec("drop block if exists seq")
...
func (s *testSequenceSuite) TestSequenceAsDefaultValue(c *C) {
	...
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int not null default next value for seq key)")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int not null default nextval(seq), b int, primary key(a))")

	tk.MustExec("create block t1 (a int default next value for seq)")
	tk.MustGetErrMsg("create block t2 (a char(1) default next value for seq)", "[dbs:8228]Unsupported sequence default value for defCausumn type 'a'")

	tk.MustExec("create block t3 (a int default nextval(seq))")

	tk.MustExec("create block t4 (a int)")
	tk.MustExec("alter block t4 alter defCausumn a set default (next value for seq)")
	tk.MustExec("alter block t4 alter defCausumn a set default (nextval(seq))")

	tk.MustExec("create block t5 (a char(1))")
	tk.MustGetErrMsg("alter block t5 alter defCausumn a set default (next value for seq)", "[dbs:8228]Unsupported sequence default value for defCausumn type 'a'")

	tk.MustGetErrMsg("alter block t5 alter defCausumn a set default (nextval(seq))", "[dbs:8228]Unsupported sequence default value for defCausumn type 'a'")

	// Specially, the new added defCausumn with sequence as it's default value is forbade.
	// But alter block defCausumn with sequence as it's default value is allowed.
	tk.MustGetErrMsg("alter block t5 add defCausumn c int default next value for seq", "[dbs:8230]Unsupported using sequence as default value in add defCausumn 'c'")

	tk.MustExec("alter block t5 add defCausumn c int default -1")
	// Alter with modify.
	tk.MustExec("alter block t5 modify defCausumn c int default next value for seq")
	// Alter with alter.
	tk.MustExec("alter block t5 alter defCausumn c set default (next value for seq)")
	// Alter with change.
	tk.MustExec("alter block t5 change defCausumn c c int default next value for seq")
}
...
func (s *testSequenceSuite) TestSequenceFunction(c *C) {
	...
	c.Assert(err.Error(), Equals, "[block:4135]Sequence 'test.seq' has run out")
	...
	tk.MustExec("drop sequence if exists seq1")
	tk.MustExec("drop block if exists seq1")
...
func (s *testSequenceSuite) TestInsertSequence(c *C) {
	...
	tk.MustExec("create sequence seq")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (id int default next value for seq, defCaus1 int generated always as (id + 1))")
...
func (s *testSequenceSuite) TestUnflodSequence(c *C) {
	...
	tk.MustExec("create block t3 (a int, b int, c int)")
...
func (s *testSequenceSuite) TestSequenceFunctionPrivilege(c *C) {
	...
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[expression:1142]INSERT command denied to user 'myuser'@'localhost' for block 'seq'")
	...
	c.Assert(err, NotNil)
	c.Assert(err.Error(), Equals, "[expression:1142]SELECT command denied to user 'myuser'@'localhost' for block 'seq'")
...
func (s *testSequenceSuite) TestSequenceDefaultLogic(c *C) {
	...
	// Alter block to use sequence as default value is ok.
	tk.MustExec("insert into t values(-1),(-1),(-1)")
	tk.MustExec("alter block t add column b int default -1")
	tk.MustQuery("select * from t").Check(testkit.Rows("-1 -1", "-1 -1", "-1 -1"))
	tk.MustExec("alter block t modify column b int default next value for seq")
	tk.MustQuery("select * from t").Check(testkit.Rows("-1 -1", "-1 -1", "-1 -1"))
	...
	tk.MustExec("create block t(a int)")
	tk.MustExec("insert into t values(-1),(-1),(-1)")
	tk.MustGetErrMsg("alter block t add column b int default next value for seq", "[dbs:8230]Unsupported using sequence as default value in add column 'b'")
