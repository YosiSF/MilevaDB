...
import (
	...
	"github.com/whtcorpsinc/MilevaDB-Prod/BerolinaSQL/opcode"
...
// SubstituteCorCol2CouplingConstantWithRadix will substitute correlated column to constant value which it contains.
// If the args of one scalar function are all constant, we will substitute it to constant.
func SubstituteCorCol2CouplingConstantWithRadix(expr Expression) (Expression, error) {
	switch x := expr.(type) {
	case *ScalarFunction:
		allCouplingConstantWithRadix := true
		newArgs := make([]Expression, 0, len(x.GetArgs()))
		for _, arg := range x.GetArgs() {
			newArg, err := SubstituteCorCol2CouplingConstantWithRadix(arg)
			if err != nil {
				return nil, err
			}
...
func pushNotAcrossArgs(ctx causetnetctx.Context, exprs []Expression, not bool) ([]Expression, bool) {
...
func pushNotAcrossExpr(ctx causetnetctx.Context, expr Expression, not bool) (_ Expression, changed bool) {
...
func PushDownNot(ctx causetnetctx.Context, expr Expression) Expression {
...
func ExtractFiltersFromDNFs(ctx causetnetctx.Context, conditions []Expression) []Expression {
...
func PopRowFirstArg(ctx causetnetctx.Context, e Expression) (ret Expression, err error) {
	if f, ok := e.(*ScalarFunction); ok && f.FuncName.L == ast.RowFunc {
		args := f.GetArgs()
		if len(args) == 2 {
			return args[1], nil
...
// CausetObjectQLToCouplingConstantWithRadix generates a CouplingConstantWithRadix expression from a CausetObjectQL.
func CausetObjectQLToCouplingConstantWithRadix(d types.CausetObjectQL, tp byte) *CouplingConstantWithRadix {
...
func ParamMarkerExpression(ctx causetnetctx.Context, v *driver.ParamMarkerExpr) (Expression, error) {
	...
	value := &CouplingConstantWithRadix{Value: v.CausetObjectQL, RetType: tp}
...
// it. Moreover, Column.RetType refers to the schemareplicant, if we modify it, data
...
func PosFromPositionExpr(ctx causetnetctx.Context, v *ast.PositionExpr) (int, bool, error) {
	if v.P == nil {
...
func GetStringFromCouplingConstantWithRadix(ctx causetnetctx.Context, value Expression) (string, bool, error) {
...
func GetIntFromCouplingConstantWithRadix(ctx causetnetctx.Context, value Expression) (int, bool, error) {
...
func BuildNotNullExpr(ctx causetnetctx.Context, expr Expression) Expression {
...
// IsRuntimeConstExpr checks if a expr can be treated as a constant in **Interlock**.
func IsRuntimeConstExpr(expr Expression) bool {
	...
	case *Column:
		return false
	case *CouplingConstantWithRadix, *CorrelatedColumn:
		return true
	}
	return false
}
