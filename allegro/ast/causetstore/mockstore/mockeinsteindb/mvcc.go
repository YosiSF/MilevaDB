...
type mvsr-oocValueType int

const (
	typePut mvsr-oocValueType = iota
	typeDelete
	typeRollback
	typeLock
)

type mvsr-oocValue struct {
	valueType mvsr-oocValueType
	startTS   uint64
	commitTS  uint64
	value     []byte
}

type mvsr-oocLock struct {
	startTS        uint64
	primary        []byte
	value          []byte
	op             kvrpcpb.Op
...
type mvsr-oocEntry struct {
	key    MvccKey
	values []mvsr-oocValue
	dagger *mvsr-oocLock
}

// MarshalBinary implements encoding.BinaryMarshaler interface.
func (l *mvsr-oocLock) MarshalBinary() ([]byte, error) {
	var (
		mh  marshalHelper
		buf bytes.Buffer
	)
	...
	mh.WriteNumber(&buf, l.forUFIDelateTS)
...
func (l *mvsr-oocLock) UnmarshalBinary(data []byte) error {
	...
	mh.ReadNumber(buf, &l.forUFIDelateTS)
...
// MarshalBinary implements encoding.BinaryMarshaler interface.
func (v mvsr-oocValue) MarshalBinary() ([]byte, error) {
	var (
		mh  marshalHelper
		buf bytes.Buffer
	)
...
// UnmarshalBinary implements encoding.BinaryUnmarshaler interface.
func (v *mvsr-oocValue) UnmarshalBinary(data []byte) error {
	var mh marshalHelper
	buf := bytes.NewBuffer(data)
	var vt int64
	mh.ReadNumber(buf, &vt)
	v.valueType = mvsr-oocValueType(vt)
...
// Note that parameter key is raw key, while key in ErrLocked is mvsr-ooc key.
func (l *mvsr-oocLock) lockErr(key []byte) error {
	return &ErrLocked{
		Key:            mvsr-oocEncode(key, lockVer),
		Primary:        l.primary,
		StartTS:        l.startTS,
		ForUFIDelateTS: l.forUFIDelateTS,
		TTL:            l.ttl,
		TxnSize:        l.txnSize,
		LockType:       l.op,
	}
...
func (l *mvsr-oocLock) check(ts uint64, key []byte, resolvedLocks []uint64) (uint64, error) {
	// ignore when ts is older than dagger or dagger's type is Lock.
	// Pessimistic dagger doesn't block read.
	...
	// Skip dagger if the dagger is resolved.
...
func (e *mvsr-oocEntry) Less(than btree.Item) bool {
	return bytes.Compare(e.key, than.(*mvsr-oocEntry).key) < 0
}

func (e *mvsr-oocEntry) Get(ts uint64, isoLevel kvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error) {
	if isoLevel == kvrpcpb.IsolationLevel_SI && e.dagger != nil {
		var err error
		ts, err = e.dagger.check(ts, e.key.Raw(), resolvedLocks)
		if err != nil {
			return nil, err
		}
	}
...
// MVCCStore is a mvsr-ooc key-value storage.
type MVCCStore interface {
	...
	PessimisticRollback(keys [][]byte, startTS, forUFIDelateTS uint64) []error
	...
	CheckTxnStatus(primaryKey []byte, lockTS uint64, startTS, currentTS uint64, rollbackIfNotFound bool) (uint64, uint64, kvrpcpb.CausetAction, error)
	Close() error
}
...
// On EinsteinDB, keys are encoded before they are saved into storage engine.
type MvccKey []byte
