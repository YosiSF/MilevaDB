...
// testMockEinsteinDBSuite tests MVCCStore interface.
// SetUpTest should set specific MVCCStore implementation.
type testMockEinsteinDBSuite struct {
	causetstore MVCCStore
}
...
type testMVCCLevelDB struct {
	testMockEinsteinDBSuite
}

var (
	_ = Suite(&testMockEinsteinDBSuite{})
	_ = Suite(&testMVCCLevelDB{})
	_ = Suite(testMarshal{})
)

func (s *testMockEinsteinDBSuite) SetUpTest(c *C) {
	var err error
	s.causetstore, err = NewMVCCLevelDB("")
	c.Assert(err, IsNil)
}
...
func (s *testMockEinsteinDBSuite) mustGetNone(c *C, key string, ts uint64) {
	val, err := s.causetstore.Get([]byte(key), ts, kvrpcpb.IsolationLevel_SI, nil)
	c.Assert(err, IsNil)
	c.Assert(val, IsNil)
}

func (s *testMockEinsteinDBSuite) mustGetErr(c *C, key string, ts uint64) {
	val, err := s.causetstore.Get([]byte(key), ts, kvrpcpb.IsolationLevel_SI, nil)
	c.Assert(err, NotNil)
	c.Assert(val, IsNil)
}

func (s *testMockEinsteinDBSuite) mustGetOK(c *C, key string, ts uint64, expect string) {
	val, err := s.causetstore.Get([]byte(key), ts, kvrpcpb.IsolationLevel_SI, nil)
	c.Assert(err, IsNil)
	c.Assert(string(val), Equals, expect)
}

func (s *testMockEinsteinDBSuite) mustGetRC(c *C, key string, ts uint64, expect string) {
	val, err := s.causetstore.Get([]byte(key), ts, kvrpcpb.IsolationLevel_RC, nil)
	c.Assert(err, IsNil)
	c.Assert(string(val), Equals, expect)
}

func (s *testMockEinsteinDBSuite) mustPutOK(c *C, key, value string, startTS, commitTS uint64) {
	req := &kvrpcpb.PrewriteRequest{
		Mutations:    putMutations(key, value),
		PrimaryLock:  []byte(key),
		StartVersion: startTS,
	}
	errs := s.causetstore.Prewrite(req)
	for _, err := range errs {
		c.Assert(err, IsNil)
	}
	err := s.causetstore.Commit([][]byte{[]byte(key)}, startTS, commitTS)
	c.Assert(err, IsNil)
}

func (s *testMockEinsteinDBSuite) mustDeleteOK(c *C, key string, startTS, commitTS uint64) {
...
func (s *testMockEinsteinDBSuite) mustScanOK(c *C, start string, limit int, ts uint64, expect ...string) {
	s.mustRangeScanOK(c, start, "", limit, ts, expect...)
}

func (s *testMockEinsteinDBSuite) mustRangeScanOK(c *C, start, end string, limit int, ts uint64, expect ...string) {
	pairs := s.causetstore.Scan([]byte(start), []byte(end), limit, ts, kvrpcpb.IsolationLevel_SI, nil)
...
func (s *testMockEinsteinDBSuite) mustReverseScanOK(c *C, end string, limit int, ts uint64, expect ...string) {
	s.mustRangeReverseScanOK(c, "", end, limit, ts, expect...)
}

func (s *testMockEinsteinDBSuite) mustRangeReverseScanOK(c *C, start, end string, limit int, ts uint64, expect ...string) {
	pairs := s.causetstore.ReverseScan([]byte(start), []byte(end), limit, ts, kvrpcpb.IsolationLevel_SI, nil)
	c.Assert(len(pairs)*2, Equals, len(expect))
	for i := 0; i < len(pairs); i++ {
		c.Assert(pairs[i].Err, IsNil)
		c.Assert(pairs[i].Key, BytesEquals, []byte(expect[i*2]))
...
func MustPrewriteOK(c *C, causetstore MVCCStore, mutations []*kvrpcpb.Mutation, primary string, startTS uint64, ttl uint64) {
	s := testMockEinsteinDBSuite{causetstore}
	s.mustPrewriteWithTTLOK(c, mutations, primary, startTS, ttl)
}

func (s *testMockEinsteinDBSuite) mustPrewriteOK(c *C, mutations []*kvrpcpb.Mutation, primary string, startTS uint64) {
	s.mustPrewriteWithTTLOK(c, mutations, primary, startTS, 0)
}

func (s *testMockEinsteinDBSuite) mustPrewriteWithTTLOK(c *C, mutations []*kvrpcpb.Mutation, primary string, startTS uint64, ttl uint64) {
	req := &kvrpcpb.PrewriteRequest{
		Mutations:    mutations,
		PrimaryLock:  []byte(primary),
		StartVersion: startTS,
...
func (s *testMockEinsteinDBSuite) mustCommitOK(c *C, keys [][]byte, startTS, commitTS uint64) {
	err := s.causetstore.Commit(keys, startTS, commitTS)
	c.Assert(err, IsNil)
}

func (s *testMockEinsteinDBSuite) mustCommitErr(c *C, keys [][]byte, startTS, commitTS uint64) {
	err := s.causetstore.Commit(keys, startTS, commitTS)
	c.Assert(err, NotNil)
}

func (s *testMockEinsteinDBSuite) mustRollbackOK(c *C, keys [][]byte, startTS uint64) {
	err := s.causetstore.Rollback(keys, startTS)
	c.Assert(err, IsNil)
}

func (s *testMockEinsteinDBSuite) mustRollbackErr(c *C, keys [][]byte, startTS uint64) {
	err := s.causetstore.Rollback(keys, startTS)
	c.Assert(err, NotNil)
}

func (s *testMockEinsteinDBSuite) mustScanLock(c *C, maxTs uint64, expect []*kvrpcpb.LockInfo) {
	locks, err := s.causetstore.ScanLock(nil, nil, maxTs)
	c.Assert(err, IsNil)
	c.Assert(locks, DeepEquals, expect)
}

func (s *testMockEinsteinDBSuite) mustResolveLock(c *C, startTS, commitTS uint64) {
	c.Assert(s.causetstore.ResolveLock(nil, nil, startTS, commitTS), IsNil)
}

func (s *testMockEinsteinDBSuite) mustBatchResolveLock(c *C, txnInfos map[uint64]uint64) {
	c.Assert(s.causetstore.BatchResolveLock(nil, nil, txnInfos), IsNil)
}

func (s *testMockEinsteinDBSuite) mustGC(c *C, safePoint uint64) {
	c.Assert(s.causetstore.GC(nil, nil, safePoint), IsNil)
}

func (s *testMockEinsteinDBSuite) mustDeleteRange(c *C, startKey, endKey string) {
	err := s.causetstore.DeleteRange([]byte(startKey), []byte(endKey))
	c.Assert(err, IsNil)
}

func (s *testMockEinsteinDBSuite) TestGet(c *C) {
...
func (s *testMockEinsteinDBSuite) TestGetWithLock(c *C) {
	key := "key"
	value := "value"
...
func (s *testMockEinsteinDBSuite) TestDelete(c *C) {
...
func (s *testMockEinsteinDBSuite) TestCleanupRollback(c *C) {
...
func (s *testMockEinsteinDBSuite) TestReverseScan(c *C) {
...
func (s *testMockEinsteinDBSuite) TestScan(c *C) {
	// ver10: A(10) - B(_) - C(10) - D(_) - E(10)
	s.mustPutOK(c, "A", "A10", 5, 10)
	s.mustPutOK(c, "C", "C10", 5, 10)
	s.mustPutOK(c, "E", "E10", 5, 10)
...
func (s *testMockEinsteinDBSuite) TestBatchGet(c *C) {
	...
	pairs := s.causetstore.BatchGet(batchKeys, 5, kvrpcpb.IsolationLevel_SI, nil)
...
func (s *testMockEinsteinDBSuite) TestScanLock(c *C) {
	s.mustPutOK(c, "k1", "v1", 1, 2)
...
func (s *testMockEinsteinDBSuite) TestScanWithResolvedLock(c *C) {
	s.mustPrewriteOK(c, putMutations("p1", "v5", "s1", "v5"), "p1", 5)
...
func (s *testMockEinsteinDBSuite) TestCommitConflict(c *C) {
...
func (s *testMockEinsteinDBSuite) TestResolveLock(c *C) {
	s.mustPrewriteOK(c, putMutations("p1", "v5", "s1", "v5"), "p1", 5)
	s.mustPrewriteOK(c, putMutations("p2", "v10", "s2", "v10"), "p2", 10)
...
func (s *testMockEinsteinDBSuite) TestBatchResolveLock(c *C) {
...
func (s *testMockEinsteinDBSuite) TestGC(c *C) {
...
func (s *testMockEinsteinDBSuite) TestRollbackAndWriteConflict(c *C) {
	...
	err := s.causetstore.Cleanup([]byte("test"), 2, math.MaxUint64)
	c.Assert(err, IsNil)
	req = &kvrpcpb.PrewriteRequest{
...
func (s *testMockEinsteinDBSuite) TestDeleteRange(c *C) {
	for i := 1; i <= 5; i++ {
...
func (s *testMockEinsteinDBSuite) mustWriteWriteConflict(c *C, errs []error, i int) {
	c.Assert(errs[i], NotNil)
	_, ok := errs[i].(*ErrConflict)
	c.Assert(ok, IsTrue)
}

func (s *testMockEinsteinDBSuite) TestRC(c *C) {
...
func (s testMarshal) TestMarshalmvsr-oocLock(c *C) {
	l := mvsr-oocLock{
		startTS:     47,
		primary:     []byte{'a', 'b', 'c'},
		value:       []byte{'d', 'e'},
...
func (s testMarshal) TestMarshalmvsr-oocValue(c *C) {
	v := mvsr-oocValue{
		valueType: typePut,
		startTS:   42,
		commitTS:  55,
...
func (s *testMVCCLevelDB) TestCheckTxnStatus(c *C) {
	...
	ttl, commitTS, action, err := s.causetstore.CheckTxnStatus([]byte("pk"), startTS, startTS+100, 666, false)
	c.Assert(err, IsNil)
	c.Assert(ttl, Equals, uint64(666))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, kvrpcpb.CausetAction_MinCommitTSPushed)

	// MaxUint64 as callerStartTS shouldn't uFIDelate minCommitTS but return CausetAction_MinCommitTSPushed.
	ttl, commitTS, action, err = s.causetstore.CheckTxnStatus([]byte("pk"), startTS, math.MaxUint64, 666, false)
	c.Assert(err, IsNil)
	c.Assert(ttl, Equals, uint64(666))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, kvrpcpb.CausetAction_MinCommitTSPushed)
	...
	ttl, commitTS, _, err = s.causetstore.CheckTxnStatus([]byte("pk"), startTS, 0, 666, false)
	c.Assert(err, IsNil)
	...
	ttl, commitTS, action, err = s.causetstore.CheckTxnStatus([]byte("pk1"), startTS, 0, 666, false)
	c.Assert(err, IsNil)
	c.Assert(ttl, Equals, uint64(0))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, kvrpcpb.CausetAction_NoCausetAction)

	s.mustPrewriteWithTTLOK(c, putMutations("pk2", "val"), "pk2", startTS, 666)
	currentTS := uint64(777 << 18)
	ttl, commitTS, action, err = s.causetstore.CheckTxnStatus([]byte("pk2"), startTS, 0, currentTS, false)
	c.Assert(err, IsNil)
	c.Assert(ttl, Equals, uint64(0))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, kvrpcpb.CausetAction_TTLExpireRollback)

	// Cover the TxnNotFound case.
	_, _, _, err = s.causetstore.CheckTxnStatus([]byte("txnNotFound"), 5, 0, 666, false)
	c.Assert(err, NotNil)
	...
	ttl, commitTS, action, err = s.causetstore.CheckTxnStatus([]byte("txnNotFound"), 5, 0, 666, true)
	c.Assert(err, IsNil)
	c.Assert(ttl, Equals, uint64(0))
	c.Assert(commitTS, Equals, uint64(0))
	c.Assert(action, Equals, kvrpcpb.CausetAction_LockNotExistRollback)
...
func (s *testMVCCLevelDB) TestRejectCommitTS(c *C) {
	...
	_, _, _, err := s.causetstore.CheckTxnStatus([]byte("x"), 5, 100, 100, false)
	c.Assert(err, IsNil)
	err = s.causetstore.Commit([][]byte{[]byte("x")}, 5, 10)
...
func (s *testMVCCLevelDB) TestMvccGetByKey(c *C) {
	...
	debugger, ok := s.causetstore.(MVCCDebugger)
	c.Assert(ok, IsTrue)
	mvsr-oocInfo := debugger.MvccGetByKey([]byte("q1"))
	...
	c.Assert(mvsr-oocInfo, DeepEquals, except)
}
...
func (s *testMVCCLevelDB) TestTxnHeartBeat(c *C) {
	...
	// UFIDelate the ttl
	ttl, err := s.causetstore.TxnHeartBeat([]byte("pk"), 5, 888)
	c.Assert(err, IsNil)
	c.Assert(ttl, Greater, uint64(666))

	// Advise ttl is small
	ttl, err = s.causetstore.TxnHeartBeat([]byte("pk"), 5, 300)
	c.Assert(err, IsNil)
