...
func (s *testRPCHandlerSuite) TestConstructTimezone(c *C) {
	...
	c.Assert(err, IsNil)
	timeInLoc := time.Date(2020, 8, 15, 20, 0, 0, 0, loc)
	timeInUTC := time.Date(2020, 8, 15, 12, 0, 0, 0, time.UTC)
	c.Assert(timeInLoc.Equal(timeInUTC), IsTrue)

	secondsEastOfUTC = int((-8 * time.Hour).Seconds())
	loc, err = constructTimeZone("", secondsEastOfUTC)
	c.Assert(err, IsNil)
	timeInLoc = time.Date(2020, 8, 15, 12, 0, 0, 0, loc)
	timeInUTC = time.Date(2020, 8, 15, 20, 0, 0, 0, time.UTC)
	c.Assert(timeInLoc.Equal(timeInUTC), IsTrue)
