...
func (d MockEinsteinDBDriver) Open(path string) (ekv.CausetStorage, error) {
...
func (d EmbedEntangledStoreDriver) Open(path string) (ekv.CausetStorage, error) {
	u, err := url.Parse(path)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func NewMockStore(options ...MockEinsteinDBStoreOption) (ekv.CausetStorage, error) {
	opt := mockOptions{
