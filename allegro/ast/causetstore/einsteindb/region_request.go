...
// ShuttingDown is a flag to indicate milevadb-server is exiting (Ctrl+C signal
// receved for example). If this flag is set, einsteindb client should not retry on
// network error because milevadb-server expect einsteindb client to exit as soon as possible.
var ShuttingDown uint32

// RegionRequestSender sends KV/Causet requests to einsteindb server. It handles network
// errors and some region errors internally.
//
// Typically, a KV/Causet request is bind to a region, all keys that are involved
// in the request should be located in the region.
// The sending process begins with looking for the address of leader causetstore's
// address of the target region from cache, and the request is then sent to the
// destination einsteindb server over TCP connection.
// If region is uFIDelated, can be caused by leader transfer, region split, region
// merge, or region balance, einsteindb server may not able to process request and
...
type RegionRequestRuntimeStats struct {
	Stats map[einsteindbrpc.CmdType]*RPCRuntimeStats
}
...
func NewRegionRequestRuntimeStats() RegionRequestRuntimeStats {
	return RegionRequestRuntimeStats{
		Stats: make(map[einsteindbrpc.CmdType]*RPCRuntimeStats),
	}
}
...
// RegionBatchRequestSender sends BatchINTERLOCK requests to TiFlash server by stream way.
...
func (ss *RegionBatchRequestSender) sendStreamReqToAddr(bo *Backoffer, ctxs []INTERLOCKTaskAndRPCContext, req *einsteindbrpc.Request, timout time.Duration) (resp *einsteindbrpc.Response, retry bool, cancel func(), err error) {
	// use the first ctx to send request, because every ctx has same address.
	cancel = func() {}
	rpcCtx := ctxs[0].ctx
	if e := einsteindbrpc.SetContext(req, rpcCtx.Meta, rpcCtx.Peer); e != nil {
...
func recordRegionRequestRuntimeStats(stats map[einsteindbrpc.CmdType]*RPCRuntimeStats, cmd einsteindbrpc.CmdType, d time.Duration) {
	stat, ok := stats[cmd]
	if !ok {
...
func (ss *RegionBatchRequestSender) onSendFail(bo *Backoffer, ctxs []INTERLOCKTaskAndRPCContext, err error) error {
	...
	if errors.Cause(err) == context.Canceled || status.Code(errors.Cause(err)) == codes.Canceled {
		...
		return errMilevaDBShuttingDown
	}
	...
	// When a causetstore is not available, the leader of related region should be elected quickly.
	// TODO: the number of retry time should be limited:since region may be unavailable
	// when some unrecoverable disaster happened.
	err = bo.Backoff(boEinsteinDBRPC, errors.Errorf("send einsteindb request error: %v, ctxs: %v, try next peer later", err, ctxs))
	return errors.Trace(err)
}
...
// SendReq sends a request to einsteindb server.
func (s *RegionRequestSender) SendReq(bo *Backoffer, req *einsteindbrpc.Request, regionID RegionVerID, timeout time.Duration) (*einsteindbrpc.Response, error) {
	resp, _, err := s.SendReqCtx(bo, req, regionID, timeout, ekv.EinsteinDB)
	return resp, err
}

// SendReqCtx sends a request to einsteindb server and return response and RPCCtx of this RPC.
func (s *RegionRequestSender) SendReqCtx(
	...
	failpoint.Inject("einsteindbStoreSendReqResult", func(val failpoint.Value) {
		switch val.(string) {
		case "timeout":
			failpoint.Return(nil, nil, errors.New("timeout"))
		case "GCNotLeader":
			if req.Type == einsteindbrpc.CmdGC {
		...
		case "requestMilevaDBStoreError":
			if sType == ekv.MilevaDB {
				failpoint.Return(nil, nil, ErrEinsteinDBServerTimeout)
			}
		}
	})

	var replicaRead ekv.ReplicaReadType
	if req.ReplicaRead {
	...
	for {
		...
		case ekv.EinsteinDB:
			...
			rpcCtx, err = s.regionCache.GetEinsteinDBRPCContext(bo, regionID, replicaRead, seed)
		case ekv.TiFlash:
		...
		failpoint.Inject("invalidCacheAndRetry", func() {
			// cooperate with github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/gcworker/setGcResolveMaxBackoff
			if c := bo.ctx.Value("injectedBackoff"); c != nil {
				resp, err = einsteindbrpc.GenRegionErrorResp(req, &errorpb.Error{EpochNotMatch: &errorpb.EpochNotMatch{}})
...
func (s *RegionRequestSender) sendReqToRegion(bo *Backoffer, rpcCtx *RPCContext, req *einsteindbrpc.Request, timeout time.Duration) (resp *einsteindbrpc.Response, retry bool, err error) {
	if e := einsteindbrpc.SetContext(req, rpcCtx.Meta, rpcCtx.Peer); e != nil {
		return nil, false, errors.Trace(e)
	}
	// judge the causetstore limit switch.
	if limit := storeutil.StoreLimit.Load(); limit > 0 {
		if err := s.getStoreToken(rpcCtx.CausetStore, limit); err != nil {
			return nil, false, err
		}
		defer s.releaseStoreToken(rpcCtx.CausetStore)
	}
	...
	if s.Stats != nil {
		...
		failpoint.Inject("einsteindbStoreRespResult", func(val failpoint.Value) {
			if val.(bool) {
				if req.Type == einsteindbrpc.CmdINTERLOCK && bo.totalSleep == 0 {
					failpoint.Return(&einsteindbrpc.Response{
						Resp: &interlock.Response{RegionError: &errorpb.Error{EpochNotMatch: &errorpb.EpochNotMatch{}}},
					}, false, nil)
				}
			}
		})
...
func (s *RegionRequestSender) getStoreToken(st *CausetStore, limit int64) error {
...
func (s *RegionRequestSender) releaseStoreToken(st *CausetStore) {
	count := st.tokenCount.Load()
	...
	logutil.BgLogger().Warn("release causetstore token failed, count equals to 0")
}
...
func (s *RegionRequestSender) onSendFail(bo *Backoffer, ctx *RPCContext, err error) error {
	...
	if status.Code(errors.Cause(err)) == codes.Canceled {
		...
		default:
			...
			// This may happen when einsteindb is killed and exiting.
	...
	// TODO: the number of retry time should be limited:since region may be unavailable
	// when some unrecoverable disaster happened.
	err = bo.Backoff(boEinsteinDBRPC, errors.Errorf("send einsteindb request error: %v, ctx: %v, try next peer later", err, ctx))
	return errors.Trace(err)
}
...
func (s *RegionRequestSender) needReloadRegion(ctx *RPCContext) (need bool) {
	...
	s.failStoreIDs[ctx.CausetStore.storeID] = struct{}{}
...
func (s *RegionRequestSender) onRegionError(bo *Backoffer, ctx *RPCContext, seed *uint32, regionErr *errorpb.Error) (retry bool, err error) {
	if span := opentracing.SpanFromContext(bo.ctx); span != nil && span.Tracer() != nil {
		span1 := span.Tracer().StartSpan("einsteindb.onRegionError", opentracing.ChildOf(span.Context()))
		defer span1.Finish()
		bo = bo.Clone()
		bo.ctx = opentracing.ContextWithSpan(bo.ctx, span1)
	}

	metrics.EinsteinDBRegionErrorCounter.WithLabelValues(regionErrorToLabel(regionErr)).Inc()
	if notLeader := regionErr.GetNotLeader(); notLeader != nil {
		// Retry if error is `NotLeader`.
		logutil.BgLogger().Debug("einsteindb reports `NotLeader` retry later",
		...
		if notLeader.GetLeader() == nil {
			...
			s.regionCache.UFIDelateLeader(ctx.Region, notLeader.GetLeader().GetStoreId(), ctx.AccessIdx)
		}
	...
	if storeNotMatch := regionErr.GetStoreNotMatch(); storeNotMatch != nil {
		// causetstore not match
		logutil.BgLogger().Warn("einsteindb reports `StoreNotMatch` retry later",
			zap.Stringer("storeNotMatch", storeNotMatch),
			zap.Stringer("ctx", ctx))
		ctx.CausetStore.markNeedCheck(s.regionCache.notifyCheckCh)
		return true, nil
	}

	if epochNotMatch := regionErr.GetEpochNotMatch(); epochNotMatch != nil {
		logutil.BgLogger().Debug("einsteindb reports `EpochNotMatch` retry later",
	...
	if regionErr.GetServerIsBusy() != nil {
		logutil.BgLogger().Warn("einsteindb reports `ServerIsBusy` retry later",
	...
	if regionErr.GetStaleCommand() != nil {
		logutil.BgLogger().Debug("einsteindb reports `StaleCommand`", zap.Stringer("ctx", ctx))
	...
	if regionErr.GetRaftEntryTooLarge() != nil {
		logutil.BgLogger().Warn("einsteindb reports `RaftEntryTooLarge`", zap.Stringer("ctx", ctx))
		return false, errors.New(regionErr.String())
	}
	if regionErr.GetRegionNotFound() != nil && seed != nil {
		logutil.BgLogger().Debug("einsteindb reports `RegionNotFound` in follow-reader",
			zap.Stringer("ctx", ctx), zap.Uint32("seed", *seed))
		*seed = *seed + 1
	}
	if regionErr.GetMaxTimestampNotSynced() != nil {
		logutil.BgLogger().Warn("einsteindb reports `MaxTimestampNotSynced`", zap.Stringer("ctx", ctx))
	...
	logutil.BgLogger().Debug("einsteindb reports region failed",
		zap.Stringer("regionErr", regionErr),
		zap.Stringer("ctx", ctx))
	// When the request is sent to MilevaDB, there is no region in the request, so the region id will be 0.
...
func pbIsolationLevel(level ekv.IsoLevel) kvrpcpb.IsolationLevel {
	switch level {
	case ekv.RC:
