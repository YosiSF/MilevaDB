...
func (s *testRegionCacheSuite) SetUpTest(c *C) {
	...
	storeIDs, peerIDs, regionID, _ := mockeinsteindb.BootstrapWithMultiStores(s.cluster, 2)
	s.region1 = regionID
	s.store1 = storeIDs[0]
	s.store2 = storeIDs[1]
	s.peer1 = peerIDs[0]
...
func (s *testRegionCacheSuite) storeAddr(id uint64) string {
	return fmt.Sprintf("causetstore%d", id)
}
...
func (s *testRegionCacheSuite) getAddr(c *C, key []byte, replicaRead ekv.ReplicaReadType, seed uint32) string {
	loc, err := s.cache.LocateKey(s.bo, key)
	c.Assert(err, IsNil)
	ctx, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, replicaRead, seed)
	c.Assert(err, IsNil)
	if ctx == nil {
		return ""
	}
...
func (s *testRegionCacheSuite) TestDropStore(c *C) {
	...
	c.Assert(err, IsNil)
	ctx, err := s.cache.GetEinsteinDBRPCContext(bo, loc.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(err, IsNil)
	c.Assert(ctx, IsNil)
	ctx, err = s.cache.GetEinsteinDBRPCContext(bo, loc.Region, ekv.ReplicaReadFollower, rand.Uint32())
	c.Assert(err, IsNil)
	c.Assert(ctx, IsNil)
	s.checkCache(c, 0)
}
...
func (s *testRegionCacheSuite) TestUFIDelateLeader(c *C) {
	seed := rand.Uint32()
	loc, err := s.cache.LocateKey(s.bo, []byte("a"))
	c.Assert(err, IsNil)
	// einsteindb-server reports `NotLeader`
	s.cache.UFIDelateLeader(loc.Region, s.store2, 0)

	r := s.getRegion(c, []byte("a"))
	c.Assert(r, NotNil)
	c.Assert(r.GetID(), Equals, s.region1)
	c.Assert(s.getAddr(c, []byte("a"), ekv.ReplicaReadLeader, 0), Equals, s.storeAddr(s.store2))
	c.Assert(s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed), Equals, s.storeAddr(s.store1))

	r = s.getRegionWithEndKey(c, []byte("z"))
	c.Assert(r, NotNil)
	c.Assert(r.GetID(), Equals, s.region1)
	c.Assert(s.getAddr(c, []byte("z"), ekv.ReplicaReadLeader, 0), Equals, s.storeAddr(s.store2))
	c.Assert(s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed), Equals, s.storeAddr(s.store1))
}

func (s *testRegionCacheSuite) TestUFIDelateLeader2(c *C) {
	seed := rand.Uint32()
	loc, err := s.cache.LocateKey(s.bo, []byte("a"))
	c.Assert(err, IsNil)
	...
	// einsteindb-server reports `NotLeader`
	s.cache.UFIDelateLeader(loc.Region, store3, 0)

	// Store3 does not exist in cache, causes a reload from FIDel.
	r := s.getRegion(c, []byte("a"))
	c.Assert(r, NotNil)
	c.Assert(r.GetID(), Equals, s.region1)
	c.Assert(s.getAddr(c, []byte("a"), ekv.ReplicaReadLeader, 0), Equals, s.storeAddr(s.store1))
	follower := s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed)
	if seed%2 == 0 {
		c.Assert(follower, Equals, s.storeAddr(s.store2))
	} else {
		c.Assert(follower, Equals, s.storeAddr(store3))
	}
	follower2 := s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed+1)
	if (seed+1)%2 == 0 {
	...
	// einsteindb-server notifies new leader to fidel-server.
	s.cluster.ChangeLeader(s.region1, peer3)
	// einsteindb-server reports `NotLeader` again.
	s.cache.UFIDelateLeader(r.VerID(), store3, 0)
	r = s.getRegion(c, []byte("a"))
	c.Assert(r, NotNil)
	c.Assert(r.GetID(), Equals, s.region1)
	c.Assert(s.getAddr(c, []byte("a"), ekv.ReplicaReadLeader, 0), Equals, s.storeAddr(store3))
	follower = s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed)
	if seed%2 == 0 {
...
func (s *testRegionCacheSuite) TestUFIDelateLeader3(c *C) {
	seed := rand.Uint32()
	loc, err := s.cache.LocateKey(s.bo, []byte("a"))
	c.Assert(err, IsNil)
	...
	// einsteindb-server notifies new leader to fidel-server.
	s.cluster.ChangeLeader(s.region1, peer3)
	// einsteindb-server reports `NotLeader`(store2 is the leader)
	s.cache.UFIDelateLeader(loc.Region, s.store2, 0)

	// Store2 does not exist any more, causes a reload from FIDel.
	r := s.getRegion(c, []byte("a"))
	c.Assert(err, IsNil)
	c.Assert(r, NotNil)
	c.Assert(r.GetID(), Equals, s.region1)
	...
	ctx, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadLeader, seed)
	c.Assert(err, IsNil)
	...
	s.cache.UFIDelateLeader(loc.Region, s.store2, 0)
	addr := s.getAddr(c, []byte("a"), ekv.ReplicaReadLeader, 0)
	c.Assert(addr, Equals, "")
	addr = s.getAddr(c, []byte("a"), ekv.ReplicaReadLeader, 0)
	c.Assert(addr, Equals, s.storeAddr(store3))

	addr = s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed)
	addr2 := s.getAddr(c, []byte("a"), ekv.ReplicaReadFollower, seed+1)
...
func (s *testRegionCacheSuite) TestSendFailedButLeaderNotChange(c *C) {
	...
	loc, err := s.cache.LocateKey(s.bo, []byte("a"))
	c.Assert(err, IsNil)
	ctx, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(err, IsNil)
	...
	seed := rand.Uint32()
	ctxFollower1, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, seed)
	c.Assert(err, IsNil)
	if seed%2 == 0 {
	...
	ctxFollower2, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, seed)
	c.Assert(err, IsNil)
	if seed%2 == 0 {
	...
	ctxFollower2, err = s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, seed+1)
	c.Assert(err, IsNil)
	if (seed+1)%2 == 0 {
	...
	s.cache.UFIDelateLeader(loc.Region, s.store2, ctx.AccessIdx)
	ctx, err = s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(err, IsNil)
	c.Assert(ctx.Peer.Id, Equals, s.peer2)
...
func (s *testRegionCacheSuite) TestSendFailedInHibernateRegion(c *C) {
	...
	s.cache.UFIDelateLeader(loc.Region, 0, ctx.AccessIdx)
	ctx, err = s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(err, IsNil)
...
func (s *testRegionCacheSuite) TestSendFailInvalidateRegionsInSameStore(c *C) {
	...
	ctx, _ := s.cache.GetEinsteinDBRPCContext(s.bo, loc1.Region, ekv.ReplicaReadLeader, 0)
	s.checkCache(c, 2)
	s.cache.OnSendFail(s.bo, ctx, false, errors.New("test error"))

	// Get region2 cache will get nil then reload.
	ctx2, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc2.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(ctx2, IsNil)
	c.Assert(err, IsNil)
}
...
func (s *testRegionCacheSuite) TestSendFailedInMultipleNode(c *C) {
	...
	s.cache.UFIDelateLeader(loc.Region, s.store1, ctx.AccessIdx)
	ctx, err = s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(err, IsNil)
	c.Assert(ctx.Peer.Id, Equals, s.peer1)
...
func (s *testRegionCacheSuite) TestSplit(c *C) {
	...
	c.Assert(r.GetID(), Equals, s.region1)
	c.Assert(s.getAddr(c, []byte("x"), ekv.ReplicaReadLeader, 0), Equals, s.storeAddr(s.store1))
	c.Assert(s.getAddr(c, []byte("x"), ekv.ReplicaReadFollower, seed), Equals, s.storeAddr(s.store2))
	...
	// einsteindb-server reports `NotInRegion`
...
func (s *testRegionCacheSuite) TestReconnect(c *C) {
	...
	// connect einsteindb-server failed, cause drop cache
	s.cache.InvalidateCachedRegion(loc.Region)
...
func (s *testRegionCacheSuite) TestRegionEpochAheadOfEinsteinDB(c *C) {
...
func (s *testRegionCacheSuite) TestRegionEpochOnTiFlash(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(loc1.Region.id, Equals, s.region1)
	lctx, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc1.Region, ekv.ReplicaReadLeader, 0)
	c.Assert(err, IsNil)
...
func createClusterWithStoresAndRegions(regionCnt, storeCount int) *mockeinsteindb.Cluster {
	cluster := mockeinsteindb.NewCluster(mockeinsteindb.MustNewMVCCStore())
	_, _, regionID, _ := mockeinsteindb.BootstrapWithMultiStores(cluster, storeCount)
...
func (s *testRegionCacheSuite) TestUFIDelateStoreAddr(c *C) {
	mvsr-oocStore := mockeinsteindb.MustNewMVCCStore()
	defer mvsr-oocStore.Close()

	client := &RawKVClient{
		clusterID:   0,
		regionCache: NewRegionCache(mockeinsteindb.NewFIDelClient(s.cluster)),
		rpcClient:   mockeinsteindb.NewRPCClient(s.cluster, mvsr-oocStore),
	}
	defer client.Close()
	...
	c.Assert(err, IsNil)
	// einsteindb-server reports `StoreNotMatch` And retry
	store1Addr := s.storeAddr(s.store1)
	s.cluster.UFIDelateStoreAddr(s.store1, s.storeAddr(s.store2))
	s.cluster.UFIDelateStoreAddr(s.store2, store1Addr)
...
func (s *testRegionCacheSuite) TestReplaceNewAddrAndOldOfflineImmediately(c *C) {
	...
	c.Assert(err, IsNil)
	fctx, err := client.regionCache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, 0)
	c.Assert(err, IsNil)
	c.Assert(fctx.CausetStore.storeID, Equals, s.store2)
...
func (s *testRegionCacheSuite) TestFollowerReadFallback(c *C) {
	...
	ctxFollower1, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, 0)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower1.Peer.Id, Equals, s.peer2)
	ctxFollower2, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, 1)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower2.Peer.Id, Equals, peer3)
	c.Assert(ctxFollower1.Peer.Id, Not(Equals), ctxFollower2.Peer.Id)
	...
	ctx, err = s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadFollower, 0)
	c.Assert(err, IsNil)
	c.Assert(ctx.Peer.Id, Equals, peer3)
}
...
func (s *testRegionCacheSuite) TestMixedReadFallback(c *C) {
	...
	ctxFollower1, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadMixed, 0)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower1.Peer.Id, Equals, s.peer1)

	ctxFollower2, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadMixed, 1)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower2.Peer.Id, Equals, s.peer2)

	ctxFollower3, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadMixed, 2)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower3.Peer.Id, Equals, peer3)

	// send fail on store2, next follower read is going to fallback to store3
	s.cache.OnSendFail(s.bo, ctxFollower1, false, errors.New("test error"))
	ctx, err = s.cache.GetEinsteinDBRPCContext(s.bo, loc.Region, ekv.ReplicaReadMixed, 0)
	c.Assert(err, IsNil)
	c.Assert(ctx.Peer.Id, Equals, s.peer2)
}
...
func (s *testRegionCacheSuite) TestFollowerMeetEpochNotMatch(c *C) {
	...
	ctxFollower1, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc1.Region, ekv.ReplicaReadFollower, followReqSeed)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower1.Peer.Id, Equals, s.peer2)
	c.Assert(ctxFollower1.CausetStore.storeID, Equals, s.store2)
...
func (s *testRegionCacheSuite) TestMixedMeetEpochNotMatch(c *C) {
	...
	followReqSeed := uint32(0)
	ctxFollower1, err := s.cache.GetEinsteinDBRPCContext(s.bo, loc1.Region, ekv.ReplicaReadMixed, followReqSeed)
	c.Assert(err, IsNil)
	c.Assert(ctxFollower1.Peer.Id, Equals, s.peer1)
	c.Assert(ctxFollower1.CausetStore.storeID, Equals, s.store1)
...
func BenchmarkOnRequestFail(b *testing.B) {
	...
	*/
	...
	cache := NewRegionCache(mockeinsteindb.NewFIDelClient(cluster))
	defer cache.Close()
	...
	causetstore, peer, accessIdx, _ := region.WorkStorePeer(regionStore)
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			if r != nil {
				r.getStore().switchNextEinsteinDBPeer(r, rpcCtx.AccessIdx)
			}
		}
	})
