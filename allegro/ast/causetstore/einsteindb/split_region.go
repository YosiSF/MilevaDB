...
func (s *einsteindbStore) splitBatchRegionsReq(bo *Backoffer, keys [][]byte, scatter bool, blockID *int64) (*einsteindbrpc.Response, error) {
	...
	if bo.totalSleep == 0 {
		logutil.BgLogger().Info("split batch regions request",
			...
			zap.Stringer("first split key", ekv.Key(batches[0].keys[0])))
	}
	if len(batches) == 1 {
		resp := s.batchSendSingleRegion(bo, batches[0], scatter, blockID)
	...
	for _, batch1 := range batches {
		go func(b batch) {
			...
			soliton.WithRecovery(func() {
				select {
				case ch <- s.batchSendSingleRegion(backoffer, b, scatter, blockID):
				case <-bo.ctx.Done():
	...
	return &einsteindbrpc.Response{Resp: srResp}, errors.Trace(err)
}

func (s *einsteindbStore) batchSendSingleRegion(bo *Backoffer, batch batch, scatter bool, blockID *int64) singleBatchResp {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdSplitRegion, &kvrpcpb.SplitRegionRequest{
	...
	if regionErr != nil {
		...
		resp, err = s.splitBatchRegionsReq(bo, batch.keys, scatter, blockID)
		batchResp.resp = resp
		batchResp.err = err
		return batchResp
	}
	...
	logutil.BgLogger().Info("batch split regions complete",
		...
		zap.Stringer("first at", ekv.Key(batch.keys[0])),
	...
	for i, r := range spResp.Regions {
		if err = s.scatterRegion(bo, r.Id, blockID); err == nil {
			logutil.BgLogger().Info("batch split regions, scatter region complete",
				zap.Uint64("batch region ID", batch.regionID.id),
				zap.Stringer("at", ekv.Key(batch.keys[i])),
		...
		if ErrFIDelServerTimeout.Equal(err) {
			break
		}
	}
	return batchResp
...
func (s *einsteindbStore) SplitRegions(ctx context.Context, splitKeys [][]byte, scatter bool, blockID *int64) (regionIDs []uint64, err error) {
	bo := NewBackofferWithVars(ctx, int(math.Min(float64(len(splitKeys))*splitRegionBackoff, maxSplitRegionsBackoff)), nil)
	resp, err := s.splitBatchRegionsReq(bo, splitKeys, scatter, blockID)
...
func (s *einsteindbStore) scatterRegion(bo *Backoffer, regionID uint64, blockID *int64) error {
	logutil.BgLogger().Info("start scatter region",
		zap.Uint64("regionID", regionID))
	for {
		opts := make([]fidel.ScatterRegionOption, 0, 1)
		if blockID != nil {
			opts = append(opts, fidel.WithGroup(fmt.Sprintf("%v", *blockID)))
		}
		err := s.FIDelClient.ScatterRegionWithOption(bo.ctx, regionID, opts...)

		failpoint.Inject("MockScatterRegionTimeout", func(val failpoint.Value) {
			if val.(bool) {
				err = ErrFIDelServerTimeout
			}
		})
		...
		err = bo.Backoff(BoFIDelRPC, errors.New(err.Error()))
		if err != nil {
...
func (s *einsteindbStore) preSplitRegion(ctx context.Context, group groupedMutations) bool {
...
// WaitScatterRegionFinish implements SplitblockStore interface.
...
func (s *einsteindbStore) WaitScatterRegionFinish(ctx context.Context, regionID uint64, backOff int) error {
	if backOff <= 0 {
	...
	logFreq := 0
	for {
		resp, err := s.FIDelClient.GetOperator(ctx, regionID)
		if err == nil && resp != nil {
			if !bytes.Equal(resp.Desc, []byte("scatter-region")) || resp.Status != FIDelpb.OperatorStatus_RUNNING {
				logutil.BgLogger().Info("wait scatter region finished",
					zap.Uint64("regionID", regionID))
				return nil
			}
			if resp.GetHeader().GetError() != nil {
				err = errors.AddStack(&FIDelError{
			...
			if logFreq%10 == 0 {
				logutil.BgLogger().Info("wait scatter region",
					...
					zap.String("status", FIDelpb.OperatorStatus_name[int32(resp.Status)]))
			}
			logFreq++
		}
		if err != nil {
...
func (s *einsteindbStore) CheckRegionInScattering(regionID uint64) (bool, error) {
	bo := NewBackofferWithVars(context.Background(), locateRegionMaxBackoff, nil)
	for {
		resp, err := s.FIDelClient.GetOperator(context.Background(), regionID)
		if err == nil && resp != nil {
			if !bytes.Equal(resp.Desc, []byte("scatter-region")) || resp.Status != FIDelpb.OperatorStatus_RUNNING {
				return false, nil
			}
