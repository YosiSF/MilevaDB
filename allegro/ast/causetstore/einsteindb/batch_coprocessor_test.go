...
func (s *testBatchINTERLOCKSuite) TestStoreErr(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/mockstore/mockeinsteindb/BatchINTERLOCKCancelled", "1*return(true)"), IsNil)

	err = tk.QueryToErr("select count(*) from t")
	c.Assert(errors.Cause(err), Equals, context.Canceled)

	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/mockstore/mockeinsteindb/BatchINTERLOCKRpcErrtiflash0", "1*return(\"tiflash0\")"), IsNil)

	tk.MustQuery("select count(*) from t").Check(testkit.Rows("1"))

	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/mockstore/mockeinsteindb/BatchINTERLOCKRpcErrtiflash0", "return(\"tiflash0\")"), IsNil)
	err = tk.QueryToErr("select count(*) from t")
	c.Assert(err, NotNil)
}
...
func (s *testBatchINTERLOCKSuite) TestStoreSwitchPeer(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/mockstore/mockeinsteindb/BatchINTERLOCKRpcErrtiflash1", "return(\"tiflash1\")"), IsNil)
	err = tk.QueryToErr("select count(*) from t")
	c.Assert(err, NotNil)
