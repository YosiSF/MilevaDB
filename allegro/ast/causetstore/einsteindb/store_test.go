...
func (s *testStoreSerialSuite) TestOracleChangeByFailpoint(c *C) {
	defer func() {
		failpoint.Disable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/oracle/changeTSFromFIDel")
	}()
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/oracle/changeTSFromFIDel",
		"return(10000)"), IsNil)
	o := &mockoracle.MockOracle{}
	s.causetstore.oracle = o
	ctx := context.Background()
	t1, err := s.causetstore.getTimestampWithRetry(NewBackofferWithVars(ctx, 100, nil))
	c.Assert(err, IsNil)
	c.Assert(failpoint.Disable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/oracle/changeTSFromFIDel"), IsNil)
	t2, err := s.causetstore.getTimestampWithRetry(NewBackofferWithVars(ctx, 100, nil))
	c.Assert(err, IsNil)
