...
func (s *testCommitterSuite) SetUpTest(c *C) {
	mvsr-oocStore, err := mockeinsteindb.NewMVCCLevelDB("")
	c.Assert(err, IsNil)
	cluster := mockeinsteindb.NewCluster(mvsr-oocStore)
	mockeinsteindb.BootstrapWithMultiRegions(cluster, []byte("a"), []byte("b"), []byte("c"))
	s.cluster = cluster
	client := mockeinsteindb.NewRPCClient(cluster, mvsr-oocStore)
	FIDelCli := &codecFIDelClient{mockeinsteindb.NewFIDelClient(cluster)}
	spkv := NewMockSafePointKV()
	causetstore, err := newEinsteinDBStore("mockeinsteindb-causetstore", FIDelCli, spkv, client, false, nil)
...
func (s *testCommitterSuite) isKeyLocked(c *C, key []byte) bool {
	...
	c.Assert(err, IsNil)
	bo := NewBackofferWithVars(context.Background(), getMaxBackoff, nil)
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdGet, &kvrpcpb.GetRequest{
		Key:     key,
		Version: ver.Ver,
	})
...
func (s *testCommitterSuite) TestPkNotFound(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/txnNotFoundRetTTL", "return"), IsNil)
	err = txn3.LockKeys(context.Background(), lockCtx, k3)
	c.Assert(err.Error(), Equals, ErrLockAcquireFailAndNoWaitSet.Error())
	c.Assert(failpoint.Disable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/txnNotFoundRetTTL"), IsNil)
}
...
func (s *testCommitterSuite) TestPushPessimisticLock(c *C) {
	...
	// The optimistic dagger shouldn't block reads.
