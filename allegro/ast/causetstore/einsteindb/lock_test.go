...
func (s *testLockSuite) TestCheckTxnStatusTTL(c *C) {
	...
	c.Assert(status.commitTS, Equals, uint64(0))
	c.Assert(status.action, Equals, kvrpcpb.CausetAction_NoCausetAction)
...
func (s *testLockSuite) TestCheckTxnStatusNoWait(c *C) {
	...
	c.Assert(status.ttl, Equals, uint64(0))
	c.Assert(status.commitTS, Equals, uint64(0))
	c.Assert(status.action, Equals, kvrpcpb.CausetAction_LockNotExistRollback)
}
...
func (s *testLockSuite) TestZeroMinCommitTS(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/mockZeroCommitTS", mockValue), IsNil)
	s.prewriteTxnWithTTL(c, txn.(*einsteindbTxn), 1000)
	c.Assert(failpoint.Disable("github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/mockZeroCommitTS"), IsNil)
