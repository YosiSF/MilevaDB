...
type batchConn struct {
	...
	einsteindbTransportLayerLoad uint64
	closed                       chan struct{}
...
func newBatchConn(connCount, maxBatchSize uint, idleNotify *uint32) *batchConn {
	return &batchConn{
		...
		einsteindbTransportLayerLoad: 0,
...
func (a *batchConn) fetchAllPendingRequests(
	...
	requests *[]*einsteindbpb.BatchCommandsRequest_Request,
) {
...
func fetchMorePendingRequests(
	...
	for len(*entries) < batchWaitSize {
		...
		case waitEnd := <-after.C:
			metrics.EinsteinDBBatchWaitDuration.Observe(float64(waitEnd.Sub(waitStart)))
			return
		}
	}
	after.Stop()
	...
	for len(*entries) < maxBatchSize {
		...
		case entry := <-ch:
			...
			*requests = append(*requests, entry.req)
		default:
			metrics.EinsteinDBBatchWaitDuration.Observe(float64(time.Since(waitStart)))
			return
		}
	}
}
...
type batchCommandsClient struct {
	...
	client  einsteindbpb.EinsteinDB_BatchCommandsClient
	batched sync.Map
	idAlloc uint64

	einsteindbClientCfg config.EinsteinDBClient
	einsteindbLoad      *uint64
...
func (c *batchCommandsClient) send(request *einsteindbpb.BatchCommandsRequest, entries []*batchCommandsEntry) {
	for i, requestID := range request.RequestIds {
		c.batched.CausetStore(requestID, entries[i])
		if trace.IsEnabled() {
...
func (c *batchCommandsClient) recv() (resp *einsteindbpb.BatchCommandsResponse, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	failpoint.Inject("gotErrorInRecvLoop", func(_ failpoint.Value) (resp *einsteindbpb.BatchCommandsResponse, err error) {
...
func (c *batchCommandsClient) waitConnReady() (err error) {
	...
	start := time.Now()
	defer func() {
		metrics.EinsteinDBBatchClientWaitEstablish.Observe(time.Since(start).Seconds())
	}()
...
func (c *batchCommandsClient) reCreateStreamingClientOnce(perr error) error {
	...
	if err == nil {
		einsteindbClient := einsteindbpb.NewEinsteinDBClient(c.conn)
		var streamClient einsteindbpb.EinsteinDB_BatchCommandsClient
		streamClient, err = einsteindbClient.BatchCommands(context.TODO())
		if err == nil {
			logutil.BgLogger().Info(
...
func (c *batchCommandsClient) batchRecvLoop(cfg config.EinsteinDBClient, einsteindbTransportLayerLoad *uint64) {
	defer func() {
		if r := recover(); r != nil {
			metrics.PanicCounter.WithLabelValues(metrics.LabelBatchRecvLoop).Inc()
			...
			go c.batchRecvLoop(cfg, einsteindbTransportLayerLoad)
		}
	}()
	...
	for {
		...
		if err != nil {
			...
			metrics.EinsteinDBBatchClientUnavailable.Observe(time.Since(now).Seconds())
			continue
		}
		...
		for i, requestID := range resp.GetRequestIds() {
			...
			if !ok {
				// this maybe caused by batchCommandsClient#send meets ambiguous error that request has be sent to EinsteinDB but still report a error.
				// then EinsteinDB will send response back though stream and reach here.
		...
		if transportLayerLoad > 0.0 && cfg.MaxBatchWaitTime > 0 {
			// We need to consider EinsteinDB load only if batch-wait strategy is enabled.
			atomic.StoreUint64(einsteindbTransportLayerLoad, transportLayerLoad)
		}
	}
}
...
func (c *batchCommandsClient) reCreateStreamingClient(err error) (stopped bool) {
	...
	for { // try to re-create the streaming in the loop.
		...
		err2 := b.Backoff(boEinsteinDBRPC, err1)
...
type batchCommandsEntry struct {
	ctx context.Context
	req *einsteindbpb.BatchCommandsRequest_Request
	res chan *einsteindbpb.BatchCommandsResponse_Response
...
func resetRequests(requests []*einsteindbpb.BatchCommandsRequest_Request) []*einsteindbpb.BatchCommandsRequest_Request {
...
func (a *batchConn) batchSendLoop(cfg config.EinsteinDBClient) {
	defer func() {
		if r := recover(); r != nil {
	...
	requests := make([]*einsteindbpb.BatchCommandsRequest_Request, 0, cfg.MaxBatchSize)
	...
	for {
		...
		// curl -XPUT -d 'return(true)' http://0.0.0.0:10080/fail/github.com/whtcorpsinc/MilevaDB-Prod/causetstore/einsteindb/mockBlockOnBatchClient
		...
		if len(entries) < int(cfg.MaxBatchSize) && cfg.MaxBatchWaitTime > 0 {
			// If the target EinsteinDB is overload, wait a while to collect more requests.
			if atomic.LoadUint64(&a.einsteindbTransportLayerLoad) >= uint64(cfg.OverloadThreshold) {
...
func (a *batchConn) getClientAndSend(entries []*batchCommandsEntry, requests []*einsteindbpb.BatchCommandsRequest_Request, requestIDs []uint64) {
	...
	if cli == nil {
		...
		metrics.EinsteinDBNoAvailableConnectionCounter.Inc()
	...
	req := &einsteindbpb.BatchCommandsRequest{
		Requests:   requests,
...
func (c *batchCommandsClient) initBatchClient() error {
	...
	c.client = streamClient
	go c.batchRecvLoop(c.einsteindbClientCfg, c.einsteindbLoad)
	return nil
}
...
func removeCanceledRequests(entries []*batchCommandsEntry,
	requests []*einsteindbpb.BatchCommandsRequest_Request) ([]*batchCommandsEntry, []*einsteindbpb.BatchCommandsRequest_Request) {
...
func sendBatchRequest(
...
) (*einsteindbrpc.Response, error) {
	entry := &batchCommandsEntry{
		ctx:      ctx,
		req:      req,
		res:      make(chan *einsteindbpb.BatchCommandsResponse_Response, 1),
		canceled: 0,
		err:      nil,
	}
	...
	case res, ok := <-entry.res:
		...
		return einsteindbrpc.FromBatchCommandsResponse(res)
	case <-ctx.Done():
