...
func (s *testAsyncCommitSuite) TestCheckSecondaries(c *C) {
	...
	mock := mockResolveClient{
		...
		onCheckSecondaries: func(req *kvrpcpb.CheckSecondaryLocksRequest) (*einsteindbrpc.Response, error) {
		...
		onResolveLock: func(req *kvrpcpb.ResolveLockRequest) (*einsteindbrpc.Response, error) {
			if req.StartVersion != ts {
				return nil, errors.Errorf("Bad start version: %d, expected: %d", req.StartVersion, ts)
			}
	...
	gotResolve = int64(0)
	gotOther = int64(0)
	mock.onResolveLock = func(req *kvrpcpb.ResolveLockRequest) (*einsteindbrpc.Response, error) {
		if req.StartVersion != ts {
			return nil, errors.Errorf("Bad start version: %d, expected: %d", req.StartVersion, ts)
		}
		if req.CommitVersion != commitTs {
...
type mockResolveClient struct {
	inner              Client
	onResolveLock      func(*kvrpcpb.ResolveLockRequest) (*einsteindbrpc.Response, error)
	onCheckSecondaries func(*kvrpcpb.CheckSecondaryLocksRequest) (*einsteindbrpc.Response, error)
}
