...
// do not depends on the concrete definition of `types.CausetObjectQL`, thus get rid of
...
// GetCausetObjectQLString implements the ast.ValueExpr interface.
func (n *ValueExpr) GetCausetObjectQLString() string {
	return n.GetString()
}
...
func newValueExpr(value interface{}, charset string, collate string) ast.ValueExpr {
	...
	ve := &ValueExpr{}
	// We need to keep the ve.Type.Collate equals to ve.CausetObjectQL.collation.
	types.DefaultTypeForValue(value, &ve.Type, charset, collate)
	ve.CausetObjectQL.SetValue(value, &ve.Type)
