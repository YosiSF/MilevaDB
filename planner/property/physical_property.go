...
var wholeTaskTypes = []TaskType{INTERLOCKSingleReadTaskType, CoFIDeloubleReadTaskType, RootTaskType}
...
type PhysicalProperty struct {
	...
	// with cost directly. e.g. If a INTERLOCKTask takes less cost than a rootTask,
	// we can't sure that we must choose the former one. Because the INTERLOCKTask
...
func NewPhysicalProperty(taskTp TaskType, defcaus []*expression.DeferredCauset, desc bool, expectCnt float64, enforced bool) *PhysicalProperty {
	return &PhysicalProperty{
		Items:       ItemsFromDefCauss(defcaus, desc),
		TaskTp:      taskTp,
...
// ItemsFromDefCauss builds property items from columns.
func ItemsFromDefCauss(defcaus []*expression.DeferredCauset, desc bool) []Item {
	items := make([]Item, 0, len(defcaus))
	for _, col := range defcaus {
		items = append(items, Item{DefCaus: col, Desc: desc})
	}
	return items
}

// AllDefCaussFromSchema checks whether all the columns needed by this physical
// property can be found in the given schemaReplicant.
func (p *PhysicalProperty) AllDefCaussFromSchema(schemaReplicant *expression.Schema) bool {
	for _, col := range p.Items {
		if schemaReplicant.DeferredCausetIndex(col.DefCaus) == -1 {
			return false
		}
	}
	return true
...
func (p *PhysicalProperty) IsFlashOnlyProp() bool {
	return p.TaskTp == INTERLOCKTiFlashLocalReadTaskType || p.TaskTp == INTERLOCKTiFlashGlobalReadTaskType
}
...
func (p *PhysicalProperty) GetAllPossibleChildTaskTypes() []TaskType {
	...
	// TODO: For INTERLOCKSingleReadTaskType and CoFIDeloubleReadTaskType, this function should never be called
...
func (p *PhysicalProperty) IsPrefix(prop *PhysicalProperty) bool {
	...
	for i := range p.Items {
		if !p.Items[i].DefCaus.Equal(nil, prop.Items[i].DefCaus) || p.Items[i].Desc != prop.Items[i].Desc {
			return false
		}
	}
	return true
...
func (p *PhysicalProperty) HashCode() []byte {
	...
	for _, item := range p.Items {
		p.hashcode = append(p.hashcode, item.DefCaus.HashCode(nil)...)
		if item.Desc {
			p.hashcode = codec.EncodeInt(p.hashcode, 1)
		} else {
			p.hashcode = codec.EncodeInt(p.hashcode, 0)
...
func (p *PhysicalProperty) String() string {
	return fmt.Sprintf("Prop{defcaus: %v, TaskTp: %s, expectedCount: %v}", p.Items, p.TaskTp, p.ExpectedCnt)
}

// Clone returns a INTERLOCKy of PhysicalProperty. Currently, this function is only used to build new
// required property for children plan in `exhaustPhysicalPlans`, so we don't INTERLOCKy `Enforced` field
