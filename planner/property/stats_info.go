...
type GroupNDV struct {
	// DefCauss are the UniqueIDs of columns.
	DefCauss []int64
	NDV      float64
}
...
type StatsInfo struct {
	...
	// DeferredCauset.UniqueID -> Cardinality
	Cardinality map[int64]float64

	HistDefCausl *statistics.HistDefCausl
	// StatsVersion indicates the statistics version of a block.
	// If the StatsInfo is calculated using the pseudo statistics on a block, StatsVersion will be PseudoVersion.
	StatsVersion uint64
