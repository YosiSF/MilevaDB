...
func (s *testPlanNormalize) TestNormalizedPlan(c *C) {
	...
	tk.MustExec("create block t2 (a int key,b int,c int, index (b));")
	var input []string
	var output []struct {
...
func (s *testPlanNormalize) TestNormalizedPlanForDiffStore(c *C) {
	...
	tk.MustExec("create block t1 (a int, b int, c int, primary key(a))")
...
func (s *testPlanNormalize) TestNormalizedDigest(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t1,t2, bmsql_order_line, bmsql_district,bmsql_stock")
	tk.MustExec("create block t1 (a int key,b int,c int, index (b));")
	tk.MustExec("create block t2 (a int key,b int,c int, index (b));")
...
func (s *testPlanNormalize) TestNthPlanHint(c *C) {
	...
	tk.MustExec("create block tt (a int,b int, index(a), index(b));")
	...
	tk.MustExec("create block t (a int, b int, c int, index(a), index(b), index(a,b))")
...
func (s *testPlanNormalize) TestDecodePlanPerformance(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t (a varchar(10) key,b int);")
	tk.MustExec("set @@milevadb_slow_log_threshold=200000")
