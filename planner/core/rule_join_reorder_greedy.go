...
func (s *joinReorderGreedySolver) checkConnectionAndMakeJoin(leftNode, rightNode LogicalPlan) (LogicalPlan, []expression.Expression) {
	...
	INTERLOCKy(remainOtherConds, s.otherConds)
	for _, edge := range s.eqEdges {
		lDefCaus := edge.GetArgs()[0].(*expression.DeferredCauset)
		rDefCaus := edge.GetArgs()[1].(*expression.DeferredCauset)
		if leftNode.Schema().Contains(lDefCaus) && rightNode.Schema().Contains(rDefCaus) {
			...
			newSf := expression.NewFunctionInternal(s.ctx, ast.EQ, edge.GetType(), rDefCaus, lDefCaus).(*expression.ScalarFunction)
