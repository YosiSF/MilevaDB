...
func (s *joinReOrderSolver) optimizeRecursive(ctx stochastikctx.Context, p LogicalPlan) (LogicalPlan, error) {
	var err error
	...
	for len(cartesianJoinGroup) > 1 {
		...
		for i := 0; i < len(cartesianJoinGroup); i += 2 {
			...
			for i := len(s.otherConds) - 1; i >= 0; i-- {
				defcaus := expression.ExtractDeferredCausets(s.otherConds[i])
