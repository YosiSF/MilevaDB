...
func (s *pFIDelSolver) optimize(ctx context.Context, lp LogicalPlan) (LogicalPlan, error) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalBlockDual) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	return predicates, p
}
...
// uFIDelateEQCond will extract the arguments of a equal condition that connect two expressions.
func (p *LogicalJoin) uFIDelateEQCond() {
	...
	if len(lKeys) > 0 {
		...
		for i := range lKeys {
			_, lOk := lKeys[i].(*expression.DeferredCauset)
			_, rOk := rKeys[i].(*expression.DeferredCauset)
		...
		for i := range lKeys {
			...
			eqCond := expression.NewFunctionInternal(p.ctx, ast.EQ, types.NewFieldType(allegrosql.TypeTiny), lKey, rKey)
			p.EqualConditions = append(p.EqualConditions, eqCond.(*expression.ScalarFunction))
		}
	}
}

func (p *LogicalProjection) appendExpr(expr expression.Expression) *expression.DeferredCauset {
	if col, ok := expr.(*expression.DeferredCauset); ok {
		return col
	}
	expr = expression.DeferredCausetSubstitute(expr, p.schemaReplicant, p.Exprs)
...
func simplifyOuterJoin(p *LogicalJoin, predicates []expression.Expression) {
	...
	if innerPlan, ok := innerBlock.(*LogicalJoin); ok {
		simplifyOuterJoin(innerPlan, predicates)
	}
	if outerPlan, ok := outerBlock.(*LogicalJoin); ok {
	...
	for _, expr := range predicates {
		...
		if expression.ExprFromSchema(expr, outerBlock.Schema()) {
			continue
		}
...
// If it is a predicate containing a reference to an inner block that evaluates to UNKNOWN or FALSE when one of its arguments is NULL.
...
func (p *LogicalProjection) PredicatePushDown(predicates []expression.Expression) (ret []expression.Expression, retPlan LogicalPlan) {
	...
	for _, cond := range predicates {
		newFilter := expression.DeferredCausetSubstitute(cond, p.Schema(), p.Exprs)
		if !expression.HasGetSetVarFunc(newFilter) {
			canBePushed = append(canBePushed, expression.DeferredCausetSubstitute(cond, p.Schema(), p.Exprs))
		} else {
...
func (la *LogicalAggregation) PredicatePushDown(predicates []expression.Expression) (ret []expression.Expression, retPlan LogicalPlan) {
	...
	for _, cond := range predicates {
		...
		case *expression.ScalarFunction:
			extractedDefCauss := expression.ExtractDeferredCausets(cond)
			ok := true
			...
			if ok {
				newFunc := expression.DeferredCausetSubstitute(cond, la.Schema(), exprsOriginal)
...
func deriveNotNullExpr(expr expression.Expression, schemaReplicant *expression.Schema) expression.Expression {
	...
	if isNullRejected(ctx, schemaReplicant, expr) && !allegrosql.HasNotNullFlag(childDefCaus.RetType.Flag) {
		return expression.BuildNotNullExpr(ctx, childDefCaus)
	}
	return nil
}

// Conds2BlockDual builds a LogicalBlockDual if cond is constant false or null.
func Conds2BlockDual(p LogicalPlan, conds []expression.Expression) LogicalPlan {
	if len(conds) != 1 {
		return nil
	}
...
func (p *LogicalJoin) outerJoinPropConst(predicates []expression.Expression) []expression.Expression {
	...
	joinConds, predicates = expression.PropConstOverOuterJoin(p.ctx, joinConds, predicates, outerBlock.Schema(), innerBlock.Schema(), nullSensitive)
	p.AttachOnConds(joinConds)
	return predicates
}

// GetPartitionByDefCauss extracts 'partition by' columns from the Window.
func (p *LogicalWindow) GetPartitionByDefCauss() []*expression.DeferredCauset {
	partitionDefCauss := make([]*expression.DeferredCauset, 0, len(p.PartitionBy))
...
func (p *LogicalWindow) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	canBePushed := make([]expression.Expression, 0, len(predicates))
	canNotBePushed := make([]expression.Expression, 0, len(predicates))
	partitionDefCauss := expression.NewSchema(p.GetPartitionByDefCauss()...)
	for _, cond := range predicates {
		// We can push predicate beneath Window, only if all of the
		// extractedDefCauss are part of partitionBy columns.
		if expression.ExprFromSchema(cond, partitionDefCauss) {
...
// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalMemBlock) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	if p.Extractor != nil {
