...
func (s *testPrepareSerialSuite) TestPrepareCache(c *C) {
	...
	tk.MustExec("create block t(a int primary key, b int, c int, index idx1(b, a), index idx2(b))")
	...
	tk.MustExec(`create block tp(c1 int, c2 int, primary key (c1))`)
...
func (s *testPlanSerialSuite) TestPrepareCacheDeferredFunction(c *C) {
	...
	tk.MustExec("create block t1 (id int PRIMARY KEY, c1 TIMESTAMP(3) NOT NULL DEFAULT '2020-01-14 10:43:20', KEY idx1 (c1))")
...
func (s *testPrepareSuite) TestPrepareWithWindowFunction(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("create block window_prepare(a int, b double)")
...
func (s *testPrepareSerialSuite) TestPrepareCacheForPartition(c *C) {
	...
	for _, val := range []string{string(variable.StaticOnly), string(variable.DynamicOnly)} {
		...
		tk.MustExec("drop block if exists t_index_read")
		tk.MustExec("create block t_index_read (id int, k int, c varchar(10), primary key (id, k)) partition by hash(id+k) partitions 10")
		...
		// Test for BlockScan.
		tk.MustExec("drop block if exists t_block_read")
		tk.MustExec("create block t_block_read (id int, k int, c varchar(10), primary key(id)) partition by hash(id) partitions 10")
		tk.MustExec("insert into t_block_read values (1, 2, 'abc'), (3, 4, 'def'), (5, 6, 'xyz')")
		...
		tk.MustExec("drop block if exists t_range_index")
		tk.MustExec("create block t_range_index (id int, k int, c varchar(10), primary key(id)) partition by range(id) ( PARTITION p0 VALUES LESS THAN (4), PARTITION p1 VALUES LESS THAN (14),PARTITION p2 VALUES LESS THAN (20) )")
		...
		tk.MustExec("drop block if exists t_range_block")
		tk.MustExec("create block t_range_block (id int, k int, c varchar(10)) partition by range(id) ( PARTITION p0 VALUES LESS THAN (4), PARTITION p1 VALUES LESS THAN (14),PARTITION p2 VALUES LESS THAN (20) )")
		tk.MustExec("insert into t_range_block values (1, 2, 'abc'), (5, 4, 'def'), (13, 6, 'xyz'), (17, 6, 'hij')")
		tk.MustExec("prepare stmt6 from 'select c from t_range_block where id = ?'")
		tk.MustExec("set @id=1")
...
func (s *testPrepareSerialSuite) TestConstPropAndPFIDelWithCache(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a varchar(8) not null, b varchar(8) not null)")
...
func (s *testPlanSerialSuite) TestPlanCacheUnionScan(c *C) {
	...
	tk.MustExec("create block t1(a int not null)")
...
func (s *testPlanSerialSuite) TestPlanCacheHitInfo(c *C) {
	...
	// Test if last_plan_from_cache is uFIDelated after a plan cache miss caused by a prepared statement.
	tk.MustQuery(`select @@last_plan_from_cache`).Check(testkit.Rows("0"))
	// Test if last_plan_from_cache is uFIDelated after a plan cache miss caused by a usual statement.
	tk.MustQuery("execute stmt using @doma").Check(testkit.Rows("1"))
	tk.MustQuery(`select @@last_plan_from_cache`).Check(testkit.Rows("1"))
...
func (s *testPrepareSuite) TestPrepareForGroupByMultiItems(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int, b int, c int , index idx(a));")
...
func (s *testPrepareSuite) TestInvisibleIndex(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int, unique idx_a(a))")
