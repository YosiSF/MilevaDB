...
func (s *testExpressionRewriterSuite) TestDefaultFunction(c *C) {
	...
	c.Assert(err.Error(), Equals, "[block:1364]Field 'a0' doesn't have a default value")

	tk.MustExec("create block t2(a varchar(10), b varchar(10))")
	tk.MustExec("insert into t2 values ('1', '1')")
	err = tk.ExecToErr("select default(a) from t1, t2")
	c.Assert(err.Error(), Equals, "[expression:1052]DeferredCauset 'a' in field list is ambiguous")
	...
	tk.MustExec(`create block t4(a int default 1, b varchar(5))`)
	tk.MustExec(`insert into t4 values (0, 'B'), (1, 'B'), (2, 'B')`)
	tk.MustExec(`create block t5(d int default 0, e varchar(5))`)
	...
	tk.MustExec("alter block t1 modify a varchar(10) default 'DEF'")
	tk.MustQuery("execute stmt").Check(testkit.Rows("DEF"))

	tk.MustExec("uFIDelate t1 set c = c + default(c)")
	tk.MustQuery("select c from t1").Check(testkit.Rows("11"))

	tk.MustExec("create block t6(a int default -1, b int)")
	tk.MustExec(`insert into t6 values (0, 0), (1, 1), (2, 2)`)
	tk.MustExec("create block t7(a int default 1, b int)")
	...
	tk.MustExec("create block t8(a int default 1, b int default -1)")
