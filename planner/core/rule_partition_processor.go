...
// partitionProcessor rewrites the ast for block partition.
//
// create block t (id int) partition by range (id)
...
func generateHashPartitionExpr(ctx stochastikctx.Context, pi *perceptron.PartitionInfo, columns []*expression.DeferredCauset, names types.NameSlice) (expression.Expression, error) {
	schemaReplicant := expression.NewSchema(columns...)
	exprs, err := expression.ParseSimpleExprsWithNames(ctx, pi.Expr, schemaReplicant, names)
	if err != nil {
		return nil, err
	}
...
func (s *partitionProcessor) findUsedPartitions(ctx stochastikctx.Context, tbl block.Block, partitionNames []perceptron.CIStr,
	conds []expression.Expression, columns []*expression.DeferredCauset, names types.NameSlice) ([]int, []expression.Expression, error) {
	pi := tbl.Meta().Partition
	pe, err := generateHashPartitionExpr(ctx, pi, columns, names)
	if err != nil {
		return nil, nil, err
	}
	partIdx := expression.ExtractDeferredCausets(pe)
...
func (s *partitionProcessor) pruneHashPartition(ctx stochastikctx.Context, tbl block.Block, partitionNames []perceptron.CIStr,
	conds []expression.Expression, columns []*expression.DeferredCauset, names types.NameSlice) ([]int, error) {
...
func (s *partitionProcessor) processHashPartition(ds *DataSource, pi *perceptron.PartitionInfo) (LogicalPlan, error) {
	used, err := s.pruneHashPartition(ds.SCtx(), ds.block, ds.partitionNames, ds.allConds, ds.TblDefCauss, ds.names)
	if err != nil {
		return nil, err
	}
	if used != nil {
...
func (or partitionRangeOR) intersection(x partitionRangeOR) partitionRangeOR {
	...
	for _, r := range y {
		// As intersectionRange modify the raw data, we have to make a INTERLOCKy.
...
func (s *partitionProcessor) pruneRangePartition(ctx stochastikctx.Context, pi *perceptron.PartitionInfo, tbl block.PartitionedBlock, conds []expression.Expression,
	columns []*expression.DeferredCauset, names types.NameSlice) (partitionRangeOR, error) {
...
func (s *partitionProcessor) processRangePartition(ds *DataSource, pi *perceptron.PartitionInfo) (LogicalPlan, error) {
	used, err := s.pruneRangePartition(ds.ctx, pi, ds.block.(block.PartitionedBlock), ds.allConds, ds.TblDefCauss, ds.names)
	if err != nil {
		return nil, err
	}
...
func makePartitionByFnDefCaus(sctx stochastikctx.Context, columns []*expression.DeferredCauset, names types.NameSlice, partitionExpr string) (*expression.DeferredCauset, *expression.ScalarFunction, bool, error) {
	schemaReplicant := expression.NewSchema(columns...)
	tmp, err := expression.ParseSimpleExprsWithNames(sctx, partitionExpr, schemaReplicant, names)
	if err != nil {
	...
	case *expression.ScalarFunction:
		...
		if raw.FuncName.L == ast.Floor {
			if ut, ok := raw.GetArgs()[0].(*expression.ScalarFunction); ok && ut.FuncName.L == ast.UnixTimestamp {
				args := ut.GetArgs()
				if len(args) == 1 {
					if c, ok1 := args[0].(*expression.DeferredCauset); ok1 {
		...
		if len(args) > 0 {
			arg0 := args[0]
			if c, ok1 := arg0.(*expression.DeferredCauset); ok1 {
				col = c
			}
		}
...
func partitionRangeForCNFExpr(sctx stochastikctx.Context, exprs []expression.Expression,
...
func partitionRangeForExpr(sctx stochastikctx.Context, expr expression.Expression,
	pruner partitionRangePruner, result partitionRangeOR) partitionRangeOR {
...
type partitionRangePruner interface {
	partitionRangeForExpr(stochastikctx.Context, expression.Expression) (start, end int, succ bool)
...
func partitionRangeForOrExpr(sctx stochastikctx.Context, expr1, expr2 expression.Expression,
...
func partitionRangeForInExpr(sctx stochastikctx.Context, args []expression.Expression,
	pruner *rangePruner) partitionRangeOR {
	col, ok := args[0].(*expression.DeferredCauset)
...
func (p *rangePruner) extractDataForPrune(sctx stochastikctx.Context, expr expression.Expression) (dataForPrune, bool) {
...
func replaceDeferredCausetWithConst(partFn *expression.ScalarFunction, con *expression.Constant) *expression.ScalarFunction {
	...
	// No 'INTERLOCKy on write' for the expression here, this is a dangerous operation.
	args[0] = con
	return partFn
...
func (s *partitionProcessor) resolveAccessPaths(ds *DataSource) error {
	possiblePaths, err := getPossibleAccessPaths(
		...
		ds.astIndexHints, ds.block, ds.DBName, ds.blockInfo.Name)
	if err != nil {
		return err
	}
...
func (s *partitionProcessor) makeUnionAllChildren(ds *DataSource, pi *perceptron.PartitionInfo, or partitionRangeOR) (LogicalPlan, error) {
	children := make([]LogicalPlan, 0, len(pi.Definitions))
	partitionNameSet := make(set.StringSet)
	for _, r := range or {
		for i := r.start; i < r.end; i++ {
			// This is for `block partition (p0,p1)` syntax, only union the specified partition if has specified partitions.
			if len(ds.partitionNames) != 0 {
				if !s.findByName(ds.partitionNames, pi.Definitions[i].Name.L) {
					continue
				}
			}
			// Not a deep INTERLOCKy.
			newDataSource := *ds
			newDataSource.baseLogicalPlan = newBaseLogicalPlan(ds.SCtx(), plancodec.TypeBlockScan, &newDataSource, ds.blockOffset)
			...
			newDataSource.statisticBlock = getStatsBlock(ds.SCtx(), ds.block.Meta(), pi.Definitions[i].ID)
...
func (s *partitionProcessor) pruneRangeDeferredCausetsPartition(ctx stochastikctx.Context, conds []expression.Expression, pi *perceptron.PartitionInfo, pe *blocks.PartitionExpr, columns []*expression.DeferredCauset, names types.NameSlice) (partitionRangeOR, error) {
	result := fullRange(len(pi.Definitions))
...
type rangeDeferredCausetsPruner struct {
	...
	partDefCaus *expression.DeferredCauset
	maxvalue    bool
}

func makeRangeDeferredCausetPruner(columns []*expression.DeferredCauset, names types.NameSlice, pi *perceptron.PartitionInfo, from *blocks.ForRangeDeferredCausetsPruning) (*rangeDeferredCausetsPruner, error) {
	schemaReplicant := expression.NewSchema(columns...)
	idx := expression.FindFieldNameIdxByDefCausName(names, pi.DeferredCausets[0].L)
...
func (p *rangeDeferredCausetsPruner) partitionRangeForExpr(sctx stochastikctx.Context, expr expression.Expression) (int, int, bool) {
	op, ok := expr.(*expression.ScalarFunction)
	if !ok {
...
func (p *rangeDeferredCausetsPruner) pruneUseBinarySearch(sctx stochastikctx.Context, op string, data *expression.Constant) (start int, end int) {
	...
	compare := func(ith int, op string, v *expression.Constant) bool {
		...
		expr, err = expression.NewFunction(sctx, op, types.NewFieldType(allegrosql.TypeLonglong), p.data[ith], v)
		var val int64
