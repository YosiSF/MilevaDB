...
type Plan interface {
	...
	// replaceExprDeferredCausets replace all the column reference in the plan's expression node.
	replaceExprDeferredCausets(replace map[string]*expression.DeferredCauset)
...
func enforceProperty(p *property.PhysicalProperty, tsk task, ctx stochastikctx.Context) task {
	...
	tsk = finishINTERLOCKTask(ctx, tsk)
...
func optimizeByShuffle(pp PhysicalPlan, tsk task, ctx stochastikctx.Context) task {
	if tsk.plan() == nil {
		return tsk
	}
...
func optimizeByShuffle4Window(pp *PhysicalWindow, ctx stochastikctx.Context) *PhysicalShuffle {
	...
	partitionBy := make([]*expression.DeferredCauset, 0, len(pp.PartitionBy))
...
type LogicalPlan interface {
	...
	PruneDeferredCausets([]*expression.DeferredCauset) error
	...
	recursiveDeriveStats(colGroups [][]*expression.DeferredCauset) (*property.StatsInfo, error)

	// DeriveStats derives statistic info for current plan node given child stats.
	// We need selfSchema, childSchema here because it makes this method can be used in
	// cascades planner, where LogicalPlan might not record its children or schemaReplicant.
	DeriveStats(childStats []*property.StatsInfo, selfSchema *expression.Schema, childSchema []*expression.Schema, colGroups [][]*expression.DeferredCauset) (*property.StatsInfo, error)
	...
	ExtractDefCausGroups(colGroups [][]*expression.DeferredCauset) [][]*expression.DeferredCauset

	// PreparePossibleProperties is only used for join and aggregation. Like group by a,b,c, all permutation of (a,b,c) is
	// valid, but the ordered indices in leaf plan is limited. So we can get all possible order properties by a pre-walking.
	PreparePossibleProperties(schemaReplicant *expression.Schema, childrenProperties ...[][]*expression.DeferredCauset) [][]*expression.DeferredCauset
	...
	// ExtractCorrelatedDefCauss extracts correlated columns inside the LogicalPlan.
	ExtractCorrelatedDefCauss() []*expression.CorrelatedDeferredCauset
...
type PhysicalPlan interface {
	...
	ToPB(ctx stochastikctx.Context, storeType ekv.StoreType) (*fidelpb.Executor, error)
	...
	// ExtractCorrelatedDefCauss extracts correlated columns inside the PhysicalPlan.
	ExtractCorrelatedDefCauss() []*expression.CorrelatedDeferredCauset
...
func (p *basePhysicalPlan) ExtractCorrelatedDefCauss() []*expression.CorrelatedDeferredCauset {
	return nil
}
...
// HasMaxOneRow returns if the LogicalPlan will output at most one event.
...
func newBaseLogicalPlan(ctx stochastikctx.Context, tp string, self LogicalPlan, offset int) baseLogicalPlan {
...
func newBasePhysicalPlan(ctx stochastikctx.Context, tp string, self PhysicalPlan, offset int) basePhysicalPlan {
...
func (p *baseLogicalPlan) ExtractCorrelatedDefCauss() []*expression.CorrelatedDeferredCauset {
	return nil
}

// PruneDeferredCausets implements LogicalPlan interface.
func (p *baseLogicalPlan) PruneDeferredCausets(parentUsedDefCauss []*expression.DeferredCauset) error {
	if len(p.children) == 0 {
		return nil
	}
...
func (p *basePlan) replaceExprDeferredCausets(replace map[string]*expression.DeferredCauset) {
}
...
func (p *basePlan) SCtx() stochastikctx.Context {
	return p.ctx
}
