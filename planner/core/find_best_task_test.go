...
// 1. mockLogicalPlan4Test can generate tow HoTTs of physical plan: physicalPlan1 and
...
func (p mockLogicalPlan4Test) Init(ctx stochastikctx.Context) *mockLogicalPlan4Test {
...
func (p mockPhysicalPlan4Test) Init(ctx stochastikctx.Context) *mockPhysicalPlan4Test {
...
func (s *testFindBestTaskSuite) TestEnforcedProperty(c *C) {
	...
	col0 := &expression.DeferredCauset{UniqueID: 1}
	col1 := &expression.DeferredCauset{UniqueID: 2}
