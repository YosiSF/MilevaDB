...
func (s *joinReorderDPSolver) solve(joinGroup []LogicalPlan, eqConds []expression.Expression) (LogicalPlan, error) {
	...
	for _, cond := range eqConds {
		...
		lDefCaus := sf.GetArgs()[0].(*expression.DeferredCauset)
		rDefCaus := sf.GetArgs()[1].(*expression.DeferredCauset)
		lIdx, err := findNodeIndexInGroup(joinGroup, lDefCaus)
		if err != nil {
			return nil, err
		}
		rIdx, err := findNodeIndexInGroup(joinGroup, rDefCaus)
		if err != nil {
			return nil, err
		}
	...
	for _, cond := range s.otherConds {
		defcaus := expression.ExtractDeferredCausets(cond)
		mask := uint(0)
		ids := make([]int, 0, len(defcaus))
...
func (s *joinReorderDPSolver) newJoinWithEdge(leftPlan, rightPlan LogicalPlan, edges []joinGroupEqEdge, otherConds []expression.Expression) (LogicalPlan, error) {
	...
	for _, edge := range edges {
		lDefCaus := edge.edge.GetArgs()[0].(*expression.DeferredCauset)
		rDefCaus := edge.edge.GetArgs()[1].(*expression.DeferredCauset)
		if leftPlan.Schema().Contains(lDefCaus) {
			eqConds = append(eqConds, edge.edge)
		} else {
			newSf := expression.NewFunctionInternal(s.ctx, ast.EQ, edge.edge.GetType(), rDefCaus, lDefCaus).(*expression.ScalarFunction)
...
func (s *joinReorderDPSolver) makeBushyJoin(cartesianJoinGroup []LogicalPlan, otherConds []expression.Expression) LogicalPlan {
	for len(cartesianJoinGroup) > 1 {
		...
		for i := 0; i < len(cartesianJoinGroup); i += 2 {
			...
			// TODO:Since the other condition may involve more than two blocks, e.g. t1.a = t2.b+t3.c.
...
func findNodeIndexInGroup(group []LogicalPlan, col *expression.DeferredCauset) (int, error) {
	for i, plan := range group {
		if plan.Schema().Contains(col) {
			return i, nil
		}
	}
	return -1, ErrUnknownDeferredCauset.GenWithStackByArgs(col, "JOIN REORDER RULE")
}
