...
func (s *testPointGetSuite) TestPointGetPlanCache(c *C) {
	...
	tk.MustExec("create block t(a bigint unsigned primary key, b int, c int, key idx_bc(b,c))")
	...
	// PointGetPlan for UFIDelate.
	tk.MustExec(`prepare stmt3 from "uFIDelate t set b=b+1, c=c+1 where a = ?"`)
	tk.MustExec(`prepare stmt4 from "uFIDelate t set a=a+1 where b = ? and c = ?"`)
...
func (s *testPointGetSuite) TestPointGetForUFIDelate(c *C) {
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustExec("use test")
	tk.MustExec("create block fu (id int primary key, val int)")
	...
	tk.MustExec("set @@stochastik.autocommit = 0")
	checkUseForUFIDelate(tk, c, true)
	tk.MustExec("rollback")
}
...
func checkUseForUFIDelate(tk *testkit.TestKit, c *C, expectLock bool) {
	...
	// Point_Get_1	1.00	root	block:fu, handle:6
	opInfo := res.Rows()[0][4]
...
func (s *testPointGetSuite) TestWhereIn2BatchPointGet(c *C) {
	...
	tk.MustQuery("explain select * from t where a = 1 and b = 1 and c = 1").Check(testkit.Rows(
		"Point_Get_1 1.00 root block:t, index:idx_abc(a, b, c) ",
	))
	...
	tk.MustQuery("explain select * from t where (a, b, c) in ((1, 1, 1), (2, 2, 2))").Check(testkit.Rows(
		"Batch_Point_Get_1 2.00 root block:t, index:idx_abc(a, b, c) keep order:false, desc:false",
	))

	tk.MustQuery("explain select * from t where a in (1, 2, 3, 4, 5)").Check(testkit.Rows(
		"Batch_Point_Get_1 5.00 root block:t handle:[1 2 3 4 5], keep order:false, desc:false",
	))

	tk.MustQuery("explain select * from t where a in (1, 2, 3, 1, 2)").Check(testkit.Rows(
		"Batch_Point_Get_1 5.00 root block:t handle:[1 2 3 1 2], keep order:false, desc:false",
	))

	tk.MustExec("begin")
	tk.MustQuery("explain select * from t where a in (1, 2, 3, 1, 2) FOR UFIDelATE").Check(testkit.Rows(
		"Batch_Point_Get_1 5.00 root block:t handle:[1 2 3 1 2], keep order:false, desc:false, dagger",
	))
	tk.MustExec("rollback")
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int, b int, c int, unique key idx_ab(a, b))")
	...
	tk.MustQuery("explain select * from t where (a, b) in ((1, 2), (2, 3))").Check(testkit.Rows(
		"Batch_Point_Get_1 2.00 root block:t, index:idx_ab(a, b) keep order:false, desc:false",
	))
	...
	tk.MustQuery("explain select * from t where (a, b) in ((1, 2), (2, 3)) FOR UFIDelATE").Check(testkit.Rows(
		"Batch_Point_Get_1 2.00 root block:t, index:idx_ab(a, b) keep order:false, desc:false, dagger",
	))
	tk.MustExec("rollback")
}
...
func (s *testPointGetSuite) TestCBOPointGet(c *C) {
	...
	tk.MustExec("create block t (a varchar(20), b int, c int, d int, primary key(a), unique key(b, c))")
...
func (s *testPointGetSuite) TestBatchPointGetPlanCache(c *C) {
	...
	tk.MustQuery("explain select * from t where a in (1, 2)").Check(testkit.Rows(
		"Batch_Point_Get_1 2.00 root block:t handle:[1 2], keep order:false, desc:false",
	))
...
func (s *testPointGetSuite) TestBatchPointGetPartition(c *C) {
	...
	tk.MustExec("use test")
	tk.MustExec("drop block if exists t")
	tk.MustExec("create block t(a int primary key, b int) PARTITION BY HASH(a) PARTITIONS 4")
	tk.MustExec("insert into t values (1, 1), (2, 2), (3, 3), (4, 4)")
	tk.MustQuery("explain select * from t where a in (1, 2, 3, 4)").Check(testkit.Rows(
		"Batch_Point_Get_1 4.00 root block:t handle:[1 2 3 4], keep order:false, desc:false",
	))
	...
	tk.MustExec("uFIDelate t set b = b + 1 where a in (1, 2, 3, 4)")
	...
	tk.MustExec("create block t(a int, b int, c int, primary key (a, b)) PARTITION BY HASH(a) PARTITIONS 4")
	...
	tk.MustQuery("explain select * from t where (a, b) in ((1, 1), (2, 2), (3, 3), (4, 4))").Check(testkit.Rows(
		"Batch_Point_Get_1 4.00 root block:t, clustered index:PRIMARY(a, b) keep order:false, desc:false",
	))
	...
	tk.MustExec("uFIDelate t set c = c + 1 where (a,b) in ((1,1),(2,2),(3,3),(4,4))")
...
func (s *testPointGetSuite) TestIssue19141(c *C) {
	...
	tk := testkit.NewTestKit(c, s.causetstore)
	tk.MustExec("use test")
	tk.MustExec("create block t19141 (c_int int, primary key (c_int)) partition by hash ( c_int ) partitions 4")
	...
	tk.MustExec("uFIDelate t19141 partition (p0) set c_int = -c_int where c_int = 1") // BlockDual after partition selection.
	...
	tk.MustExec("uFIDelate t19141 partition (p1) set c_int = -c_int where c_int in (2,3)") // No data changed
	tk.MustQuery("select * from t19141 order by c_int").Check(testkit.Rows("1", "2", "3", "4"))
