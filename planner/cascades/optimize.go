...
func (opt *Optimizer) GetImplementationMemrules(node plannercore.LogicalPlan) []ImplementationMemrule {
...
func (opt *Optimizer) FindBestPlan(sctx stochastikctx.Context, logical plannercore.LogicalPlan) (p plannercore.PhysicalPlan, cost float64, err error) {
...
func (opt *Optimizer) onPhasePreprocessing(sctx stochastikctx.Context, plan plannercore.LogicalPlan) (plannercore.LogicalPlan, error) {
...
func (opt *Optimizer) onPhaseImplementation(sctx stochastikctx.Context, g *memo.Group) (plannercore.PhysicalPlan, float64, error) {
	prop := &property.PhysicalProperty{
		ExpectedCnt: math.MaxFloat64,
	}
	preparePossibleProperties(g, make(map[*memo.Group][][]*expression.DeferredCauset))
...
// and LogicalJoin.
func preparePossibleProperties(g *memo.Group, propertyMap map[*memo.Group][][]*expression.DeferredCauset) [][]*expression.DeferredCauset {
	if prop, ok := propertyMap[g]; ok {
		return prop
	}
	groupPropertyMap := make(map[string][]*expression.DeferredCauset)
	for elem := g.Equivalents.Front(); elem != nil; elem = elem.Next() {
		expr := elem.Value.(*memo.GroupExpr)
		childrenProperties := make([][][]*expression.DeferredCauset, len(expr.Children))
	...
	resultProps := make([][]*expression.DeferredCauset, 0, len(groupPropertyMap))
