...
// TransformationMemruleBatch is a batch of transformation rules.
type TransformationMemruleBatch map[memo.Operand][]Transformation

// DefaultMemruleBatches contain all the transformation rules.
// Each batch will be applied to the memo independently.
var DefaultMemruleBatches = []TransformationMemruleBatch{
	MilevaDBLayerOptimizationBatch,
	EinsteinDBLayerOptimizationBatch,
	PostTransformationBatch,
}

// MilevaDBLayerOptimizationBatch does the optimization in the MilevaDB layer.
var MilevaDBLayerOptimizationBatch = TransformationMemruleBatch{
...
// EinsteinDBLayerOptimizationBatch does the optimization related to EinsteinDB layer.
// For example, rules about pushing down Operators like Selection, Limit,
// Aggregation into EinsteinDB layer should be inside this batch.
var EinsteinDBLayerOptimizationBatch = TransformationMemruleBatch{
...
// the constraints of the execution engine of MilevaDB.
// For example, TopN/Sort only support `order by` defCausumns in MilevaDB layer,
// as for scalar functions, we need to inject a Projection for them
// below the TopN/Sort.
var PostTransformationBatch = TransformationMemruleBatch{
	memo.OperandProjection: {
		NewMemruleEliminateProjection(),
		NewMemruleMergeAdjacentProjection(),
	},
...
type baseMemrule struct {
	pattern *memo.Pattern
}

// Match implements Transformation Interface.
func (r *baseMemrule) Match(expr *memo.ExprIter) bool {
	return true
}

// GetPattern implements Transformation Interface.
func (r *baseMemrule) GetPattern() *memo.Pattern {
	return r.pattern
}

// PushSelDownBlockScan pushes the selection down to BlockScan.
type PushSelDownBlockScan struct {
	baseMemrule
}

// NewMemrulePushSelDownBlockScan creates a new Transformation PushSelDownBlockScan.
// The pattern of this rule is: `Selection -> BlockScan`
func NewMemrulePushSelDownBlockScan() Transformation {
	rule := &PushSelDownBlockScan{}
	ts := memo.NewPattern(memo.OperandBlockScan, memo.EngineEinsteinDBOrTiFlash)
	p := memo.BuildPattern(memo.OperandSelection, memo.EngineEinsteinDBOrTiFlash, ts)
	rule.pattern = p
	return rule
}
...
func (r *PushSelDownBlockScan) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*plannercore.LogicalSelection)
	ts := old.Children[0].GetExpr().ExprNode.(*plannercore.LogicalBlockScan)
	if ts.HandleDefCauss == nil {
		return nil, false, false, nil
	}
	accesses, remained := ranger.DetachCondsForDeferredCauset(ts.SCtx(), sel.Conditions, ts.HandleDefCauss.GetDefCaus(0))
	if accesses == nil {
		return nil, false, false, nil
	}
	newTblScan := plannercore.LogicalBlockScan{
		Source:         ts.Source,
	...
	schemaReplicant := old.GetExpr().Group.Prop.Schema
	tblScanGroup := memo.NewGroupWithSchema(tblScanExpr, schemaReplicant)
...
// NewMemrulePushSelDownIndexScan creates a new Transformation PushSelDownIndexScan.
// The pattern of this rule is `Selection -> IndexScan`.
func NewMemrulePushSelDownIndexScan() Transformation {
	rule := &PushSelDownIndexScan{}
	rule.pattern = memo.BuildPattern(
		memo.OperandSelection,
		memo.EngineEinsteinDBOnly,
		memo.NewPattern(memo.OperandIndexScan, memo.EngineEinsteinDBOnly),
	)
	return rule
}
...
func (r *PushSelDownIndexScan) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	if is.AccessConds != nil {
		...
		INTERLOCKy(conditions, sel.Conditions)
		INTERLOCKy(conditions[len(sel.Conditions):], is.AccessConds)
	}
	res, err := ranger.DetachCondAndBuildRangeForIndex(is.SCtx(), conditions, is.IdxDefCauss, is.IdxDefCausLens)
	if err != nil {
	...
	newIs := plannercore.LogicalIndexScan{
		...
		Ranges:             res.Ranges,
		Index:              is.Index,
		DeferredCausets:    is.DeferredCausets,
		FullIdxDefCauss:    is.FullIdxDefCauss,
		FullIdxDefCausLens: is.FullIdxDefCausLens,
		IdxDefCauss:        is.IdxDefCauss,
		IdxDefCausLens:     is.IdxDefCausLens,
...
// PushSelDownEinsteinDBSingleGather pushes the selection down to child of EinsteinDBSingleGather.
type PushSelDownEinsteinDBSingleGather struct {
	baseMemrule
}

// NewMemrulePushSelDownEinsteinDBSingleGather creates a new Transformation PushSelDownEinsteinDBSingleGather.
// The pattern of this rule is `Selection -> EinsteinDBSingleGather -> Any`.
func NewMemrulePushSelDownEinsteinDBSingleGather() Transformation {
	any := memo.NewPattern(memo.OperandAny, memo.EngineEinsteinDBOrTiFlash)
	tg := memo.BuildPattern(memo.OperandEinsteinDBSingleGather, memo.EngineMilevaDBOnly, any)
	p := memo.BuildPattern(memo.OperandSelection, memo.EngineMilevaDBOnly, tg)

	rule := &PushSelDownEinsteinDBSingleGather{}
	rule.pattern = p
	return rule
}
...
func (r *PushSelDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	sel := old.GetExpr().ExprNode.(*plannercore.LogicalSelection)
	sg := old.Children[0].GetExpr().ExprNode.(*plannercore.EinsteinDBSingleGather)
	...
	// The field content of EinsteinDBSingleGather would not be modified currently, so we
	// just reference the same tg instead of making a INTERLOCKy of it.
	//
	// TODO: if we save pushed filters later in EinsteinDBSingleGather, in order to do partition
	//       pruning or skyline pruning, we need to make a INTERLOCKy of the EinsteinDBSingleGather here.
...
// EnumeratePaths converts DataSource to block scan and index scans.
type EnumeratePaths struct {
	baseMemrule
}

// NewMemruleEnumeratePaths creates a new Transformation EnumeratePaths.
// The pattern of this rule is: `DataSource`.
func NewMemruleEnumeratePaths() Transformation {
	rule := &EnumeratePaths{}
	rule.pattern = memo.NewPattern(memo.OperandDataSource, memo.EngineMilevaDBOnly)
	return rule
}
...
func (r *EnumeratePaths) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	for _, gather := range gathers {
		...
		expr.Children[0].SetEngineType(memo.EngineEinsteinDB)
...
// and pushed the partial Aggregation down to the child of EinsteinDBSingleGather.
type PushAggDownGather struct {
	baseMemrule
}

// NewMemrulePushAggDownGather creates a new Transformation PushAggDownGather.
// The pattern of this rule is: `Aggregation -> EinsteinDBSingleGather`.
func NewMemrulePushAggDownGather() Transformation {
	rule := &PushAggDownGather{}
	rule.pattern = memo.BuildPattern(
		memo.OperanPosetDaggregation,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandEinsteinDBSingleGather, memo.EngineMilevaDBOnly),
	)
	return rule
}
...
func (r *PushAggDownGather) Match(expr *memo.ExprIter) bool {
	if expr.GetExpr().HasAppliedMemrule(r) {
		return false
	}
	...
	if childEngine != memo.EngineEinsteinDB {
		// TODO: Remove this check when we have implemented TiFlashAggregation.
		return false
	}
	return plannercore.CheckAggCanPushINTERLOCK(agg.SCtx(), agg.AggFuncs, agg.GroupByItems, ekv.EinsteinDB)
}
...
func (r *PushAggDownGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	gather := old.Children[0].GetExpr().ExprNode.(*plannercore.EinsteinDBSingleGather)
	...
	INTERLOCKy(gbyItems, agg.GroupByItems)
	...
	finalAggExpr.AddAppliedMemrule(r)
...
// NewMemrulePushSelDownSort creates a new Transformation PushSelDownSort.
// The pattern of this rule is: `Selection -> Sort`.
func NewMemrulePushSelDownSort() Transformation {
	rule := &PushSelDownSort{}
	rule.pattern = memo.BuildPattern(
		memo.OperandSelection,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandSort, memo.EngineMilevaDBOnly),
	)
	return rule
}
...
// NewMemrulePushSelDownProjection creates a new Transformation PushSelDownProjection.
// The pattern of this rule is: `Selection -> Projection`.
func NewMemrulePushSelDownProjection() Transformation {
	rule := &PushSelDownProjection{}
	rule.pattern = memo.BuildPattern(
		memo.OperandSelection,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandProjection, memo.EngineMilevaDBOnly),
	)
	return rule
}
...
func (r *PushSelDownProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	for _, cond := range sel.Conditions {
		if !expression.HasGetSetVarFunc(cond) {
			canBePushed = append(canBePushed, expression.DeferredCausetSubstitute(cond, projSchema, proj.Exprs))
		} else {
...
// NewMemrulePushSelDownAggregation creates a new Transformation PushSelDownAggregation.
// The pattern of this rule is `Selection -> Aggregation`.
func NewMemrulePushSelDownAggregation() Transformation {
	rule := &PushSelDownAggregation{}
	rule.pattern = memo.BuildPattern(
		memo.OperandSelection,
		memo.EngineAll,
		memo.NewPattern(memo.OperanPosetDaggregation, memo.EngineAll),
	)
	return rule
}
...
func (r *PushSelDownAggregation) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	for _, cond := range sel.Conditions {
		...
		case *expression.ScalarFunction:
			...
			canPush := true
			for _, defCaus := range extractedDefCauss {
				if !groupByDeferredCausets.Contains(defCaus) {
					canPush = false
					break
				}
			}
...
// NewMemrulePushSelDownWindow creates a new Transformation PushSelDownWindow.
// The pattern of this rule is `Selection -> Window`.
func NewMemrulePushSelDownWindow() Transformation {
...
func (r *PushSelDownWindow) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// get partition DeferredCausets' Schema
	partitionDefCaussSchema := expression.NewSchema(window.GetPartitionByDefCauss()...)

	for _, cond := range sel.Conditions {
		if expression.ExprFromSchema(cond, partitionDefCaussSchema) {
...
// NewMemruleTransformLimitToTopN creates a new Transformation TransformLimitToTopN.
// The pattern of this rule is `Limit -> Sort`.
func NewMemruleTransformLimitToTopN() Transformation {
...
// NewMemrulePushLimitDownProjection creates a new Transformation.
// The pattern of this rule is `Limit->Projection->X` to `Projection->Limit->X`.
func NewMemrulePushLimitDownProjection() Transformation {
...
// NewMemrulePushLimitDownUnionAll creates a new Transformation PushLimitDownUnionAll.
// The pattern of this rule is `Limit->UnionAll->X`.
func NewMemrulePushLimitDownUnionAll() Transformation {
	rule := &PushLimitDownUnionAll{}
	rule.pattern = memo.BuildPattern(
		memo.OperandLimit,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandUnionAll, memo.EngineMilevaDBOnly),
	)
	return rule
}

// Match implements Transformation interface.
// Use appliedMemruleSet in GroupExpr to avoid re-apply rules.
func (r *PushLimitDownUnionAll) Match(expr *memo.ExprIter) bool {
	return !expr.GetExpr().HasAppliedMemrule(r)
}
...
func (r *PushLimitDownUnionAll) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	newLimitExpr.AddAppliedMemrule(r)
...
// NewMemrulePushSelDownJoin creates a new Transformation PushSelDownJoin.
// The pattern of this rule is `Selection -> Join`.
func NewMemrulePushSelDownJoin() Transformation {
	rule := &PushSelDownJoin{}
	rule.pattern = memo.BuildPattern(
		memo.OperandSelection,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandJoin, memo.EngineMilevaDBOnly),
	)
	return rule
}
...
func (r *PushSelDownJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	case plannercore.LeftOuterJoin, plannercore.LeftOuterSemiJoin, plannercore.AntiLeftOuterSemiJoin,
		...
		INTERLOCKy(remainCond, sel.Conditions)
		...
		// Return block dual when filter is constant false or null.
		dual := plannercore.Conds2BlockDual(join, remainCond)
		if dual != nil {
			return []*memo.GroupExpr{memo.NewGroupExpr(dual)}, false, true, nil
		}
		if join.JoinType == plannercore.RightOuterJoin {
	...
	// TODO: UFIDelate EqualConditions like what we have done in the method join.uFIDelateEQCond() before.
	...
	if len(remainCond) > 0 {
		...
		newSelExpr.AddAppliedMemrule(r)
...
// NewMemrulePushSelDownUnionAll creates a new Transformation PushSelDownUnionAll.
// The pattern of this rule is `Selection -> UnionAll`.
func NewMemrulePushSelDownUnionAll() Transformation {
...
// NewMemruleEliminateProjection creates a new Transformation EliminateProjection.
// The pattern of this rule is `Projection -> Any`.
func NewMemruleEliminateProjection() Transformation {
	rule := &EliminateProjection{}
	rule.pattern = memo.BuildPattern(
		memo.OperandProjection,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandAny, memo.EngineMilevaDBOnly),
	)
	return rule
}

// OnTransform implements Transformation interface.
// This rule tries to eliminate the projection whose output defCausumns are the same with its child.
func (r *EliminateProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	oldDefCauss := old.GetExpr().Group.Prop.Schema.DeferredCausets
	for i, defCaus := range child.Group.Prop.Schema.DeferredCausets {
		if !defCaus.Equal(nil, oldDefCauss[i]) {
			return nil, false, false, nil
		}
	}
	...
	for elem := child.Group.Equivalents.Front(); elem != nil; elem = elem.Next() {
		...
		INTERLOCKyChildExpr := memo.NewGroupExpr(childExpr.ExprNode)
		INTERLOCKyChildExpr.SetChildren(childExpr.Children...)
		finalGroupExprs = append(finalGroupExprs, INTERLOCKyChildExpr)
	}
...
// NewMemruleMergeAdjacentProjection creates a new Transformation MergeAdjacentProjection.
// The pattern of this rule is `Projection -> Projection`.
func NewMemruleMergeAdjacentProjection() Transformation {
...
func (r *MergeAdjacentProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	replace := make(map[string]*expression.DeferredCauset)
	for i, defCaus := range childGroup.Prop.Schema.DeferredCausets {
		if defCausOrigin, ok := child.Exprs[i].(*expression.DeferredCauset); ok {
			replace[string(defCaus.HashCode(nil))] = defCausOrigin
		}
	}
	...
	for i, expr := range proj.Exprs {
		...
		newProj.Exprs[i] = plannercore.ReplaceDeferredCausetOfExpr(newExpr, child, childGroup.Prop.Schema)
	}
...
// NewMemrulePushTopNDownOuterJoin creates a new Transformation PushTopNDownOuterJoin.
// The pattern of this rule is: `TopN -> Join`.
func NewMemrulePushTopNDownOuterJoin() Transformation {
...
func pushTopNDownOuterJoinToChild(topN *plannercore.LogicalTopN, outerGroup *memo.Group) *memo.Group {
	for _, by := range topN.ByItems {
		...
		for _, defCaus := range defcaus {
			if !outerGroup.Prop.Schema.Contains(defCaus) {
				return outerGroup
			}
		}
	}
...
func (r *PushTopNDownOuterJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	newTopNExpr.AddAppliedMemrule(r)
...
// NewMemrulePushTopNDownProjection creates a new Transformation PushTopNDownProjection.
// The pattern of this rule is `TopN->Projection->X` to `Projection->TopN->X`.
func NewMemrulePushTopNDownProjection() Transformation {
...
func (r *PushTopNDownProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	newTopN.ByItems = make([]*soliton.ByItems, 0, len(topN.ByItems))
	for _, by := range topN.ByItems {
		newTopN.ByItems = append(newTopN.ByItems, &soliton.ByItems{
			Expr: expression.DeferredCausetSubstitute(by.Expr, old.Children[0].Group.Prop.Schema, proj.Exprs),
			Desc: by.Desc,
		})
	}
...
// NewMemrulePushTopNDownUnionAll creates a new Transformation PushTopNDownUnionAll.
// The pattern of this rule is `TopN->UnionAll->X`.
func NewMemrulePushTopNDownUnionAll() Transformation {
...
// PushTopNDownEinsteinDBSingleGather pushes the top-n down to child of EinsteinDBSingleGather.
type PushTopNDownEinsteinDBSingleGather struct {
	baseMemrule
}

// NewMemrulePushTopNDownEinsteinDBSingleGather creates a new Transformation PushTopNDownEinsteinDBSingleGather.
// The pattern of this rule is `TopN -> EinsteinDBSingleGather`.
func NewMemrulePushTopNDownEinsteinDBSingleGather() Transformation {
	rule := &PushTopNDownEinsteinDBSingleGather{}
	rule.pattern = memo.BuildPattern(
		memo.OperandTopN,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandEinsteinDBSingleGather, memo.EngineMilevaDBOnly),
...
// Match implements Transformation interface.
// Use appliedMemruleSet in GroupExpr to avoid re-apply rules.
func (r *PushTopNDownEinsteinDBSingleGather) Match(expr *memo.ExprIter) bool {
	return !expr.GetExpr().HasAppliedMemrule(r)
}

// OnTransform implements Transformation interface.
// It transforms `TopN -> EinsteinDBSingleGather` to `TopN(Final) -> EinsteinDBSingleGather -> TopN(Partial)`.
func (r *PushTopNDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	topN := old.GetExpr().ExprNode.(*plannercore.LogicalTopN)
	...
	finalTopNExpr.AddAppliedMemrule(r)
...
// NewMemruleMergeAdjacentTopN creates a new Transformation MergeAdjacentTopN.
// The pattern of this rule is `TopN->TopN->X`.
func NewMemruleMergeAdjacentTopN() Transformation {
...
func (r *MergeAdjacentTopN) Match(expr *memo.ExprIter) bool {
	...
	// We can use this rule when the sort defCausumns of parent TopN is a prefix of child TopN.
...
func (r *MergeAdjacentTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	if child.Count <= topN.Offset {
		blockDual := plannercore.LogicalBlockDual{RowCount: 0}.Init(child.SCtx(), child.SelectBlockOffset())
		blockDual.SetSchema(old.GetExpr().Schema())
		blockDualExpr := memo.NewGroupExpr(blockDual)
		return []*memo.GroupExpr{blockDualExpr}, true, true, nil
	}
...
// NewMemruleMergeAggregationProjection creates a new Transformation MergeAggregationProjection.
// The pattern of this rule is: `Aggregation -> Projection`.
func NewMemruleMergeAggregationProjection() Transformation {
...
func (r *MergeAggregationProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	olPosetDagg := old.GetExpr().ExprNode.(*plannercore.LogicalAggregation)
	proj := old.Children[0].GetExpr().ExprNode.(*plannercore.LogicalProjection)
	projSchema := old.Children[0].GetExpr().Schema()

	groupByItems := make([]expression.Expression, len(olPosetDagg.GroupByItems))
	for i, item := range olPosetDagg.GroupByItems {
		groupByItems[i] = expression.DeferredCausetSubstitute(item, projSchema, proj.Exprs)
	}

	aggFuncs := make([]*aggregation.AggFuncDesc, len(olPosetDagg.AggFuncs))
	for i, aggFunc := range olPosetDagg.AggFuncs {
		aggFuncs[i] = aggFunc.Clone()
		newArgs := make([]expression.Expression, len(aggFunc.Args))
		for j, arg := range aggFunc.Args {
			newArgs[j] = expression.DeferredCausetSubstitute(arg, projSchema, proj.Exprs)
		}
	...
	newAgg := plannercore.LogicalAggregation{
		...
		AggFuncs:     aggFuncs,
	}.Init(olPosetDagg.SCtx(), olPosetDagg.SelectBlockOffset())
...
// NewMemruleEliminateSingleMaxMin creates a new Transformation EliminateSingleMaxMin.
// The pattern of this rule is `max/min->X`.
func NewMemruleEliminateSingleMaxMin() Transformation {
...
func (r *EliminateSingleMaxMin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// If there's no defCausumn in f.GetArgs()[0], we still need limit and read data from real block because the result should be NULL if the input is empty.
	if len(expression.ExtractDeferredCausets(f.Args[0])) > 0 {
		...
		byItems = append(byItems, &soliton.ByItems{
			Expr: f.Args[0],
			Desc: desc,
		})
	...
	newAggExpr.AddAppliedMemrule(r)
	return []*memo.GroupExpr{newAggExpr}, false, false, nil
}
...
// NewMemruleMergeAdjacentSelection creates a new Transformation MergeAdjacentSelection.
// The pattern of this rule is `Selection->Selection->X`.
func NewMemruleMergeAdjacentSelection() Transformation {
...
// NewMemruleMergeAdjacentLimit creates a new Transformation MergeAdjacentLimit.
// The pattern of this rule is `Limit->Limit->X`.
func NewMemruleMergeAdjacentLimit() Transformation {
...
// TransformLimitToBlockDual convert limit to BlockDual.
type TransformLimitToBlockDual struct {
	baseMemrule
}

// NewMemruleTransformLimitToBlockDual creates a new Transformation TransformLimitToBlockDual.
// The pattern of this rule is `Limit->X`.
func NewMemruleTransformLimitToBlockDual() Transformation {
	rule := &TransformLimitToBlockDual{}
	rule.pattern = memo.BuildPattern(
		memo.OperandLimit,
		memo.EngineAll,
	)
...
// Match implements Transformation interface.
func (r *TransformLimitToBlockDual) Match(expr *memo.ExprIter) bool {
	limit := expr.GetExpr().ExprNode.(*plannercore.LogicalLimit)
	return 0 == limit.Count
}

// OnTransform implements Transformation interface.
// This rule tries to convert limit to blockDual.
func (r *TransformLimitToBlockDual) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*plannercore.LogicalLimit)
	blockDual := plannercore.LogicalBlockDual{RowCount: 0}.Init(limit.SCtx(), limit.SelectBlockOffset())
	blockDual.SetSchema(old.GetExpr().Schema())
	blockDualExpr := memo.NewGroupExpr(blockDual)
	return []*memo.GroupExpr{blockDualExpr}, true, true, nil
}
...
// NewMemrulePushLimitDownOuterJoin creates a new Transformation PushLimitDownOuterJoin.
// The pattern of this rule is `Limit -> Join`.
func NewMemrulePushLimitDownOuterJoin() Transformation {
...
// PushLimitDownEinsteinDBSingleGather pushes the limit down to child of EinsteinDBSingleGather.
type PushLimitDownEinsteinDBSingleGather struct {
	baseMemrule
}

// NewMemrulePushLimitDownEinsteinDBSingleGather creates a new Transformation PushLimitDownEinsteinDBSingleGather.
// The pattern of this rule is `Limit -> EinsteinDBSingleGather`.
func NewMemrulePushLimitDownEinsteinDBSingleGather() Transformation {
	rule := &PushLimitDownEinsteinDBSingleGather{}
	rule.pattern = memo.BuildPattern(
		memo.OperandLimit,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandEinsteinDBSingleGather, memo.EngineMilevaDBOnly),
...
// Match implements Transformation interface.
// Use appliedMemruleSet in GroupExpr to avoid re-apply rules.
func (r *PushLimitDownEinsteinDBSingleGather) Match(expr *memo.ExprIter) bool {
	return !expr.GetExpr().HasAppliedMemrule(r)
}

// OnTransform implements Transformation interface.
// It transforms `Limit -> EinsteinDBSingleGather` to `Limit(Final) -> EinsteinDBSingleGather -> Limit(Partial)`.
func (r *PushLimitDownEinsteinDBSingleGather) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	limit := old.GetExpr().ExprNode.(*plannercore.LogicalLimit)
	...
	finalLimitExpr.AddAppliedMemrule(r)
...
func (*outerJoinEliminator) prepareForEliminateOuterJoin(joinExpr *memo.GroupExpr) (ok bool, innerChildIdx int, outerGroup *memo.Group, innerGroup *memo.Group, outerUniqueIDs set.Int64Set) {
	...
	for _, outerDefCaus := range outerGroup.Prop.Schema.DeferredCausets {
...
func (*outerJoinEliminator) isInnerJoinKeysContainUniqueKey(innerGroup *memo.Group, joinKeys *expression.Schema) (bool, error) {
	...
	for _, keyInfo := range innerGroup.Prop.Schema.Keys {
		...
		for _, defCaus := range keyInfo {
			if !joinKeys.Contains(defCaus) {
...
// NewMemruleEliminateOuterJoinBelowAggregation creates a new Transformation EliminateOuterJoinBelowAggregation.
// The pattern of this rule is `Aggregation->Join->X`.
func NewMemruleEliminateOuterJoinBelowAggregation() Transformation {
...
func (r *EliminateOuterJoinBelowAggregation) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// only when agg only use the defCausumns from outer block can eliminate outer join.
	if !plannercore.IsDefCaussAllFromOuterBlock(agg.GetUsedDefCauss(), outerUniqueIDs) {
		return nil, false, false, nil
	}
	// outer join elimination with duplicate agnostic aggregate functions.
	_, aggDefCauss := plannercore.GetDupAgnosticAggDefCauss(agg, nil)
	if len(aggDefCauss) > 0 {
...
// NewMemruleEliminateOuterJoinBelowProjection creates a new Transformation EliminateOuterJoinBelowProjection.
// The pattern of this rule is `Projection->Join->X`.
func NewMemruleEliminateOuterJoinBelowProjection() Transformation {
...
func (r *EliminateOuterJoinBelowProjection) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// only when proj only use the defCausumns from outer block can eliminate outer join.
	if !plannercore.IsDefCaussAllFromOuterBlock(proj.GetUsedDefCauss(), outerUniqueIDs) {
		return nil, false, false, nil
	}
...
// NewMemruleTransformAggregateCaseToSelection creates a new Transformation TransformAggregateCaseToSelection.
// The pattern of this rule is `Agg->X`.
func NewMemruleTransformAggregateCaseToSelection() Transformation {
...
func (r *TransformAggregateCaseToSelection) transform(agg *plannercore.LogicalAggregation) (ok bool, newConditions []expression.Expression, newAggFuncs []*aggregation.AggFuncDesc) {
	...
	if nullFlip || zeroFlip {
		outputIdx = 2
		newConditions = []expression.Expression{expression.NewFunctionInternal(ctx, ast.UnaryNot, types.NewFieldType(allegrosql.TypeTiny), conditionFromCase)}
	} else {
		outputIdx = 1
...
func (r *TransformAggregateCaseToSelection) isOnlyOneNotNull(ctx stochastikctx.Context, args []expression.Expression, argsNum int, outputIdx int) bool {
...
// NewMemruleTransformAggToProj creates a new Transformation TransformAggToProj.
// The pattern of this rule is `Agg`.
func NewMemruleTransformAggToProj() Transformation {
...
func (r *TransformAggToProj) Match(expr *memo.ExprIter) bool {
	...
	schemaByGroupby := expression.NewSchema(agg.GetGroupByDefCauss()...)
...
// NewMemruleInjectProjectionBelowTopN creates a new Transformation InjectProjectionBelowTopN.
...
// TopN to prune the extra DeferredCausets.
// The reason why we need this rule is that, TopNExecutor in MilevaDB does not support ScalarFunction
...
func NewMemruleInjectProjectionBelowTopN() Transformation {
...
func (r *InjectProjectionBelowTopN) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	for i := range oldTopNSchema.DeferredCausets {
		topProjExprs[i] = oldTopNSchema.DeferredCausets[i]
	}
	...
	bottomProjSchema := make([]*expression.DeferredCauset, 0, oldTopNSchema.Len()+len(topN.ByItems))
	for _, defCaus := range oldTopNSchema.DeferredCausets {
		bottomProjExprs = append(bottomProjExprs, defCaus)
		bottomProjSchema = append(bottomProjSchema, defCaus)
	}
	newByItems := make([]*soliton.ByItems, 0, len(topN.ByItems))
	for _, item := range topN.ByItems {
		itemExpr := item.Expr
		...
		newByItems = append(newByItems, &soliton.ByItems{Expr: newDefCaus, Desc: item.Desc})
	}
...
// NewMemruleInjectProjectionBelowAgg creates a new Transformation NewMemruleInjectProjectionBelowAgg.
// It will extract the ScalarFunctions of `AggFuncDesc` and `GroupByItems` into a Projection and injects it below Agg.
// The reason why we need this rule is that, AggExecutor in MilevaDB does not support ScalarFunction
...
func NewMemruleInjectProjectionBelowAgg() Transformation {
...
func (r *InjectProjectionBelowAgg) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	hasScalarFunc := false
	INTERLOCKyFuncs := make([]*aggregation.AggFuncDesc, 0, len(agg.AggFuncs))
	for _, aggFunc := range agg.AggFuncs {
		INTERLOCKyFunc := aggFunc.Clone()
		//WrapCastForAggArgs will modify AggFunc, so we should clone AggFunc.
		INTERLOCKyFunc.WrapCastForAggArgs(agg.SCtx())
		INTERLOCKyFuncs = append(INTERLOCKyFuncs, INTERLOCKyFunc)
		for _, arg := range INTERLOCKyFunc.Args {
	...
	projSchemaDefCauss := make([]*expression.DeferredCauset, 0, len(INTERLOCKyFuncs)+len(agg.GroupByItems))
	...
	for _, f := range INTERLOCKyFuncs {
		for i, arg := range f.Args {
			...
			case *expression.DeferredCauset:
				...
				projSchemaDefCauss = append(projSchemaDefCauss, expr)
			default:
				projExprs = append(projExprs, expr)
	...
	projSchema := expression.NewSchema(projSchemaDefCauss...)
...
// inner children has no correlated defCausumns from it's outer schemaReplicant.
type TransformApplyToJoin struct {
	baseMemrule
}

// NewMemruleTransformApplyToJoin creates a new Transformation TransformApplyToJoin.
// The pattern of this rule is: `Apply -> (X, Y)`.
func NewMemruleTransformApplyToJoin() Transformation {
	rule := &TransformApplyToJoin{}
	rule.pattern = memo.NewPattern(memo.OperandApply, memo.EngineMilevaDBOnly)
	return rule
}
...
func (r *TransformApplyToJoin) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	// Because apply.CorDefCauss will only be used and uFIDelated by this rule during Transformation.
	apply.CorDefCauss = r.extractCorDeferredCausetsBySchema(groupExpr.Children[1], groupExpr.Children[0].Prop.Schema)
...
func (r *TransformApplyToJoin) extractCorDeferredCausetsBySchema(innerGroup *memo.Group, outerSchema *expression.Schema) []*expression.CorrelatedDeferredCauset {
	corDefCauss := r.extractCorDeferredCausetsFromGroup(innerGroup)
	return plannercore.ExtractCorDeferredCausetsBySchema(corDefCauss, outerSchema, false)
}

func (r *TransformApplyToJoin) extractCorDeferredCausetsFromGroup(g *memo.Group) []*expression.CorrelatedDeferredCauset {
	...
	for elem := g.Equivalents.Front(); elem != nil; elem = elem.Next() {
		...
		corDefCauss = append(corDefCauss, expr.ExprNode.ExtractCorrelatedDefCauss()...)
		for _, child := range expr.Children {
			corDefCauss = append(corDefCauss, r.extractCorDeferredCausetsFromGroup(child)...)
		}
	}
	// We may have duplicate CorrelatedDeferredCausets here, but it won't influence
	// the logic of the transformation. Apply.CorDefCauss will be deduplicated in
	// `ResolveIndices`.
	return corDefCauss
}
...
// NewMemrulePullSelectionUpApply creates a new Transformation PullSelectionUpApply.
// The pattern of this rule is: `Apply -> (Any<outer>, Selection<inner>)`.
func NewMemrulePullSelectionUpApply() Transformation {
	rule := &PullSelectionUpApply{}
	rule.pattern = memo.BuildPattern(
		memo.OperandApply,
		memo.EngineMilevaDBOnly,
		memo.NewPattern(memo.OperandAny, memo.EngineMilevaDBOnly),       // outer child
		memo.NewPattern(memo.OperandSelection, memo.EngineMilevaDBOnly), // inner child
	)
	return rule
}
...
func (r *PullSelectionUpApply) OnTransform(old *memo.ExprIter) (newExprs []*memo.GroupExpr, eraseOld bool, eraseAll bool, err error) {
	...
	newApply := plannercore.LogicalApply{
		...
		CorDefCauss: apply.CorDefCauss,
	}.Init(apply.SCtx(), apply.SelectBlockOffset())
	// UFIDelate Join conditions.
