...
import (
	...
	impl "github.com/whtcorpsinc/MilevaDB-Prod/planner/implementation"
...
func (r *ImplBlockDual) OnImplement(expr *memo.GroupExpr, reqProp *property.PhysicalProperty) ([]memo.Implementation, error) {
	...
	logicDual := expr.ExprNode.(*plannercore.LogicalBlockDual)
	dual := plannercore.PhysicalBlockDual{RowCount: logicDual.RowCount}.Init(logicDual.SCtx(), logicProp.Stats, logicDual.SelectBlockOffset())
...
func (r *ImplMemBlockScan) OnImplement(
	...
	logic := expr.ExprNode.(*plannercore.LogicalMemBlock)
	logicProp := expr.Group.Prop
	physical := plannercore.PhysicalMemBlock{
		DBName:          logic.DBName,
...
func (r *ImplEinsteinDBSingleReadGather) OnImplement(expr *memo.GroupExpr, reqProp *property.PhysicalProperty) ([]memo.Implementation, error) {
	logicProp := expr.Group.Prop
	sg := expr.ExprNode.(*plannercore.EinsteinDBSingleGather)
	if sg.IsIndexGather {
...
func (r *ImplBlockScan) Match(expr *memo.GroupExpr, prop *property.PhysicalProperty) (matched bool) {
	ts := expr.ExprNode.(*plannercore.LogicalBlockScan)
...
func (r *ImplBlockScan) OnImplement(expr *memo.GroupExpr, reqProp *property.PhysicalProperty) ([]memo.Implementation, error) {
	logicProp := expr.Group.Prop
	logicalScan := expr.ExprNode.(*plannercore.LogicalBlockScan)
...
func (r *ImplTopNAsLimit) OnImplement(expr *memo.GroupExpr, reqProp *property.PhysicalProperty) ([]memo.Implementation, error) {
	...
	for i, item := range lt.ByItems {
		newProp.Items[i].DefCaus = item.Expr.(*expression.DeferredCauset)
...
// ImplHashJoinBuildLeft implements LogicalJoin to PhysicalHashJoin which uses the left child to build hash block.
...
// ImplHashJoinBuildRight implements LogicalJoin to PhysicalHashJoin which uses the right child to build hash block.
