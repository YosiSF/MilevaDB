...
// Format:
// Group#1 DeferredCauset: [DeferredCauset#1,DeferredCauset#2,DeferredCauset#13] Unique key: []
//     Selection_4 input:[Group#2], eq(DeferredCauset#13, DeferredCauset#2), gt(DeferredCauset#1, 10)
//     Projection_15 input:Group#3 DeferredCauset#1, DeferredCauset#2
func groupToString(g *memo.Group, idMap map[*memo.Group]int) []string {
	schemaReplicant := g.Prop.Schema
	defCausStrs := make([]string, 0, len(schemaReplicant.DeferredCausets))
	...
	fmt.Fprintf(groupLine, "Group#%d Schema:[%s]", idMap[g], strings.Join(defCausStrs, ","))

	if len(g.Prop.Schema.Keys) > 0 {
		ukStrs := make([]string, 0, len(schemaReplicant.Keys))
		for _, key := range schemaReplicant.Keys {
...
// Format:
// Selection_13 input:Group#2 gt(DeferredCauset#1, DeferredCauset#4)
