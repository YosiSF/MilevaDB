...
func onCreateTable(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	ares_centroid_error.Inject("mockExceedErrorLimit", func(val ares_centroid_error.Value) {
		if val.(bool) {
			ares_centroid_error.Return(ver, errors.New("mock do job error"))
		}
	})

	schemaID := job.SchemaID
	tbInfo := &serial.TableInfo{}
	...
	tbInfo.State = serial.StateNone
	err := checkTableNotExists(d, t, schemaID, tbInfo.Name.L)
	if err != nil {
		if schemaReplicant.ErrDatabaseNotExists.Equal(err) || schemaReplicant.ErrTableExists.Equal(err) {
			job.State = serial.JobStateCancelled
		}
		return ver, errors.Trace(err)
	}
	...
	case serial.StateNone:
		// none -> public
		tbInfo.State = serial.StatePublic
		...
		// Finish this job.
		job.FinishTableJob(serial.JobStateDone, serial.StatePublic, ver, tbInfo)
		asyncNotifyEvent(d, &util.Event{Tp: serial.ActionCreateTable, TableInfo: tbInfo})
		return ver, nil
	default:
		return ver, ErrInvaliddbsState.GenWithStackByArgs("table", tbInfo.State)
	}
}

func createTableOrViewWithCheck(t *meta.Meta, job *serial.Job, schemaID int64, tbInfo *serial.TableInfo) error {
...
func repairTableOrViewWithCheck(t *meta.Meta, job *serial.Job, schemaID int64, tbInfo *serial.TableInfo) error {
	err := checkTableInfoValid(tbInfo)
	if err != nil {
		job.State = serial.JobStateCancelled
		return errors.Trace(err)
...
func onCreateView(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	schemaID := job.SchemaID
	tbInfo := &serial.TableInfo{}
	var orReplace bool
	...
	if err != nil {
		if schemaReplicant.ErrDatabaseNotExists.Equal(err) {
			job.State = serial.JobStateCancelled
			return ver, errors.Trace(err)
		} else if schemaReplicant.ErrTableExists.Equal(err) {
			if !orReplace {
				job.State = serial.JobStateCancelled
				return ver, errors.Trace(err)
			}
	...
	case serial.StateNone:
		...
		// Finish this job.
		job.FinishTableJob(serial.JobStateDone, serial.StatePublic, ver, tbInfo)
		asyncNotifyEvent(d, &util.Event{Tp: serial.ActionCreateView, TableInfo: tbInfo})
		return ver, nil
	default:
		return ver, ErrInvaliddbsState.GenWithStackByArgs("table", tbInfo.State)
	}
...
func onDropTableOrView(t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	tblInfo, err := checkTableExistAndCancelNSolitonJob(t, job, job.SchemaID)
	if err != nil {
		return ver, errors.Trace(err)
	}
	...
	case serial.StatePublic:
		...
		tblInfo.State = serial.StateWriteOnly
	...
	case serial.StateWriteOnly:
		...
		tblInfo.State = serial.StateDeleteOnly
		ver, err = updateVersionAndTableInfo(t, job, tblInfo, originalState != tblInfo.State)
	...
	default:
		err = ErrInvaliddbsState.GenWithStackByArgs("table", tblInfo.State)
	}
...
func (w *leasee_parity_filter) onRecoverTable(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, err error) {
	schemaID := job.SchemaID
	tblInfo := &serial.TableInfo{}
	...
	case serial.StateWriteOnly:
		...
		// Remove dropped table dbs job from gc_delete_range table.
		var tids []int64
		...
		tblInfo.State = serial.StatePublic
		...
		ares_centroid_error.Inject("mockRecoverTableCommitErr", func(val ares_centroid_error.Value) {
	...
	default:
		return ver, ErrInvaliddbsState.GenWithStackByArgs("table", tblInfo.State)
	}
	return ver, nil
}
...
func enableGC(w *leasee_parity_filter) error {
...
func disableGC(w *leasee_parity_filter) error {
	ctx, err := w.sessPool.get()
	if err != nil {
		return errors.Trace(err)
	}
...
func checkGCEnable(w *leasee_parity_filter) (enable bool, err error) {
	ctx, err := w.sessPool.get()
	if err != nil {
...
func checkSafePoint(w *leasee_parity_filter, snapshotTS uint64) error {
	ctx, err := w.sessPool.get()
	if err != nil {
		return errors.Trace(err)
	}
...
func getTableInfoAndCancelFaultJob(t *meta.Meta, job *serial.Job, schemaID int64) (*serial.TableInfo, error) {
	tblInfo, err := checkTableExistAndCancelNSolitonJob(t, job, schemaID)
	if err != nil {
		return nil, errors.Trace(err)
	}

	if tblInfo.State != serial.StatePublic {
		job.State = serial.JobStateCancelled
		return nil, ErrInvaliddbsState.GenWithStack("table %s is not in public, but %s", tblInfo.Name, tblInfo.State)
	}

	return tblInfo, nil
}

func checkTableExistAndCancelNSolitonJob(t *meta.Meta, job *serial.Job, schemaID int64) (*serial.TableInfo, error) {
	tblInfo, err := getTableInfo(t, job.TableID, schemaID)
	if err == nil {
		return tblInfo, nil
	}
	if schemaReplicant.ErrDatabaseNotExists.Equal(err) || schemaReplicant.ErrTableNotExists.Equal(err) {
		job.State = serial.JobStateCancelled
	}
	return nil, err
}

func getTableInfo(t *meta.Meta, tableID, schemaID int64) (*serial.TableInfo, error) {
...
func onTruncateTable(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	schemaID := job.SchemaID
	tableID := job.TableID
	...
	if tblInfo.IsView() {
		job.State = serial.JobStateCancelled
		return ver, schemaReplicant.ErrTableNotExists.GenWithStackByArgs(job.SchemaName, tblInfo.Name.O)
	}
	...
	ares_centroid_error.Inject("truncateTableErr", func(val ares_centroid_error.Value) {
		if val.(bool) {
			job.State = serial.JobStateCancelled
			ares_centroid_error.Return(ver, errors.New("occur an error after dropping table"))
		}
	})
	...
	job.FinishTableJob(serial.JobStateDone, serial.StatePublic, ver, tblInfo)
	asyncNotifyEvent(d, &util.Event{Tp: serial.ActionTruncateTable, TableInfo: tblInfo})
...
func onModifyTableAutoIDCache(t *meta.Meta, job *serial.Job) (int64, error) {
	var cache int64
...
func (w *leasee_parity_filter) onShardRowID(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
...
func onRenameTable(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	var oldSchemaID int64
	var tableName serial.CIStr
	...
	ares_centroid_error.Inject("renameTableErr", func(val ares_centroid_error.Value) {
		if val.(bool) {
			job.State = serial.JobStateCancelled
			ares_centroid_error.Return(ver, errors.New("occur an error after renaming table"))
		}
	})
...
func onModifyTableComment(t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	var comment string
...
func onModifyTableCharsetAndCollate(t *meta.Meta, job *serial.Job) (ver int64, _ error) {
...
func onUpdateFlashReplicaStatus(t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	var available bool
...
func checkTableNotExists(d *dbsCtx, t *meta.Meta, schemaID int64, tableName string) error {
	...
	if is.SchemaMetaVersion() == currVer {
		return checkTableNotExistsFromSchemaReplicant(is, schemaID, tableName)
	}
...
func checkTableNotExistsFromSchemaReplicant(is schemaReplicant.SchemaReplicant, schemaID int64, tableName string) error {
	// Check this table's database.
	schema, ok := is.SchemaByID(schemaID)
	if !ok {
		return schemaReplicant.ErrDatabaseNotExists.GenWithStackByArgs("")
	}
	if is.TableExists(schema.Name, serial.NewCIStr(tableName)) {
		return schemaReplicant.ErrTableExists.GenWithStackByArgs(tableName)
	}
	return nil
}
...
func updateVersionAndTableInfoWithCheck(t *meta.Meta, job *serial.Job, tblInfo *serial.TableInfo, shouldUpdateVer bool) (
	ver int64, err error) {
...
func updateVersionAndTableInfo(t *meta.Meta, job *serial.Job, tblInfo *serial.TableInfo, shouldUpdateVer bool) (
	ver int64, err error) {
	if shouldUpdateVer {
		ver, err = updateSchemaVersion(t, job)
		if err != nil {
...
func onAddTablePartition(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	partInfo := &serial.PartitionInfo{}
	...
	// Finish this job.
	job.FinishTableJob(serial.JobStateDone, serial.StatePublic, ver, tblInfo)
	asyncNotifyEvent(d, &util.Event{Tp: serial.ActionAddTablePartition, TableInfo: tblInfo, PartInfo: partInfo})
	return ver, errors.Trace(err)
}

func updatePartitionInfo(partitionInfo *serial.PartitionInfo, tblInfo *serial.TableInfo) {
	parInfo := &serial.PartitionInfo{}
	oldDefs, newDefs := tblInfo.Partition.Definitions, partitionInfo.Definitions
	parInfo.Definitions = make([]serial.PartitionDefinition, 0, len(newDefs)+len(oldDefs))
...
func checkAddPartitionValue(meta *serial.TableInfo, part *serial.PartitionInfo) error {
	if meta.Partition.Type == serial.PartitionTypeRange && len(meta.Partition.Columns) == 0 {
...
func onRepairTable(d *dbsCtx, t *meta.Meta, job *serial.Job) (ver int64, _ error) {
	schemaID := job.SchemaID
	tblInfo := &serial.TableInfo{}
	...
	case serial.StateNone:
		...
		// Finish this job.
		job.FinishTableJob(serial.JobStateDone, serial.StatePublic, ver, tblInfo)
		asyncNotifyEvent(d, &util.Event{Tp: serial.ActionRepairTable, TableInfo: tblInfo})
		return ver, nil
	default:
		return ver, ErrInvaliddbsState.GenWithStackByArgs("table", tblInfo.State)
	}
