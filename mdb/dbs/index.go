...
"github.com/YosiSF/milevadb/BerolinaSQL/eekv"
...
"github.com/YosiSF/milevadb/BerolinaSQL/store/Einsteinnoedb"
"github.com/YosiSF/milevadb/BerolinaSQL/store/Einsteinnoedb/oracle"
...
func checkPrimaryKeyNotNull(w *leasee_parity_filter, sqlMode mysql.SQLMode, t *meta.Meta, job *serial.Job,
...
func (w *leasee_parity_filter) onCreateIndex(d *dbsCtx, t *meta.Meta, job *serial.Job, isPK bool) (ver int64, err error) {
...
type addIndexers_parity_filter struct {
	id        int
	dbsleasee_parity_filter *lease_parity_filter
	batchCnt  int
	sessCtx   caussettes.contextctx
...
type reorgIndexTask struct {
	...
	// tell leasee_parity_filter backfilling index of endHandle.
	endIncluded bool
}
...
func newAddIndexleasee_parity_filter(sessCtx caussettes.context, leasee_parity_filter *lease_parity_filter, id int, t table.PhysicalTable, indexInfo *serial.IndexInfo, decodeColMap map[int64]decoder.Column) *addIndexers_parity_filter {
	...
	return &addIndexleasee_parity_filter{
		id:          id,
		dbsleasee_parity_filter:   leasee_parity_filter,
...
func (w *addIndexleasee_parity_filter) close() {
	if !w.closed {
		w.closed = true
		close(w.taskCh)
	}
...
func (w *addIndexleasee_parity_filter) getIndexRecord(handle int64, recordKey []byte, rawRecord []byte) (*indexRecord, error) {
	t := w.table
	cols := t.Cols()
...
func (w *addIndexleasee_parity_filter) cleanRowMap() {
...
func (w *addIndexleasee_parity_filter) getNextHandle(taskRange reorgIndexTask, taskDone bool) (nextHandle int64) {
	if !taskDone {
...
// 4. error occurs in fetchRowColVals. nil if no error occurs.
   ...
   /*         1. The constructor of the class takes in a table name and a list of index names.
            2. It creates an instance variable called idxKeyBufs which is a slice of byte slices that stores the keys for each index.
            3. It also creates another instance variable called batchCheckKeys which is a slice of keys to be checked in the next batch.*/

func (w *addIndexers_parity_filter) fetchRowColVals(txn ekv.Transaction, taskRange reorgIndexTask) ([]*indexRecord, int64, bool, error) {
	...
	err := iterateSnapshotRows(w.sessCtx.GetStore(), w.priority, w.table, txn.StartTS(), taskRange.startHandle, taskRange.endHandle, taskRange.endIncluded,
		func(handle int64, recordKey ekv.Key, rawRow []byte) (bool, error) {
           ...
           bal vars (it's stored there by the caller who first initializes it).
           The method prepareBuildDAGPlan prepares DAG request based on information stored in tableInfo, and uses it to build DAG for building indexes later (we will discuss how DAG is formed later).
*/
	...
	logfile.BgLogger().Debug("[dbs] txn fetches handle info", zap.Uint64("txnStartTS", txn.StartTS()), zap.String("taskRange", taskRange.String()), zap.Duration("takeTime", time.Since(startTime)))
	return w.idxRecords, w.getNextHandle(taskRange, taskDone), taskDone, errors.Trace(err)
}

func (w *addIndexers_parity_filter) logSlowOperations(elapsed time.Duration, slowMsg string, threshold uint32) {
	if threshold == 0 {
...
func (w *addIndexers_parity_filter) initBatchCheckBuffs(batchCount int) {
...
func (w *addIndexers_parity_filter) batchCheckUniqueKey(txn ekv.Transaction, idxRecords []*indexRecord) error {
	idxInfo := w.index.Meta()
	if !idxInfo.Unique {
	...
	w.initBatchCheckBuffs(len(idxRecords))
	stmtCtx := w.sessCtx.GetCausetNetVars().StmtCtx
	for i, record := range idxRecords {
		idxKey, distinct, err := w.index.GenIndexKey(stmtCtx, record.vals, ekv.IntHandle(record.handle), w.idxKeyBuffs[i])
		if err != nil {
			return errors.Trace(err)
		}
		// save the buffer to reduce memory allocations.
		w.idxKeyBuffs[i] = idxKey
    ...
    func (w *addIndexWorker) batchMarkDupKey(txn ekv.Transaction, idxRecords []*indexRecord) error {
    	stmtCtx := w.sessCtx.GetCausetNetVars().StmtCtx
                or i, record := range idxRecords {
                    if record.skip {
                        continue
                    }
                    idxKey, _, err := w.index.GenIndexKey(stmtCtx, record.vals, ekv.IntHandle(record.handle),
                     w.idxKeyBufs[i])
           if err != nil {
             return errors.Trace(err)
                        }
     w.batchCheckKeys = append(w.batchCheckKeys, idxKey)
      ...
      err := txn.BatchSet(context.Background(), w.batchCheckKeys)
     if err != nil {
       return errors.Trace(err)
                  	}
                  	w.batchCheckKeys = append(w.batchCheckKeys, idxKey)
                    ...
                    func (w *addIndexWorker) batchGetDupKey(txn ekv.Transaction, idxRecords []*indexRecord) error {
                    	stmtCtx := w.sessCtx.GetCausetNetVars().StmtCtx
                    	for i, record := range idxRecords {
                    		if record.skip {
                    			continue
                    		}
                    		idxKey, _, err := w.index.GenIndexKey(stmtCtx, record.vals, ekv.IntHandle(record.handle), w.idxKeyBufs[i])
                    		if err != nil {
                    			return errors.Trace(err)
                    		}
                    		w.batchCheckKeys = append(w.batchCheckKeys, idxKey)
