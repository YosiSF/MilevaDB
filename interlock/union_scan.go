...
type DirtyTable struct {
	...
	// addedRows ...
	// the key is handle.
	addedRows   *ekv.HandleMap
	deletedRows *ekv.HandleMap
}

// AddRow adds a row to the DirtyDB.
func (dt *DirtyTable) AddRow(handle ekv.Handle) {
	dt.addedRows.Set(handle, true)
}

// DeleteRow deletes a row from the DirtyDB.
func (dt *DirtyTable) DeleteRow(handle ekv.Handle) {
...
type UnionScanExec struct {
	...
	dirty *DirtyTable
	// usedIndex is the column offsets of the index which Src Interlock has used.
	usedIndex            []int
	desc                 bool
...
func (us *UnionScanExec) Open(ctx context.Context) error {
	if err := us.baseInterlock.Open(ctx); err != nil {
		return err
	}
	return us.open(ctx)
}
