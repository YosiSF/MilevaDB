...
func (s *testConfigSuite) TestLogConfig(c *C) {
	...
	var testLoad = func(confStr string, expectedEnableErrorStack, expectedDisableErrorStack, expectedEnableTimestamp, expectedDisableTimestamp nullableBool, resultedDisableTimestamp, resultedDisableErrorVerbose bool, valid Checker) {
		...
		c.Assert(conf.Log.ToLogConfig(), DeepEquals, logutil.NewLogConfig("info", "text", "milevadb-slow.log", conf.Log.File, resultedDisableTimestamp, func(config *zaplog.Config) { config.DisableErrorVerbose = resultedDisableErrorVerbose }))
		f.Truncate(0)
		f.Seek(0, 0)
	}
...
func (s *testConfigSuite) TestConfig(c *C) {
	...
	conf.EinsteinDBClient.CommitTimeout = "10s"
	conf.EinsteinDBClient.RegionCacheTTL = 600
	conf.Log.EnableSlowLog = logutil.DefaultMilevaDBEnableSlowLog
...
`)
	...
	_, err = f.WriteString(`
token-limit = 0
enable-block-dagger = true
alter-primary-key = true
delay-clean-block-dagger = 5
...
`)
	...
	c.Assert(allegrosql.ServerVersion, Equals, conf.ServerVersion)
	...
	c.Assert(conf.EinsteinDBClient.CommitTimeout, Equals, "41s")
	c.Assert(conf.EinsteinDBClient.EnableAsyncCommit, Equals, true)
	c.Assert(conf.EinsteinDBClient.AsyncCommitKeysLimit, Equals, uint(123))
	c.Assert(conf.EinsteinDBClient.MaxBatchSize, Equals, uint(128))
	c.Assert(conf.EinsteinDBClient.RegionCacheTTL, Equals, uint(6000))
	c.Assert(conf.EinsteinDBClient.StoreLimit, Equals, int64(0))
	c.Assert(conf.EinsteinDBClient.TTLRefreshedTxnSize, Equals, int64(8192))
	...
	c.Assert(conf.StmtSummary.MaxALLEGROSQLLength, Equals, uint(1024))
...
`)
	...
	// Test for log config.
	c.Assert(conf.Log.ToLogConfig(), DeepEquals, logutil.NewLogConfig("info", "text", "milevadb-slow.log", conf.Log.File, false, func(config *zaplog.Config) { config.DisableErrorVerbose = conf.Log.getDisableErrorStack() }))
...
BvUPi88z3wGa8rmhn9dOvkwauLFU5i5dqoz6m9HXmaEKzAAigGzgU8vFIDelt/Dxxgu
c933WW1E0hCtvuGxWFIFtoJMQoyH0Pl4ACmY/6CokCCZKDInrFIDelhhf3MGRjkkw==
-----END CERTIFICATE-----
`)
	c.Assert(err, IsNil)
	c.Assert(f.Close(), IsNil)
...
MIIEowIBAAKCAQEAxAsmOXGeEnHEHZOFwoBDcDi7BFIDelrmb7rQXGB5PJ3SKcbsUJ5
...
JPxYOdKPBhUU8y6lMReiRsAkSSg6be7AOFhZT3oc7f4AWZixYPnFU2SFIDel+GnkRXA
...
func (s *testConfigSuite) TestOOMCausetActionValid(c *C) {
	c1 := NewConfig()
	tests := []struct {
		oomCausetAction string
		valid           bool
	}{
		{"log", true},
		{"Log", true},
	...
	for _, tt := range tests {
		c1.OOMCausetAction = tt.oomCausetAction
...
func (s *testConfigSuite) TestParsePath(c *C) {
	etcdAddrs, disableGC, err := ParsePath("einsteindb://node1:2379,node2:2379")
	c.Assert(err, IsNil)
	c.Assert(etcdAddrs, DeepEquals, []string{"node1:2379", "node2:2379"})
	c.Assert(disableGC, IsFalse)

	_, _, err = ParsePath("einsteindb://node1:2379")
	c.Assert(err, IsNil)
	_, disableGC, err = ParsePath("einsteindb://node1:2379?disableGC=true")
	c.Assert(err, IsNil)
...
func (s *testConfigSuite) TestEncodeDefTempStorageDir(c *C) {
	...
	dirPrefix := filepath.Join(os.TemFIDelir(), osUID+"_milevadb")
	for _, test := range tests {
		tempStorageDir := encodeDefTempStorageDir(os.TemFIDelir(), test.host, test.statusHost, test.port, test.statusPort)
