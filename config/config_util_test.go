...
func (s *testConfigSuite) TestCloneConf(c *C) {
	...
	c.Assert(c1.CausetStore, Not(Equals), c2.CausetStore)
...
func (s *testConfigSuite) TestMergeConfigItems(c *C) {
	...
	newConf.OOMCausetAction = "panic"
	newConf.MemQuotaQuery = 123
	newConf.EinsteinDBClient.StoreLimit = 123

	// rejected
	newConf.CausetStore = "tiflash"
	newConf.Port = 2333
	...
	c.Assert(oldConf.OOMCausetAction, Equals, newConf.OOMCausetAction)
	c.Assert(oldConf.MemQuotaQuery, Equals, newConf.MemQuotaQuery)
	c.Assert(oldConf.EinsteinDBClient.StoreLimit, Equals, newConf.EinsteinDBClient.StoreLimit)
	c.Assert(oldConf.Log.SlowThreshold, Equals, newConf.Log.SlowThreshold)

	c.Assert(oldConf.CausetStore, Equals, oriConf.CausetStore)
...
func (s *testConfigSuite) TestAtomicWriteConfig(c *C) {
	...
	confPath := filepath.Join(os.TemFIDelir(), "test-write-config.toml")
...
`
	...
	c.Assert(toJSONStr(flatMap["isolation-read.engines"]), Equals, `["einsteindb","tiflash","milevadb"]`)
}
