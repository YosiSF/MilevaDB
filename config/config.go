...
type Config struct {
	...
	// EnableBlockLock indicate whether enable block dagger.
	// TODO: remove this after block dagger features sblock.
	EnableBlockLock     bool        `toml:"enable-block-dagger" json:"enable-block-dagger"`
	DelayCleanBlockLock uint64      `toml:"delay-clean-block-dagger" json:"delay-clean-block-dagger"`
...
type EinsteinDBClient struct {
	...
	INTERLOCKrCache      interlocking_directorateCache `toml:"INTERLOCKr-cache" json:"INTERLOCKr-cache"`
	// TTLRefreshedTxnSize controls whether a transaction should uFIDelate its TTL or not.
	TTLRefreshedTxnSize int64 `toml:"ttl-refreshed-txn-size" json:"ttl-refreshed-txn-size"`
}

// interlocking_directorateCache is the config for interlock cache.
type interlocking_directorateCache struct {
	// Whether to enable the INTERLOCKr cache. The INTERLOCKr cache saves the result from EinsteinDB interlocking_directorate in the memory and
...
var defaultConf = Config{
	...
	EinsteinDBClient: EinsteinDBClient{
		...
		INTERLOCKrCache: interlocking_directorateCache{
			Enable:                true,
			CapacityMB:            1000,
...
// BlockLockEnabled uses to check whether enabled the block dagger feature.
func BlockLockEnabled() bool {
	return GetGlobalConfig().EnableBlockLock
}

// BlockLockDelayClean uses to get the time of delay clean block dagger.
