...
func (t *testTopNSlowQuerySuite) TestPush(c *C) {
	...
	// UFIDelate all data in the heap.
...
func (t *testTopNSlowQuerySuite) TestQueue(c *C) {
	...
	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "aaa"})
	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "bbb"})
	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "ccc"})

	query := q.recent.Query(1)
	c.Assert(query[0].ALLEGROALLEGROSQL, Equals, "ccc")
	query = q.recent.Query(2)
	c.Assert(query[0].ALLEGROALLEGROSQL, Equals, "ccc")
	c.Assert(query[1].ALLEGROALLEGROSQL, Equals, "bbb")
	query = q.recent.Query(6)
	c.Assert(query[0].ALLEGROALLEGROSQL, Equals, "ccc")
	c.Assert(query[1].ALLEGROALLEGROSQL, Equals, "bbb")
	c.Assert(query[2].ALLEGROALLEGROSQL, Equals, "aaa")

	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "ddd"})
	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "eee"})
	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "fff"})
	q.Append(&SlowQueryInfo{ALLEGROALLEGROSQL: "ggg"})

	query = q.recent.Query(3)
	c.Assert(query[0].ALLEGROALLEGROSQL, Equals, "ggg")
	c.Assert(query[1].ALLEGROALLEGROSQL, Equals, "fff")
	c.Assert(query[2].ALLEGROALLEGROSQL, Equals, "eee")
	query = q.recent.Query(6)
	c.Assert(query[0].ALLEGROALLEGROSQL, Equals, "ggg")
	c.Assert(query[1].ALLEGROALLEGROSQL, Equals, "fff")
	c.Assert(query[2].ALLEGROALLEGROSQL, Equals, "eee")
	c.Assert(query[3].ALLEGROALLEGROSQL, Equals, "ddd")
	c.Assert(query[4].ALLEGROALLEGROSQL, Equals, "ccc")
}
