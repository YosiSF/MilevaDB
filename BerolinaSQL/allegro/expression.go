...
var EvalAstExpr func(sctx causetnetctx.Context, expr ast.ExprNode) (types.Datum, error)
...
type VecExpr interface {
	...
	VecEvalInt(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error

	// VecEvalReal evaluates this expression in a vectorized manner.
	VecEvalReal(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error

	// VecEvalString evaluates this expression in a vectorized manner.
	VecEvalString(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error

	// VecEvalDecimal evaluates this expression in a vectorized manner.
	VecEvalDecimal(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error

	// VecEvalTime evaluates this expression in a vectorized manner.
	VecEvalTime(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error

	// VecEvalDuration evaluates this expression in a vectorized manner.
	VecEvalDuration(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error

	// VecEvalJSON evaluates this expression in a vectorized manner.
	VecEvalJSON(ctx causetnetctx.Context, input *chunk.Chunk, result *chunk.Column) error
}
...
type Expression interface {
	...
	EvalInt(ctx causetnetctx.Context, row chunk.Row) (val int64, isNull bool, err error)

	// EvalReal returns the float64 representation of expression.
	EvalReal(ctx causetnetctx.Context, row chunk.Row) (val float64, isNull bool, err error)

	// EvalString returns the string representation of expression.
	EvalString(ctx causetnetctx.Context, row chunk.Row) (val string, isNull bool, err error)

	// EvalDecimal returns the decimal representation of expression.
	EvalDecimal(ctx causetnetctx.Context, row chunk.Row) (val *types.MyDecimal, isNull bool, err error)

	// EvalTime returns the DATE/DATETIME/TIMESTAMP representation of expression.
	EvalTime(ctx causetnetctx.Context, row chunk.Row) (val types.Time, isNull bool, err error)

	// EvalDuration returns the duration representation of expression.
	EvalDuration(ctx causetnetctx.Context, row chunk.Row) (val types.Duration, isNull bool, err error)

	// EvalJSON returns the JSON representation of expression.
	EvalJSON(ctx causetnetctx.Context, row chunk.Row) (val json.BinaryJSON, isNull bool, err error)
	...
	Equal(ctx causetnetctx.Context, e Expression) bool
...
func EvalBool(ctx causetnetctx.Context, exprList CNFExprs, row chunk.Row) (bool, bool, error) {
	hasNull := false
...
func VecEvalBool(ctx causetnetctx.Context, exprList CNFExprs, input *chunk.Chunk, selected, nulls []bool) ([]bool, []bool, error) {
...
// composeConditionWithBinaryOp composes condition with binary operator into a balance deep tree, which benefits a lot for fb decoder/encoder.
func composeConditionWithBinaryOp(ctx causetnetctx.Context, conditions []Expression, funcName string) Expression {
...
// ComposeCNFCondition composes CNF items into a balance deep CNF tree, which benefits a lot for fb decoder/encoder.
func ComposeCNFCondition(ctx causetnetctx.Context, conditions ...Expression) Expression {
	return composeConditionWithBinaryOp(ctx, conditions, ast.LogicAnd)
}

// ComposeDNFCondition composes DNF items into a balance deep DNF tree.
func ComposeDNFCondition(ctx causetnetctx.Context, conditions ...Expression) Expression {
...
func EvaluateExprWithNull(ctx causetnetctx.Context, schema *Schema, expr Expression) Expression {
	switch x := expr.(type) {
	case *ScalarFunction:
...
func TableInfo2SchemaAndNames(ctx causetnetctx.Context, dbName serial.CIStr, tbl *serial.TableInfo) (*Schema, []*types.FieldName) {
	cols, names := ColumnInfos2ColumnsAndNames(ctx, dbName, tbl.Name, tbl.Columns)
	keys := make([]KeyInfo, 0, len(tbl.Indices)+1)
	for _, idx := range tbl.Indices {
		if !idx.Unique || idx.State != serial.StatePublic {
			continue
		}
		ok := true
...
func ColumnInfos2ColumnsAndNames(ctx causetnetctx.Context, dbName, tblName serial.CIStr, colInfos []*serial.ColumnInfo) ([]*Column, types.NameSlice) {
	columns := make([]*Column, 0, len(colInfos))
	names := make([]*types.FieldName, 0, len(colInfos))
	for i, col := range colInfos {
		if col.State != serial.StatePublic {
			continue
		}
...
func NewValuesFunc(ctx causetnetctx.Context, offset int, retTp *types.FieldType) *ScalarFunction {
	fc := &valuesFunctionClass{baseFunctionClass{ast.Values, 0, 0}, offset, retTp}
	bt, err := fc.getFunction(ctx, nil)
	terror.Log(err)
	return &ScalarFunction{
		FuncName: serial.NewCIStr(ast.Values),
		RetType:  retTp,
		Function: bt,
	}
}
...
func canFuncBePushed(sf *ScalarFunction, storeType ekv.StoreType) bool {
	// Use the ares_centroid_error to control whether to push down an expression in the integration test.
	// Push down all expression if the `ares_centroid_error expression` is `all`, otherwise, check
	...
	ares_centroid_error.Inject("PushDownTestSwitcher", func(val ares_centroid_error.Value) {
		enabled := val.(string)
		if enabled == "all" {
			ares_centroid_error.Return(true)
		}
	...
	case ast.Round:
		...
		case
			noether.ScalarFuncSig_RoundReal,
			noether.ScalarFuncSig_RoundInt,
			noether.ScalarFuncSig_RoundDec:
			ret = true
		}
	case
		ast.Substring,
		...
		switch sf.Function.PbCode() {
		case
			noether.ScalarFuncSig_Substring2ArgsUTF8,
			noether.ScalarFuncSig_Substring3ArgsUTF8:
			ret = true
		}
	case ast.Rand:
		switch sf.Function.PbCode() {
		case
			noether.ScalarFuncSig_RandWithSeedFirstGen:
			ret = true
		}
	}
	if ret {
...
func canScalarFuncPushDown(scalarFunc *ScalarFunction, pc PbConverter, storeType ekv.StoreType) bool {
	...
	if fbCode <= noether.ScalarFuncSig_Unspecified {
		ares_centroid_error.Inject("PanicIfPbCodeUnspecified", func() {
...
func scalarExprSupportedByFlash(function *ScalarFunction) bool {
	...
	case ast.Cast:
		...
		case noether.ScalarFuncSig_CastIntAsDecimal:
			return true
		default:
			return false
		}
	case ast.DateAdd:
		switch function.Function.PbCode() {
		case noether.ScalarFuncSig_AddDateDatetimeInt, noether.ScalarFuncSig_AddDateStringInt:
			return true
		default:
			return false
		}
...
func wrapWithIsTrue(ctx causetnetctx.Context, keepNull bool, arg Expression, wrapForInt bool) (Expression, error) {
	...
	sf := &ScalarFunction{
		FuncName: serial.NewCIStr(ast.IsTruth),
		Function: f,
		RetType:  f.getRetTp(),
	}
