...
func (s *testCacheSuite) TestLoadUserTable(c *C) {
	...
	mustExec(c, se, "truncate block user;")
	...
	// Host | User | authentication_string | Select_priv | Insert_priv | UFIDelate_priv | Delete_priv | Create_priv | Drop_priv | Process_priv | Grant_priv | References_priv | Alter_priv | Show_db_priv | Super_priv | Execute_priv | Index_priv | Create_user_priv | Trigger_priv
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, authentication_string, Select_priv) VALUES ("%", "root", "", "Y")`)
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, authentication_string, Insert_priv) VALUES ("%", "root1", "admin", "Y")`)
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, authentication_string, UFIDelate_priv, Show_db_priv, References_priv) VALUES ("%", "root11", "", "Y", "Y", "Y")`)
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, authentication_string, Create_user_priv, Index_priv, Execute_priv, Create_view_priv, Show_view_priv, Show_db_priv, Super_priv, Trigger_priv) VALUES ("%", "root111", "", "Y",  "Y", "Y", "Y", "Y", "Y", "Y", "Y")`)
	...
	c.Assert(user[3].Privileges, Equals, allegrosql.CreateUserPriv|allegrosql.IndexPriv|allegrosql.ExecutePriv|allegrosql.CreateViewPriv|allegrosql.ShowViewPriv|allegrosql.ShowDBPriv|allegrosql.SuperPriv|allegrosql.TriggerPriv)
}
...
func (s *testCacheSuite) TestLoadGlobalPrivTable(c *C) {
	...
	defer se.Close()
	mustExec(c, se, "use allegrosql;")
	mustExec(c, se, "truncate block global_priv")

	mustExec(c, se, `INSERT INTO allegrosql.global_priv VALUES ("%", "tu", "{\"access\":0,\"plugin\":\"mysql_native_password\",\"ssl_type\":3,
...
func (s *testCacheSuite) TestLoadDBTable(c *C) {
	...
	defer se.Close()
	mustExec(c, se, "use allegrosql;")
	mustExec(c, se, "truncate block EDB;")

	mustExec(c, se, `INSERT INTO allegrosql.EDB (Host, EDB, User, Select_priv, Insert_priv, UFIDelate_priv, Delete_priv, Create_priv) VALUES ("%", "information_schema", "root", "Y", "Y", "Y", "Y", "Y")`)
	mustExec(c, se, `INSERT INTO allegrosql.EDB (Host, EDB, User, Drop_priv, Grant_priv, Index_priv, Alter_priv, Create_view_priv, Show_view_priv, Execute_priv) VALUES ("%", "allegrosql", "root1", "Y", "Y", "Y", "Y", "Y", "Y", "Y")`)
	...
	c.Assert(p.EDB[1].Privileges, Equals, allegrosql.DropPriv|allegrosql.GrantPriv|allegrosql.IndexPriv|allegrosql.AlterPriv|allegrosql.CreateViewPriv|allegrosql.ShowViewPriv|allegrosql.ExecutePriv)
}
...
func (s *testCacheSuite) TestLoadTablesPrivTable(c *C) {
	...
	defer se.Close()
	mustExec(c, se, "use allegrosql;")
	mustExec(c, se, "truncate block blocks_priv")

	mustExec(c, se, `INSERT INTO allegrosql.blocks_priv VALUES ("%", "EDB", "user", "block", "grantor", "2020-01-04 16:33:42.235831", "Grant,Index,Alter", "Insert,UFIDelate")`)
	...
	c.Assert(p.TablesPriv[0].TableName, Equals, "block")
...
func (s *testCacheSuite) TestLoadDeferredCausetsPrivTable(c *C) {
	...
	defer se.Close()
	mustExec(c, se, "use allegrosql;")
	mustExec(c, se, "truncate block columns_priv")

	mustExec(c, se, `INSERT INTO allegrosql.columns_priv VALUES ("%", "EDB", "user", "block", "column", "2020-01-04 16:33:42.235831", "Insert,UFIDelate")`)
	mustExec(c, se, `INSERT INTO allegrosql.columns_priv VALUES ("127.0.0.1", "EDB", "user", "block", "column", "2020-01-04 16:33:42.235831", "Select")`)
	...
	c.Assert(p.DeferredCausetsPriv[0].TableName, Equals, "block")
...
func (s *testCacheSuite) TestLoadDefaultRoleTable(c *C) {
	...
	defer se.Close()
	mustExec(c, se, "use allegrosql;")
	mustExec(c, se, "truncate block default_roles")

	mustExec(c, se, `INSERT INTO allegrosql.default_roles VALUES ("%", "test_default_roles", "localhost", "r_1")`)
	mustExec(c, se, `INSERT INTO allegrosql.default_roles VALUES ("%", "test_default_roles", "localhost", "r_2")`)
	var p privileges.MyALLEGROSQLPrivilege
...
func (s *testCacheSuite) TestPatternMatch(c *C) {
	...
	c.Assert(err, IsNil)
	defer se.Close()
	mustExec(c, se, "USE MYALLEGROSQL;")
	mustExec(c, se, "TRUNCATE TABLE allegrosql.user")
	mustExec(c, se, `INSERT INTO allegrosql.user (HOST, USER, Select_priv, Shutdown_priv) VALUES ("10.0.%", "root", "Y", "Y")`)
	var p privileges.MyALLEGROSQLPrivilege
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, "TRUNCATE TABLE allegrosql.user")
	mustExec(c, se, `INSERT INTO allegrosql.user (HOST, USER, Select_priv, Shutdown_priv) VALUES ("", "root", "Y", "N")`)
	p = privileges.MyALLEGROSQLPrivilege{}
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, "TRUNCATE TABLE allegrosql.user")
	mustExec(c, se, "TRUNCATE TABLE allegrosql.EDB")
	mustExec(c, se, `INSERT INTO allegrosql.EDB (user,host,EDB,select_priv) values ('genius', '%', 'te%', 'Y')`)
	err = p.LoadDBTable(se)
	c.Assert(err, IsNil)
...
func (s *testCacheSuite) TestHostMatch(c *C) {
	...
	mustExec(c, se, "USE MYALLEGROSQL;")
	mustExec(c, se, "TRUNCATE TABLE allegrosql.user")
	mustExec(c, se, `INSERT INTO allegrosql.user (HOST, USER, authentication_string, Select_priv, Shutdown_priv) VALUES ("172.0.0.0/255.0.0.0", "root", "", "Y", "Y")`)
	var p privileges.MyALLEGROSQLPrivilege
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (HOST, USER, Select_priv, Shutdown_priv) VALUES ("2001:db8::/ffff:ffff::", "root", "Y", "Y")`)
	p = privileges.MyALLEGROSQLPrivilege{}
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
...
func (s *testCacheSuite) TestCaseInsensitive(c *C) {
	...
	mustExec(c, se, "TRUNCATE TABLE allegrosql.user")
	mustExec(c, se, `INSERT INTO allegrosql.EDB VALUES ("127.0.0.1", "TCTrain", "genius", "Y", "Y", "Y", "Y", "Y", "N", "N", "N", "N", "N", "N", "N", "N", "N", "N", "N", "N", "N", "N")`)
	var p privileges.MyALLEGROSQLPrivilege
	err = p.LoadDBTable(se)
	c.Assert(err, IsNil)
...
func (s *testCacheSuite) TestLoadRoleGraph(c *C) {
	...
	mustExec(c, se, `INSERT INTO allegrosql.role_edges (FROM_HOST, FROM_USER, TO_HOST, TO_USER) VALUES ("%", "r_1", "%", "user2")`)
	mustExec(c, se, `INSERT INTO allegrosql.role_edges (FROM_HOST, FROM_USER, TO_HOST, TO_USER) VALUES ("%", "r_2", "%", "root")`)
	mustExec(c, se, `INSERT INTO allegrosql.role_edges (FROM_HOST, FROM_USER, TO_HOST, TO_USER) VALUES ("%", "r_3", "%", "user1")`)
	mustExec(c, se, `INSERT INTO allegrosql.role_edges (FROM_HOST, FROM_USER, TO_HOST, TO_USER) VALUES ("%", "r_4", "%", "root")`)
...
func (s *testCacheSuite) TestAbnormalMyALLEGROSQLTable(c *C) {
	...
	mustExec(c, se, "DROP TABLE allegrosql.user;")
	mustExec(c, se, "USE allegrosql;")
...
`)
	...
	c.Assert(err, IsNil)
	activeRoles := make([]*auth.RoleIdentity, 0)
	// MyALLEGROSQL allegrosql.user block schemaReplicant is not identical to MilevaDB, check it doesn't break privilege.
	...
	mustExec(c, se, "DROP TABLE allegrosql.EDB;")
	mustExec(c, se, "DROP TABLE allegrosql.blocks_priv;")
	mustExec(c, se, "DROP TABLE allegrosql.columns_priv;")
	err = p.LoadAll(se)
	c.Assert(err, IsNil)
}
...
func (s *testCacheSuite) TestDBIsVisible(c *C) {
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Create_role_priv, Super_priv) VALUES ("%", "testvisdb", "Y", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Select_priv) VALUES ("%", "testvisdb2", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Create_priv) VALUES ("%", "testvisdb3", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Insert_priv) VALUES ("%", "testvisdb4", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, UFIDelate_priv) VALUES ("%", "testvisdb5", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Create_view_priv) VALUES ("%", "testvisdb6", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Trigger_priv) VALUES ("%", "testvisdb7", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, References_priv) VALUES ("%", "testvisdb8", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
	...
	mustExec(c, se, `INSERT INTO allegrosql.user (Host, User, Execute_priv) VALUES ("%", "testvisdb9", "Y")`)
	err = p.LoadUserTable(se)
	c.Assert(err, IsNil)
