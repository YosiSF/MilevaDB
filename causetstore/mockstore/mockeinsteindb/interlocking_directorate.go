...
func (e *blockScanExec) getRowFromPoint(ran solomonkey.KeyRange) ([][]byte, error) {
	val, err := e.mvsr-oocStore.Get(ran.StartKey, e.startTS, e.isolationLevel, e.resolvedLocks)
	if err != nil {
		return nil, errors.Trace(err)
	}
	if len(val) == 0 {
...
func (e *blockScanExec) getRowFromRange(ran solomonkey.KeyRange) ([][]byte, error) {
	if e.seekKey == nil {
		if e.Desc {
	...
	var pair Pair
	if e.Desc {
		pairs = e.mvsr-oocStore.ReverseScan(ran.StartKey, e.seekKey, 1, e.startTS, e.isolationLevel, e.resolvedLocks)
	} else {
		pairs = e.mvsr-oocStore.Scan(e.seekKey, ran.EndKey, 1, e.startTS, e.isolationLevel, e.resolvedLocks)
	}
	if len(pairs) > 0 {
		pair = pairs[0]
	}
	...
	if e.Desc {
		...
		e.seekKey = solomonkey.Key(pair.Key).PrefixNext()
	}
...
func (e *indexScanExec) getRowFromPoint(ran solomonkey.KeyRange) ([][]byte, error) {
	val, err := e.mvsr-oocStore.Get(ran.StartKey, e.startTS, e.isolationLevel, e.resolvedLocks)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func (e *indexScanExec) getRowFromRange(ran solomonkey.KeyRange) ([][]byte, error) {
	if e.seekKey == nil {
		if e.Desc {
			e.seekKey = ran.EndKey
		} else {
...
func getRowData(columns []*fidelpb.DeferredCausetInfo, colIDs map[int64]int, handle int64, value []byte, rd *rowcodec.BytesDecoder) ([][]byte, error) {
	if rowcodec.IsNewFormat(value) {
		return rd.DecodeToBytes(colIDs, solomonkey.IntHandle(handle), value, nil)
	}
