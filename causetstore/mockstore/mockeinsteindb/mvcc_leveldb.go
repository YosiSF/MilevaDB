...
import (
	...
	"github.com/whtcorpsinc/milevadb/soliton/deadlock"
...
func (mvcc *MVCCLevelDB) getValue(key []byte, startTS uint64, isoLevel kvrpcpb.IsolationLevel, resolvedLocks []uint64) ([]byte, error) {
	...
	iter := newIterator(mvcc.EDB, &soliton.Range{
		Start: startKey,
	})
	defer iter.Release()
...
func (mvcc *MVCCLevelDB) Scan(startKey, endKey []byte, limit int, startTS uint64, isoLevel kvrpcpb.IsolationLevel, resolvedLock []uint64) []Pair {
	...
	iter, currKey, err := newScanIterator(mvcc.EDB, startKey, endKey)
	defer iter.Release()
	if err != nil {
...
func (mvcc *MVCCLevelDB) ReverseScan(startKey, endKey []byte, limit int, startTS uint64, isoLevel kvrpcpb.IsolationLevel, resolvedLocks []uint64) []Pair {
	...
	iter := mvcc.EDB.NewIterator(&soliton.Range{
		Limit: mvccEnd,
	}, nil)
	defer iter.Release()
...
func (mvcc *MVCCLevelDB) PessimisticLock(req *kvrpcpb.PessimisticLockRequest) *kvrpcpb.PessimisticLockResponse {
	...
	for _, m := range mutations {
		...
		if lockWaitTime == ekv.LockNoWait {
	...
	if err := mvcc.EDB.Write(batch, nil); err != nil {
...
func (mvcc *MVCCLevelDB) PessimisticRollback(keys [][]byte, startTS, forUFIDelateTS uint64) []error {
	mvcc.mu.Lock()
	defer mvcc.mu.Unlock()
	...
	for _, key := range keys {
		err := pessimisticRollbackKey(mvcc.EDB, batch, key, startTS, forUFIDelateTS)
		errs = append(errs, err)
		if err != nil {
			anyError = true
		}
...
func (mvcc *MVCCLevelDB) Prewrite(req *kvrpcpb.PrewriteRequest) []error {
	...
	for i, m := range mutations {
		...
		err = prewriteMutation(mvcc.EDB, batch, m, startTS, primary, ttl, txnSize, isPessimisticLock, minCommitTS)
		errs = append(errs, err)
		if err != nil {
			anyError = true
		}
...
func (mvcc *MVCCLevelDB) Commit(keys [][]byte, startTS, commitTS uint64) error {
	...
	batch := &leveldb.Batch{}
	for _, k := range keys {
		err := commitKey(mvcc.EDB, batch, k, startTS, commitTS)
		if err != nil {
			return errors.Trace(err)
		}
	}
	return mvcc.EDB.Write(batch, nil)
}
...
func (mvcc *MVCCLevelDB) Rollback(keys [][]byte, startTS uint64) error {
	...
	batch := &leveldb.Batch{}
	for _, k := range keys {
		err := rollbackKey(mvcc.EDB, batch, k, startTS)
		if err != nil {
			return errors.Trace(err)
		}
	}
		...
		if ok && dec.dagger.startTS == lockTS {
			...
			if callerStartTS == math.MaxUint64 {
				...
				if dagger.minCommitTS < callerStartTS+1 {
					...
					batch.Put(writeKey, writeValue)
					if err1 = mvcc.EDB.Write(batch, nil); err1 != nil {
						err = errors.Trace(err1)
						return
					}
				}
...
func (mvcc *MVCCLevelDB) RawPut(key, value []byte) {
	...
	terror.Log(mvcc.EDB.Put(key, value, nil))
}
...
func (mvcc *MVCCLevelDB) RawBatchPut(keys, values [][]byte) {
	...
	terror.Log(mvcc.EDB.Write(batch, nil))
}
...
func (mvcc *MVCCLevelDB) RawBatchGet(keys [][]byte) [][]byte {
	...
	for _, key := range keys {
		value, err := mvcc.EDB.Get(key, nil)
		terror.Log(err)
...
func (mvcc *MVCCLevelDB) RawDelete(key []byte) {
	...
	terror.Log(mvcc.EDB.Delete(key, nil))
}
