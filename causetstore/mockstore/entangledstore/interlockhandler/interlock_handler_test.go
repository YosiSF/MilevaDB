...
func prepareTestTableData(c *C, keyNumber int, blockID int64) *data {
	...
	for i := 0; i < keyNumber; i++ {
		...
		c.Assert(err, IsNil)
		rowKeyEncodedData := blockcodec.EncodeRowKeyWithHandle(blockID, solomonkey.IntHandle(i))
...
func getTestPointRange(blockID int64, handle int64) solomonkey.KeyRange {
	startKey := blockcodec.EncodeRowKeyWithHandle(blockID, solomonkey.IntHandle(handle))
	endKey := make([]byte, len(startKey))
	INTERLOCKy(endKey, startKey)
	convertToPrefixNext(endKey)
	return solomonkey.KeyRange{
		StartKey: startKey,
		EndKey:   endKey,
	}
}
...
func newPosetPosetDagContext(causetstore *testStore, keyRanges []solomonkey.KeyRange, posetPosetDagReq *fidelpb.PosetDagRequest, startTs uint64) *posetPosetDagContext {
...
func (ts testSuite) TestPointGet(c *C) {
	...
	posetPosetDagRequest := newPosetPosetDagBuilder().
		...
		build()
	posetPosetDagCtx := newPosetPosetDagContext(causetstore, []solomonkey.KeyRange{getTestPointRange(blockID, handle)},
...
func (ts testSuite) TestClosureExecutor(c *C) {
	...
	posetPosetDagCtx := newPosetPosetDagContext(causetstore, []solomonkey.KeyRange{getTestPointRange(blockID, 1)},
		posetPosetDagRequest, posetPosetDagRequestStartTs)
...
func newTestStore(dbPrefix string, logPrefix string) (*testStore, error) {
	...
	kvPath := filepath.Join(dbPath, "solomonkey")
