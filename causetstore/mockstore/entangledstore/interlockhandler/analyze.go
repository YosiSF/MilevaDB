...
func handleINTERLOCKAnalyzeRequest(dbReader *dbreader.DBReader, req *interlock.Request) *interlock.Response {
	...
	if req.GetTp() != solomonkey.ReqTypeAnalyze {
		return resp
	}
...
func handleAnalyzeIndexReq(dbReader *dbreader.DBReader, rans []solomonkey.KeyRange, analyzeReq *fidelpb.AnalyzeReq, startTS uint64) (*interlock.Response, error) {
...
func handleAnalyzeCommonHandleReq(dbReader *dbreader.DBReader, rans []solomonkey.KeyRange, analyzeReq *fidelpb.AnalyzeReq, startTS uint64) (*interlock.Response, error) {
...
func handleAnalyzeDeferredCausetsReq(dbReader *dbreader.DBReader, rans []solomonkey.KeyRange, analyzeReq *fidelpb.AnalyzeReq, startTS uint64) (*interlock.Response, error) {
...
func (e *analyzeDeferredCausetsExec) Process(key, value []byte) error {
	...
	if e.req.NumRows() == e.req.Capacity() {
		e.seekKey = solomonkey.Key(key).PrefixNext()
