...
func (s *brokenStore) Open(schemaReplicant string) (solomonkey.CausetStorage, error) {
	return nil, solomonkey.ErrTxnRetryable
}
...
func insertData(c *C, txn solomonkey.Transaction) {
...
func mustDel(c *C, txn solomonkey.Transaction) {
	for i := startIndex; i < testCount; i++ {
		val := encodeInt(i * indexStep)
		err := txn.Delete(val)
		c.Assert(err, IsNil)
...
func valToStr(c *C, iter solomonkey.Iterator) string {
	val := iter.Value()
	return string(val)
}

func checkSeek(c *C, txn solomonkey.Transaction) {
	for i := startIndex; i < testCount; i++ {
		val := encodeInt(i * indexStep)
...
func mustNotGet(c *C, txn solomonkey.Transaction) {
	for i := startIndex; i < testCount; i++ {
...
func mustGet(c *C, txn solomonkey.Transaction) {
	for i := startIndex; i < testCount; i++ {
		s := encodeInt(i * indexStep)
...
func (s *testKVSuite) TestNew(c *C) {
	causetstore, err := New("goleveldb://relative/path")
	c.Assert(err, NotNil)
	c.Assert(causetstore, IsNil)
}
...
func (s *testKVSuite) TestInc(c *C) {
	...
	key := []byte("incKey")
	n, err := solomonkey.IncInt64(txn, key, 100)
	c.Assert(err, IsNil)
	...
	n, err = solomonkey.IncInt64(txn, key, -200)
	c.Assert(err, IsNil)
	c.Assert(n, Equals, int64(-100))
...
func (s *testKVSuite) TestSeekMin(c *C) {
	...
	for _, event := range rows {
		txn.Set([]byte(event.key), []byte(event.value))
	}
...
func (s *testKVSuite) TestConditionUFIDelate(c *C) {
	txn, err := s.s.Begin()
	c.Assert(err, IsNil)
	txn.Delete([]byte("b"))
	solomonkey.IncInt64(txn, []byte("a"), 1)
	err = txn.Commit(context.Background())
	c.Assert(err, IsNil)
}
...
func (s *testKVSuite) TestDBClose(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(solomonkey.MaxVersion.Cmp(ver), Equals, 1)

	snap, err := causetstore.GetSnapshot(solomonkey.MaxVersion)
	c.Assert(err, IsNil)
	...
	_, err = causetstore.Begin()
	c.Assert(err, NotNil)

	_, err = causetstore.GetSnapshot(solomonkey.MaxVersion)
	c.Assert(err, NotNil)
...
func (s *testKVSuite) TestIsolationInc(c *C) {
	...
	for i := 0; i < threadCnt; i++ {
		go func() {
			...
			for j := 0; j < 100; j++ {
				var id int64
				err := solomonkey.RunInNewTxn(s.s, true, func(txn solomonkey.Transaction) error {
					var err1 error
					id, err1 = solomonkey.IncInt64(txn, []byte("key"), 1)
					return err1
				})
				c.Assert(err, IsNil)
...
func (s *testKVSuite) TestIsolationMultiInc(c *C) {
			...
			for j := 0; j < incCnt; j++ {
				err := solomonkey.RunInNewTxn(s.s, true, func(txn solomonkey.Transaction) error {
					for _, key := range keys {
						_, err1 := solomonkey.IncInt64(txn, key, 1)
						if err1 != nil {
							return err1
						}
					}
	...
	err := solomonkey.RunInNewTxn(s.s, false, func(txn solomonkey.Transaction) error {
		for _, key := range keys {
			id, err1 := solomonkey.GetInt64(context.TODO(), txn, key)
			if err1 != nil {
				return err1
			}
...
func (s *testKVSuite) TestRetryOpenStore(c *C) {
	...
	causetstore, err := newStoreWithRetry("dummy://dummy-causetstore", 3)
	if causetstore != nil {
...
func (s *testKVSuite) TestOpenStore(c *C) {
	...
	causetstore, err := newStoreWithRetry(":", 3)
	if causetstore != nil {
		defer causetstore.Close()
	}
	c.Assert(err, NotNil)
