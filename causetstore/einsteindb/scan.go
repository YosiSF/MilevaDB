...
// Scanner support einsteindb scan
type Scanner struct {
	snapshot     *einsteindbSnapshot
	batchSize    int
	cache        []*pb.KvPair
	idx          int
	nextStartKey ekv.Key
	endKey       ekv.Key
...
func newScanner(snapshot *einsteindbSnapshot, startKey []byte, endKey []byte, batchSize int, reverse bool) (*Scanner, error) {
...
func (s *Scanner) Next() error {
	...
	for {
		...
		if (!s.reverse && (len(s.endKey) > 0 && ekv.Key(current.Key).Cmp(s.endKey) >= 0)) ||
			(s.reverse && len(s.nextStartKey) > 0 && ekv.Key(current.Key).Cmp(s.nextStartKey) < 0) {
			s.eof = true
			s.Close()
			return nil
		}
...
func (s *Scanner) getData(bo *Backoffer) error {
	...
	sender := NewRegionRequestSender(s.snapshot.causetstore.regionCache, s.snapshot.causetstore.client)
	var reqEndKey, reqStartKey []byte
	var loc *KeyLocation
	var err error
	for {
		if !s.reverse {
			loc, err = s.snapshot.causetstore.regionCache.LocateKey(bo, s.nextStartKey)
		} else {
			loc, err = s.snapshot.causetstore.regionCache.LocateEndKey(bo, s.nextEndKey)
		}
		if err != nil {
			return errors.Trace(err)
		}
		...
		req := einsteindbrpc.NewReplicaReadRequest(einsteindbrpc.CmdScan, sreq, s.snapshot.replicaRead, &s.snapshot.replicaReadSeed, pb.Context{
			Priority:     s.snapshot.priority,
			NotFillCache: s.snapshot.notFillCache,
		...
		err = s.snapshot.causetstore.CheckVisibility(s.startTS())
		if err != nil {
			return errors.Trace(err)
		}
		...
		if !s.reverse {
			s.nextStartKey = ekv.Key(lastKey).Next()
		} else {
