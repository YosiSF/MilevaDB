...
type RangeTaskHandler = func(ctx context.Context, r ekv.KeyRange) (RangeTaskStat, error)
...
func (s *RangeTaskRunner) RunOnRange(ctx context.Context, startKey, endKey ekv.Key) error {
	s.completedRegions = 0
	metrics.EinsteinDBRangeTaskStats.WithLabelValues(s.name, lblCompletedRegions).Set(0)
	...
	taskCh := make(chan *ekv.KeyRange, s.concurrency)
...
Loop:
	for {
		...
		rangeEndKey, err := s.causetstore.GetRegionCache().BatchLoadRegionsFromKey(bo, key, s.regionsPerTask)
		if err != nil {
		...
		metrics.EinsteinDBRangeTaskPushDuration.WithLabelValues(s.name).Observe(time.Since(pushTaskStartTime).Seconds())
...
func (s *RangeTaskRunner) createWorker(taskCh chan *ekv.KeyRange, wg *sync.WaitGroup) *rangeTaskWorker {
...
func (w *rangeTaskWorker) run(ctx context.Context, cancel context.CancelFunc) {
	...
	for r := range w.taskCh {
		...
		metrics.EinsteinDBRangeTaskStats.WithLabelValues(w.name, lblCompletedRegions).Add(float64(stat.CompletedRegions))
		metrics.EinsteinDBRangeTaskStats.WithLabelValues(w.name, lblFailedRegions).Add(float64(stat.FailedRegions))
