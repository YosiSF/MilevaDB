...
func (t CmdType) String() string {
	...
	case CmdINTERLOCK:
		return "Causet"
	case CmdINTERLOCKStream:
		return "INTERLOCKStream"
	case CmdBatchINTERLOCK:
		return "BatchINTERLOCK"
...
func NewReplicaReadRequest(typ CmdType, pointer interface{}, replicaReadType ekv.ReplicaReadType, replicaReadSeed *uint32, ctxs ...kvrpcpb.Context) *Request {
...
// BatchINTERLOCK returns interlock request in request.
func (req *Request) BatchINTERLOCK() *interlock.BatchRequest {
...
func (req *Request) ToBatchCommandsRequest() *einsteindbpb.BatchCommandsRequest_Request {
	...
	case CmdINTERLOCK:
		return &einsteindbpb.BatchCommandsRequest_Request{Cmd: &einsteindbpb.BatchCommandsRequest_Request_interlocking_directorate{interlocking_directorate: req.Causet()}}
	case CmdPessimisticLock:
...
func FromBatchCommandsResponse(res *einsteindbpb.BatchCommandsResponse_Response) (*Response, error) {
	...
	case *einsteindbpb.BatchCommandsResponse_Response_interlocking_directorate:
		return &Response{Resp: res.interlocking_directorate}, nil
...
// INTERLOCKStreamResponse combinates einsteindbpb.EinsteinDB_interlocking_directorateStreamClient and the first Recv() result together.
...
type INTERLOCKStreamResponse struct {
	einsteindbpb.EinsteinDB_interlocking_directorateStreamClient
	*interlock.Response // The first result of Recv()
	Timeout             time.Duration
	Lease               // Shared by this object and a background goroutine.
}

// BatchINTERLOCKStreamResponse comprises the Batchinterlocking_directorateClient , the first result and timeout detector.
type BatchINTERLOCKStreamResponse struct {
	einsteindbpb.EinsteinDB_Batchinterlocking_directorateClient
...
func SetContext(req *Request, region *metapb.Region, peer *metapb.Peer) error {
	...
	case CmdINTERLOCKStream:
		req.Causet().Context = ctx
	case CmdBatchINTERLOCK:
		req.BatchINTERLOCK().Context = ctx
	case CmdMvccGetByKey:
...
func CallRPC(ctx context.Context, client einsteindbpb.EinsteinDBClient, req *Request) (*Response, error) {
	...
	case CmdINTERLOCK:
		resp.Resp, err = client.interlocking_directorate(ctx, req.Causet())
	case CmdINTERLOCKStream:
		var streamClient einsteindbpb.EinsteinDB_interlocking_directorateStreamClient
		streamClient, err = client.interlocking_directorateStream(ctx, req.Causet())
		resp.Resp = &INTERLOCKStreamResponse{
			EinsteinDB_interlocking_directorateStreamClient: streamClient,
		}
	case CmdBatchINTERLOCK:
		var streamClient einsteindbpb.EinsteinDB_Batchinterlocking_directorateClient
		streamClient, err = client.Batchinterlocking_directorate(ctx, req.BatchINTERLOCK())
		resp.Resp = &BatchINTERLOCKStreamResponse{
			EinsteinDB_Batchinterlocking_directorateClient: streamClient,
		}
	case CmdMvccGetByKey:
...
func (resp *INTERLOCKStreamResponse) Recv() (*interlock.Response, error) {
	deadline := time.Now().Add(resp.Timeout).UnixNano()
	atomic.StoreInt64(&resp.Lease.deadline, deadline)

	ret, err := resp.EinsteinDB_interlocking_directorateStreamClient.Recv()

	atomic.StoreInt64(&resp.Lease.deadline, 0) // Stop the lease check.
	return ret, errors.Trace(err)
}

// Close closes the INTERLOCKStreamResponse object.
func (resp *INTERLOCKStreamResponse) Close() {
...
// Recv overrides the stream client Recv() function.
func (resp *BatchINTERLOCKStreamResponse) Recv() (*interlock.BatchResponse, error) {
	deadline := time.Now().Add(resp.Timeout).UnixNano()
	atomic.StoreInt64(&resp.Lease.deadline, deadline)

	ret, err := resp.EinsteinDB_Batchinterlocking_directorateClient.Recv()
...
// Close closes the INTERLOCKStreamResponse object.
func (resp *BatchINTERLOCKStreamResponse) Close() {
	atomic.StoreInt64(&resp.Lease.deadline, 1)
	// We also call cancel here because CheckStreamTimeoutLoop
	// is not guaranteed to cancel all items when it exits.
	if resp.Lease.Cancel != nil {
