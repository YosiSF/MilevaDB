...
var (
	einsteindbRawkvCmdHistogramWithGet           = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("get")
	einsteindbRawkvCmdHistogramWithBatchGet      = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("batch_get")
	einsteindbRawkvCmdHistogramWithBatchPut      = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("batch_put")
	einsteindbRawkvCmdHistogramWithDelete        = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("delete")
	einsteindbRawkvCmdHistogramWithBatchDelete   = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("batch_delete")
	einsteindbRawkvCmdHistogramWithRawScan       = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("raw_scan")
	einsteindbRawkvCmdHistogramWithRawReversScan = metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues("raw_reverse_scan")

	einsteindbRawkvSizeHistogramWithKey   = metrics.EinsteinDBRawkvSizeHistogram.WithLabelValues("key")
	einsteindbRawkvSizeHistogramWithValue = metrics.EinsteinDBRawkvSizeHistogram.WithLabelValues("value")
)
...
func (c *RawKVClient) Get(key []byte) ([]byte, error) {
	start := time.Now()
	defer func() { einsteindbRawkvCmdHistogramWithGet.Observe(time.Since(start).Seconds()) }()

	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRawGet, &kvrpcpb.RawGetRequest{Key: key})
...
func (c *RawKVClient) BatchGet(keys [][]byte) ([][]byte, error) {
	start := time.Now()
	defer func() {
		einsteindbRawkvCmdHistogramWithBatchGet.Observe(time.Since(start).Seconds())
	}()
...
func (c *RawKVClient) Put(key, value []byte) error {
	start := time.Now()
	defer func() { einsteindbRawkvCmdHistogramWithBatchPut.Observe(time.Since(start).Seconds()) }()
	einsteindbRawkvSizeHistogramWithKey.Observe(float64(len(key)))
	einsteindbRawkvSizeHistogramWithValue.Observe(float64(len(value)))
...
func (c *RawKVClient) BatchPut(keys, values [][]byte) error {
	start := time.Now()
	defer func() {
		einsteindbRawkvCmdHistogramWithBatchPut.Observe(time.Since(start).Seconds())
	}()
...
func (c *RawKVClient) Delete(key []byte) error {
	start := time.Now()
	defer func() { einsteindbRawkvCmdHistogramWithDelete.Observe(time.Since(start).Seconds()) }()

	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRawDelete, &kvrpcpb.RawDeleteRequest{
		Key: key,
	})
	resp, _, err := c.sendReq(key, req, false)
	if err != nil {
...
func (c *RawKVClient) BatchDelete(keys [][]byte) error {
	start := time.Now()
	defer func() {
		einsteindbRawkvCmdHistogramWithBatchDelete.Observe(time.Since(start).Seconds())
	}()
...
func (c *RawKVClient) DeleteRange(startKey []byte, endKey []byte) error {
	...
	defer func() {
		...
		metrics.EinsteinDBRawkvCmdHistogram.WithLabelValues(label).Observe(time.Since(start).Seconds())
	}()
...
func (c *RawKVClient) Scan(startKey, endKey []byte, limit int) (keys [][]byte, values [][]byte, err error) {
	start := time.Now()
	defer func() { einsteindbRawkvCmdHistogramWithRawScan.Observe(time.Since(start).Seconds()) }()
	...
	for len(keys) < limit && (len(endKey) == 0 || bytes.Compare(startKey, endKey) < 0) {
		req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRawScan, &kvrpcpb.RawScanRequest{
			StartKey: startKey,
			EndKey:   endKey,
...
func (c *RawKVClient) ReverseScan(startKey, endKey []byte, limit int) (keys [][]byte, values [][]byte, err error) {
	start := time.Now()
	defer func() {
		einsteindbRawkvCmdHistogramWithRawReversScan.Observe(time.Since(start).Seconds())
	}()
...
func (c *RawKVClient) sendBatchReq(bo *Backoffer, keys [][]byte, cmdType einsteindbrpc.CmdType) (*einsteindbrpc.Response, error) { // split the keys
	...
	case einsteindbrpc.CmdRawBatchGet:
		resp = &einsteindbrpc.Response{Resp: &kvrpcpb.RawBatchGetResponse{}}
	case einsteindbrpc.CmdRawBatchDelete:
		resp = &einsteindbrpc.Response{Resp: &kvrpcpb.RawBatchDeleteResponse{}}
	}
...
func (c *RawKVClient) doBatchReq(bo *Backoffer, batch batch, cmdType einsteindbrpc.CmdType) singleBatchResp {
	...
	switch cmdType {
	case einsteindbrpc.CmdRawBatchGet:
		req = einsteindbrpc.NewRequest(cmdType, &kvrpcpb.RawBatchGetRequest{
			Keys: batch.keys,
		})
	case einsteindbrpc.CmdRawBatchDelete:
		req = einsteindbrpc.NewRequest(cmdType, &kvrpcpb.RawBatchDeleteRequest{
			Keys: batch.keys,
		})
	}
...
func (c *RawKVClient) sendDeleteRangeReq(startKey []byte, endKey []byte) (*einsteindbrpc.Response, []byte, error) {
	...
	for {
		...
		req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRawDeleteRange, &kvrpcpb.RawDeleteRangeRequest{
			StartKey: startKey,
			EndKey:   actualEndKey,
		})
...
func (c *RawKVClient) doBatchPut(bo *Backoffer, batch batch) error {
	...
	req := einsteindbrpc.NewRequest(einsteindbrpc.CmdRawBatchPut, &kvrpcpb.RawBatchPutRequest{Pairs: kvPair})
