...
func (s *testTiclientSuite) TestSplitRegionIn2PC(c *C) {
	if *WithEinsteinDB {
		c.Skip("scatter will timeout with single node EinsteinDB")
	}
	...
	checkKeyRegion := func(bo *Backoffer, start, end []byte, checker Checker) {
		...
		loc1, err := s.causetstore.regionCache.LocateKey(bo, start)
		c.Assert(err, IsNil)
		loc2, err := s.causetstore.regionCache.LocateKey(bo, end)
		c.Assert(err, IsNil)
	...
	for _, m := range mode {
		...
		if m == "pessimistic" {
			...
			lockCtx := &ekv.LockCtx{}
			lockCtx.ForUFIDelateTS = txn.startTS
			keys := make([]ekv.Key, 0, preSplitThresholdInTest)
