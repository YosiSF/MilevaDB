...
import (
	...
	"github.com/whtcorpsinc/MilevaDB-Prod/spacetime"
...
func (*testSuite) TestT(c *C) {
	...
	err = solomonkey.RunInNewTxn(causetstore, true, func(txn solomonkey.Transaction) error {
...
func checkApplyCreateNonExistsSchemaDoesNotPanic(c *C, txn solomonkey.Transaction, builder *schemareplicant.Builder) {
...
func checkApplyCreateNonExistsBlockDoesNotPanic(c *C, txn solomonkey.Transaction, builder *schemareplicant.Builder, dbID int64) {
	m := spacetime.NewMeta(txn)
...
func (testSuite) TestConcurrent(c *C) {
	defer testleak.AfterTest(c)()
	storeCount := 5
	stores := make([]solomonkey.CausetStorage, storeCount)
	...
	wg.Add(storeCount)
	for _, causetstore := range stores {
		go func(s solomonkey.CausetStorage) {
			defer wg.Done()
...
func genGlobalID(causetstore solomonkey.CausetStorage) (int64, error) {
	var globalID int64
	err := solomonkey.RunInNewTxn(causetstore, true, func(txn solomonkey.Transaction) error {
		var err error
