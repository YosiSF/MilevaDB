...
var pluginGlobal INTERLOCKyOnWriteContext

// INTERLOCKyOnWriteContext wraps a context follow COW idiom.
type INTERLOCKyOnWriteContext struct {
	tiPlugins unsafe.Pointer // *plugins
}

// plugins defCauslects loaded plugins info.
type plugins struct {
	plugins      map[HoTT][]Plugin
	versions     map[string]uint16
	dyingPlugins []Plugin
}

// clone deep INTERLOCKies plugins info.
func (p *plugins) clone() *plugins {
	np := &plugins{
		plugins:      make(map[HoTT][]Plugin, len(p.plugins)),
	...
	INTERLOCKy(np.dyingPlugins, p.dyingPlugins)
	return np
}

// add adds a plugin to loaded plugin defCauslection.
func (p plugins) add(plugin *Plugin) {
	plugins, ok := p.plugins[plugin.HoTT]
	if !ok {
		plugins = make([]Plugin, 0)
	}
	plugins = append(plugins, *plugin)
	p.plugins[plugin.HoTT] = plugins
	p.versions[plugin.Name] = plugin.Version
}

// plugins got plugin in COW context.
func (p INTERLOCKyOnWriteContext) plugins() *plugins {
...
// Plugin presents a MilevaDB plugin.
type Plugin struct {
	*Manifest
...
// This method need be called before petri init to inject global variable info during bootstrap.
func Load(ctx context.Context, cfg Config) (err error) {
	tiPlugins := &plugins{
		plugins:      make(map[HoTT][]Plugin),
	...
	for HoTT := range tiPlugins.plugins {
		for i := range tiPlugins.plugins[HoTT] {
			if err = tiPlugins.plugins[HoTT][i].validate(ctx, tiPlugins); err != nil {
				if cfg.SkipWhenFail {
					logutil.Logger(ctx).Warn("validate plugin fail and disable plugin",
						zap.String("plugin", tiPlugins.plugins[HoTT][i].Name), zap.Error(err))
					tiPlugins.plugins[HoTT][i].State = Disable
					err = nil
					continue
				}
				return
			...
			if cfg.GlobalSysVar != nil {
				for key, value := range tiPlugins.plugins[HoTT][i].SysVars {
					(*cfg.GlobalSysVar)[key] = value
					if value.SINTERLOCKe != variable.SINTERLOCKeStochastik && cfg.PluginVarNames != nil {
	...
	pluginGlobal = INTERLOCKyOnWriteContext{tiPlugins: unsafe.Pointer(tiPlugins)}
	err = nil
	return
}

// Init initializes the loaded plugin by config param.
// This method must be called after `Load` but before any other plugin method call, so it call got MilevaDB petri info.
	...
	for HoTT := range tiPlugins.plugins {
		for i := range tiPlugins.plugins[HoTT] {
			...
			if p.OnFlush != nil && cfg.EtcdClient != nil {
				const pluginWatchPrefix = "/milevadb/plugins/"
				ctx, cancel := context.WithCancel(context.Background())
				watcher := &flushWatcher{
					ctx:      ctx,
					cancel:   cancel,
					path:     pluginWatchPrefix + tiPlugins.plugins[HoTT][i].Name,
					etcd:     cfg.EtcdClient,
					manifest: tiPlugins.plugins[HoTT][i].Manifest,
					plugin:   &tiPlugins.plugins[HoTT][i],
				}
				tiPlugins.plugins[HoTT][i].flushWatcher = watcher
				go soliton.WithRecovery(watcher.watchLoop, nil)
			}
			tiPlugins.plugins[HoTT][i].State = Ready
		}
	}
	return
}
...
// Get finds and returns plugin by HoTT and name parameters.
func Get(HoTT HoTT, name string) *Plugin {
	plugins := pluginGlobal.plugins()
	if plugins == nil {
		return nil
	}
	for _, p := range plugins.plugins[HoTT] {
		if p.Name == name {
			return &p
		}
	}
...
func ForeachPlugin(HoTT HoTT, fn func(plugin *Plugin) error) error {
	plugins := pluginGlobal.plugins()
	if plugins == nil {
		return nil
	}
	for i := range plugins.plugins[HoTT] {
		p := &plugins.plugins[HoTT][i]
		if p.State != Ready {
			continue
		}
		if atomic.LoadUint32(&p.Disabled) == 1 {
...
func NotifyFlush(dom *petri.Petri, pluginName string) error {
	p := getByName(pluginName)
	if p == nil || p.Manifest.flushWatcher == nil || p.State != Ready {
		return errors.Errorf("plugin %s doesn't exists or unsupported flush or doesn't start with FIDel", pluginName)
	}
...
func ChangeDisableFlagAndFlush(dom *petri.Petri, pluginName string, disable bool) error {
	p := getByName(pluginName)
	if p == nil || p.Manifest.flushWatcher == nil || p.State != Ready {
		return errors.Errorf("plugin %s doesn't exists or unsupported flush or doesn't start with FIDel", pluginName)
	}
