...
import (
	...
	stats "github.com/whtcorpsinc/MilevaDB-Prod/statistics"
...
func loadStats(tblInfo *perceptron.TableInfo, path string) (*stats.Block, error) {
...
// When the randCnt falls in the midbse of bucket, we return the idx of lower bound which is an even number.
...
func (h *histogram) randDate(unit string, mysqlFmt string, dateFmt string) string {
	...
	if idx%2 == 0 {
		...
		diff := types.TimestamFIDeliff(unit, lower, upper)
		if diff == 0 {
