...
import (
	...
	"github.com/whtcorpsinc/MilevaDB-Prod/soliton/filesort"
...
func encodeRow(b []byte, event *comparableRow) ([]byte, error) {
	var (
		err  error
		head = make([]byte, 8)
		body []byte
	...
	body, err = codec.EncodeKey(sc, body, event.key...)
	if err != nil {
		return b, errors.Trace(err)
	}
	body, err = codec.EncodeKey(sc, body, event.val...)
	if err != nil {
		return b, errors.Trace(err)
	}
	body, err = codec.EncodeKey(sc, body, types.NewIntCauset(event.handle))
	if err != nil {
		return b, errors.Trace(err)
	}
...
func decodeRow(fd *os.File) (*comparableRow, error) {
	var (
		...
		n    int
		head = make([]byte, 8)
		dcod = make([]types.Causet, 0, keySize+valSize+1)
	)
	...
	if n != rowSize {
		return nil, errors.New("incorrect event")
	}
	if err != nil {
		return nil, errors.Trace(err)
	}
 ...
 *		One 64-bit integer represent the event size in bytes, followed by the
 *      the actual event bytes.
 */
func export() error {
	var outputBytes []byte

	fileName := filepath.Join(tmFIDelir, "data.out")
...
func driveGenCmd() {
	...
	if _, err = os.Stat(tmFIDelir); err != nil {
		if os.IsNotExist(err) {
			log.Fatal("tmFIDelir does not exist")
		}
		log.Fatal(err.Error())
	}
	...
	terror.MustNil(err)
	cLog("Done!")
	cLogf("Data placed in: %s", filepath.Join(tmFIDelir, "data.out"))
...
func driveRunCmd() {
	...
	dir, err = ioutil.TemFIDelir(tmFIDelir, "benchfilesort_test")
	terror.MustNil(err)
...
func init() {
	...
	genCmd.StringVar(&tmFIDelir, "dir", cwd, "where to causetstore the generated rows")
	...
	runCmd.StringVar(&tmFIDelir, "dir", cwd, "where to load the generated rows")
